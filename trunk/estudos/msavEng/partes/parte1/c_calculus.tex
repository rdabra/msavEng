...
\end{alignat*}
where $|s|(x):=|\fua{s}{x}|$ and $|s|,g_1,g_2\in S$. In this context, if a Cauchy sequence of integrable step functions $s_1,s_2,\cdots\in S$ exists and converges in measure $\mu$ to the nonnegative measurable function in $\map{g}{M}{\real^+}$, we say that $g$ is integrable or Labesgue integrable on $E$ and scalar 
\begin{equation}\label{eq:integ}
\int_E g := \lim_{i\to \infty}\int_E s_i 
\end{equation} 
is the integral or Labesgue integral of $g$ on $E$. For future purposes, we must say that if $s_1,s_2,\cdots\in S$ and $t_1,t_2,\cdots\in S$ are convergent Chauchy sequences of integrable nonnegative step functions, then sequence $ \alpha s_1+\beta t_1,\alpha s_2+\beta t_2,\cdots$ also is, $\alpha,\beta\in\real$. Moreover, 
\begin{equation}\label{eq:cauchySum}
s_1,s_2,\cdots\to g\,\,\, \wedge\,\,\, t_1,t_2,\cdots\to h \implies \alpha s_1+\beta t_1,\alpha s_2+\beta t_2,\cdots\to \alpha g+\beta h\,. 
\end{equation}

{\footnotesize
\begin{proof}
Since scalar multiplication does not affect the properties of above sequences, we will verify only the effect of the sum. First we prove that $  s_1+ t_1,s_2+ t_2,\cdots$ is also a Cauchy Sequence. If both expressions $\lim_{\min{\lpa i,j\rpa}\to\infty}{\fua{\varrho}{s_i,s_j}}=0$ and $\lim_{\min{\lpa i,j\rpa}\to\infty}{\fua{\varrho}{t_i,t_j}}=0$ are valid, then, by summing these two equalities, we obtain $\lim_{\min{\lpa i,j\rpa}\to\infty}\fua{\varrho}{s_i,s_j}+\fua{\varrho}{t_i,t_j}=0\,$. From the property of triangular inequality of norms, since
\begin{equation*}
\fua{\varrho}{s_i,s_j}+\fua{\varrho}{t_i,t_j} = \|s_i-s_j\| + \|t_i-t_j\| \geqslant \|s_i-s_j + t_i-t_j\|\,,
\end{equation*}
it is obvious that $\lim_{\min{\lpa i,j\rpa}\to\infty}\|(s_i+ t_i) -(s_j +t_j)\|=0\,$, which proves the sum to be a Cauchy sequence. The Cauchy convergence can be  proved by this same procedure.  
\end{proof}}
...
{\footnotesize
\begin{proof}
In order to prove item i, since the integrable step functions $s_i$ that converges to $f$ are nonnegative, so are their coefficients. Therefore, by definition \eqref{eq:defInteSimp}, since $\mu$ is nonnegative then $\int_{E}s_i$ is also nonnegative and so the integral on \eqref{eq:integ}. Now, for item ii, considering equality \eqref{eq:measuSum}, definition \eqref{eq:integ} and mapping $\map{h}{M}{\real^+}$, we have
\begin{align*}
...
\end{align*}
Therefore, we prove second property by 
\begin{equation*}
...
\end{equation*}
and, by the same development, $\int_E f^-=0$, which prove item iii. Now, we prove item iv. It is easy to obtain equality
\begin{equation*}
...
\end{equation}
is called the integral of tensor function $\tnr{\psi}$ on $\pmb{\mathcal{U}}$.