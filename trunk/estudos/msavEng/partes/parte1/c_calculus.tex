...
\end{dcases}\,.
\end{equation*}
In the particular case of a measure space $(M,\css{M},\mu)$, step function $s_\css{A}$ is defined to be \textsb{integrable}\index{function!integrable step} on set $E\in\css{M}$ when $\fua{\mu}{E\cap A_i} < \infty$. If this is the case, scalar
\begin{equation}
...
\end{alignat}
where $|s|(x):=|\fua{s}{x}|$ and $|s|,g_1,g_2\in S$. In this context, if a Cauchy sequence of nonnegative integrable step functions $s_1,s_2,\cdots\in S$ exists and converges in measure $\mu$ to the measurable nonnegative function in $\map{g}{M}{\real}$, we say that $g$ is integrable or Labesgue integrable on $E$ and scalar 
\begin{equation}\label{eq:integ}
\int_E g := \lim_{i\to \infty}\int_E s_i 
\end{equation} 
is the integral or Labesgue integral of $g$ on $E$.
