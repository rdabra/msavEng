...
Considering measure space $(M,\css{M},\mu)$, a measurable field $(\cam{F},\css{F})$ and a mapping  $\map{f}{M}{\cam{F}}$, a sequence of functions $f_1,f_2,\cdots$ that also map $M$ to $\cam{F}$ converges to function $f$ in measure\index{convergence!in measure} $\mu$ when, given an arbitrary real number $\epsilon>0$,
\begin{equation}
