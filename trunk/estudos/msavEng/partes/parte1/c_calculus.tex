...
The integration approach we shall deal with in this section relies fundamentally on Lebesgue's theory, which is built from the generalization of the concept of size, that is, length, area, volume or \textsb{hypervolume}\index{hypervolume} of respectively one, two, three or $n$ dimensional affine Hilbert spaces. In simple terms, this generalization is strong, based on assigning a real number to a set, whatever measure this scalar expresses. But this general measure must obey certain rules that require and lead to new important definitions. In this context, from specific scalar functions with domains called measurable, integrable tensor functions can be defined.
...
\begin{itemize}
	\setlength\itemsep{.1em}
	\item[i.] If nonnegative function $f$ is integrable, then its integral on $E$ is also nonnegative;
...
There are Lipschitz surfaces in three dimensional Euclidean spaces for which it is possible to define a boundary. In general terms, the intersection of a bounded set $A$ with an open ball centered at an element of boundary $\partial A$ is not a subset of $A$. Considering this idea on a three-dimensional Euclidean space $U_\real$ , the boundary of a surface $\partial\pmb{\mathcal{U}}\subset U_\real$ is a curve $\partial\partial\pmb{\mathcal{U}}\subset\partial\pmb{\mathcal{U}}$ where $B_{\vto{x},r}\cap\partial\pmb{\mathcal{U}}\not\subset\partial\pmb{\mathcal{U}}$ for all $\vto{x}\in\partial\partial\pmb{\mathcal{U}}$ and $r\in\real$. For Lipschitz surfaces with boundaries, we present the following classical theorem that establishes a relationship between an integral on a surface and an integral on its boundary.

\begin{mteo}{Kelvin-Stokes}{stokes}
Let $(U_\real,\tnr{A}_B)$ be a three dimensional oriented Euclidean space and $\map{\vtf{\varphi}}{\pmb{\mathcal{U}}}{\ete{\real}{U\times V}}$ a continuous mapping where  $\vtf{\varphi}$ is continuously differentiable on Lipschitz surface $\partial\pmb{\mathcal{U}}\subset U_\real$, which is oriented by an outward unit normal vector function $\vtf{n}$. Let $\map{\vtf{\tau}}{\partial\partial\pmb{\mathcal{U}}}{U_\real}$ be a continuous mapping where $\fua{\vtf{n}}{\vto{x}}\perp\fua{\vtf{\tau}}{\vto{x}}$ such that $\fua{\tnr{A}_B}{\fua{\vtf{n}}{\vto{x}}\times\fua{\vtf{\tau}}{\vto{x}},\fua{\vtf{n}}{\vto{x}},\fua{\vtf{\tau}}{\vto{x}}}>0$ and $\partial\partial\pmb{\mathcal{U}}$ is the boundary of $\partial\pmb{\mathcal{U}}$. Considering the continuous mappings $\map{\vtf{\varphi}_{\partial\partial\pmb{\mathcal{U}}}}{\partial\partial\pmb{\mathcal{U}}}{V_\real}$ and $\map{\vtf{\varphi}_{\partial\pmb{\mathcal{U}}}}{\partial\pmb{\mathcal{U}}}{V_\real}$ where $\fua{\vtf{\varphi}_{\partial\partial\pmb{\mathcal{U}}}}{\vto{x}}=\fua{\vtf{\tau}}{\vto{x}}\odot_1\fua{\vtf{\varphi}}{\vto{x}}$ and $\fua{\vtf{\varphi}_{\partial\pmb{\mathcal{U}}}}{\vto{x}}=\fua{\vtf{n}}{\vto{x}}\odot_1\fua{\crl{\vtf{\varphi}}}{\vto{x}}$,
\begin{equation*}
\int_{\partial{\pmb{\mathcal{U}}}}\vtf{\varphi}_{\partial\pmb{\mathcal{U}}} = \int_{\partial\partial{\pmb{\mathcal{U}}}}\vtf{\varphi}_{\partial\partial\pmb{\mathcal{U}}}\,\,.
\end{equation*}
\end{mteo}
%\hspace{1pt}
{\footnotesize
