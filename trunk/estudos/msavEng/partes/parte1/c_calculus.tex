...
\end{equation*}
The tensor function $\fua{[\fua{\tnr{\varphi}'_G}{\tnr{X}_0}]}{\tnr{X}}$ is called the \textsb{Gâteaux differential}\index{differential!Gâteaux} of $\tnr{\varphi}$ at $\tnr{X}_0$ and $\tnr{\varphi}'_G$ is the \textsb{Gâteaux derivative}\index{derivative!Gâteaux} of $\tnr{\varphi}$. Every Fréchet differentiable function is also Gâteaux differential, but the converse is obviously not true. What is less obvious is the following: even when Gâteaux derivative $\fua{\tnr{\varphi}'_G}{\tnr{X}_0}$ is bounded and linear, Gâteaux differentiability, which means differentiability along all directions at a point, does not assure total differentiability. This present study will require only the stronger restrictions from the Fréchet differentiation. For further developments of the Gâteaux derivative, see \aut{Wouk}\cite{wouk_1979_1}, chapter 12.} of $\tnr{\psi}$ along $\tnr{H}$. For the particular case of open set $\pmb{\mathcal{U}}$ constituted by scalars, that is $\pmb{\mathcal{U}}\subset\cam{F}$, there is no much sense in talking about directional derivative since vector space $\cam{F}$ is one-dimensional and then has only one direction. In this context, from previous equality, if we conveniently choose 1 as direction, then
\begin{equation}
\fua{\tnr{\psi}'}{x}=\fua{[\fua{\tnr{\psi}'}{x}]}{1}=\lim_{\alpha\to 0} \dfrac{\fua{\tnr{\psi}}{x+\alpha}-\fua{\tnr{\psi}}{x}}{\alpha}
\end{equation}
