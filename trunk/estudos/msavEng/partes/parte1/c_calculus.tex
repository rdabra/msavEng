...
From what we have studied so far, it is finally possible to present a fundamental identity of elementary Calculus, which is the objective of this section, called the Gauss-Ostrogradsky or \textsb{Divergence}\index{theorem!Divergence} theorem. We shall not deal with its most general form, the notorious Generalized Stokes' theorem, because it would demand us to cover an extensive material on Exterior Calculus, which is out of the scope of this work\footnote{For an introduction on Exterior Calculus, see \aut{Flanders}\cite{flanders_1989_1}, \aut{Loomis \& Sternberg}\cite{loomis_2014_1}  and \aut{Spivak}\cite{spivak_1965_1}.}. In general terms, the following theorem relates the ``flux'' of a function -- understood as projections of function values on unit normals -- on the boundary of its domain with its divergence on the interior.     
...
{\footnotesize
\begin{proof}
For a simplified and elegant proof of this theorem in the context of $n$-dimensional Euclidean spaces, we recommend the interested reader to take a look at the approach of \aut{Figueiredo}\cite{figueiredo_1964_1}. 
\end{proof}
}
