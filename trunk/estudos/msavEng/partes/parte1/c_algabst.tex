...
{\footnotesize
\begin{proof}
For the second item, if the relationship between $v$ and $u$ is biunivocal in $v=\fua{g}{u}$ for any $v\in V$, then the relationship between $u$ e $w$ is also biunivocal in $\fua{f^{-1}}{w}=\fua{g}{u}$ or $w=\fua{f}{\fua{g}{u}}$ for any $w\in W$; from where $f\circ g$ results a bijection. Now, considering $u$, $v$ e $w$ any elements of $U$, $V$ and $W$ respectively, the first equality in ii is verified as follows:
\begin{equation*}
...
Now, let's consider the mapping $\map{\fun{k}}{\crt{G}{n}}{\con{W}}$ whose domain is the catersian product of $n$ sets, each of them defining a group. In this case, the function $k$ can be called a group homomorphism if, for any group $G_i$ and any elements $g_{i_1},g_{i_2}\in G_i$,
\begin{align}
...
\end{itemize}
When $\varphi$ observe these axioms, the structure of $B$ is preserved, now called a \textsb{$G$-set}\index{set!G-}, since its fundamental mathematical attributes remain unaltered. Thereby, given an element $b_0\in B$, the group action $\varphi$ is classified as \textsb{simply transitive}\index{grupo!action!simply transitive} if
\begin{equation}
...
In the context of invertible matrices, a matrix whose inverse equals to its conjugate transpose, that is, when  $\mat{U}^{-1}=\mat{U}^\dagger$ where $\mat{U}\in\overline{M}$, is called  \textsb{unitary}\index{matrix!unitary}. Considering the property $\det{\mat{H}}^\dagger=\overline{\det{\mat{H}}}$, we have
\begin{equation}
...
A given square matrix $\mat{A}$ that equals its transpose conjugate $\mat{A}^\dagger$ is called \textsb{Hermitian}\index{matrix!Hermitian}. Real symmetric matrices are examples of Hermitian matrices: if the elements of the square matrix  $\mat{S}$ are real, then $\mat{S}^\dagger=\mat{S}^\text{T}$, and since $\mat{S}$ is symmetric, $\mat{S}^\dagger=\mat{S}$. Similarly to antisymmetric matrices, an \textsb{anti-Hermitian}\index{matrix!anti-Hermitian} matrix $\mat{A}$ equals the negative of its transpose conjugate, that is, $\mat{A}=-\mat{A}^\dagger$. Thus, let $\mat{B}\in\overline{M}$ be a matrix from which we write the following development:  
\begin{equation}
...
\end{equation}
Considering the properties of conjugate transposes, we can obtain that the matrix $\mat{B}_1$ equals its transpose conjugate and matrix $\mat{B}_2$ equals the negative of its transpose conjugate; thereby, we say that they are respectively the \textsb{Hermitian} and \textsb{anti-Hermitian}\index{matrix!Hermitian part of}\index{matrix!anti-Hermitian part of} parts of $\mat{B}$. Thus, this result is generalized by saying that every square matrix can be decomposed additively in a Hermitian and an anti-Hermitian parts.


Any square matrix $\mat{N}$ is called \textsb{normal}\index{matrix!normal}\label{nm:Normal} when it comutes with its conjugate transpose, that is, when $\mat{N}^\dagger\mat{N}=\mat{N}\mat{N}^\dagger$. These matrices, for making the product $\mat{N}^\dagger\mat{N}$ an Hermitian matrix, \emph{are always susceptible of diagonalization by a unitary matrix}. In order to understand what this means, let's consider firstly the matrices $\mat{A},\mat{B}\in\overline{M}$ and say that they are called \textsb{similar}\index{matrix!similar} when an invertible matrix $\mat{Q}$ exists such that
\begin{equation}
...
\end{equation}
where $\mat{X}$ is a non zero $n\times 1$ matrix. When the inequality imposes the left side to be always positive, $\mat{A}$ is called  \textsb{positive-definite}\index{matrix!positive-definite}. A necessary and sufficient condition that ensures positivity of a matrix is that the spectrum of its hermitian part be constituted of non negative elements\footnote{This condition will be verified after the definition of eigenvalue on the next chapter %à p.\pageref{sec:autoPares}
...
We will finish this chapter by presenting the following fundamental equality, a property of any square matrix that will make further developments feasible.  
