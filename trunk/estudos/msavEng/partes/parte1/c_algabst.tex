...
For future purposes, it is important define here the \textsb{function graph}\index{function!graph} of a mapping $\map{f}{\con{U}^{\times n}}{\con{V}}$, which is the set of ordered $(n+1)$-tuples 
\begin{equation*}
\{(x_1,\cdots,x_n,\fua{f}{x_1,\cdots,x_n}):x_i\in \con{U}_i\}\subseteq \con{U}^{\times n}\times V.
\end{equation*}
If $\con{U}_i$ and $\con{V}$ are sets of real numbers, it is possible to depict pictorially the classical cartesian  representation of each tuple $(x_1,\cdots,x_n,\fua{f}{x_1,\cdots,x_n})$.   
