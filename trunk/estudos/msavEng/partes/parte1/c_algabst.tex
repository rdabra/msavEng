...
We already learned that sequences are collections of ordered elements; or, more precisely, elements arranged in a queue, where each one has a position labeled by an index. Generalizing this idea, we shall build collections whose elements are arranged in a higher number of perspectives, like a table (bidimensional) arrangement, for instance. Thereby, let $H$ be a collection of scalars, arranged by the mapping $\map{\fun{h}}{\crt{N}{q}}{\cam{F}}$, where the function $h$ is called \textsb{addressing}\index{array!addressing of} and each $N_i = \lch 1,2,\cdots,n_i \rch$ is a finite subset of $\mathbb{N}^*$ whose elements are ordinals. In this context, we call the collection $H$ an array, represented by $\gloref{array}$, whose scalars $\fua{\fun{h}}{( i_1,\cdots,i_q)}$ are notated by $\gloref{elArray1}$, where the subscript show explicitly the element position. The description of perspectives or \textsb{size}\index{array!size of} of an array is given by the $n$-tuple $(n_1 , \cdots , n_q)$ or, more usually, $n_1\times\cdots\times n_q\,$, where the number of perspectives $q$ expresses the \textsb{order}\index{array!order of} of the array and $n_i$ the \textsb{dimension}\index{array!dimension of} of each perspective.

Arrays can be added and multiplied. If an array  $\mat{A+B}$ is the sum of $\mat{A}$ and $\mat{B}$, these three arrays have equal size $n_1\times\cdots\times n_q\,$ and each element
\begin{equation}\label{eq:adicaoArray}
\mat{(A+B)}_{i_1\cdots i_q}=\mat{A}_{i_1\cdots i_q}+\mat{B}_{i_1\cdots i_q}\,.
\end{equation}
On the other hand, the multiplication $\mat{AB}$ requires that the last $q$ elements of the size of $A$ and the first $q$ elements of the size of $B$ are equal; in other words, if array $\mat{A}$ has size $m_1\times\cdots\times m_p\times n_1\times\cdots\times n_q $, then $\mat{B}$ must have size $n_1\times\cdots\times n_q\times l_1\times\cdots\times l_s$. Thereby, the resulting size of array $\mat{AB}$ is $m_1\times\cdots\times m_p\times l_1\times\cdots\times l_s$ and each element
\begin{equation}
...
\end{equation}
It is also very important for our study to present the so called the \textsb{scalar product}\index{matrix!scalar product of} or \textsb{Frobenius product} of arrays: given two arrays $\mat{A}$ and $\mat{B}$ with the same size $m_1\times\cdots\times m_p$, the scalar product 
\begin{equation}
\gloref{scalP}:=\sum_{i_1}^{m_1}\cdots\sum_{i_p}^{m_p}\mat{A}_{i_1\cdots i_p}\overline{\mat{B}_{i_1\cdots i_p}}\,.
\end{equation}
Speaking of scalars, it is also possible to multiply a scalar $\alpha$ and an array $\mat{C}$ with size $n_1\times\cdots\times n_q$ according to
\begin{equation}
...
Because of its definition, the Levi-Civita Symbol is an array of order $n$ whose size is $n\times\cdots\times n$. Arrays having the same dimension $n$ in each perspective, like the Levi-Civita Symbol, are usually called \textsb{hypercubic}\index{array!hypercubic} of dimension $n$. Therefore, $\epsilon$ is a hypercubic array of order $n$ and dimension $n$. For instance, the Levi-Civita Symbol of order two and dimension two results in a tabular arrangement; more precisely,
\begin{equation}
\epsilon=
\begin{bmatrix}
    0 & 1  \\
...
At this point, it is convenient to say that every $n_1\times n_2$ array, whose elements are arranged in a tabular format, is called a \textsb{matrix}\index{matrix}. The elements of a matrix are addressed by lines and columns: the escalar with $ij$ position is located on the line $i$ and column $j$. If $n_1=n_2=n$, we have a hypercubic matrix, called \textsb{square matrix}\index{matrix!square}, whose dimension is $n$. Henceforth, recalling the additive group $Y$ of arrays, we will consider
$\bar{Y}\subset Y$ a set of hypercubic arrays of dimension $n$, $M\subset Y$ a set of matrices and  $\bar{M}\subset \bar{Y}$ a set of square matrices with dimension $n$. For example, the set $\bar{M}$ has the $n\times n$ Kronecker Delta, represented in \eqref{eq:matrizIdentidade}, as an element, which we usually call \textsb{identity matrix}\index{matrix!identity} $\mat{I}$ because $\mat{A}\mat{I} = \mat{I}\mat{A} = \mat{A}$, for an arbitrary $\mat{A}\in\bar{M}$.

If there exists a matrix $\mat{B}\in\bar{M}$ such that $\mat{A}\mat{B} = \mat{I}$, we call it the \textsb{inverse}\index{matrix!inverse} of $\mat{A}$, notated by $\mat{A}^{-1}$. The condition of invertibility of a matrix depends on the value of a function called \textsb{determinant}\index{determinant}, which defines a mapping $\map{\gloref{determ}}{\bar{M}}{\cam{F}}$ where $\cam{F}$ is a real or complex field and $\det$ is described by the equality
...
\end{itemize}
Additionally, if $\mat{A}$ is invertible and $\mat{A}^{-1}=\mat{A}^\text{T}$, it is classified as \textsb{orthogonal}\index{matrix!orthogonal}. In this context, the property $\det{\mat{A}}^\text{T}=\det{\mat{A}}$ enables us to write 
\begin{equation}\label{eq:detMatOrtog}
1=\det{\mat{AA}^{-1}}=\det{\mat{AA}^\text{T}}= \det{\mat{A}}\det\mat{A} = (\det{\mat{A}}) ^2\,,
\end{equation}
which means that $\det{\mat{A}}=\pm 1$. Considering this result, matrix $\mat{A}$ is called a \textsb{proper orthogonal matrix}\index{matrix!proper orthogonal} when it has a positive determinant or an \textsb{improper orthogonal matrix}\index{matrix!improper orthogonal} when its determinant is negative. 
...
In the context of invertible matrices, a matrix whose inverse equals its conjugate transpose, that is, when  $\mat{U}^{-1}=\mat{U}^\dagger$ where $\mat{U}\in\bar{M}$, is called  \textsb{unitary}\index{matrix!unitary}. Considering the property $\det{\mat{H}}^\dagger=\overline{\det{\mat{H}}}$, we have
\begin{equation}
1=\det{\mat{UU}^{-1}}=\det{\mat{UU}^\dagger}= \det{\mat{U}}\overline{\det\mat{U}} = | \det{\mat{U}} |^2\,,
\end{equation}
from where we can conclude that $\det{\mat{U}}=\pm 1$. Thereby, given a matrix $\mat{A}\in\bar{M}$, the determinant $\det{\mat{U}\mat{A}}=\pm\det{\mat{A}}$; which reveals, except for an eventual sign, the \emph{neutrality} of the unitary matrix in the determinant of matrix products. If the unitary matrix elements are reals, it results that $\mat{U}^\dagger=\mat{U}^\text{T}=\mat{U}^{-1}$, or that it is orthogonal. Moreover, for any invertible matrix $\mat{A}$, the determinant of the inverse is the inverse of the determinant, according to the following expression:
\begin{equation}
...
Among other benefits, the trace function is a convenient tool to present what is known by \textsb{characteristic polynomial}\index{matrix!characteristic polynomial of}\label{pg:PolinomioCarac} of matrix: it is the function in operation $\map{g}{\cam{F}}{\cam{F}}$, whose rule is
\begin{equation}
