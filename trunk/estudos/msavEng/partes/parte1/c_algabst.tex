\chapter{Coleções e Relações}


Podemos dizer que é no mínimo inócuo estudar um determinado objeto matemático fundamental sem que ele esteja reunido com outros objetos matemáticos, compartilhando algum tipo de característica comum eleita para análise. Esse ajuntamento criterioso, que pode encerrar objetos e que define uma espécie de escopo, de abrangência, vamos denominá-lo coleção\index{coleção}. A partir daí, é possível estabelecer relações entre objetos coligidos, chamados \textsb{elementos}\index{elementos}, e também relações entre elementos de diferentes coleções. Tais relações\index{relação} são expressas por meio de \textsb{regras}\index{regra} que descrevem genericamente como elas devem ocorrer. Por conta de seu caráter genérico, essas regras, bem como coleções e elementos, são todos descritos segundo uma linguagem tipicamente algébrica.
...
A coleção algébrica menos restrita em termos conceituais é denominada conjunto\index{conjunto}\rodape{O conceito de conjunto aqui considerado carece do rigor necessário a abordagens matemáticas mais formais. A definição de conjunto pela Teoria Axiomática de Conjuntos está fora do escopo deste livro. Ver \aut{Cameron}\cite{cameron_1999_1}.}, que pode conter os mais variados elementos distintos, numa quantidade finita ou infinita deles. Assim, de uma coleção finita $a,b,b,a,c$ podemos definir o conjunto finito $\lch \gloref{eleA},b,c \rch$, formado por três elementos distintos\footnote{Um conjunto não admite repetição de elementos. Ver \aut{Shen \& Vereshchagin}\cite{shen_2002_1}.}. Tal distinção, aqui representada por letras diferentes, depende das características do elemento eleitas para análise (fig. \ref{fg:conjunto}). Importante ressaltar que a sequência descritiva dos elementos de um conjunto não altera sua definição: por exemplo, as definições $\gloref{conjA}\gloref{defPor}\lch \ele{a},\ele{b}\rch$ e $\con{A}:=\lch \ele{b},\ele{a}\rch$ são idênticas. Por falar em comparações, os conjuntos $\con{A}$ e $\con{B}$ são ditos iguais, $\con{A}=\con{B}$, se os mesmos elementos os constituem; caso contrário, eles são diferentes: $\con{A}\neq\con{B}$.


\begin{figure}[!ht]
	\centering
	\begin{center}
		\scalebox{.72}{\input{partes/parte1/figs/c_algabst/conjunto.pstex_t}}
	\end{center}
	\titfigura{Neste exemplo, dois elementos são idênticos quando tiverem os mesmos número de lados, altura e cor de preenchimento. Se para a obtenção de $\varDelta$, o critério de seleção de objetos tiver sido ``\texttt{triângulos}'', $\varDelta$ é uma coleção; se tiver sido ``\texttt{triângulos de altura > 1}'', $\varDelta$ também é uma coleção; mas, se tiver sido ``\texttt{triângulos brancos, pretos ou cinzas, de altura > 1}'', $\varDelta$ é um conjunto.}\label{fg:conjunto}
\end{figure}


O \textsb{conjunto vazio}\index{conjunto!vazio} $\gloref{emptyset}$ é aquele desprovido de elementos e sua existência reforça no conceito de conjunto a ideia de seleção regida por algum critério: o conjunto vazio é resultado de uma seleção em cujo critério nenhum objeto se enquadra. Por exemplo, na coleção de todos os seres humanos vivos, o conjunto daqueles que possuem idade superior a duzentos anos é vazio; algo que não ocorreria fosse a idade mínima cento e um anos. No âmbito específico dos conjuntos, a citada ``seleção regida por critério'' é denominada \textsb{especificação}\index{conjunto!especificação de}, cuja sintaxe matemática é a seguinte:
\begin{equation}
\texttt{\emph{conjunto} }:=\,\,\,\lch  \texttt{ \emph{seleção} } : \texttt{ \emph{critério} }  \rch\,.
\end{equation}
A partir desse padrão e considerando $\ele{x}$ um elemento qualquer,
\begin{equation}
\con{P}:=\lch x\gloref{pertence} \gloref{inte}: x \bmod 2 = 0 \rch
\end{equation}
é o conjunto dos números pares. A especificação anterior lê-se ``o conjunto $\con{P}$ é definido pelos elementos pertencentes ao conjunto dos inteiros tal que o resto de sua divisão por dois é zero''.


A relação entre um objeto e um conjunto consiste em averiguar, através da ideia intuitiva de pertencimento, se o primeiro é ou não elemento do segundo. Aliás, essa ideia é de fundamental importância na chamada Teoria Ingênua de Conjuntos\footnote{Ver \aut{Halmos}\cite{halmos_19742_1}.}. Dito isso, se $\ele{a}$ é elemento do conjunto $\con{A}$, dizemos que ele \textsb{pertence}\index{pertence} ao conjunto ou que $\ele{a}\in\con{A}\,$; caso contrário, dizemos que não pertence a $\con{A}$, ou $\ele{a}\gloref{notin}\con{A}\,$. Quando todos os elementos de um conjunto $\con{A}_1$ pertencem ao conjunto $\con{A}$, diz-se que $\con{A}_1$ é \textsb{subconjunto}\index{subconjunto} de $\con{A}$. Nessas condições, se $\con{A}_1\neq\con{A}$ diz-se que ele é \textsb{subconjunto próprio}\index{subconjunto!próprio} de $\con{A}$ ou que $\con{A}_1\gloref{subset}\con{A}$; mas, se $\con{A}_1$ for subconjunto de $\con{A}$ ou igual a $\con{A}$, ele é dito \textsb{subconjunto impróprio}\index{subconjunto!impróprio} de $\con{A}$, descrito $\con{A}_1\gloref{subseteq}\con{A}$. Assim, concluímos que todo conjunto é subconjunto impróprio dele mesmo. O conjunto $\emptyset$ não seria subconjunto impróprio de um conjunto qualquer $\con{A}$ se ele possuísse algum elemento que não pertencesse a $\con{A}$; como isso é impossível porque $\emptyset$ não possui elementos, então $\emptyset\subseteq\con{A}$.

As relações entre conjuntos também dependem da ideia de pertencimento. A \textsb{união}\index{conjunto!união} dos conjuntos $\con{A}_1$ e $\con{A}_2$ é o conjunto $\con{A}_1\gloref{cup}\con{A}_2$, ao qual todos os elementos de $\con{A}_1$ e $\con{A}_2$ pertencem. Representamos $\gloref{bigcup}\con{A}_i$ a união de $n$ conjuntos quaisquer $\con{A}_i$. O conjunto definido pelos elementos que pertencem tanto a $\con{A}_1$ quanto a $\con{A}_2$ é a \textsb{intersecção}\index{conjunto!intersecção} $\con{A}_1\gloref{cap}\con{A}_2$. Em outras palavras, se o elemento $x \in \con{A}_1\cap\con{A}_2$ então $\lpa x \in \con{A}_1\rpa \gloref{wedge} \lpa x \in \con{A}_2\rpa$, onde $\wedge$ significa ``E'' em português. De maneira similar à união, $\gloref{bigcap}\con{A}_i$ é como escrevemos a intersecção de $n$ conjuntos $\con{A}_i$. Utilizando diagramas de Venn (figura \ref{fg:interDistrib}), pode-se constatar que a intersecção é distributiva na união, ou seja, que
\begin{equation}
A_1\cap\lpa A_2\cup A_3\rpa=\lpa A_1\cap A_2\rpa\cup\lpa A_1\cap A_3\rpa.
\end{equation}
Quando um conjunto $\con{A}_1\cap\con{A}_2$ é vazio dizemos que $\con{A}_1$ e $\con{A}_2$ são \textsb{disjuntos}\index{conjuntos!disjuntos}. Nessa situação, se $x\in\con{A}_1\cup\con{A}_2$ então
$x\in\con{A}_1\gloref{vee} x\in\con{A}_2$, onde o símbolo $\vee$ significa ``OU'' em termos literais.
\begin{figure}[!ht]
\centering
\begin{center}
\scalebox{.72}{\input{partes/parte1/figs/c_algabst/interseccao.pstex_t}}
\end{center}
\titfigura{A intersecção é distributiva na união.}\label{fg:interDistrib}
\end{figure}

O conjunto \textsb{diferença}\index{conjunto!diferença} $\con{A}_1\gloref{setminus}\con{A}_2$ é composto pelos elementos de $\con{A}_1$ que não pertencem a $\con{A}_2$. Se o conjunto $\con{A}_1$ é subconjunto de $\con{A}$, então o conjunto $\gloref{complementar}_1:=\con{A}\setminus\con{A}_1$ é dito \textsb{complementar}\index{conjunto!complementar} de $\con{A}_1$ em $\con{A}$. Com isso, se $A_1$ e $A_2$ forem subconjuntos de $A$, é possível constatar, também por diagramas de Venn, que o complemento da união $\lpa A_1\cup A_2\rpa' = A_1'\cap A_2'$, que o complemento da intersecção $\lpa A_1\cap A_2\rpa' = A_1'\cup A_2'$ e que a diferença $A_1\setminus A_2= A_1\cap A_2'$. A partir dessas igualdades, podemos dizer que na união a diferença é distribuída conforme
...
{\footnotesize
\begin{proof}
Sejam os conjuntos $A_1,A_2,A_3\subset A$. Temos então que $A_1\setminus\lpa A_2\cup A_3\rpa=A_1\cap\lpa A_2\cup A_3\rpa'=A_1\cap A_2'\cap A_3'$. Temos também que $\lpa A_1\setminus A_2\rpa\cap \lpa A_1\setminus A_3\rpa=A_1\cap A_2'\cap A_1\cap A_3'=A_1\cap A_2'\cap A_3'$. Concluímos então a igualdade \eqref{eq:distribUniao}. Para provarmos \eqref{eq:distriInters}, eis o seguinte:
\begin{align}
...
\section{Sequências}\index{sequência}

Quando os elementos de uma coleção precisam estar ordenados, diz-se em Matemática que ela é uma sequência.
Para tal ordenação, os elementos que constituem a sequência possuem suas posições respectivas, identificadas sequencialmente por números inteiros não negativos, definidores do conjunto $\gloref{naturais}$, chamados \textsb{índices}\index{índice}. Assim, todo elemento que constitui uma sequência possui uma determinada posição, rotulada por um índice único. A igualdade de duas sequências é alcançada quando ambas são formadas pelos mesmos elementos igualmente posicionados. Na sequência $\lpa a,b,c\rpa$ por exemplo, o elemento $b$ tem posição 2, caso o índice inicie em 1; disso resultando $\lpa a,b,c\rpa\neq\lpa a,c,b\rpa$. Por conta dessa necessária característica posicional, uma sequência pode se constituir de elementos indistintos: a coleção $a,b,c,a$ é válida como sequência $\lpa a,b,c,a\rpa$, onde os dois elementos $a$ possuem a primeira e a quarta posições.

Se uma sequência é finita, ela é denominada \textsb{tupla}\index{tupla}. A tupla de um ú\-ni\-co elemento é denominada \textsb{mônada}\index{mônada}, de dois elementos \textsb{dupla}\index{dupla}, de três elementos \textsb{tripla}\index{tripla}, de quatro elementos \textsb{quádrupla}\index{quádrupla}, de $n$ elementos \textsb{ênupla}\index{ênupla} ou $n$-tupla, onde $n\in\mathbb{N}$. Denomina-se \textsb{transposição}\index{transposição} a troca de posições entre pares de elementos de uma $n$-tupla, $n>1$, mantendo-se os demais inalterados se for o caso. A \textsb{sequência vazia}\index{sequência!vazia} existe e é chamada $0$-tupla.
...
Seja então uma coleção $H$ cujos elementos são escalares, arranjados segundo o mapeamento $\map{\fun{h}}{\crt{N}{q}}{\mathcal{R}}$, onde a função $h$ é chamada \textsb{endereçamento}\index{array!endereçamento de} e cada $N_i = \lch 1,2,\cdots,n_i \rch$ é um subconjunto finito de $\mathbb{N}^*$ formado por números ordinais\footnote{Números inteiros destinados a rotular posições em sequência.}. Nessas condições, chamamos a coleção $H$ de array\rodape{Na falta de um termo adequado em português, optou-se pela denominação inglesa.}, representado por $\gloref{array}$, cujos elementos escalares $\fua{\fun{h}}{( i_1,\cdots,i_q)}$ são escritos nas notações $h_{i_1\cdots i_q}$ ou  $\gloref{elArray1}$, onde os índices subscritos explicitam a posição do escalar no arranjo. A descrição das perspectivas ou a \textsb{dimensão}\index{array!dimensão de} de um array é dada pela ênupla $(n_1 , \cdots , n_q)$, ou pela representação mais usual $n_1\times\cdots\times n_q\,$.
