...
Therefore, from axiom ii on page \pageref{ax:pointSpaces}, equalities
\begin{equation*}
\epu{S}_a=\{\vto{x}\oplus a:\forall \vto{x}\in S_\cam{F}\}=\{(\vto{x}+\vto{w})\oplus b:\forall \vto{x}\in S_\cam{F}\} 
\end{equation*}
and $\epu{V}_b=\{\vto{y}\oplus b:\forall \vto{y}\in V_\cam{F}\}$ lead to $\eamsd{S}{F}{a}{n}\subseteq\eamsd{V}{F}{b}{r}$ if $S_\cam{F}\subseteq V_\cam{F}$ since $\{\vto{x}+\vto{w}:\forall \vto{x}\in S_\cam{F}\}\subseteq\{\vto{y}:\forall \vto{y}\in V_\cam{F}\}$ or $S_\cam{F} \cup \{\vto{w}\}\subseteq V_\cam{F}$ . For the case of $V_\cam{F}\subseteq S_\cam{F}$, we similarly arrive at $\eamsd{V}{F}{b}{r}\subseteq\eamsd{S}{F}{a}{n}\,$. For the third property, let's verify the condition where $a\notin \eamsd{V}{F}{b}{r}$. In this case, there is no vector in $V_\cam{F}$ relating the pair of points $(b,a)$, that is, $\vv{ba}\notin V_\cam{F}$. Now, supposing that $\eamsd{S}{F}{a}{n}\cap\eamsd{V}{F}{b}{r}=\emptyset$ is valid, we have for all $\vto{x}\in S_\cam{F}$ and $\vto{y}\in V_\cam{F}$ that 
\begin{align*}
\vto{x}\oplus a &\neq \vto{y}\oplus b\\
-\vto{x}\oplus(\vto{x}\oplus a) &\neq -\vto{x}\oplus(\vto{y}\oplus b)\\
a &\neq (\vto{y}-\vto{x})\oplus b\,,
\end{align*}
which confirms our supposition since it is indeed true that we can't obtain $a$ from $b$.
\end{proof}}
