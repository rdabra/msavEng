...
\end{itemize}
Figure \ref{fg:paralelo} depicts a line and plane parallel to each other for the case of a fi\-ve-di\-men\-sio\-nal real affine space. Now, we say that hyperplanes $\eamsd{W}{F}{c}{r}$ and $\eamsd{S}{F}{a}{n}$ are \textsb{perpendicular}\index{perpendicularity}, represented by $\eamsd{W}{F}{c}{r}\perp\eamsd{S}{F}{a}{n}\,$, if vectors of $W_\cam{F}$ and $V_\cam{F}$ do not have an incidence interrelationship, that is, if $W_\cam{F}\perp V_\cam{F}$. Geometrical representation of perpendicularity is more intuitive in the context of affine Euclidean spaces through the concept of angle, which we shall present later in this section.
...
\end{proof}}
 ...
 Considering previous conditions, given a point $o\in\eamsd{S}{F}{a}{n}$ and a basis $B=\{\vto{v}_1,\cdots,\vto{v}_n\}$ of $S_\cam{F}$, where $n<m$, we call double $(o,B)$ an \textsb{affine coordinate system}\index{affine!coordinate system} of $\eamsd{S}{F}{a}{n}$ since point space $\epu{S}_a$ can be obtained from $o$ and $B$ as follows:
...
From the previous corollary and equality \eqref{eq:projgeo}, if there is no geometric projection between $\vv{ox}$ and $\vv{oy}$, that is, $\vartheta=0$, we conclude that the angle $\theta=\pi(k+1/2)$, $k=0,1,..\,$. Thereby, if line $\eamsd{W}{\real}{b}{1}$ and plane $\eamsd{V}{\real}{a}{2}$, subspaces of affine Euclidean space $\eamd{S}{\real}{5}$, are perpendicular, we can represent them as depicted on figure XX.


%copiar a figura \label{fg:paralelo} e fazer perpendicular
...
Considering previous conditions, an affinity $\mathit{f}$ is said to be centered at point $a$, represented by $\mathit{f}_a$, when $\mathit{g}$ is the identity function, that is, when point $\fua{\mathit{f}_a}{\vto{x}\oplus a}=\fua{\vtf{f}}{\vto{x}}\oplus a$. From this equality, when $\fua{\vtf{f}}{\vto{x}}=\vto{u}+\vto{x}$, we have $\fua{\mathit{f}_a}{\vto{x}\oplus a}=\vto{u}\oplus(\vto{x}\oplus a)$ and then affinity $\mathit{f}_a$ is called a \textsb{translation}\index{translation}, represented by $\mathit{t}_\vto{u}$, described by the rule
...
\noindent Given an arbitrary vector $\vto{v}\in U_\cam{F}$, from equality $\fua{\mathit{t}_\vto{v}\circ\mathit{t}_\vto{u}}{x}=(\vto{v}+\vto{u})\oplus x$,
...
Still considering previous conditions, an affinity $\mathit{r}$ is called \textsb{isometric}\index{affinity!isometric} when its correspondent linear operator $\vtf{r}$ is isometric, that is, $\vtf{r}\in\grc{I}{\cam{F}}{U}$. In the context of Euclidean spaces, we already know that an isometric operator is also an orthogonal operator\footnote{See p. \pageref{prg:IsometOrth}.}, that is, the isometric group $\grc{I}{\real}{U}$ equals the orthogonal group $\grc{O}{\real}{U}$, whose elements preserve inner product and have determinant $\pm 1$. The centered isometric affinity $\mathit{r}_a$, described by $\fua{\mathit{r}_a}{\vto{x}\oplus a}=\fua{\vtf{r}}{\vto{x}}\oplus a$, is called a \textsb{rotation}\index{rotation} if isometric operator $\vtf{r}$ is proper orthogonal, or $\vtf{r}\in\grc{O^+}{\real}{U}$; otherwise, it is called a \textsb{reflection}\index{reflection}. The affinity tensor $\tnr{R}$ of a rotation is called a \textsb{rotation tensor}\index{rotation!tensor} and, similarly, tensor $\overline{\tnr{R}}$ of a reflection is called a \textsb{reflection tensor}\index{reflection!tensor}. Now, recalling the concept of unimodular tensors presented in section \ref{sec:TensSpac}, let $(U_\real,\tnr{A}_B)$ be an $m$-dimensional Euclidean space oriented by the basis $B=\{\vun{u}_1,\cdots,\vun{u}_m\}$ where $\tnr{A}_B\in\etng{m}{\real}{U^m}$ is a unimodular tensor. Considering $\mathit{r}_a$ a rotation and $\overline{\mathit{r}}_a$ a reflection, since every pair of Euclidean orthonormal basis can be related through an orthogonal operator\footnote{See p. \pageref{prg:orthBases}.}, we conclude that rotation  $C=\{\fua{\vtf{r}}{\vun{u}_1},\cdots,\fua{\vtf{r}}{\vun{u}_m}\}$ and reflection $\overline{C}=\{\fua{\overline{\vtf{r}}}{\vun{u}_1},\cdots,\fua{\overline{\vtf{r}}}{\vun{u}_m}\}$ of basis $B$ are positively and negatively oriented bases respectively because $\fua{\tnr{A}_C}{\vun{u}_1,\cdots,\vun{u}_m}=-\fua{\tnr{A}_{\overline{C}}}{\vun{u}_1,\cdots,\vun{u}_m}=1$ from definition \eqref{eq:basisAltern}. In order to geometrically represent in a simple way basis rotation and reflection, we specify the affine Euclidean space in question to be two-dimensional, that is, $m=2$. Thereby, for the basis vectors $\vun{u}_1$ and $\vun{u}_2$ the smallest angle $\theta$ between them leads to equalities $0=\vun{u}_1\cdot\vun{u}_2=\|\vun{u}_1\|\|\vun{u}_2\|\cos\theta=\cos\theta$, and since orthogonal functions preserve inner product, then $\theta$ is also the smallest angle between $\fua{\vtf{r}}{\vun{u}_1}$ and $\fua{\vtf{r}}{\vun{u}_2}$, or between $\fua{\overline{\vtf{r}}}{\vun{u}_1}$ and $\fua{\overline{\vtf{r}}}{\vun{u}_2}$, that is, $0=\vun{u}_1\cdot\vun{u}_2=\fua{\vtf{r}}{\vun{u}_1}\cdot\fua{\vtf{r}}{\vun{u}_2}=\fua{\overline{\vtf{r}}}{\vun{u}_1}\cdot\fua{\overline{\vtf{r}}}{\vun{u}_2}=0$. From these angle and size preserving features, we can intuitively represent an anticlockwise acute angle $\phi$ rotation and reflection of basis $B$ according to the following figure XX.


%Figura com tres conjuntos do dia 03/05

\noindent However, for the mathematical spirit, intuition is necessary but not sufficient. Since both isometric affinities on the figure preserve angles and sizes, how do we verify mathematically that the function on the right is indeed a rotation and on the left a reflection? By proving that the rotated and reflected bases are positively and negatively oriented respectively in relation to $(U_\real,\tnr{A}_B)$, that is, by calculating $\fua{\tnr{A}_C}{\vun{u}_1,\vun{u}_2}$ and $\fua{\tnr{A}_{\overline{C}}}{\vun{u}_1,\vun{u}_2}$. From definition \eqref{eq:basisAltern} and equality \eqref{eq:geoProdInt}, development
\begin{align*}
\fua{\tnr{A}_{C}}{\vun{u}_1,\vun{u}_2}&=\sum_{i=1}^2\sum_{j=1}^2\fua{\vtf{f}^\con{C}_{i}}{\vto{x}_1}\fua{\vtf{f}^\con{C}_{j}}{\vto{x}_2}\epsilon_{ij}\\
&=(\vto{x}_1\cdot\fua{\vtf{r}}{\vun{u}_1})(\vto{x}_2\cdot\fua{\vtf{r}}{\vun{u}_2})-(\vto{x}_1\cdot\fua{\vtf{r}}{\vun{u}_2})(\vto{x}_2\cdot\fua{\vtf{r}}{\vun{u}_1})\\
...
Considering a rotation $\mathit{r}_a$ , in the context of a three-dimensional Euclidean affine oriented space $(U_\real,\tnr{A}_B)$, we now search for a vector $\vto{u}\in U_\mathcal{\real}$, usually called an \textsb{axial vector}\index{vector!axial}, that observes the property $\fua{\vtf{r}}{\vto{u}}=\fua{\vtf{r}}{-\vto{u}}$ because certain physical quantities, such as angular momentum, are accordingly described by axial vectors. However, with the algebraic apparatus presented so far, there is no axial vector $\vto{u}\neq\vto{0}$ on $U_\real$ and then we need to define a binary operation whose resulting vector always observes this new property. Then, given two arbitrary vectors $\vto{x},\vto{y}\in U_\real$, from operation $\map{\times}{U_\real^2}{U_\real}$, vector $\vto{x}\times\vto{y}$, called the \textsb{cross product}\index{product!cross} of vectors $\vto{x}$ and $\vto{y}$, is axial if  
\begin{equation}
\vto{x}\times\vto{y} = \tnr{A}_B\odot_2 (\vto{x}\otimes\vto{y})\,.
\end{equation}
The triple $(U_\real,\tnr{A}_B,\times)$ is an example of what is known as algebra over real numbers, whose generalized concept, founded on multivectors defined by an exterior product, is out of the scope of this elementary study. For an additional arbitrary vector $\vto{z}\in U_\real$ and scalars $\alpha,\beta\in\real$, the cross product has the following properties. 
\begin{itemize}
\setlength\itemsep{.1em}
\item[i.] Self cross product: $\vto{x}\times\vto{x}=\vto{0}$;
\item[ii.] Anticommutativity: $\vto{x}\times\vto{y}=-(\vto{y}\times\vto{x})$;
\item[iii.] Distributivity over addition: $\vto{x}\times(\vto{y}+\vto{z})=(\vto{x}\times\vto{y})+(\vto{x}\times\vto{z})$;
\item[iv.] Scalar multiplication: $\alpha\beta(\vto{x}\times\vto{y})=(\alpha\vto{x})\times(\beta\vto{y})=(\beta\vto{x})\times(\alpha\vto{y})$.
\item[....]
\end{itemize}
...
% - DEFINIR PRODUTO EXTERNO DE N VETORES
% - DIZER QUE PRODUTO EXTERNO DE N VETORES DO ESPAÇO v PRODUZ UMA ENTIDADE CHAMADA SIMPLE MULTIVETOR OU SIMPLE N-VETOR QUE PERTENCE AO ESPACO AV
% - DEFINIR BIVETOR E TRIVETOR SIMPLES EM ESPAÇO EUCLIDIANO E DIZER QUE O PRODUTO EXTERNO NESSE CASO É CHAMADO DE CROSS PRODUCT
% - DIZER QUE O TAMANHO DE UM BIVETOR SIMPLES NO CONTEXTO DE UM ESPACO EUCLIDIANO É A AREA DO PARALELOGRAMO DEFINIDO POR SEUS ARGUMENTOS
% - DIZER QUE O TAMANHO DE UM TRIVETOR SIMPLES NO CONTEXTO DE UM ESPACO EUCLIDIANO É O VOLUME DO PARALELEPIPEDIO DEFINIDO POR SEUS AGUMENTOS
