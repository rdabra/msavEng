...
\end{itemize}
In this context, the triple $(U_\cam{F},\epu{U},\oplus)$ is called an affine space, henceforth represented by $\eam{U}{F}$, which will also be used to refer to the point space $\epu{U}$, as we adopted similarly for other cases in order to simplify notation. Moreover, vector space $U_\cam{F}$ is usually called the \textsb{direction space}\index{space!direction} of affine space $\eam{U}{F}$. Therefore, we can say that an affine space is defined by a direction space, a point space and a group action. From axiom iii above, every double of points uniquely identifies a certain vector: if $\vto{u}\oplus a=b$, we define that double $(a,b)$ identifies vector $\vto{u}$, when this vector is represented by $\vv{ab}$. \emph{The converse is not true: a certain vector can be identified by different doubles of points because $\vto{u}\oplus x=y$ for arbitrary $x\in\eam{U}{F}$ and then $\vv{ab}=\vv{xy}$}. Now, from axioms i and ii, considering point $\vto{v}\oplus a=c$, we conclude that $-\vto{v}\oplus c=a$, and then vector $\vto{v}=\vv{ac}=-\vv{ca}$. From this characteristic relationship between points and vectors, the classical pictorial representation of a vector identified by a double of points $(a,b)$ as an arrow whose tail ``starts'' on $a$ and head points to $b$ arises naturally (figure \ref{fg:vetorSeta}).
...
\begin{equation}
\epu{S}_a=\{\vto{x}\oplus a:\forall \vto{x}\in V_\cam{F}\}= \{(\vto{x}+\vv{oa})\oplus o:\forall \vto{x}\in \spn{(B)}\}\,.
\end{equation}
...
\begin{equation}
\fua{\zeta}{\vv{ox},\alpha\vv{ox}}=\dfrac{\overline{\alpha}(\vv{ox}\cdot \vv{ox})}{|\alpha|\|\vv{ox}\|}=\dfrac{\overline{\alpha}\|\vv{ox}\|}{|\alpha|}=\|\vv{ox}\|\sqrt{\dfrac{\overline{\alpha}}{\alpha}}\,,\,\forall\alpha\in\{\cam{F}\setminus 0\}.
\end{equation}
For the specific case of affine Euclidean spaces, these equalities lead to $\fua{\zeta}{\vv{ox},\alpha\vv{ox}}=\|\vv{ox}\|$ for all $\alpha\in\{\real\setminus 0\}$, where the projection does not depend on the real value $\alpha$. Concerning projections on the context of affine Hilbert spaces, there is a property, presented on the following theorem, that will be important for future geometric concepts.
...
Now, for an affine Euclidean space $\eamd{S}{R}{n}$, if $\theta$ is the angle defined by vectors $\vv{ox},\vv{oy}\in S_\cam{R}$, the linear combination  $\alpha\vv{oy}/\|\vv{oy}\|$ and the scalar $\alpha\in\real$ are called respectively the vector and scalar geometric projections of $\vv{ox}$ onto $\vv{oy}$ when 
\begin{equation}
\alpha:=\|\vv{ox}\|\cos\theta\,,
\end{equation}
which can be visualized in figure XX. In this context, the following corollary establishes a connection between geometric and algebraic projections.



\begin{mcoro}{Projection of Euclidean Vectors}{projEuclid}
Considering an affine Euclidean space $\eamd{S}{R}{n}$, if $\alpha$ is the scalar geometric projection of $\vv{ox}\in S_\cam{R}$ onto $\vv{oy}\in S_\cam{R}$, then 
$-\alpha\leq\fua{\zeta}{\vv{ox},\vv{oy}}\leq+\alpha$.
\end{mcoro}

{\footnotesize
\begin{proof}
For the case of affine Euclidean spaces, projection $\fua{\zeta}{\vv{ox},\vv{oy}}$ is always a real number, where concepts of ``greater than'' and ``less than'' are admissible. Thereby, from definition \ref{eq:projection} and previous theorem, we have for all $\theta\in\real$:
\begin{eqnarray*}
&-\|\vv{ox}\|\leq\fua{\zeta}{\vv{ox},\vv{oy}}\leq+\|\vv{ox}\|&\\
&-\cos\theta\|\vv{ox}\|\leq\cos\theta\fua{\zeta}{\vv{ox},\vv{oy}}\leq+\cos\theta\|\vv{ox}\|&\\
&-\alpha\leq\cos\theta\fua{\zeta}{\vv{ox},\vv{oy}}\leq+\alpha.&
\end{eqnarray*}
Choosing $\theta=0$, we prove the theorem.
\end{proof}}

% FAZER FIGURA SOBRE PROJECOES GEOMÉRICAS


\begin{comment}
\begin{mcoro}{Projection of Euclidean Vectors}{projEuclid}
In the context of an affine Euclidean space $\eamd{S}{R}{n}$, where $\hat{\vv{ox}},\hat{\vv{oy}}\in S_\cam{R}$ are the normalized vectors of $\vv{ox}$ and $\vv{oy}$  respectively, it is valid that $-1\leq\hat{\vv{ox}}\cdot\hat{\vv{oy}}\leq+1$.
\end{mcoro}

{\footnotesize
\begin{proof}
For the case of affine Euclidean spaces, projection $\fua{\zeta}{\vv{ox},\vv{oy}}$ is always a real number, where concepts of ``greater than'' and ``less than'' are admissible. Thereby, from definition \ref{eq:projection}, the following development proves the corollary:
\begin{eqnarray*}
&-\|\vv{ox}\|\leq\fua{\zeta}{\vv{ox},\vv{oy}}\leq+\|\vv{ox}\|&\\
&-1\leq\dfrac{\fua{\zeta}{\vv{ox},\vv{oy}}}{\|\vv{ox}\|}\leq+1&\\
&-1\leq\dfrac{\vv{ox}\cdot\vv{oy}}{\|\vv{ox}\|\|\vv{oy}\|}\leq+1\,.&
\end{eqnarray*}
\end{proof}}
\end{comment}

From the previous corollary, it is possible to define an angle between vectors 
