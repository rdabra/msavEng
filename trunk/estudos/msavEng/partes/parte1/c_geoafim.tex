...
If $\eamd{U}{F}{m}$ is an affine space where $\eamsd{S}{F}{a}{n}$ is one of its subspaces and $k\notin \eamsd{S}{F}{a}{n}$ one of its points, there is one and only one subspace $\eamsd{V}{F}{b}{n}\parallel\eamsd{S}{F}{a}{n}$, where $k\in\eamsd{V}{F}{b}{n}$.
\end{mteo}

{\footnotesize
\begin{proof}
If $\eamsd{S}{F}{a}{n}$ exists, so does $\eamsd{S}{F}{b}{n}$, where $b\neq a$. Since these subspaces are obviously parallel, the existence of subspace $\eamsd{V}{F}{b}{n}=\eamsd{S}{F}{b}{n}$ is proved. Uniqueness is verified by the following rationale: considering both subspaces $\eamsd{V}{F}{b}{n}$ and $\eamsd{W}{F}{c}{n}$ parallel to $\eamsd{S}{F}{a}{n}$, with point $k\in\eamsd{V}{F}{b}{n}$ and $k\in\eamsd{W}{F}{c}{n}$, let's suppose 
$\eamsd{W}{F}{c}{n}\neq\eamsd{V}{F}{b}{n}$ such that $\eamsd{W}{F}{c}{n}\cap\eamsd{V}{F}{b}{n}=\{k\}$. Thereby, we can say that  $\vto{x}\oplus k\neq\vto{y}\oplus k$, for all non zero $\vto{x}\in V_\cam{F}$ and $\vto{y}\in W_\cam{F}$. Therefore, we can write that $k\neq(\vto{y}-\vto{x})\oplus k$ since $S_\cam{F}$, $V_\cam{F}$ and $W_\cam{F}$ are all subsets of $U_\cam{F}$. But, since the previous inequality is valid for any non zero $\vto{x}$ and $\vto{y}$, by specifying $\vto{y}=\vto{x}$ we arrive at the absurd inequality $k\neq k$, which invalidates supposition $\eamsd{W}{F}{c}{n}\neq\eamsd{V}{F}{b}{n}$ or supposition $\eamsd{W}{F}{c}{n}\cap\eamsd{V}{F}{b}{n}=\{k\}$. If $n=1$, for example, the subspaces spaces involved can be different lines that share a point $k$ then it is perfectly tru
\end{proof}}
