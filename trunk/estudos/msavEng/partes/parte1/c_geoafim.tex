...
Considering a rotation $\mathit{r}_a$ and a rotoreflection $\overline{\mathit{r}}_a$ where $\overline{\vtf{r}}=-\vtf{r}$, in the context of a three-dimensional Euclidean affine oriented space $(U_\real,\tnr{A}_B)$, we now search for a vector $\vto{u}\in U_\mathcal{\real}$, usually called an \textsb{axial vector}\index{vector!axial}, that observes the property $\fua{\vtf{r}}{\vto{u}}=-\fua{\vtf{r}}{\vto{u}}$ because certain physical quantities, such as angular momentum, are accordingly described by axial vectors. However, with the algebraic apparatus presented so far, there is no axial vector $\vto{u}\neq\vto{0}$ on $U_\real$ and then we need to define a binary operation whose resulting vector always observes this new property. Given two arbitrary vectors $\vto{x},\vto{y}\in U_\real$, from operation $\map{\times}{U_\real^2}{U_\real}$, vector $\vto{x}\times\vto{y}$, called the \textsb{cross product}\index{product!cross} of $\vto{x}$ and $\vto{y}$, is axial if  
\begin{equation}
