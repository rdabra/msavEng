...
\end{equation} 
It is straightforward to note that since $\vtf{f}$ and $\mathit{f}$ are bijections, so results affinity $\mathit{f}$. Among other properties, it is possible to obtain that every affinity preserves parallelism and hyperplane dimensionality; in other words, parallel hyperplanes on the  domain remain parallel on the image and $r$-dimensional hyperplanes on the domain, $r\leq n$, remain $r$-dimensional on the image: for example, lines are mapped to lines, planes are mapped to planes and so on. Morever, an affinity $\mathit{f}$ is said to be centered at point $a$, represented by $\mathit{f}_a$, when $\mathit{g}$ is the identity function, that is, when $\fua{\mathit{f}_a}{\vto{v}\oplus a}=\fua{\vtf{f}}{\vto{v}}\oplus a$. 
