...
\end{equation}
From this definition, if $(V_\cam{F})_i$ is a Hilbert space and $B_i=\{\vun{v}_1^{(i)},\cdots,\vun{v}_{n_i}^{(i)}\}$ one of its orthonormal bases, the following is valid for $\alpha=\fua{\tnr{I}}{\vto{x}_1,\cdots,\vto{x}_p,\vto{y}_1,\cdots,\vto{y}_p}$:
\begin{align*}
\alpha&=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\sum_{{j_1}=1}^{n_1}\cdots\sum_{{j_p}=1}^{n_p}\prod_{k=1}^p\fua{\vtf{f}^\con{B_k}_{i_k}}{\vto{x}_k}\fua{\vtf{f}^\con{B_k}_{j_k}}{\vto{y}_k}\fua{\tnr{I}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_p}^{(p)},{{\vun{v}_{j_1}}}^{(1)},\cdots,\vun{v}_{j_p}^{(p)}}\\
...
\begin{equation}
\vtf{f}_{j_1\cdots j_m}^{B}(\tnr{I})= \fua{\tnr{I}}{\vun{v}_{j_1}^{(1)},\cdots,\vun{v}_{j_p}^{(p)},\vun{v}_{j_1}^{(1)},\cdots,\vun{v}_{j_p}^{(p)}}=\prod_{k=1}^p\fua{\vtf{f}_{i_k}^{B_k}}{\vun{v}_{j_k}^{(k)}}=\prod_{k=1}^p\delta_{i_kj_k}\,.
\end{equation}
...
&=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\prod_{k=1}^p\vun{v}_{i_k}\cdot\vto{x}_k\vun{v}_{i_1}^*\otimes\cdots\otimes\vun{v}_{i_p}^*\nonumber\\
&=(\sum_{{i_1}=1}^{n_1}\fua{\vtf{f}_{i_1}^{B_1}}{\vto{x}_1}\vun{v}_{i_1})^*\otimes\cdots\otimes(\sum_{{i_p}=1}^{n_1}\fua{\vtf{f}_{i_p}^{B_p}}{\vto{x}_p}\vun{v}_{i_p})^*\nonumber\nonumber\\
...
&=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\fua{\tnr{A}\otimes\tnr{B}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_p}^{(p)},\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_p}^{(p)}}\nonumber\\
&=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\fua{\tnr{A}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_p}^{(p)}}\fua{\tnr{B}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_p}^{(p)}}\nonumber\\
...
\begin{align*}
\fua{\vtf{\psi}}{\tnr{X}}&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_p=1}^{n_p} \vtf{f}_{i_1\cdots i_p}^{C}(\fua{\vtf{\psi}}{\tnr{X}})\,\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{C_p}\nonumber\\
&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_p=1}^{n_p}\sum_{j_1=1}^{l_1}\cdots\sum_{j_m=1}^{l_m}\vtf{f}_{i_1\cdots i_p}^{C}(\fua{\vtf{\psi}}{\tnr{X}})\,\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{C_p}\nonumber\\
&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_p=1}^{n_p}\sum_{j_1=1}^{l_1}\cdots\sum_{j_m=1}^{l_m}\underbrace{\vtf{f}_{i_1\cdots i_p}^{C}(\fua{\vtf{\psi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}})}_{{\maf{\tnr{\psi}}{B}{C}}_{i_1\cdots i_p j_1\cdots j_m}}\vtf{f}_{j_1\cdots j_m}^{B}(\tnr{X})\,\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{C_p}\,,\nonumber
\end{align*}
leads to
\begin{equation}
\mav{\fua{\tnr{\psi}}{\tnr{X}}}{C}=\maf{\tnr{\psi}}{B}{C}\mav{\tnr{X}}{B}\,,
\end{equation}
...
\begin{equation}
\mav{[\fua{\alpha\tnr{\psi}+\beta\tnr{\varphi}]}{\tnr{X}}}{C}=(\alpha\maf{\tnr{\psi}}{B}{C}+\beta\maf{\tnr{\varphi}}{B}{C})\mav{\tnr{X}}{B}\,.
\end{equation}
...
\begin{equation}
\mav{\tnr{\gamma}\circ\tnr{\psi}(\tnr{X})}{D}=\maf{\tnr{\gamma}}{C}{D}\mav{\fua{\tnr{\psi}}{\tnr{X}}}{C}=\maf{\tnr{\gamma}}{C}{D}\maf{\tnr{\psi}}{B}{C}\mav{\tnr{X}}{B}\,.
\end{equation}
As array $\mav{\fua{\tnr{\gamma}\circ\tnr{\psi}}{\tnr{X}}}{D}=\maf{(\tnr{\gamma}\circ\tnr{\psi})}{B}{D}\mav{\vto{u}}{B}$, we can state that
\begin{equation}
\maf{(\tnr{\gamma}\circ\tnr{\psi})}{B}{D}=\maf{\tnr{\gamma}}{C}{D}\maf{\tnr{\psi}}{B}{C}\,.
\end{equation}
...
\end{alignat}
Now, we shall verify if it is possible to establish a relation between a Hilbert-adjoint tensor function and a conjugate transpose array and also between a transpose function and a transpose array. If a function $\tnr{\psi}\in\evl{\cam{F}}{\ete{\cam{F}}{U^{\times p}}}{\ete{\cam{F}}{V^{\times p}}}$ has a Hilbert-adjoint, sets $B$ and $D$ are bases of $\ete{\cam{F}}{U^{\times p}}$ and $\ete{\cam{F}}{V^{\times p}}$ respectively, then, from the definition of Hilbert-adjoint tensor function, 
\begin{align}
\fua{\tnr{\psi}}{\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}}\cdot \vtf{f}_{j_1}^{D_1}\otimes\cdots\otimes\vtf{f}_{j_p}^{D_p} &= \overline{\tnr{\psi}^\dagger(\vtf{f}_{j_1}^{D_1}\otimes\cdots\otimes\vtf{f}_{j_p}^{D_p})\cdot\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}}\nonumber\\
\vtf{f}^D_{j_1\cdots j_p}[\fua{\tnr{\psi}}{\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}}] &=\overline{\vtf{f}^B_{i_1\cdots i_p}[\fua{\tnr{\psi}^\dagger}{\vtf{f}_{j_1}^{D_1}\otimes\cdots\otimes\vtf{f}_{j_p}^{D_p}}]}\nonumber\\
\overline{\vtf{f}^D_{i_1\cdots i_p}[\fua{\tnr{\psi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_p}^{B_p}}]^\text{T}} &=\vtf{f}^B_{i_1\cdots i_p}[\fua{\tnr{\psi}^\dagger}{\vtf{f}_{j_1}^{D_1}\otimes\cdots\otimes\vtf{f}_{j_p}^{D_p}}]\nonumber\\
...
The notion of invariance is of paramount importance in Physics because the fundamental description of mechanical phenomena must be not only qualitatively immune to arbitrary changes of reference frame but also quantitatively indifferent when such changes are of orthogonal type. From a constitutive viewpoint, materials have their invariance too, when some or all of their inherent mechanical properties present themselves indistinctly on certain directions, that is, the overall mechanical behavior of a portion of matter in study is the same when this portion is subjected to specific motions. A tensor, as an algebraic entity that suitably embodies a physical magnitude, can be used to describe accordingly a mechanical measurement or a constitutive material property, including their eventual invariance requirements which we referred to. Thereby, it is the purpose of this section to briefly develop in mathematical terms this intuitive concept of invariance applied to tensors.
