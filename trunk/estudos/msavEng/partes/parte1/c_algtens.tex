...
\end{equation}
\item[ii.] If basis $B=\{\vto{u}_1,\cdots,\vto{u}_m\}$, then ${\tnr{A}_B}_{(r,s)}=-\tnr{A}_B$ and $\fua{\tnr{A}_B}{\vto{u}_1,\cdots,\vto{u}_m}=1$. 
...
\end{align*}
From property ii, scalar $\tnr{A}_B(\vto{u}_1,\cdots,\vto{u}_m)$ can never be zero and so scalar $\tnr{A}(\vto{u}_1,\cdots,\vto{u}_m)$ when tensor $\tnr{A}\in\etng{m}{\cam{F}}{V^m}$ is non-zero, since $\tnr{A}=\alpha\tnr{A}_B$. Thereby, if $C=\{\vto{v}_1,\cdots,\vto{v}_m\}$ were linearly independent, it would be a basis of $V_\cam{F}$ and then from \eqref{eq:regraCoordTens}, tensor $\tnr{A}=\tnr{A}(\vto{v}_1,\cdots,\vto{v}_m)\tnr{A}_C$. Since $\tnr{A}$ and $\tnr{A}_C$ are non zero, then 
\end{proof}
}
