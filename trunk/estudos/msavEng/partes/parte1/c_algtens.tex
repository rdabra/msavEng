...
&=\sum_{i_1=1}^{n}\cdots\sum_{i_m=1}^{n}{\maf{\tnr{\psi}}{B}{C}}_{i_1\cdots i_m j_1\cdots j_m}\sum_{k_1=1}^{n}\cdots\sum_{k_m=1}^{n}\,,
\end{align*}


\begin{multline*}
\fua{\tnr{P}}{\cdots,\fua{\tnr{\psi}}{\vtf{f}_{k_1^{(2)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m^{(2)}}^{B_m}},\cdots} = \sum_{i_1^{(1)}=1}^{n}\cdots\sum_{i_m^{(1)}=1}^{n}\sum_{j_1^{(1)}=1}^{n}\cdots\sum_{j_m^{(1)}=1}^{n}\sum_{i_1^{(2)}=1}^{n}\cdots\sum_{i_m^{(2)}=1}^{n}\\
