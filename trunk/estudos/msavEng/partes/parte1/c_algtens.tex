...
Therefore, from definition \eqref{eq:prodTens}, it is clear that $\fua{\tnr{V}\otimes\tnr{W}}{\vto{x}_1,\cdots,\vto{x}_p,\vto{y}_1,\cdots,\vto{y}_q}=\fua{\tnr{W}\otimes\tnr{V}}{\vto{y}_1,\cdots,\vto{y}_q,\vto{x}_1,\cdots,\vto{x}_q}$. 
and then if $\tnr{U}=\vtf{u}_1^*$

Considering $\tnr{U}=\tnr{V}\otimes\tnr{W}$, tensor $\tnr{U}^{\text{T}}\in\ete{\cam{F}}{W^{\times q}\times V^{\times p}}$, where  $\tnr{U}^{\text{T}}=\tnr{W}\otimes\tnr{V}$, is called the \textsb{transpose tensor}\index{tensor!transpose} of $\tnr{U}\in\ete{\cam{F}}{V^{\times p}\times W^{\times q}}$.
...
\item[iii.] If tensor $\tnr{A}$ is an arbitrary non-zero element of tensor space $\etng{m}{\cam{F}}{V^m}$ and subset $B=\{\vto{u}_1,\cdots,\vto{u}_m\}$ one of its bases, then $\tnr{A}=\fua{\tnr{A}}{\vto{u}_1,\cdots,\vto{u}_m}\tnr{A}_B$;
...
\item[v.] Given an arbitrary operator $\vtf{g}\in\grc{U}{\cam{F}}{V}$, an alternating tensor $\tnr{A}\in\etng{m}{\cam{F}}{V^m}$ and a basis $B=\{\vto{u}_1,\cdots,\vto{u}_m\}$ of $V_\cam{F}$, then
...
\end{align*}
from which we conclude $\tnr{P}=\alpha\tnr{A}_B$, that is, $\{\tnr{A}_B\}$ is a basis of one-dimensional $\etng{m}{\cam{F}}{V^m}$ and then property i is proved. Let's verify now item ii. From the definition of $\tnr{A}_B$ it is clear that
\begin{align*}
...
\noindent A tensor $\tnr{H}\in\etng{m}{\cam{F}}{U^m}$, where $U_\cam{F}$ is a Hilbert space, is called \textsb{unimodular}\index{tensor!unimodular} if scalar $|\fua{\tnr{H}}{\vun{u}_1,\cdots,\vun{u}_m}|=1$ for every orthonormal basis $\{\vun{u}_1,\cdots,\vun{u}_m\}$. If tensors $\tnr{X}$ and $\tnr{H}$ are unimodular, we have $|\fua{\tnr{X}}{\vun{u}_1,\cdots,\vun{u}_m}|=|\fua{\tnr{H}}{\vun{u}_1,\cdots,\vun{u}_m}|$ and then $\tnr{X}=\pm\tnr{H}$. Thereby, we can affirm that if tensor $\tnr{H}$ is unimodular, so is $-\tnr{H}$. In this context, considering $B=\{\vun{u}_1,\cdots,\vun{u}_m\}$ an arbitrary orthonormal basis of $U_\cam{F}$, we can obtain that the basis tensor $\tnr{A}_B$ of $\etng{m}{\cam{F}}{U^m}$ is unimodular and then since any other vector in this alternating tensor space can be expressed as $\alpha\tnr{A}_B$, there are only two unimodular tensors in $\etng{m}{\cam{F}}{U^m}$ relative to basis $B$, namely, $\tnr{A}_B$ and $-\tnr{A}_B$. In this sense, considering $\tnr{A}\in\etng{m}{\cam{F}}{U^m}$ unimodular, the double $(U_\cam{F},\tnr{A})$ is said to be an \textsb{oriented vector space}\index{vector space!oriented}, where arbitrary basis $W=\{\vto{w}_1,\cdots,\vto{w}_m\}$ is called \textsb{positively}\index{basis!positively oriented} or \textsb{negatively oriented}\index{basis!negatively oriented} if scalar $\fua{\tnr{A}}{\vto{w}_1,\cdots,\vto{w}_m}$ is $>0$ or $<0$ respectively. Particularly, if $W$ is orthonormal, it is positively or negatively oriented if $\fua{\tnr{A}}{\vto{w}_1,\cdots,\vto{w}_m}$ is $1$ or $-1$. Thereby, given two orthonormal bases $B$ and $C=\{\vun{v}_1,\cdots,\vun{v}_m\}$ of $(U_\cam{F},\tnr{A}_B)$, if $C$ is positively or negatively oriented, then $\fua{\tnr{A}_B}{\vun{v}_1,\cdots,\vun{v}_m}=\alpha\fua{\tnr{A}_C}{\vun{v}_1,\cdots,\vun{v}_m}$, from which $\alpha=1$ or $\alpha=-1$ and then $\fua{\tnr{A}_C}{\vun{u}_1,\cdots,\vun{u}_m}=1$ or $\fua{\tnr{A}_C}{\vun{u}_1,\cdots,\vun{u}_m}=-1$. Therefore, if $C$ is positively (negatively) oriented in $(U_\cam{F},\tnr{A}_B)$ then $B$ is positively (negatively) oriented in $(U_\cam{F},\tnr{A}_C)$. In this context, we obtain that a basis $\{\fua{\vtf{h}}{\vun{u}_1},\cdots,\fua{\vtf{h}}{\vun{u}_m}\}$, for all $\vtf{h}\in\grc{U^+}{\cam{F}}{V}$, is positively oriented. Moreover, if basis $\{\vun{v}_1,\cdots,\vun{v}_m\}$ is negatively oriented, so is $\{\fua{\vtf{h}}{\vun{v}_1},\cdots,\fua{\vtf{h}}{\vun{v}_m}\}$; and then we can say that \emph{basis orientation is immune to proper unitary operators}. Now, since $\tnr{A}_B$ is unimodular, given an arbitrary alternating tensor $\tnr{P}\in\etng{m}{\cam{F}}{U^m}$,
...
\end{equation}
when basis $\{\vun{x}_{1},\cdots,\vun{x}_{m}\}$ of $(U_\cam{F},\tnr{A}_B)$ is positively or negatively oriented respectively. In other words, the coordinates of an alternating tensor are identical for any positively (negatively) oriented basis. Therefore, we can conclude that $\vtf{f}_{i_1\cdots i_m}^{X}(\tnr{A}_B)=\pm\epsilon_{i_1\cdots i_m}$.
...
\end{equation*}
when we say that $\{\fua{\tnr{\psi}}{\tnr{B}_1},\cdots,\fua{\tnr{\psi}}{\tnr{B}_p}\}$, which is also a basis according to theorem \ref{teo:basesUnit}, has the same orientation of $B$. Now, according to theorem \ref{teo:repRieszTensor}, in the particular case where operator $\tnr{\psi}$ is a $1$-cotensor of a second order tensor $\tnr{T}\in \eteg{n}{\cam{F}}{A\times A}$, that is, $\tnr{T}_1^*=\tnr{\psi}$, it is possible to state that $\hdet{\tnr{T}}:=\hdet{\tnr{\psi}}$ and then
\begin{equation}\label{eq:detTensorSeg}
\fua{\tnr{P}}{\fua{\tnr{T}_1^*}{\vto{x}_1^*},\cdots,{\fua{\tnr{T}_1^*}{\vto{x}_p^*}}}=\hdet{\tnr{T}}{\fua{\tnr{P}}{\vto{x}_1^*,\cdots,\vto{x}_p^*}}\,.
\end{equation}
