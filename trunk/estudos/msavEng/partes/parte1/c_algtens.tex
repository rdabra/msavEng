...
\fua{\tnr{I}}{\vto{x}_{1},\cdots,\vto{x}_{p},\vto{y}_{1}^c,\cdots,\vto{y}_{p}^c}=\prod_{k=1}^p \vto{x}_k\cdot\vto{y}_k\,.
\end{equation}
...
\begin{align*}
\alpha&=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\sum_{{j_1}=1}^{n_1}\cdots\sum_{{j_p}=1}^{n_p}\prod_{k=1}^p\fua{\vtf{f}^\con{B_k}_{i_k}}{\vto{x}_k}\fua{\overline{\vtf{f}^\con{B_k}_{j_k}}}{\vto{y}_k}\fua{\tnr{I}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_p}^{(p)},{{\vun{v}_{j_1}}}^{c(1)},\cdots,\vun{v}_{j_p}^{c(p)}}\\
&=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\sum_{{j_1}=1}^{n_1}\cdots\sum_{{j_p}=1}^{n_p}\prod_{k=1}^p\fua{\vtf{f}^\con{B_k}_{i_k}}{\vto{x}_k}\fua{\overline{\vtf{f}^\con{B_k}_{j_k}}}{\vto{y}_k}\delta_{i_1j_1}\cdots\delta_{i_pj_p}\\
&=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\prod_{k=1}^p\fua{\vtf{f}^\con{B_k}_{i_k}}{\vto{x}_k}\fua{\overline{\vtf{f}^\con{B_k}_{i_k}}}{\vto{y}_k}\,,
\end{align*}
considering equalities \eqref{eq:decompVectConj}. Thereby, from the definition of tensor product, we can consider tensor 
\begin{equation}
\tnr{I}=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}\otimes\overline{\vtf{f}_{i_1}^{B_1}}\otimes\cdots\otimes\overline{\vtf{f}_{i_p}^{B_p}}
\end{equation}
...
\end{equation}     
Moreover, if the tensor space in question $\ete{\cam{F}}{U^{\times m}}=\ete{\cam{F}}{V^{\times q}\otimes V^{\times q}}$, the product
\begin{equation}
\tnr{I}\cdot \tnr{B}=\fua{\tnr{I}}{\vto{b}_1,\cdots,\vto{b}_m}= \prod_{i=1}^q\vto{b}_i\cdot\vto{b}_{i+q}^c\,,
\end{equation}   
according to \eqref{eq:identidade}.
{\footnotesize
\begin{proof}
...
\end{equation*}
be a mapping where the tensor spaces involved are Hilbert spaces. If for every tensor $\tnr{T}\in \ete{\cam{F}}{W^{\times p}\times A^{\times q}}$, a $p$-cotensor  $\tnr{T}^*_p:=\fua{\tnr{\Phi}}{\tnr{T}}$ is described by the rule  
\begin{equation*}
\fua{\vtf{T}^*_p}{\tnr{X}}=\tnr{X}\odot_p\tnr{T}\,,
\end{equation*}
then $\tnr{\Phi}$ is an antilinear bijection.
\end{mteo}
...
% seguir a sequencia que está no livro velho


\begin{mcoro}{Cotensors}{eqNorm}
If mapping $\map{\tnr{\Phi}}{\ete{\cam{F}}{W^{\times p}}}{\eted{\cam{F}}{W^{\times p}}}$ is considered, the rule of tensor functional $\tnr{T}^*:=\tnr{T}^*_p$, called the cotensor of $\tnr{T}$, results
\begin{equation*}
\fua{\vtf{T}^*}{\tnr{X}}=\tnr{X}\cdot\tnr{T}\,,
\end{equation*}
and when $\tnr{T}^*$ is continuous, $\|\tnr{T}^*\|_{\eted{\cam{F}}{W^{\times p}}}=\|\tnr{T}\|_{\ete{\cam{F}}{W^{\times p}}}$.
\end{mcoro}
{\footnotesize
\begin{proof}
...
\end{equation*}
where $\frt{T}$ results a \textsb{representative function}\index{function!representative} of tensor $\tnr{T}$ since their values are the same when variables $\vto{x}_i\in (U_i)_\cam{F}$ are arranged as above. Now, it is possible to study tensors through their conjugate cotensors, which substitute them numerically.
