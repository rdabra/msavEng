...
% Abaixo Errado!!!
When the tensor space is defined by conjugate vector spaces $\overline{(U_i)_\cam{F}}$, we represent it $\ete{\cam{F}}{\overline{U^{\times m}}}$, whose arbitrary element \gloref{tensorConj} is called the \textsb{conjugate tensor}\index{tensor!conjugate} of $\tnr{T}\in\ete{\cam{F}}{U^{\times m}}$.
%O certo começa aqui...
As any other vector space, a tensor space have its conjugate, ..........  
...
\end{eqnarray}
because the last equality results a rule for $\vtf{c}^\otimes$ since $\tnr{X}$ is arbitrary. Now, let's verify if equality $\fua{\vtf{c}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\fua{{\vtf{c}}}{\vto{x}_1,\cdots,\vto{x}_m}$ is valid. Considering $\tnr{X}=\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*$, equality \eqref{eq:conjMultFun}, the properties of complex conjugates and rule \eqref{eq:regraCoordTens}, the previous development can proceed as follows:  
\begin{eqnarray}
...
When the order of a contractive product equals the order of its terms, resulting a scalar, the order of the contractive product representation is not made explicit: for arbitrary tensors $\tnr{A},\tnr{B}\in\ete{\cam{F}}{U^{\times m}}$, we define $\tnr{A}\odot\tnr{B}=\tnr{A}\odot_m\tnr{B}$. Now, we can consider that there exists a  multiantilinear functional $\overline{\tnr{B}}$ where
\begin{equation}
