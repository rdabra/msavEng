...
This theory of determinant presented so far is obviously valid for the context of order $m=1$, that is, the context of a tensor space $\etng{n}{\cam{F}}{U^n}$. Equalities \eqref{eq:alternDete} consist a specific example of the validity of \eqref{eq:detFunDef} for alternating tensors. The following theorem considers the result obtained by equality \eqref{eq:opeTensOrdin}, where a unitary tensor operator is always related to an ordinary unitary operator. 

\begin{mteo}{Related Unitary Operators and Alternating Tensors}{unimodAltern}
Given a tensor $\tnr{A}\in\etng{n}{\cam{F}}{U^n}$, a unitary tensor operator $\tnr{\psi}\in\grt{U}{\cam{F}}{U^{n}}$ related to a unitary operator $\vtf{g}\in\grc{U}{\cam{F}}{U}$ by $\fua{\vtf{g}}{\vto{x}_{1}}^*\otimes\cdots\otimes\fua{\vtf{g}}{\vto{x}_{n}}^*=\fua{\tnr{\psi}}{\vto{x}_{1}^*\otimes\cdots\otimes\vto{x}_{n}^*}$, tensor $\tnr{A}=\dete{\vtf{g}}\fua{\tnr{\psi}}{\tnr{A}}$.
\end{mteo}
{\footnotesize
\begin{proof}
...
\begin{mcoro}{Determinant of Related Unitary Operators}{deterUnita}
If unitary tensor operator $\tnr{\psi}$ is related to unitary operator $\vtf{g}$ then $\dete{\vtf{g}}=\hdet{\tnr{\psi}}$.
\end{mcoro}
{\footnotesize
\begin{proof}
Considering a unimodular tensor $\tnr{A}_B$, where $B=\{\vun{u}_1,\cdots,\vun{u}_n\}$ is a basis of $U_\cam{F}$, from the previous theorem we know that $\tnr{A}_B=\dete{\vtf{g}}\fua{\tnr{\psi}}{\tnr{A}_B}$. As $\tnr{A}_B$ is a basis, so is $\tnr{A}_C:=\fua{\tnr{\tnr{\psi}}}{\tnr{A}_B}$, according to theorem \ref{teo:basesUnit}, and then from $\dete{\vtf{g}}\fua{\tnr{A}_C}{\vun{u}_1,\cdots,\vun{u}_n}=\fua{\tnr{A}_B}{\vun{u}_1,\cdots,\vun{u}_n}=1$,  $\hdet{\tnr{\psi}}=1$ if $\dete{\vtf{g}}=1$ and $\hdet{\tnr{\psi}}=-1$ if $\dete{\vtf{g}}=-1$. 
\end{proof}
}
