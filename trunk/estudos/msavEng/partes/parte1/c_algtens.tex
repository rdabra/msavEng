...
\end{equation}
where an arbitrary unitary tensor operator is always related to a unitary operator. TENHO QUE PROVAR QUE O DET DE PSI É IGUAL AO DET DE G 
...
Admitting a tensor space $\ete{\cam{F}}{A^p}$ and a group $\grt{U}{\cam{F}}{A^p}$, where $A=U^{m}$ and $p$ is even, by equality \eqref{eq:coordIdent} we know that the coordinates of identity tensor $\tnr{I}\in\ete{\cam{F}}{A^p}$ are the same for any chosen basis. Therefore, from theorem above, it is clear that $\tnr{I}$ is isotropic, that is, $\tnr{I}\in\eto{\cam{F}}{A^p}$. In this context, again by equality \eqref{eq:coordIdent}, it is straightforward to see that the coordinates of an arbitrary tensor belonging to $\spn(\{\tnr{I}\})$ are the same for any chosen basis of $\ete{\cam{F}}{A^p}$. In this context, it is possible to affirm that \emph{any linear combination of an identity tensor is isotropic}. Now, given an arbitrary unimodular tensor $\tnr{H}\in\etng{m}{\cam{F}}{U^m}$, considering $B_i$ an orthonormal basis in the context of theorem \ref{teo:geraEte}, polyadic tensors $\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}$ and $\fua{\tnr{\varphi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}}$ elements of bases $B$ and $C$ of $\etng{m}{\cam{F}}{U^m}$ respectively, where $\tnr{\varphi}\in\tnr{\mathcal{U}}^+_{\cam{F}}({U^{m}})$, from equalities 
\begin{equation*}
