...
\end{equation}
the tensor $\tnr{T}_\pi\in\ete{\cam{F}}{V^{m}}$ is called the $\pi$ \textsb{permutation}\index{tensor!permutation of} of $\tnr{T}\in\tnr{V}^{m}_\cam{F}$. There are two important properties: a) the permutation of sum is the sum of permutations, that is, if $\tnr{T}=\tnr{A}+\tnr{B}$ then $(\tnr{A}+\tnr{B})_\pi=\tnr{A}_\pi+\tnr{B}_\pi$; b) tensor $(\alpha\tnr{T})_\pi=\alpha\tnr{T}_\pi$, $\forall\,\alpha\in\cam{F}$. Now, considering the specific rule $\fua{\pi}{x}=a\delta_{xb}+b\delta_{xa}+x(1-\delta_{xa}-\delta_{xb})$ that interchanges indexes $a$ and $b$, we call $\tnr{T}_\pi$ the $(a,b)$ \textsb{transposition}\index{tensor!transposition of} of $\tnr{T}$, represented by $\tnr{T}_{(a,b)}$.  
...
\end{equation}
as an example of identity tensor. Moreover, in the context of tensor spaces $\ete{\cam{F}}{V^2}$, where $p=1$, defined by Euclidean spaces, we already know that, if basis $B_1$ is orthonormal, equality $\sum_{{i_1}=1}^{n_1}\fua{\vtf{f}^\con{B_1}_{i_1}}{\vto{x}_1}\fua{\vtf{f}^\con{B_1}_{i_1}}{\vto{y}_1}=\vto{x}_1\cdot\vto{y}_1$ is valid, that is, expression \eqref{eq:identidade} results $\fua{\tnr{I}}{\vto{x}_1,\vto{y}_1}=\vto{x}_1\cdot\vto{y}_1$ and  
\begin{equation}
...
\nonumber
\end{equation}
where $B$ is a basis of $\ete{\cam{F}}{U}$ defined by orthonormal bases  $\con{B}_i=\{\vun{v}_{1}^{(i)},\cdots,\vun{v}_{n_i}^{(i)}\}$ of $X_i$. According to rule \eqref{eq:regraOrtonmFunc}, the function $\vtf{c}^\otimes$ that observes $\fua{\vtf{c}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\fua{{\vtf{c}}}{\vto{x}_1,\cdots,\vto{x}_m}$ really exists and is uniquely determined by $\vtf{c}$, as can be verified in  
\begin{eqnarray}
...
i_m}^{B}}{\tnr{X}}\fua{{\vtf{c}}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_m}^{(m)}}\,,\nonumber
\end{eqnarray}
...
\fua{\vtf{c}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}&=&\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
		i_m}^{B}}{\tnr{X}}\fua{{\vtf{c}}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_m}^{(m)}}\nonumber\\
&=&
\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\tnr{X}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_m}^{(m)}}\fua{{\vtf{c}}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_m}^{(m)}}\nonumber\\
&=&
\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\prod_{k=1}^m\fua{\vto{x}_k^*}{\vun{v}_{i_k}^{(k)}} \,\,\fua{{\vtf{c}}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_m}^{(m)}}\nonumber\\
&=&
\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\prod_{k=1}^m \overline{\fua{\vtf{f}_{i_k}^{\con{B}_k}}{\vto{x}_k}}\,\,\fua{{\vtf{c}}}{\vun{v}_{i_1}^{(1)},\cdots,\vun{v}_{i_m}^{(m)}}\nonumber\\
&=&
\fua{{\vtf{c}}}{\sum_{i_1=1}^{n_1}\fua{\vtf{f}_{i_1}^{\con{B}_1}}{\vto{x}_1}
\vun{v}_{i_1}^{(1)},\cdots,\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_m}^{\con{B}_m}}{\vun{x}_m}\vun{v}_{i_m}^{(m)}}\nonumber\\
...
\otimes\cdots\otimes\vto{x}_{s-1}^*\otimes\vto{x}_{s+1}^*\otimes\cdots\otimes\vto{x}_m^*\,.
\end{equation}
It is important to observe that the product $\vto{x}_{r}^c\cdot\vto{x}_{s}$ is antilinear also on the left term since  $\vtf{c}_2$ has $\vto{x}_{r}$ as one of its arguments\footnote{See property \eqref{eq:antiLinConjVect}.}. Moreover, in the context of Euclidean spaces, from equalities \eqref{eq:prodIntGen} and \eqref{eq:identidade}, we can write that $\vto{x}_{s}^c\cdot\vto{x}_{r}=\vto{x}_{s}\cdot\vto{x}_{r}\,$. 
...
\end{align*} 
we call tensor $\fua{{\vtf{c}}_{2p}^\otimes}{\tnr{A}\otimes\tnr{B}}$ the $p$-th order \textsb{contractive product}\index{contractive!product} of $\tnr{A}$ and $\tnr{B}$, represented by $\tnr{A}\diamond_p\tnr{B}$. Thereby, we can write in simpler terms that   
\begin{equation}\label{eq:ProdTens}
...
\end{equation}
where $\tnr{C}\in\ete{\cam{F}}{W^{\times p}\times C^{\times n}\times Z^{\times r}}$ and $\tnr{D}\in\ete{\cam{F}}{Z^{\times r}\times D^{\times s}}$. 
...
From this concept of contractive product, it is now convenient to define a new product that is linear on the first argument and antilinear on the second with an obvious purpose of generalizing the inner product. In order to do this, let's consider the same conditions that led to definition \eqref{eq:ProdTens} and state that the tensor 
\begin{equation}\label{eq:prodInGen}
\tnr{A}\odot_p\tnr{B}:=\tnr{A}\diamond_p ((\vto{y}_1^c)^*\otimes\cdots\otimes(\vto{y}_{p}^c)^*\otimes(\vto{b}_1^c)^*\otimes\cdots\otimes(\vto{b}_{n}^c)^*)
\end{equation}
is called the $p$-th order \textsb{generalized inner product} of $\tnr{A}$ and $\tnr{B}$, which is antilinear on the second argument through $\vto{y}_i^*$ and $\vto{b}_i^*$. Thereby, we have
\begin{equation}
\tnr{A}\odot_p\tnr{B}=(\prod_{i=1}^p\vto{y}_i\cdot\vto{x}_i)\vto{a}_1^*\otimes\cdots\otimes\vto{a}_{q}^*\otimes(\vto{b}_{1}^c)^*\otimes\cdots\otimes(\vto{b}_n^c)^*\,.
\end{equation}
In the context of Euclidean spaces, it is not unusual to see a first order generalized inner product of a dyadic tensor and a covector being used to define the so called ``tensor product of vectors''\footnote{See \aut{Gurtin}\cite{gurtin_1981}, p.4; \aut{Truesdell \& Noll}\cite{truesdell_1992}, p.15; \aut{Ogden}\cite{ogden_1997}, p.16.}. Although we shall not adopt this rather obscure concept, its frequent use by renowned authors forces us to clarify it in terms of our rigorous approach as follows. Firstly, since vectors and covectors have a one to one correspondence, these authors inadvertently use the term ``vector'' either as an element of a vector space or as a covector, depending on the context: in $\vto{u}(\vto{v})=\vto{v}\cdot\vto{u}$, for example, $\vto{u}$ is a covector on the left-hand side and a vector on the right. Thereby, they get rid of the burden of defining a covector and all of its preliminary concepts. In this sense, our representation of a dyadic tensor $\vto{u}^*\otimes\vto{v}^*$ is subverted to $\vto{u}\otimes\vto{v}$ and the contractive product operator is suppressed from our precise representation $(\vto{u}^*\otimes\vto{v}^*)\odot_1 \vto{x}^*=(\vto{x}\cdot\vto{v})\vto{u}^*$ in order to avoid defining a contraction and its supporting concepts. Finally, they define the ``tensor product'' $\vto{u}\otimes\vto{v}$ through its effects, that is, as a ``tensor'' which causes $(\vto{u}\otimes\vto{v})\vto{x}=(\vto{v}\cdot\vto{x})\vto{u}$, for an arbitrary $\vto{x}$. Again,  \emph{this pragmatic approach of taking vectors as first order tensors and all of its consequences will not be adopted in this text}. 

When the order of a generalized inner product equals the order of its terms, the result is a scalar. In this context, considering arbitrary tensors $\tnr{A},\tnr{B}\in\ete{\cam{F}}{U^{\times m}}$, where $\tnr{A}=\vto{a}^*_1\otimes\cdots\otimes\vto{a}^*_m$ and $\tnr{B}=\vto{b}^*_1\otimes\cdots\otimes\vto{b}^*_m$, the scalar
...
{\footnotesize
\begin{proof}
First we prove that the generalized inner product is indeed antilinear on the second argument through the following equalities:
\begin{align*}
\tnr{A}\odot_p\alpha\beta\tnr{B} &=\tnr{A}\,\odot_p\,(\overline{\alpha}\vto{y}_{1})^*\otimes\cdots\otimes(\vto{y}_{p})^*\otimes (\overline{\beta}\vto{b}_1)^*\otimes\cdots\otimes(\vto{b}_{n})^*\\
&=\tnr{A}\,\diamond_p\,((\overline{\alpha}\vto{y}_{1})^c)^*\otimes\cdots\otimes(\vto{y}_{p}^c)^*\otimes ((\overline{\beta}\vto{b}_1)^c)^*\otimes\cdots\otimes(\vto{b}_{n}^c)^*\\
&=\tnr{A}\,\diamond_p\,\overline{\alpha}(\vto{y}_{1}^c)^*\otimes\cdots\otimes(\vto{y}_{p}^c)^*\otimes \overline{\beta}(\vto{b}_1^c)^*\otimes\cdots\otimes(\vto{b}_{n}^c)^*\\
&=\overline{\alpha\beta}(\tnr{A}\odot_p\tnr{B})\,.
\end{align*}
...
In this study, a tensor function is treated like an ordinary function, on which the same functional definitions and nomenclature already presented can be applied properly. For instance, it is possible to consider a tensor function that is the composition of other tensor functions and consequently to define an inverse tensor function. Moreover, if we consider a  mapping $\map{\tnr{\psi}}{\ete{\cam{F}}{U^{\times p}}}{\ete{\cam{F}}{V^{\times q}}}$, the existence of inner product of tensors enables us to write, for arbitrary tensors $\tnr{U}\in\ete{\cam{F}}{U^{\times p}}$ and $\tnr{V}\in\ete{\cam{F}}{V^{\times p}}$, equality 
\begin{equation}
\fua{\tnr{\psi}}{\tnr{U}}\cdot\vto{V}=\tnr{U}\cdot\fua{\tnr{\psi}^\dagger}{\tnr{V}}\,,
\end{equation}
where the function in $\map{\tnr{\psi}^\dagger}{\ete{\cam{F}}{V^{\times q}}}{\ete{\cam{F}}{U^{\times p}}}$ is called the adjoint tensor function of $\tnr{\psi}$, as defined on section \ref{sec:FuncLin}. In this context, all properties of ordinary adjoint functions presented on page \pageref{eq:propAdj} are also valid and for the particular case of an operation $\map{\tnr{\varphi}}{\ete{\cam{F}}{U^{\times p}}}{\ete{\cam{F}}{U^{\times p}}}$, operator $\tnr{\varphi}$ is said to be Hermitian when  $\tnr{\varphi}=\tnr{\varphi}^\dagger$ and anti-Hermitian when $\tnr{\varphi}=-\tnr{\varphi}^\dagger$. If tensor function $\tnr{\psi}$, as defined previously, is a linear bijection whose inverse equals the adjoint, it is called unitary. Similarly to ordinary unitary operators, the set $O$ of all unitary tensor operators on a certain tensor space defines a group $(O,\circ)$ called unitary group of tensors. A linear operator $\tnr{\kappa}$ defined on a Hilbert tensor space $\ete{\cam{F}}{W^{\times m}}$ is called isometric if the distance $\varrho\,[\fua{\tnr{\kappa}}{\tnr{W}_1},\fua{\tnr{\kappa}}{\tnr{W}_2}]=\fua{\varrho}{\tnr{W}_1,\tnr{W}_2}$, where tensors $\tnr{W}_1$ and $\tnr{W}_2$ are arbitrary. Again, the set $I$ of all isometric tensor operators can define an isometry group $(I,\circ)$ of tensors. In the context of Euclidean tensor spaces, adjoint functions are called transpose, Hermitian functions are said to be symmetric, unitary functions are orthogonal and an isometry group is also an unitary group whose elements preserve the inner product. 


Fundamentally speaking, the concept of tensor function, up to now, has enabled us to arrive at dual and inner product tensor spaces from the definitions of linear tensor functional and contractive product respectively. In comparative terms, we are now in a similar situation to that which preceded the essential Riesz-Fréchet Theorem in section \ref{sec:FuncLin}. Then, a very interesting question naturally arises: ``Considering a Hilbert tensor space, is it possible for each of its tensors to have a corresponding `cotensor' just like a vector in Hilbert spaces has a covector?'' An affirmative answer to this question would be quite convenient for our purposes because we could properly replace in some convenient cases the study of tensors, which are multilinear functionals, with the simpler study of the so called cotensors. In practice, considering a suitable relationship between the values of tensor and cotensor, we could be able to approach Tensor Differentiation by working with single variable tensor functions instead of tensors themselves, inherently multivariable. The remaining text of this section shows that the concept of \textsb{cotensor}\index{cotensor} is indeed feasible and a future chapter will deal  with the differentiation of single variable tensor functions and their relation to tensor differentiation.
...
Let 
\begin{equation*}
\map{\tnr{\Phi}}{\ete{\cam{F}}{W^{\times p}\times B^{\times n}}}{\evl{\cam{F}}{\ete{\cam{F}}{A^{\times q}\times W^{\times p}}}{\ete{\cam{F}}{A^{\times q}\times B^{\times n}}}}
\end{equation*}
be a mapping where the tensor spaces involved are Hilbert spaces. If for every tensor $\tnr{T}\in \ete{\cam{F}}{W^{\times p}\times B^{\times n}}$, a $p$-cotensor  $\tnr{T}^*_p:=\fua{\tnr{\Phi}}{\tnr{T}}$ is described by the rule  
\begin{equation*}
\fua{\vtf{T}^*_p}{\tnr{X}}=\tnr{X}\odot_p\tnr{T}\,,
\end{equation*}
then $\tnr{\Phi}$ is an antilinear bijection. If $\tnr{T}^*_p$ is continuous,  $\|\tnr{T}^*_p\|=\|\tnr{T}\|$.
\end{mteo}

{\footnotesize
\begin{proof}
Considering $\tnr{A},\tnr{B}\in\ete{\cam{F}}{W^{\times p}\times B^{\times n}}$, function $\tnr{\Phi}$ is antilinear from the following:
\begin{equation*}
...
\end{equation*}
If $\tnr{\Phi}$ were not an injection, there would be different $p$-cotensors $\tnr{A}^*_p$ and $\tnr{B}^*_p$ where $\tnr{X}\odot_p\tnr{A}=\tnr{X}\odot_p\tnr{B}$. From this supposition, equality $\tnr{X}\odot_p(\tnr{A}-\tnr{B})=\tnr{0}$ do not confirm $\tnr{A}^*_p\neq\tnr{B}^*_p$ because it is valid for arbitrary $\tnr{X}$. In order to prove that $\tnr{\Phi}$ is a surjection, we need to obtain for an arbitrary tensor function $\vtf{\psi}$ a tensor $\vto{A}\in \ete{\cam{F}}{W^{\times p}\times B^{\times n}}$ such that $\fua{\tnr{\Phi}}{\tnr{A}}=\vtf{\psi}$. Considering the same conditions of equality \eqref{eq:coordTensor} and equalities \eqref{eq:propProdIndTens} and betaking what we could call mathematical sagacity, 
\begin{align*}
\fua{\vtf{\psi}}{\tnr{X}}&=\fua{[\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\vtf{f}_{i_1\cdots i_m}^{B}]}{\tnr{X}}\\&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\fua{\tnr{X}}{\vto{u}_{i_1}^{(1)},\cdots,\vto{u}_{i_m}^{(m)}}\\
&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\tnr{X}\cdot(\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m})\\
&=\tnr{X}\cdot\underbrace{(\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\overline{\alpha_{i_1\cdots i_m}}\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m})}_{\tnr{A}}\\
&=\tnr{X}\odot_{q+p}\tnr{A}\\
\\&=[\fua{\tnr{\Phi}}{\tnr{A}}](\tnr{X})\,.
\end{align*}
Using definition \eqref{eq:normaFuncaoTens} on the present conditions, leaving space representation on norms implicit, we obtain equality $\|\tnr{A}^*\|=\sup\{ |\tnr{X}\cdot\tnr{A}|/\|\tnr{X}\|\}$ for all non zero $\tnr{X}$. If $\tnr{A}$ is zero, it is evident that $\|\tnr{A}^*\|=\|\tnr{A}\|$; otherwise, $\tnr{A}^*$ is non zero and we conclude that $\|\tnr{A}^*\|\geqslant |\, \tnr{X}\cdot\tnr{A}|/\|\tnr{X}\|$. Cauchy-Schwarz Inequality states that $|\, \tnr{X}\cdot\tnr{A}| \leqslant \|\tnr{X}\|\,\,\|\tnr{A}\|$. Subtracting these two previous inequalities, we arrive at $(\|\tnr{A}^*\|-\|\tnr{A}\|)\|\tnr{X}\|\geqslant 0$, whose left side can be zero for arbitrary non zero $\tnr{A}^*$, $\tnr{A}$ and $\tnr{X}$; therefore, $\|\tnr{A}^*\|=\|\tnr{A}\|$.    
	\end{proof}
}
...
\begin{mteo}{Riesz-Fréchet Representation of Tensors}{repRieszTensor}
Let $\map{\tnr{\Phi}}{\ete{\cam{F}}{A^{\times q}\times W^{\times p}}}{\eted{\cam{F}}{U^{\times m}}}$ be a mapping where $\ete{\cam{F}}{U^{\times m}}$ is a Hilbert tensor space and $\eted{\cam{F}}{U^{\times m}}$ its dual space. If for every $\tnr{T}\in \ete{\cam{F}}{U^{\times m}}$, a cotensor  $\tnr{T}^*:=\fua{\tnr{\Phi}}{\tnr{T}}$ is described by the rule  
\begin{equation*}
\fua{\vtf{T}^*}{\tnr{X}}=\tnr{X}\cdot\tnr{T}\,,
\end{equation*}
then $\tnr{\Phi}$ is an antilinear bijection. If $\tnr{T}^*$ is continuous,  $\|\tnr{T}^*\|_{\eted{\cam{F}}{U^{\times m}}}=\|\tnr{T}\|_{\ete{\cam{F}}{U^{\times m}}}$.
\end{mteo}

{\footnotesize
\begin{proof}
Considering $\tnr{A},\tnr{B}\in\ete{\cam{F}}{U^{\times m}}$, function $\tnr{\Phi}$ is antilinear from the following equalities:
\begin{equation*}
\fua{\lco\fua{\tnr{\Phi}}{\alpha\tnr{A}+\beta\tnr{B}}\rco}{\tnr{X}}=\tnr{X}\cdot(\alpha\tnr{A}+\beta\tnr{B})\cdot=\overline{\alpha}\tnr{A}^*(\tnr{X})+\overline{\beta}\tnr{B}^*(\tnr{X})=\fua{\lco\overline{\alpha}\fua{\tnr{\Phi}}{\tnr{A}}+\overline{\beta}\fua{\tnr{\Phi}}{\tnr{B}}\rco}{\tnr{X}}\,.
\end{equation*}
If $\tnr{\Phi}$ were not an injection, there would be different cotensors $\tnr{A}^*$ and $\tnr{B}^*$ where $\fua{\tnr{A}^*}{\tnr{X}}=\fua{\tnr{B}^*}{\tnr{X}}$ or $\tnr{X}\cdot\tnr{A}=\tnr{X}\cdot\tnr{B}$. From this supposition, the following equalities $(\tnr{A}-\tnr{B})\cdot\tnr{X}=\fua{(\tnr{A}-\tnr{B})^*}{\tnr{X}}=0$ do not confirm $\tnr{A}^*\neq\tnr{B}^*$ since $(\tnr{A}-\tnr{B})^*=\tnr{A}^*-\tnr{B}^*$. In order to prove that $\tnr{\Phi}$ is a surjection, we need to obtain for an arbitrary functional $\vtf{\psi}\in \eted{\cam{F}}{U^{\times m}}$ a tensor $\vto{A}\in \ete{\cam{F}}{U^{\times m}}$ such that $\fua{\tnr{\Phi}}{\tnr{A}}=\vtf{\psi}$.
Considering the same conditions of equality \eqref{eq:coordTensor} and equalities \eqref{eq:propProdIndTens}, we can say that
\begin{align*}
\fua{\vtf{\psi}}{\tnr{X}}&=\fua{[\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\vtf{f}_{i_1\cdots i_m}^{B}]}{\tnr{X}}\\&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\fua{\tnr{X}}{\vto{u}_{i_1}^{(1)},\cdots,\vto{u}_{i_m}^{(m)}}\\
&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\tnr{X}\cdot(\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m})\\
&=\tnr{X}\cdot\underbrace{(\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\overline{\alpha_{i_1\cdots i_m}}\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m})}_{\tnr{A}}\\&=[\fua{\tnr{\Phi}}{\tnr{A}}](\tnr{X})\,.
\end{align*}
Using definition \eqref{eq:normaFuncaoTens} on the present conditions, leaving space representation on norms implicit, we obtain equality $\|\tnr{A}^*\|=\sup\{ |\tnr{X}\cdot\tnr{A}|/\|\tnr{X}\|\}$ for all non zero $\tnr{X}$. If $\tnr{A}$ is zero, it is evident that $\|\tnr{A}^*\|=\|\tnr{A}\|$; otherwise, $\tnr{A}^*$ is non zero and we conclude that $\|\tnr{A}^*\|\geqslant |\, \tnr{X}\cdot\tnr{A}|/\|\tnr{X}\|$. Cauchy-Schwarz Inequality states that $|\, \tnr{X}\cdot\tnr{A}| \leqslant \|\tnr{X}\|\,\,\|\tnr{A}\|$. Subtracting these two previous inequalities, we arrive at $(\|\tnr{A}^*\|-\|\tnr{A}\|)\|\tnr{X}\|\geqslant 0$, whose left side can be zero for arbitrary non zero $\tnr{A}^*$, $\tnr{A}$ and $\tnr{X}$; therefore, $\|\tnr{A}^*\|=\|\tnr{A}\|$.    
\end{proof}
}
...
Considering the conditions of this Riesz-Fréchet Representation of Tensors and a function space $\cam{F}^V_\cam{F}$, where the set $V=\ete{\cam{F}}{U^{\times m}}$, let $\frt{T}\in \cam{F}^V_\cam{F}$ be an antilinear tensor functional described by the rule  
\begin{equation}
\fua{\frt{T}}{\tnr{X}} = \overline{\fua{\vtf{T}^*}{\tnr{X}}}=\tnr{T}\cdot\tnr{X}\,,
\end{equation}
...
\begin{equation*}
\fua{\frt{T}}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*} = \tnr{T}\cdot(\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*)=\fua{\tnr{T}}{\vto{x}_1,\cdots,\vto{x}_m}\,,
\end{equation*}
where $\frt{T}$ results a \emph{substitute function} of tensor $\tnr{T}$ since their values are the same when variables $\vto{x}_i\in (U_i)_\cam{F}$ are arranged as above. Now, it is possible to study tensors through their conjugate cotensors, which substitute them numerically.
