...
\end{equation}
Now, considering these equalities, we extend the concept of positivity that can be applied to the representative operator $\tnr{\Upsilon}_{\tnr{T}}\in\evl{\cam{F}}{U}{U}$, described by definition \eqref{eq:linPosDef}, to tensor $\tnr{T}\in\ete{\cam{F}}{U^2}$. Thereby, a second order tensor $\tnr{T}$ is said to be \textsb{nonnegative}\index{tensor!nonnegative} if 
...
\end{equation}
and \textsb{positive-definite}\index{tensor!positive-definite} if the real value on the left side is never zero. 
...
{\footnotesize
\begin{proof}
From \eqref{eq:secTens2Lin}, we know that $\fua{\tnr{G}}{\vto{u},\vto{v}}=\vto{v}\cdot\fua{\tnr{\Upsilon}_{\tnr{G}}}{\vto{u}^c}$. But if function $\tnr{\Upsilon}_{\tnr{G}}$ is symmetric, then
\begin{equation*}
