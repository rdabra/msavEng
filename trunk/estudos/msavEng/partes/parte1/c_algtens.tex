...
Additionally to symmetric and antisymmetric, there is another notable tensor that will enable very important concepts for our study, like isotropy and anti-isotropy for example: I'm referring to the identity tensor, which is defined in tensor spaces of even order. We call \gloref{tensorI}$\,\in\ete{\cam{F}}{V^{\times p}\times V^{\times p}}$ an \textsb{identity tensor}\index{tensor!identity} of order $2p$ if, considering $\vto{x}_i$ and $\vto{y}_i$ arbitrary vectors of Hilbert spaces $(V_\cam{F})_i$, scalars 
 \begin{equation}
 \fua{\tnr{I}}{\vto{x}_{1},\cdots,\vto{x}_{p},\vto{y}_{1},\cdots,\vto{y}_{p}}=\prod_{k=1}^p \vto{x}_k\cdot\vto{y}_k\,.
 \end{equation}
From this definition, given bases $B_i=\{\vto{v}_1^{(i)},\cdots,\vto{v}_{n_i}^{(i)}\}$ of $(V_\cam{F})_i$, 
\begin{equation}\label{eq:identidade}
\fua{\tnr{I}}{\vto{x}_1,\cdots,\vto{x}_p,\vto{y}_1,\cdots,\vto{y}_p}=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\,\prod_{k=1}^p\fua{\vtf{f}^\con{B_k}_{i_k}}{\vto{x}_k}\overline{\fua{\vtf{f}^\con{B^\perp_k}_{i_k}}{\vto{y}_k}}\,,
\end{equation}
according to \eqref{eq:prodIntGen}. From the definition of tensor product, we can consider 
\begin{equation}
\tnr{I}=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}\otimes\overline{\vtf{f}_{i_1}^{B^\perp_1}}\otimes\cdots\otimes\overline{\vtf{f}_{i_p}^{B^\perp_p}}
\end{equation}
...
considering $\vto{x}_i$ and $\vto{y}_i$ arbitrary vectors of $(V_\cam{F})_i$, we can write scalar $\fua{\tnr{I}}{\vto{x}_1,\cdots,\vto{x}_p,\vto{y}_1,\cdots,\vto{y}_p}$ in the format 
\begin{equation*}
