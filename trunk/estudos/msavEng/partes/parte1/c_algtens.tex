...
\fua{\tnr{P}^{\tnr{\psi}}}{\tnr{X}_1,\cdots,\tnr{X}_p}:=\fua{\tnr{P}}{\fua{\tnr{\psi}}{\tnr{X}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{X}_p}}}\,,\,\forall\,\tnr{X}_i\in\ete{\cam{F}}{V^{\times m}}\,,
\end{equation}  
where $\tnr{P}^{\tnr{\psi}}$ is clearly antisymmetric, that is, $\tnr{P}^{\tnr{\psi}}$ also belongs to $\etn{\cam{F}}{A^p}$. We already know the interesting feature that any antisymmetric tensor space is one-di\-men\-sional and thereby the set $\{\tnr{P}\}$ can be a basis of $\etn{\cam{F}}{A^p}$, when it is valid to write that $\tnr{P}^{\tnr{\psi}}=\alpha\tnr{P}$, where $\alpha\in\cam{F}$ is the coordinate of $\tnr{P}^{\tnr{\psi}}$ on $\{\tnr{P}\}$. If a non-zero arbitrary tensor $\tnr{T}\in\etn{\cam{F}}{A^p}$ is considered, let $\beta\in\cam{F}$ be its coordinate on $\{\tnr{P}\}$. As the previous definition \eqref{eq:defDetPsi} is valid for an arbitrary tensor of $\etn{\cam{F}}{A^p}$, the following development
\begin{align*}
