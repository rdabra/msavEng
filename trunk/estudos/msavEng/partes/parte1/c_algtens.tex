...
If a tensor space can be structured as any other ordinary vector space, it is possible to consider a function space $V^U_\cam{F}$, where domain $U={\ete{\cam{F}}{U^{\times m}}}$ and codomain $V=\ete{\cam{F}}{V^{\times n}}$ are complete tensor spaces, with a properly defined metric. Thereby, similarly to vector function spaces, as presented in section \ref{sec:FuncLin}, an element of this tensor function space $V^U_\cam{F}$ can be linear or, if $U=\ete{\cam{F}}{U^{\times m}}$
