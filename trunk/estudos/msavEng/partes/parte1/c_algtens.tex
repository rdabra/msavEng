...
\end{equation}
where $\tnr{C}\in\ete{\cam{F}}{W^{\times p}\times C^{\times n}\times Z^{\times r}}$ and $\tnr{D}\in\ete{\cam{F}}{Z^{\times r}\times D^{\times s}}$. It is also important to say that if both tensors $\tnr{A}$ and $\tnr{B}$ belong to $\ete{\cam{F}}{W^{\times p}}$, the scalar $\tnr{A}\diamond\tnr{B}:=\tnr{A}\diamond_p\tnr{B}$ is called simply the contractive product of the tensors in question. DESENVOLVER O PRODUTO CONTRATIVO COMPLETO EM TERMOS DE COORDENADAS PARA TERMINAR A ÚLTIMA EXPRESSÃO.
...
\end{equation} 
If tensor $\tnr{X}=\vto{x}^*_1\otimes\cdots\otimes\vto{x}^*_m\otimes\vto{y}^*_1\otimes\cdots\otimes\vto{y}^*_m$ belongs to  $\ete{\cam{F}}{U^{\times m}\otimes U^{\times m}}$, 
...
&=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\fua{\vtf{f}_{i_1\cdots i_p}}{\tnr{A}}\fua{\vtf{f}_{i_1\cdots i_p}}{\tnr{B}}\nonumber\\
&=????????\,.
\end{align}
