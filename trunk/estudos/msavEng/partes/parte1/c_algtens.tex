...
&=[\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\underbrace{\fua{\vtf{\psi}}{\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m}}}_{\alpha_{i_1\cdots i_m}}\,\vtf{f}_{i_1\cdots i_m}^{B}]\,(\tnr{X}) 
\end{align*}
...
Let $\map{\tnr{\Phi}}{U_\cam{F}}{U^*_\cam{F}}$ be a mapping where $U_\cam{F}$ is a Hilbert space and 
...
\end{equation}
then $\Phi$ results an antilinear bijection. When $\vtf{u}^*$ is continuous,  $\|\vtf{u}^*\|_{U^*_\cam{F}}=\|\vto{u}\|_{U_\cam{F}}$.
\end{mteo}
