...
Fundamentally speaking, the concept of tensor function, up to now, has enabled us to arrive at dual and inner product tensor spaces from the definitions of linear tensor functional and contractive product respectively. In comparative terms, we are now in a similar situation to that which preceded the essential Riesz-Fréchet Theorem in section \ref{sec:FuncLin}. Then, a very interesting question naturally arises: ``Since a Hilbert tensor space can now be defined, is it possible for each of its tensors to have a corresponding cotensor just like a vector in Hilbert spaces has a covector?'' An affirmative answer to this question would be quite convenient for our purposes because we could properly replace the study of tensors, which are multilinear functionals, with the simpler study of the so called cotensors, defined as linear functionals. In practice, considering a suitable relationship between the values of tensor and cotensor, we could be able to approach Tensor Calculus by working with single variable tensor functions instead of tensors themselves, inherently multivariable. The remaining text of this section shows that the concept of \textsb{cotensor}\index{cotensor} is indeed feasible and a future chapter will deal  with the Calculus of single variable tensor functions.


\begin{mteo}{Riesz-Fréchet Representation of Tensors}{repRieszTensor}
Let $\map{\tnr{\Phi}}{\ete{\cam{F}}{U^{\times m}}}{\eted{\cam{F}}{U^{\times m}}}$ be a mapping where $\ete{\cam{F}}{U^{\times m}}$ is a Hilbert tensor space and 
$\eted{\cam{F}}{U^{\times m}}$ its dual space. If for every $\tnr{T}\in \ete{\cam{F}}{U^{\times m}}$, a cotensor  $\tnr{T}^*:=\fua{\tnr{\Phi}}{\tnr{T}}$ is described by the rule  
\begin{equation*}
