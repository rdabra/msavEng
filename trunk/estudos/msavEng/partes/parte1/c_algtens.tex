...
\end{itemize}
are valid properties for all $\alpha,\beta\in\cam{F}$ and  $\tnr{U},\tnr{V},\tnr{W}\in\ete{\cam{F}}{U^{\times m}}$. Moreover, the following theorem states that a set of specific polyadic tensors, defined by tuples of coordinate functionals, is able to span a tensor space. 
...
Henceforth, we shall be dealing only with tensors defined by Hilbert spaces, since it is not our purpose to work with first order tensors which are not covectors. In this scope of study, a function which enables a relationship between tensors of any order we call it a tensor function, which, unless otherwise stated, we shall represent it with bold greek letters: the function in $\map{\vtf{\psi}}{\ete{\cam{F}}{U^{\times m}}}{\ete{\cam{F}}{V^{\times p}}}$ is then a tensor function. It is important to say that, when conditions described at the beginning of section \ref{sec:FuncLin} are considered, tensor functions can constitute function spaces, which may be structured to follow the same nomenclature presented in figure \ref{fig:esquemaEspacos}. Moreover, this context of tensors on Hilbert spaces enables us to always relate a multiantilinear function and a linear tensor function, which is a fundamental feature to ensure uniqueness and existence of a fundamental linear tensor function to be presented in this section, which we call contraction. The following theorem\footnote{Adapted from \aut{Backus}\cite{backus_1997_1} after generalizing a property from  \aut{Marcus}\cite{marcus_1973_1}, p.14.}, borrowed from the realm of Multilinear Algebra, describes such feature by establishing a one to one correspondence between a multilinear function and a linear tensor function. From this biunivocal relationship, the multiantilinear function is usually said to be ``lifted'' to a tensor function.   
...
{\footnotesize
	\begin{proof}
		...
		\begin{equation}
		\tnr{X}=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
...
\begin{equation*}
\fua{{\vtf{c}}_2}{\vto{x}_1,\cdots,\vto{x}_m}:=(\vto{x}_{r}^c\cdot\vto{x}_{s})\,\vto{x}_1^*\otimes\cdots\otimes\vto{x}_{r-1}^*\otimes\vto{x}_{r+1}^*
...
\begin{equation}
\trt{r}{s} \tnr{X}=(\vto{x}_{r}^c\cdot\vto{x}_{s})\,\vto{x}_1^*\otimes\cdots\otimes\vto{x}_{r-1}^*\otimes\vto{x}_{r+1}^*
\otimes\cdots\otimes\vto{x}_{s-1}^*\otimes\vto{x}_{s+1}^*\otimes\cdots\otimes\vto{x}_m^*\,.
\end{equation}
It is important to observe that in the context of Euclidean spaces, from equalities \eqref{eq:prodIntGen} and \eqref{eq:identidade}, we can write that $\vto{x}_{r}^c\cdot\vto{x}_{s}=\vto{x}_{r}\cdot\vto{x}_{s}\,$. 
...
\\&\overline{\fua{\tnr{I}}{\vto{x}_{1},\cdots,\vto{x}_{p},\vto{y}_1,\cdots,\vto{y}_p}}\,\vto{a}_1^*\otimes\cdots\otimes\vto{a}_{q}^*\otimes\vto{b}_{1}^*\otimes\cdots\otimes\vto{b}_n^*\,,
\end{align*} 
...
\begin{equation}\label{eq:ProdTens}
\tnr{A}\odot_p\tnr{B}=\overline{\fua{\tnr{I}}{\vto{x}_{1},\cdots,\vto{x}_{p},\vto{y}_1,\cdots,\vto{y}_p}}\vto{a}_1^*\otimes\cdots\otimes\vto{a}_{q}^*\otimes\vto{b}_{1}^*\otimes\cdots\otimes\vto{b}_n^*.
\end{equation}
For the sake of generality when manipulating contractive products algebraically, we consider that $\tnr{A}\odot_0\tnr{B}:=\tnr{A}\otimes\tnr{B}$. Moreover, it is trivial to obtain that these type of products are also associative:
\begin{equation}
...
When the order of a contractive product equals the order of its terms, resulting a scalar, the order of the contractive product representation is omitted: for arbitrary tensors $\tnr{A},\tnr{B}\in\ete{\cam{F}}{U^{\times m}}$, we define $\tnr{A}\odot\tnr{B}=\tnr{A}\odot_m\tnr{B}$. Since a vector space and its conjugate are constituted by the same elements,   




Now, we can consider that there exists a  multiantilinear functional $\overline{\tnr{B}}$ where
\begin{equation}
\fua{\overline{\tnr{B}}}{\vto{u}_1,\cdots,\vto{u}_m}=\overline{\fua{\tnr{B}}{\vto{u}_1,\cdots,\vto{u}_m}}\,,
\end{equation}
for all $u_i\in (U_i)_\cam{F}$. 



In particular, the scalar values in equality \eqref{eq:ProdTens} are always real when only Euclidean spaces are considered and then, from \eqref{eq:identidade}, these real values can be arranged in such a way that 
\begin{equation*}
\overline{\fua{\tnr{I}}{\vto{x}_{1},\cdots,\vto{x}_{p},\vto{y}_1,\cdots,\vto{y}_p}}=\prod_{i=1}^{p}\vto{x}_{i}\cdot\vto{y}_{i}\,.
\end{equation*}
From this result, we can state that the contractive product $\tnr{W}_1\odot\tnr{W}_2$, where both tensors belong to $\ete{\real}{W^{\times p}}$, results a real number and thus \emph{generalizes the concept of inner product in Euclidean spaces}\footnote{Also in the context of Euclidean spaces, it is not unusual to see a first order contractive product of a dyadic tensor and a covector being used to define the so called ``tensor product of vectors'' (See \aut{Gurtin}\cite{gurtin_1981}, p.4; \aut{Truesdell \& Noll}\cite{truesdell_1992}, p.15; \aut{Ogden}\cite{ogden_1997}, p.16). Although we shall not use this rather obscure concept, its frequent use by renowned authors forces us to clarify it in terms of our rigorous approach as follows. First, since vectors and covectors have a one to one correspondence, these authors inadvertently use the term ``vector'' either as an element of a vector space or as a covector, depending on the context: in $\vto{u}(\vto{v})=\vto{v}\cdot\vto{u}$, for example, $\vto{u}$ is a covector on the left side and a vector on the right. Thereby, they get rid of the burden of defining a covector and all of its preliminary concepts. In this sense, our representation of a dyadic tensor $\vto{u}^*\otimes\vto{v}^*$ is subverted to $\vto{u}\otimes\vto{v}$ and the contractive product operator is suppressed from the precise representation $(\vto{u}^*\otimes\vto{v}^*)\odot_1 \vto{x}^*=(\vto{v}\cdot\vto{x})\vto{u}^*$ in order to avoid defining a contraction and its supporting concepts. Finally, they define the ``tensor product'' $\vto{u}\otimes\vto{v}$ through its effects, that is, as a ``tensor'' which causes $(\vto{u}\otimes\vto{v})\vto{x}=(\vto{v}\cdot\vto{x})\vto{u}$, where vector $\vto{x}$ is arbitrary.}. Since this generalized inner product observes the axioms of inner products on page \pageref{prop:produtoInterno}, tensor space $\ete{\real}{W^{\times p}}$ is naturally an inner product space. 


If a tensor space can be structured as any other ordinary vector space, it is possible to consider  
a function space  $\ete{\cam{F}}{V^{\times n}}_\cam{F}^{\ete{\cam{F}}{U^{\times m}}}$
...
Henceforth, we shall be dealing only with tensors defined by Hilbert spaces, since it is not our purpose to work with first order tensors which are not covectors. In this scope of study, a function which enables a relationship between tensors of any order we call it a tensor function, which, unless otherwise stated, we shall represent it with bold greek letters: the function in $\map{\vtf{\psi}}{\ete{\cam{F}}{U^{\times m}}}{\ete{\cam{F}}{V^{\times p}}}$ is then a tensor function. It is important to say that, when conditions described at the beginning of section \ref{sec:FuncLin} are considered, tensor functions can constitute function spaces, which may be structured to follow the same nomenclature presented in figure \ref{fig:esquemaEspacos}. Moreover, this context of tensors on Hilbert spaces enables us to always relate a multiantilinear function and a linear tensor function, which is a fundamental feature to ensure uniqueness and existence of a fundamental linear tensor function to be presented in this section, which we call contraction. The following theorem\footnote{Adapted from \aut{Backus}\cite{backus_1997_1} after generalizing a property from  \aut{Marcus}\cite{marcus_1973_1}, p.14.}, borrowed from the realm of Multilinear Algebra, describes such feature by establishing a one to one correspondence between a multiantilinear function and a linear tensor function. From this biunivocal relationship, the multiantilinear function is usually said to be ``lifted'' to a tensor function.   

\begin{mteo}{Lifting of Multiantilinear Functions}{lift}
	Considering function spaces $V^{U^{\times m}}_\cam{F}$ and  $\evl{\cam{F}}{\ete{\cam{F}}{U^{\times m}}}{V}$, where sets $U_i$ and $V$ define Hilbert spaces, for every multiantilinear function $\vtf{c}\in V^{U^{\times m}}_\cam{F}$ there is one and only one linear tensor function  \gloref{elevC}$\in\evl{\cam{F}}{\ete{\cam{F}}{U^{\times m}}}{V}$ such that 
	\begin{equation*}
	\fua{\vtf{c}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\fua{{\vtf{c}}}{\vto{x}_1,\cdots,\vto{x}_m}\,,\,\,\forall\,\vto{x}_i\in U_i.
	\end{equation*}
\end{mteo}
\hspace{1pt}
{\footnotesize
	\begin{proof}
		The following proof is adapted from \aut{Backus}\cite{backus_1997_1}, pp. 43-45. According to theorem \ref{teo:geraEte}, let $\tnr{X}\in\ete{\cam{F}}{U^{\times m}}$ be an arbitrary tensor described by 
		\begin{equation}
		\tnr{X}=\sum_{i_1=1}^{n_1}\cdots\sum_{i_p=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
				i_p}^{B}}{\tnr{X}}\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m}\,,
		\nonumber
		\end{equation}
		where $B$ is a basis of $\ete{\cam{F}}{U^{\times m}}$ defined by orthonormal bases  $\con{B}_i=\{\vun{u}_{1}^{(i)},\cdots,\vun{u}_{n_i}^{(i)}\}$ of $U_i$. According to rule \eqref{eq:regraOrtonmFunc}, the function $\vtf{c}^\otimes$ that observes $\fua{\vtf{c}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\fua{{\vtf{c}}}{\vto{x}_1,\cdots,\vto{x}_m}$ really exists and is uniquely determined by $\vtf{c}$, as can be verified in  
		\begin{eqnarray}
		\fua{\vtf{c}^\otimes}{\tnr{X}}&=&\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
				i_m}^{B}}{\tnr{X}}\fua{\vtf{c}^\otimes}{\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m}}\nonumber\\
		&=&\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
...
As we mentioned in the previous paragraph, a special linear tensor function whose existence and uniqueness is assured by lifting multiantilinear functions is called contraction. In simple words, this function relates a tensor of order $m\geq p$ and a tensor of order $m-p$, when we say that it ``contracts'' the order of its argument. Mathematically, in the context of the previous theorem, the tensor function \gloref{contrac}$\in\evl{\cam{F}}{\ete{\cam{F}}{U^{\times m}}}{\ete{\cam{F}}{U^{\times m-p}}}$ is said to be a \textsb{contraction}\index{contraction} of order $p$ if it is lifted from the antilinear function in $\map{\tnr{c}_p}{U^{\times m}_\cam{F}}{\ete{\cam{F}}{U^{\times m-p}}}$ in such a way that the $(m-p)$-th order tensor
\begin{equation}
\fua{\vtf{c}_p^\otimes}{\tnr{X}}=\fua{{\vtf{c}}_p}{\vtf{x}_1,\cdots,\vtf{x}_m}\,,
\end{equation}
where $\tnr{X}$ is an arbitrary element of $\ete{\cam{F}}{U^{\times m}}$ that can be described as a polyadic tensor $\vtf{x}_1^*\otimes\cdots\otimes\vtf{x}_m^*$, according to equalities \eqref{eq:TensPoliadic}. 

In our study, we deal with two different types of contraction, expressed by two different rules for ${\vtf{c}}_p$. The first type is called trace of tensor, valid for tensor spaces of order greater than one and defined as follows. Still considering the above conditions, if $U^{\times m}=U_1\times\cdots\times U_{r}\times\cdots\times U_{s}\times\cdots\times U_m$ where $U_{r}=U_{s}$ and $m\geq 2$, the tensor $\fua{\vtf{c}_2^\otimes}{\tnr{X}}$ of order $m-2$ is a contraction called the $r,s$ trace of $\tnr{X}$, represented by \gloref{tracoRS}$\tnr{X}$, when 
\begin{equation*}
\fua{{\vtf{c}}_2}{\vto{x}_1,\cdots,\vto{x}_m}:=\overline{\fua{\tnr{I}}{\vto{x}_{r},\vto{x}_{s}}}\,\vto{x}_1^*\otimes\cdots\otimes\vto{x}_{r-1}^*\otimes\vto{x}_{r+1}^*
\otimes\cdots\otimes\vto{x}_{s-1}^*\otimes\vto{x}_{s+1}^*\otimes\cdots\otimes\vto{x}_m^*\,,
\end{equation*}
where $\tnr{I}$ is the second order identity tensor of $\ete{\cam{F}}{U_r\times U_s}$. In other words, the lifting of this function $\vtf{c}_2$ results the $r,s$ trace function and thus
\begin{equation}
\trt{r}{s} \tnr{X}=\overline{\fua{\tnr{I}}{\vto{x}_{r},\vto{x}_{s}}}\,\vto{x}_1^*\otimes\cdots\otimes\vto{x}_{r-1}^*\otimes\vto{x}_{r+1}^*
\otimes\cdots\otimes\vto{x}_{s-1}^*\otimes\vto{x}_{s+1}^*\otimes\cdots\otimes\vto{x}_m^*\,.
\end{equation}
It is important to observe that in the context of Euclidean spaces, from equalities \eqref{eq:prodIntGen} and \eqref{eq:identidade}, we can write that $\overline{\fua{\tnr{I}}{\vto{x}_{r},\vto{x}_{s}}}=\vto{x}_{r}\cdot\vto{x}_{s}\,$. 

Since the contraction that follows will take part in the majority of forthcoming definitions in this chapter, it is more relevant than the $r,s$ trace for the purposes of our study. In order to define this important contraction using conditions above, let's consider that $U^{\times m}=A^{\times q}\times W^{\times p}\times W^{\times p}\times B^{\times n}$,
where $m=q+2p+n$, that tensor $\tnr{A}:=\vto{a}_1^*\otimes\cdots\otimes\vto{a}_{q}^*\otimes\vto{x}_1^*\otimes\cdots\otimes\vto{x}_{p}^*$ belongs to tensor space $\ete{\cam{F}}{A^{\times q}\times W^{\times p}}$ and tensor $\tnr{B}:=\vto{y}_1^*\otimes\cdots\otimes\vto{y}_{p}^*\otimes\vto{b}_1^*\otimes\cdots\otimes\vto{b}_{n}^*$ belongs to $\ete{\cam{F}}{W^{\times p}\times B^{\times n}}$. When 
\begin{align*}
\fua{{\vtf{c}}_{2p}}{\vto{a}_1,\cdots,\vto{a}_q,&\vto{x}_1,\cdots,\vto{x}_p,\vto{y}_1,\cdots,\vto{y}_p,\vto{b}_1,\cdots,\vto{b}_n}:=
\\&\overline{\fua{\tnr{I}}{\vto{x}_{1},\cdots,\vto{x}_{p},\vto{y}_1,\cdots,\vto{y}_p}}\,\vto{a}_1^*\otimes\cdots\otimes\vto{a}_{q}^*\otimes\vto{b}_{1}^*\otimes\cdots\otimes\vto{b}_n^*\,,
\end{align*} 
