...
Considering  an arbitrary unitary operator $\tnr{\psi}\in\grt{U}{\cam{F}}{U^{\times m}}$, tensor bases $B=\{\tnr{B}_1,\cdots,\tnr{B}_p\}$ and  $C=\{\fua{\tnr{\psi}}{\tnr{B}_1},\cdots,\fua{\tnr{\psi}}{\tnr{B}_p}\}$ of $\etom{\cam{F}}{U^{\times m}}$, where $p=n^m$, equalities $\fua{\tnr{f}_{i}^B}{\tnr{X}}=\fua{\tnr{f}_{i}^C}{\tnr{X}}$ are valid for arbitrary $\tnr{X}\in\etom{\cam{F}}{U^{\times m}}$. Moreover, if $\fua{\tnr{f}_{i}^B}{\tnr{T}}=\fua{\tnr{f}_{i}^C}{\tnr{T}}$ for a certain tensor $\tnr{T}\in\eteg{n}{\cam{F}}{U^{\times m}}$, then $\tnr{T}$ also belongs to $\etom{\cam{F}}{U^{\times m}}$.
\end{mteo}
\hspace{1pt}
{\footnotesize
\begin{proof}
From theorem \ref{teo:basesUnit}, we know that set $C$ is indeed a basis of $\etom{\cam{F}}{U^{\times m}}$. As $\fua{\tnr{\psi}}{\tnr{X}}=\tnr{X}$, equalities
\begin{alignat*}{3}
...
Admitting a tensor space $\eteg{n}{\cam{F}}{U^{\times m}\times U^{\times m}}$ and a group $\etom{\cam{F}}{U^{\times m}\times U^{\times m}}$, by equality \eqref{eq:coordIdent} we know that the coordinates of identity tensor $\tnr{I}\in\eteg{n}{\cam{F}}{U^{\times m}\times U^{\times m}}$ are the same for any chosen basis. Therefore, from theorem above, it is clear that $\tnr{I}$ is anti isotropic, that is, $\tnr{I}\in\etom{\cam{F}}{U^{\times m}\times U^{\times m}}$.
