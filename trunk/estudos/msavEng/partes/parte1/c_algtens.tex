...
\begin{itemize}
\setlength\itemsep{.1em}
\item[i.] A set $\{\tnr{A}_B\}$ is a basis of alternating tensor space $\etng{m}{\cam{F}}{V^m}$ where 
...
\item[iii.] If $\fua{\tnr{A}}{\vto{v}_1,\cdots,\vto{v}_m}=0$, for all non-zero $\tnr{A}\in\etng{m}{\cam{F}}{V^m}$, then $\{\vto{v}_1,\cdots,\vto{v}_m\}\subset V_\cam{F}$ is linearly dependent.
\item[iv.] Given an arbitrary operator $\vtf{g}\in\grc{U}{\cam{F}}{V}$, an alternating tensor $\tnr{A}\in\etng{m}{\cam{F}}{V^m}$ and a basis $B=\{\vto{u}_1,\cdots,\vto{u}_m\}$ of $V_\cam{F}$, then
...
\end{align*}
from which we conclude $\tnr{P}=\alpha\tnr{A}_B$, that is, $\{\tnr{A}_B\}$ is a basis of one-dimensional $\etng{m}{\cam{F}}{V^m}$ and then property i is proved. Let's verify now items ii and iii. From the definition of $\tnr{A}_B$ it is clear that 
\begin{align*}
...
\end{align*}
Let's prove property iii. Since non zero tensor $\tnr{A}=\alpha\tnr{A}_B$, then $\alpha\neq 0$ and
\begin{equation*}
\alpha\tnr{A}_B(\vto{v}_1,\cdots,\vto{v}_m)=\alpha\sum_{i_1=1}^m\cdots\sum_{i_m=1}^m\fua{\vtf{f}^\con{B}_{i_1}}{\vto{v}_1}\cdots\fua{\vtf{f}^\con{B}_{i_m}}{\vto{v}_m}\epsilon_{i_1\cdots i_m}=0\,.
\end{equation*}
Theses equalities are valid only if $\epsilon_{i_1\cdots i_m}$ is zero. This is only possible when some vector $\vto{v}_{a}$ is a linear combination of the other vectors. Without losing generality, this statement can be verified if we admit $a=1$, which leads to $\vto{v}_{1}=\sum_{j=2}^m\beta_j\vto{v}_j$. Thereby, 
\begin{equation*}
\alpha\sum_{j=2}^m\beta_j\tnr{A}_B(\vto{v}_j,\vto{v}_2,\cdots,\vto{v}_m)=\alpha\sum_{j=2}^m\beta_j\sum_{i_j=1}^m\cdots\sum_{i_m=1}^m\fua{\vtf{f}^\con{B}_{i_j}}{\vto{v}_j}\fua{\vtf{f}^\con{B}_{i_2}}{\vto{v}_2}\cdots\fua{\vtf{f}^\con{B}_{i_m}}{\vto{v}_m}\epsilon_{i_ji_2\cdots i_m}
\end{equation*}
is obviously equal to zero since $\epsilon_{i_ji_2\cdots i_m}=0$ because $i_j\in\{i_2,\cdots,i_m\}$. Now, in order to verify item iv, from \eqref{eq:igAntis} and \eqref{eq:DeterminanteSimplificado}, we develop as follows 
\begin{align*}
...
A tensor $\tnr{H}\in\etng{m}{\cam{F}}{U^m}$, where $U_\cam{F}$ is a Hilbert space, is called \textsb{unimodular}\index{tensor!unimodular} if scalar $|\fua{\tnr{H}}{\vun{u}_1,\cdots,\vun{u}_m}|=1$ for every orthonormal basis $\{\vun{u}_1,\cdots,\vun{u}_m\}$. If tensors $\tnr{X}$ and $\tnr{H}$ are unimodular, we have $|\fua{\tnr{X}}{\vun{u}_1,\cdots,\vun{u}_m}|=|\fua{\tnr{H}}{\vun{u}_1,\cdots,\vun{u}_m}|$ and then $\tnr{X}=\pm\tnr{H}$. Thereby, we can affirm that if tensor $\tnr{H}$ is unimodular, so is $-\tnr{H}$. In this context, we can obtain that the basis tensor $\tnr{A}_B$ of $\etng{m}{\cam{F}}{U^m}$ is unimodular and then since any other vector in this alternating tensor space can be expressed as $\alpha\tnr{A}_B$, there are only two unimodular tensors in $\etng{m}{\cam{F}}{U^m}$ relative to basis $B$, namely, $\tnr{A}_B$ and $-\tnr{A}_B$. In this sense, considering $\tnr{A}\in\etng{m}{\cam{F}}{U^m}$ unimodular and $B=\{\vun{u}_1,\cdots,\vun{u}_m\}$ an arbitrary orthonormal basis of $U_\cam{F}$, the double $(U_\cam{F},\tnr{A})$ is said to be an \textsb{oriented vector space}\index{vector space!oriented}, where basis $B$ is called \textsb{positively}\index{basis!positively oriented} or \textsb{negatively oriented}\index{basis!negatively oriented} if scalar $\fua{\tnr{A}}{\vun{u}_1,\cdots,\vun{u}_m}$ is $+1$ or $-1$ respectively. Note that basis $B$ of $(U_\cam{F},\tnr{A}_B)$ is always positively oriented. In this context, we obtain that a basis $\{\fua{\vtf{h}}{\vun{u}_1},\cdots,\fua{\vtf{h}}{\vun{u}_m}\}$, for all $\vtf{h}\in\grc{U^+}{\cam{F}}{V}$, is positively oriented. Moreover, if basis $\{\vun{v}_1,\cdots,\vun{v}_m\}$ is negatively oriented, so is $\{\fua{\vtf{h}}{\vun{v}_1},\cdots,\fua{\vtf{h}}{\vun{v}_m}\}$; and then we can say that \emph{basis orientation is immune to proper unitary operators}. Now, since $\tnr{A}_B$ is unimodular, given an arbitrary alternating tensor $\tnr{P}\in\etng{m}{\cam{F}}{U^m}$, its coordinate elements
\begin{equation*}
\vtf{f}_{i_1\cdots i_m}^{X}(\tnr{P})= \fua{\tnr{P}}{\vun{x}_{i_1},\cdots,\vun{x}_{i_m}}=
\fua{\alpha\tnr{A}_B}{\vun{x}_{i_1},\cdots,\vun{x}_{i_m}}=\alpha\epsilon_{i_1\cdots i_m}\fua{\tnr{A}_B}{\vun{x}_{1},\cdots,\vun{x}_{m}}
\end{equation*}
equal to $\alpha\epsilon_{i_1\cdots i_m}$ or $-\alpha\epsilon_{i_1\cdots i_m}$ when basis $\{\vun{x}_{1},\cdots,\vun{x}_{m}\}$ of $(U_\cam{F},\tnr{A}_B)$ is positively or negatively oriented respectively. In other words, the coordinates of an alternating tensor are identical for any positively (negatively) oriented basis. 


{\footnotesize
\begin{proof}
Lets verify $\tnr{A}_B$ is indeed unimodular. Given $B=\{\vun{u}_1,\cdots,\vun{u}_m\}$ and $C=\{\vun{v}_1,\cdots,\vun{v}_m\}$ arbitrary orthonormal bases of $U_\cam{F}$,  we know that there is always a unitary operator $\vtf{g}\in\grc{U}{\cam{F}}{U}$ defined by \eqref{eq:unitOrthon} where $\fua{\vtf{g}}{\vun{u}_i}=\vun{v}_i$. Therefore, from definitions \eqref{eq:DeterminanteSimplificado} and \eqref{eq:basisAltern}, we have  
\begin{align*} 
...
\end{equation}
from which we conclude for an arbitrary basis $B$ of $\ete{\cam{F}}{V^{\times p}\times V^{\times p}}$ that
\begin{equation}\label{eq:coordIdent}
\vtf{f}_{i_1\cdots i_pj_1\cdots j_p}^{B}(\tnr{I})=\prod_{k=1}^p \delta_{i_kj_k}
\end{equation}
It is important to say that the previous results obtained for an identity tensor of order $2p$ are also valid for identity tensors of order $\alpha p$, where $\alpha$ is even.
...
\end{equation*}
from which we conclude that $C$ spans $\ete{\cam{F}}{U^{\times p}}$ and then $\fua{\vtf{f}^{B}_{i_1\cdots i_p}}{\tnr{T}}=\fua{\vtf{f}^{C}_{i_1\cdots i_p}}{\tnr{X}}$.
\end{proof}
}

\begin{mcoro}{Orthonormal Bases and Unitary Tensor Operators}{orthobasesUnit}
Given $\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{C_p}:=\fua{\tnr{\psi}}{\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}}$, if basis $B_i$ of $(U_\cam{F})_i$ is orthonormal, so is $C_i$.
\end{mcoro}

{\footnotesize  
\begin{proof}
As $\tnr{\psi}$ preserves inner product, the following development prove the theorem.
\begin{align*}
\prod_{k=1}^p\delta_{i_kj_k}&=\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}\cdot\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_p}^{B_p}\\
&=\fua{\tnr{\psi}}{\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}}\cdot\fua{\tnr{\psi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_p}^{B_p}}\\
&=\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{C_p}\cdot\vtf{f}_{j_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{j_p}^{C_p}\,.
\end{align*}
\end{proof}
}

Considering the context of previous corollary for a tensor space $\eteg{n}{\cam{F}}{U^{p}}$, we have $\vtf{f}_{i_1}^{C}\otimes\cdots\otimes\vtf{f}_{i_p}^{C}:=\fua{\tnr{\psi}}{\vtf{f}_{i_1}^{B}\otimes\cdots\otimes\vtf{f}_{i_p}^{B}}$. As we already know, the pair of bases $B$ and $C$ can be related through a unitary operator $\vtf{g}\in\grc{U}{\cam{F}}{U}$, that is, if $B=\{\vun{u}_1,\cdots,\vun{u}_n\}$ then basis  $C=\{\fua{\vtf{g}}{\vun{u}_1},\cdots,\fua{\vtf{g}}{\vun{u}_n}\}$. Thereby, it is possible to write that 
\begin{equation}\label{eq:opeTensOrdin}
\fua{\vtf{g}}{\vun{u}_{i_1}}^*\otimes\cdots\otimes\fua{\vtf{g}}{\vun{u}_{i_p}}^*:=\fua{\tnr{\psi}}{\vun{u}_{i_1}^*\otimes\cdots\otimes\vun{u}_{i_p}^*}\,,
\end{equation}
where an arbitrary unitary tensor operator is always related to a unitary operator. 

The concept of tensor function, up to now, has enabled us to arrive at dual and inner product tensor spaces from the definitions of linear tensor functional and contractive product respectively. In comparative terms, we are now in a similar situation to that which preceded the essential Riesz-Fréchet Theorem in section \ref{sec:FuncLin}. Then, an interesting question naturally arises: ``Considering a Hilbert tensor space, is it possible for each of its tensors to have a corresponding `cotensor' just like a vector in Hilbert spaces has a covector?'' An affirmative answer to this question would be convenient for our purposes because we could properly replace in suitable cases the multilinearity of tensors by the linearity of the so called cotensors. In practice, considering a proper relationship between the values of tensor and cotensor, we could be able to approach tensor differentiation by working with single variable tensor functions instead of tensors themselves, inherently multivariable. The remaining text of this section shows that the concept of \textsb{cotensor}\index{cotensor} is indeed feasible.
...
\end{equation}
Under conditions suitable for defining identity tensors, we already know that equality \eqref{eq:coordIdent} is valid for any basis. Therefore, we consider
\begin{equation}
...
\end{equation}  
is valid for all $\tnr{P}\in\{\etn{\cam{F}}{A^p}\setminus \tnr{0}\}$ and all $\tnr{X}_i\in\eteg{n}{\cam{F}}{A}$. According to theorem \ref{teo:repRieszTensor}, in the particular case where operator $\tnr{\psi}$ is a $m$-cotensor of $\tnr{T}\in \ete{\cam{F}}{A^2}$, that is, $\tnr{T}_m^*=\tnr{\psi}$, it is possible to state that $\hdet{\tnr{T}}:=\hdet{\tnr{\psi}}$ and  
\begin{equation}
\fua{\tnr{P}}{\fua{\tnr{T}_m^*}{\tnr{X}_1},\cdots,{\fua{\tnr{T}_m^*}{\tnr{X}_p}}}=\hdet{\tnr{T}}{\fua{\tnr{P}}{\tnr{X}_1,\cdots,\tnr{X}_p}}\,.
\end{equation}
Now, considering arbitrary functions $\tnr{\psi},\tnr{\varphi}\in\evl{\cam{F}}{\eteg{n}{\cam{F}}{A}}{\eteg{n}{\cam{F}}{A}}$, the following properties of determinant of tensor operators are valid:
\begin{itemize}
\setlength\itemsep{.1em}
\item[i.] $\hdet{\tnr{i}} =  1$;
...
\item[v.] If $B$ and $C$ are bases of $\eteg{n}{\cam{F}}{A}$, then $\hdet{\tnr{\psi}}=\hdet{\maf{\tnr{\psi}}{B}{C}}$;
...
This theory of determinant presented so far is obviously valid for the context of order $m=1$, that is, the context of an alternating tensor space $\etng{n}{\cam{F}}{U^n}$. Equalities \eqref{eq:alternDete} is an example of this 
The following theorem considers the result obtained by equality \eqref{eq:opeTensOrdin}, where a unitary tensor operator is always related to an ordinary unitary operator. 

\begin{mteo}{Related Unitary Operators and Alternating Tensors}{orthogAltern}
Considering a tensor $\tnr{A}\in\etng{n}{\cam{F}}{U^n}$, a unitary tensor operator $\tnr{\psi}\in\grt{U}{\cam{F}}{U^{n}}$ related to a unitary operator $\vtf{g}\in\grc{U}{\cam{F}}{U}$ through $\fua{\vtf{g}}{\vun{u}_{i_1}}^*\otimes\cdots\otimes\fua{\vtf{g}}{\vun{u}_{i_n}}^*=\fua{\tnr{\psi}}{\vun{u}_{i_1}^*\otimes\cdots\otimes\vun{u}_{i_n}^*}$, where  set $B=\{\vun{u}_1,\cdots,\vun{u}_n\}$ is an orthonormal basis of $U_\cam{F}$, tensor $\tnr{A}=\dete{\vtf{g}}\fua{\tnr{\psi}}{\tnr{A}}$.
\end{mteo}
{\footnotesize
\begin{proof}
As unitary operators preserve inner product, from \eqref{eq:igAntis}, \eqref{eq:propProdIndTens} and \eqref{eq:detFunDef}, we can write 
\begin{align*}
\fua{\tnr{A}}{\fua{\vtf{g}}{{\vun{u}_{i_1}}},\cdots,\fua{\vtf{g}}{\vun{u}_{i_n}}}&=\dete{\vtf{g}}\,\fua{\tnr{A}}{\vun{u}_{i_1},\cdots,\vun{u}_{i_n}}\\
\epsilon_{i_1\cdots i_n} \tnr{A} \cdot (\fua{\vtf{g}}{{\vun{u}_{1}}}^*\otimes\cdots\otimes\fua{\vtf{g}}{\vun{u}_{n}}^*)&=\epsilon_{i_1\cdots i_n}\dete{\vtf{g}}\, \tnr{A} \cdot (\vun{u}_{1}^*\otimes\cdots\otimes\vun{u}_{n}^*)\\
\tnr{A} \cdot \fua{\tnr{\psi}}{\vun{u}_{1}^*\otimes\cdots\otimes\vun{u}_{n}^*} &= \dete{\vtf{g}}\, \tnr{A} \cdot (\vun{u}_{1}^*\otimes\cdots\otimes\vun{u}_{n}^*)\\
\fua{\tnr{\psi}\circ\tnr{\psi}^{-1}}{\tnr{A}} \cdot \fua{\tnr{\psi}}{\vun{u}_{1}^*\otimes\cdots\otimes\vun{u}_{n}^*}&=\dete{\vtf{g}}\, \tnr{A} \cdot (\vun{u}_{1}^*\otimes\cdots\otimes\vun{u}_{n}^*)\\
\fua{\tnr{\psi}^{-1}}{\tnr{A}} \cdot (\vun{u}_{1}^*\otimes\cdots\otimes\vun{u}_{n}^*)&=\dete{\vtf{g}}\, \tnr{A} \cdot (\vun{u}_{1}^*\otimes\cdots\otimes\vun{u}_{n}^*)\\
\tnr{A} &= \dete{\vtf{g}}\, \fua{\tnr{\psi}}{\tnr{A}}\,.
\end{align*} 
\end{proof}
}

Now that we know the determinant of linear tensor operators and its properties, let $\eteg{n}{\cam{F}}{U^{\times m}}$ be a Hilbert tensor space and function $\tnr{\varphi}$ be an arbitrary element of a group $\grt{G}{\cam{F}}{U^{\times m}}$ of invertible continuous linear tensor operators whose domain is $\eteg{n}{\cam{F}}{U^{\times m}}$. A tensor $\tnr{T}\in\eteg{n}{\cam{F}}{U^{\times m}}$ is said to be \textsb{invariant}\index{tensor!invariant} on group $\grt{G}{\cam{F}}{U^{\times m}}$ if $\fua{\tnr{\varphi}}{\tnr{T}}=\tnr{T}$. This equality and the linearity of $\tnr{\varphi}$ enable us to affirm that a set $V$ constituted by all tensors of $\eteg{n}{\cam{F}}{U^{\times m}}$ that are invariant on group $\grt{G}{\cam{F}}{U^{\times m}}$ is a tensor subspace of $\eteg{n}{\cam{F}}{U^{\times m}}$, when $V$ is represented by the invariant tensor space $\eti{\cam{F}}{U^{\times m}}$ on group $\grt{G}{\cam{F}}{U^{\times m}}$. This group is represented by $\grt{V}{\cam{F}}{U^{\times m}}$ and called the \textsb{invariance group}\index{group!invariant} of $\eti{\cam{F}}{U^{\times m}}$ if it is constituted by all the invertible continuous linear tensor operators on which every element of $\eti{\cam{F}}{U^{\times m}}$ is invariant. Thus, it is evident that operator $\tnr{i}\in\grt{V}{\cam{F}}{U^{\times m}}$. The unitary group $\grt{U}{\cam{F}}{U^{\times m}}\subseteq\grt{V}{\cam{F}}{U^{\times m}}$ is the invariance group of the invariant tensor space $\eto{\cam{F}}{U^{\times m}}$, whose tensors are said to be \textsb{isotropic}\index{tensor!isotropic}\footnote{\aut{Appleby et al}\cite{appleby_1987} define an isotropic tensor relative to certain changes of bases as the tensor whose coordinates are preserved under these changes, which is assured in our approach by all unitary operators.} relative to $\grt{U}{\cam{F}}{U^{\times m}}$, and then $\eti{\cam{F}}{U^{\times m}}\subseteq\eto{\cam{F}}{U^{\times m}}$. Moreover, the unitary group $\tnr{\mathcal{U}}^+_{\cam{F}}({U^{\times m}})\subset\grt{U}{\cam{F}}{U^{\times m}}$, where $\hdet{\tnr{\varphi}}=1$, $\forall\,\tnr{\varphi}\in\tnr{\mathcal{U}}^+_{\cam{F}}({U^{\times m}})$, is the invariance group of $\etom{\cam{F}}{U^{\times m}}$, whose tensors are called \textsb{anti isotropic}\index{tensor!anti isotropic} relative to $\tnr{\mathcal{U}}^+_{\cam{F}}({U^{\times m}})$ and tensor space $\eto{\cam{F}}{U^{\times m}}\subset\etom{\cam{F}}{U^{\times m}}$. Then, we can say that every isotropic tensor is also anti isotropic. Tensors that are not isotropic are called \textsb{anisotropic}\index{tensor!anisotropic}. From definitions \eqref{eq:HilbAdj}, in the context of Euclidean spaces, we can say that (anti) isotropic tensors are defined by (proper) orthogonal groups.  


\begin{mteo}{Coordinate Invariance of Isotropic Tensors}{coordIsot}
Considering  an arbitrary unitary operator $\tnr{\psi}\in\grt{U}{\cam{F}}{U^{\times m}}$ where $\dim{({U_\cam{F}})_i=n}$, tensor bases $B=\{\tnr{B}_1,\cdots,\tnr{B}_p\}$ and  $C=\{\fua{\tnr{\psi}}{\tnr{B}_1},\cdots,\fua{\tnr{\psi}}{\tnr{B}_p}\}$ of $\eto{\cam{F}}{U^{\times m}}$, where $p=n^m$, equalities $\fua{\tnr{f}_{i}^B}{\tnr{X}}=\fua{\tnr{f}_{i}^C}{\tnr{X}}$ are valid for arbitrary $\tnr{X}\in\eto{\cam{F}}{U^{\times m}}$. Moreover, if $\fua{\tnr{f}_{i}^B}{\tnr{T}}=\fua{\tnr{f}_{i}^C}{\tnr{T}}$ for a certain tensor $\tnr{T}\in\eteg{n}{\cam{F}}{U^{\times m}}$, then $\tnr{T}$ also belongs to $\eto{\cam{F}}{U^{\times m}}$.
\end{mteo}

{\footnotesize
\begin{proof}
First, let's verify if set $V$ is indeed a vector space of invariant tensors on $\grt{G}{\cam{F}}{U^{\times m}}$. Considering operator $\tnr{\varphi}\in\grt{G}{\cam{F}}{U^{\times m}}$, $\alpha,\beta\in\cam{F}$ and $\tnr{A},\tnr{B}\in V$ arbitrary elements of their respective sets, the following equalities $\alpha\tnr{A}+\beta\tnr{B}=\fua{\alpha\tnr{\varphi}}{\tnr{A}}+\beta\fua{\tnr{\varphi}}{\tnr{B}}=\fua{\tnr{\varphi}}{\alpha\tnr{A}+\beta\tnr{B}}$ prove that sum and multiplication by scalar of invariant tensors result in an invariant tensor, since $\tnr{\varphi}$ is linear. This condition also justifies that $\tnr{0}\in V$ because $\fua{\tnr{\varphi}}{\tnr{0}}=\tnr{0}$. The verification of other axioms of vector spaces is trivial. Now, let's prove the theorem above. From theorem \ref{teo:basesUnit}, we know that set $C=\{\fua{\tnr{\psi}}{\tnr{B}_1},\cdots,\fua{\tnr{\psi}}{\tnr{B}_p}\}$ is indeed a basis of $\eto{\cam{F}}{U^{\times m}}$. As $\fua{\tnr{\psi}}{\tnr{X}}=\tnr{X}$, equalities
\begin{alignat*}{3}
\fua{\tnr{\psi}}{\tnr{X}}=\sum_{{i}=1}^p\fua{\tnr{f}_{i}^B}{\tnr{X}} \fua{\tnr{\psi}}{\tnr{B}_i} &\qquad \text{and}\qquad& \tnr{X}=\sum_{{i}=1}^p\fua{\tnr{f}_{i}^C}{\tnr{X}} \fua{\tnr{\psi}}{\tnr{B}_i}
\end{alignat*}
enable us to verify that coordinates $\fua{\tnr{f}_{i}^B}{\tnr{X}}=\fua{\tnr{f}_{i}^C}{\tnr{X}}$. Considering these same equalities for the case of a tensor $\tnr{T}$ that has equal coordinates on bases $B$ and $C$, the conclusion of $\fua{\tnr{\psi}}{\tnr{T}}=\tnr{T}$ is straightforward.
\end{proof}
}

Admitting a tensor space $\ete{\cam{F}}{A^p}$ and a group $\grt{U}{\cam{F}}{A^p}$, where $A=U^{m}$ and $p$ is even, by equality \eqref{eq:coordIdent} we know that the coordinates of identity tensor $\tnr{I}\in\ete{\cam{F}}{A^p}$ are the same for any chosen basis. Therefore, from theorem above, it is clear that $\tnr{I}$ is isotropic, that is, $\tnr{I}\in\eto{\cam{F}}{A^p}$. In this context, again by equality \eqref{eq:coordIdent}, it is straightforward to see that the coordinates of an arbitrary tensor belonging to $\spn(\{\tnr{I}\})$ are the same for any chosen basis of $\ete{\cam{F}}{A^p}$. Therefore, it is possible to affirm that \emph{any linear combination of an identity tensor is isotropic}. Now, given an arbitrary unimodular tensor $\tnr{H}\in\etng{m}{\cam{F}}{U^m}$, considering $B_i$ an orthonormal basis in the context of theorem \ref{teo:geraEte}, polyadic tensors $\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}$ and $\fua{\tnr{\varphi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}}$ elements of bases $B$ and $C$ of $\etng{m}{\cam{F}}{U^m}$ respectively, where $\tnr{\varphi}\in\tnr{\mathcal{U}}^+_{\cam{F}}({U^{m}})$, from equalities 
\begin{equation*}
\fua{\tnr{\varphi}}{\tnr{H}}=\sum_{{i_1}=1}^m\cdots\sum_{{i_m}=1}^m\fua{\tnr{f}_{i_1\cdots i_m}^B}{\tnr{H}}\fua{\tnr{\varphi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}}
\end{equation*}
and 
\begin{equation*}
\tnr{H}=\sum_{{i_1}=1}^m\cdots\sum_{{i_m}=1}^m\fua{\tnr{f}_{i_1\cdots i_m}^C}{\tnr{H}}\fua{\tnr{\varphi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}}
\end{equation*}
...
\begin{align*}
\fua{\tnr{\varphi}}{\tnr{H}}&=\sum_{{i_1}=1}^m\cdots\sum_{{i_m}=1}^m\fua{\tnr{f}_{i_1\cdots i_m}^B}{\tnr{H}}\fua{\tnr{\varphi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}}\\
&=\sum_{{i_1}=1}^m\cdots\sum_{{i_m}=1}^m\fua{\tnr{H}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}
\end{align*}
we conclude that $\tnr{H}$ is anti isotropic.



Moreover, a unimodular tensor $\tnr{H}\in\etng{m}{\cam{F}}{U^m}$,  is a very important example of anti isotropic tensor. Let's see if this is true.   
