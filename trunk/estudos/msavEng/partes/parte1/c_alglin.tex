...
\begin{alignat}{2}
\fua{\vtf{h}}{\alpha\vto{u}_1+\beta\vto{u}_2}=\alpha\fua{\vtf{h}}{\vto{u}_1}+\beta\fua{\vtf{h}}{\vto{u}_2} & \quad\text{and}\quad & \fua{\vtf{h}}{\vto{0}}=\vto{0} \,,
\end{alignat}
for all $\alpha,\beta\in\mathcal{R},\,\vto{u}_1,\vto{u}_2\in U$. Thereby, we call $\vtf{h}$ a \textsb{linear function}\index{function!linear}\label{def:linear} and the corresponding mapping a \textsb{linear transformation}\index{transformation!linear}. If the function space $V^U_\mathcal{R}$ has only linear functions, its usual representation is $\gloref{evl}$. Now, in the case of $U_\mathcal{R}=W^{\times q}_\mathcal{R}$, a function $\vtf{k}$ is said to be \textsb{multilinear}\index{function!multilinear}, or \textsb{bilinear}\index{função!bilinear} if $q=2$, when 
\begin{align}
