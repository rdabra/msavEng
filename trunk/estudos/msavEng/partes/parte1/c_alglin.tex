...
{\footnotesize
\begin{proof}
The proof of $\omega$ linearity is trivial. Now let's verify if it is an injection: considering arbitrary nonzero and distinct $\alpha,\beta\in\cam{F}$, since $\fua{x\vun{u}^*}{\vto{x}}=x(\vto{x}\cdot \vun{u})=\vto{x}\cdot x\vun{u}=\fua{(x\vun{u})^*}{\vto{x}}$, the inequality $\fua{\omega}{\alpha}=(\alpha\vun{u})^*\neq (\beta\vun{u})^*=\fua{\omega}{\beta}$ is always valid because $\alpha\vun{u}\neq \beta\vun{u}$. Injection $\omega$ is also a surjection because, for an arbitrary $\vto{v}^*\in U_\cam{F}^*$, we have $\vto{v}^*=(\|\vto{v}\|\vun{u})^*=\|\vto{v}\|\vun{u}^*=\fua{\omega}{\|\vto{v}\|}$. 
\end{proof}
}
