...
\alpha\vto{v}^c=
\begin{dcases}
\alpha\vto{v}& \text{if } \Im(\alpha)=0\\
\overline{\alpha}\vto{v}& \text{if } \Im(\alpha)\neq 0
\end{dcases}\,,
\end{equation}
for all $\alpha\in\cam{F}$. Note that when field $\cam{F}=\real$, arbitrary vector $\vto{v}$ is always equal to $\vto{v}^c$ and then $\overline{V_\cam{F}}=V_\cam{F}$. From previous equality, we can obtain the following property
...
\end{equation*}
which proves $(\beta_1\vto{x}+\beta_2\vto{y})^c=\overline{\beta_1}\vto{x}^c+ \overline{\beta_2}\vto{y}^c$ for non zero imaginary parts $\Im(\beta_1)$ and $\Im(\beta_2)$. When they are zero, verification is trivial.
\end{proof}}
