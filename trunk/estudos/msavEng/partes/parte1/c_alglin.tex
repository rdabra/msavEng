...
\end{equation}
is unitary and clearly enables $\fua{\vtf{g}}{\vun{u}_i}=\vun{v}_i$. Thereby, \emph{every pair of orthonormal bases can be related through a unitary operator}. If only real fields are considered, every orthogonal operator $\vtf{h}$ is unitary since $\vtf{h}^{-1}=\vtf{h}^\text{T}=\vtf{h}^\dagger$.
