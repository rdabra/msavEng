...
In the previous chapter we said that the set of all unary invertible operators defines a group on the operation of composition. Thereby, let's define a group $\gloref{grGene}:=(\evl{\cam{F}}{V}{V},\circ)$ constituted by all invertible linear operators on $\evl{\cam{F}}{V}{V}$. We shall verify now if the set $X\subset \grc{G}{\cam{F}}{V}$ of all unitary operators on $V_\cam{F}$, since they are unary and invertible, constitutes the group  $\gloref{grUni}:=(X,\circ)$ called \textsb{unitary group}\index{group!unitary} on $V_\cam{F}$. Equalities
\begin{equation}
