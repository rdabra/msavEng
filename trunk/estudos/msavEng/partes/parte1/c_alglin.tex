...
Henceforth, in order to shorten notation, $\gloref{prdint}$ will also be used to represent the inner product $\fua{\xi}{\vto{x},\vto{y}}$. Alternatively, if we had considered the function in $\map{\xi^c}{U_\cam{F}\times \overline{U_\cam{F}}}{\cam{F}}$ to be the inner product, it would have to follow the axioms
