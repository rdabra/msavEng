...
$\vto{u}_j \cdot \vto{v}_i$ must be $\delta_{ji}$. Thereby, we can say that subset $\{\vto{v}_1,\cdots,\vto{v}_n\}$, whose elements are biunivocaly related to the vectors of $B^*$, is the reciprocal basis $B^\perp=\{\vto{u}^1,\cdots,\vto{u}^n\}$, that is, covectors $\vtf{v}_i^*=(\vtf{u}^i)^*=\vtf{f}_i^B$. Then, from the previous theorem, the rule for coordinate functionals can be described by
...
If the covectors of $U_\cam{F}^*$ are continuous, then $U_\cam{F}^*$ is a Hilbert space.
\end{mcoro}
