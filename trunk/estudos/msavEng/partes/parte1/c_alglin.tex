...
\end{align}
for all $\alpha,\beta\in\mathcal{R}$ and $\vto{w},\vto{w}_i\in W_i$. In this context, when domain $U_\mathcal{R}=V_\mathcal{R}^q$, the vectors of $V^{V^q}_\mathcal{R}$ are called \textsb{multilinear operators}\index{operator!multilinear} and the mappings they define are \textsb{multilinear operations}\index{operation!multilinear}. Linear functions, as we presented, can also be continuous and constitute a normed function space if a norm is defined. Thereby, considering a linear mapping $\map{\vtf{h}}{Z_\mathcal{R}}{Y_\mathcal{R}}$ whose domain and codomain are Banach spaces, a necessary and sufficient condition for $\vtf{h}$ to be continuous requires that it is \textsb{bounded}\index{function!bounded}, namely, that there exists a number $\nu\in\real^+$ where 
\begin{equation}\label{eq:funcaoLimitada}
\nu\geqslant \|\fua{\vtf{h}}{\vto{z}}\|/\|\vto{z}\| \,,\forall\, \vto{z}\in \{Z_\mathcal{R}\setminus\{\vto{0}\}\}\,.
\end{equation}
From this condition, a function space $Y_\mathcal{R}^Z$ constituted of linear bounded functions can be called a normed space of linear functions when the minimum of values $\nu$ is defined as norm; in other words, when the following rule is admitted:  
...
\end{equation}
In our study, we define that \emph{a vector space $Y_\mathcal{R}^Z$ of continuous linear functions is not only normed but also metric inner product where the distance $\fua{\varrho}{\vtf{h}_1,\vtf{h}_2} := \|\vtf{h}_1-\vtf{h}_2\|$ and the inner product\label{txt:prodInt} $\vtf{h}\cdot\vtf{h}=\|\vtf{h}\|^2\,\,$ for all $\vtf{h},\vtf{h}_1,\vtf{h}_2\in Y_\mathcal{R}^Z$}.   

Any element of a function space $\mathcal{R}^{U_\mathcal{R}}$, where domain and codomain are complete vector spaces, is called a \textsb{functional}\index{functional}.   

Still considering the previous conditions, if the codomain $V_\mathcal{R}=\mathcal{R}_\mathcal{R}$, an element $\vtf{f}\in \mathcal{R}^{U_\mathcal{R}}$ that defines $\map{\vtf{f}}{U_\mathcal{R}}{\mathcal{R}_\mathcal{R}}$ is called a \textsb{functional}\index{functional}.
