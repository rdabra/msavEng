...
\end{equation}
For the specific case of affine Euclidean spaces, these equalities lead to $|\fua{\zeta}{\vv{ox},\alpha\vv{ox}}|=\|\vv{ox}\|$ for all $\alpha\in\{\real\setminus 0\}$, where the modulus of projection does not depend on the real value $\alpha$. Concerning projections on affine Hilbert spaces, there is a property, presented on the following theorem, that will be important for future geometric concepts.
...
Now, for an affine Euclidean space $\eamd{S}{R}{n}$, if $\theta$ is the angle defined by vectors $\vv{ox},\vv{oy}\in S_\cam{R}$, the linear combination  $\vartheta\vv{oy}/\|\vv{oy}\|$ and the scalar $\vartheta\in\real$ are called respectively the vector and scalar geometric projections of $\vv{ox}$ onto $\vv{oy}$ when 
...
|\cos\theta|&\leq|\vartheta|/|\fua{\zeta}{\vv{ox},\vv{oy}}|\,.
\end{align*}
The last inequality is only valid for all $\theta$ if $|\vartheta|\geq|\fua{\zeta}{\vv{ox},\vv{oy}}|$. Now let's choose $\vv{oy}=\alpha\vv{ox}$, where $\alpha\in\{\real\setminus 0\}$ and $\cos\theta=\pm 1$ since both vectors $\vv{ox}$ and $\vv{oy}$ define the same line. In this case, we know from \eqref{eq:projgeo} that $|\vartheta|=\|\vv{ox}\|$ and also that $|\fua{\zeta}{\vv{ox},\vv{oy}}|=\|\vv{ox}\|$, from which we conclude that $\fua{\zeta}{\vv{ox},\vv{oy}}=\vartheta$. 
\end{proof}}
