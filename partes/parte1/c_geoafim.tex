...
Considering the last property above, the geometrical definition of inner product \eqref{eq:geoProdInt} and arbitrary vectors $\vto{x},\vto{y},\vto{z}\in U$, if $\theta_1$ is the smallest angle defined by $\overline{o\vto{x}\oplus o}$ and $\overline{o(\vto{y}\times\vto{z})\oplus o}$, and angle $\theta_2$ defined by  $\overline{o\vto{y}\oplus o}$ and $\overline{o\vto{z}\oplus o}$, scalar
\begin{equation}
...
\end{equation}
where $\alpha$ is the area of parallelogram defined by $\overline{o\vto{y}\oplus o}$ and $\overline{o\vto{z}\oplus o}$, as we already know. Moreover, it is possible to conclude that from vector $\vto{u}=\beta(\vto{y}\times\vto{z})/\|\vto{y}\times\vto{z}\|$, the line segments $\overline{o\vto{x}\oplus o}$, $\overline{o\vto{y}\oplus o}$ and $\overline{o\vto{z}\oplus o}$ define a parallelepiped of height $\overline{o\vto{u}\oplus o}$ and volume $\alpha\beta$, represented in figure XXX.

%fazer figura do paralelepípedo.
 
\noindent Considering definition \eqref{eq:detTensorSeg} in the present context of an Euclidean three-di\-men\-sional space and $\{\vto{x},\vto{y},\vto{z}\}\subset U$ a linearly independent set, for a second order tensor $\tnr{T}\in\ete{\cam{F}}{U^2}$ equalities
\begin{equation}
...
\end{equation}
show that $\hdet{\tnr{T}}$ measures the volume change on a parallelepiped ``changed'' by $\tnr{T}$.
