...
\begin{itemize}
	...
	\item[vii.] Double cross product: $\vto{u}\times(\vto{v}\times\vto{w})=(\vto{u}\cdot\vto{w})\vto{v}-(\vto{u}\cdot\vto{v})\vto{w}$;
...
{\footnotesize
\begin{proof}
Considering an arbitrary non zero vector $\vto{x}\in U_\real$ and $\tnr{A}_B=\vto{a}\otimes\vto{b}\otimes\vto{c}$, item i is verified by the following equalities:
\begin{equation*}
...
\end{equation*}
By this same procedure, proof of property iv is trivial. Now, property v verified by expression
\begin{equation*}
...
\end{equation*}
and, since $\tnr{A}_B$ is antisymmetric, equalities $(\vto{u}\times\vto{v})\cdot\vto{u}=\tnr{A}_B(\vto{u},\vto{u},\vto{v})=0$ prove item vi. Considering \eqref{eq:coordAltern}, development
\begin{align*}
\vto{u}\times(\vto{v}\times\vto{w})&=
\end{align*} 
