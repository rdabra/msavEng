...
\end{proof}}
\noindent Note that, \emph{in the conext of a certain coordinate system, this vectorization feature enables to identify a point by a vector and vice-versa} 
...
\end{equation}
is a positive-definite Hermitian operator, usually called a \textsb{stretch operator}\index{operator!stretch}, representing, according to \eqref{eq:hemritianTensor} and \eqref{eq:nonnegTensor}, a positive-definite tensor $\tnr{S}=\tnr{S}^\dagger$, called \textsb{stretch tensor}\index{tensor!stretch}, which is the affinity tensor of $\mathit{s}$. In this context and by considering the conditions of the bases in \eqref{eq:hermitTensor}, from which equality $[\tnr{S}]^{B}={[{\vtf{s}}_{B_2}]^{B_1}}$ can be concluded, we can write that the eigenvalues 
\begin{equation}
...
\begin{mteo}{Antisymmetric Tensors and Axial Vectors}{antSymVectors}
