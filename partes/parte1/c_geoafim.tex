...
% COLOCAR A FIGURA DA LINHA E PLANO PARALELOS (30/01)
...
% FAZER FIGURA SOBRE PROJECOES GEOMÉRICAS (3/1)




\section{Affinities}
The subject of Geometry is to study not only morphological sets and their elements, as we have done on the previous section, but also the correlation of geometrical objects, that is, functions whose domain and image are morphological sets. Thereby, in the context of Affine Geometry, this section deals exclusively with affinities, here understood as ``geometrical'' functions that correlate points, defined by linear vector functions. In more precise terms, given a bijective mapping $\map{\mathit{g}}{\eamd{S}{F}{n}}{\eamd{S}{F}{n}}$, an affine space $\eamd{S}{F}{n}$ and a vector space $\evl{\cam{F}}{S}{S}$, we call mapping $\map{\mathit{f}}{\eamd{S}{F}{n}}{\eamd{S}{F}{n}}$ an \textsb{affine transformation}\index{affine!transformation} and its function an \textsb{affinity}\index{affinity} when there is a unique linear bijection $\vtf{f}\in\evl{\cam{F}}{S}{S}$ where
\begin{equation}
\fua{\mathit{f}}{\vto{v}\oplus a}=\fua{\vtf{f}}{\vto{v}}\oplus\fua{\mathit{g}}{a}\,,\,\,\forall\,a\in\eamd{S}{F}{n}\,,\,\vto{v}\in S_\cam{F}\,. 
\end{equation} 
It is straightforward to note that since $\vtf{f}$ and $\mathit{f}$ are bijections, so results affinity $\mathit{f}$. Among other properties, it is possible to obtain that every affinity preserves parallelism and hyperplane dimensionality; in other words, parallel hyperplanes on the  domain remain parallel on the image and $r$-dimensional hyperplanes on the domain, $r\leq n$, remain $r$-dimensional on the image: for example, lines are mapped to lines, planes are mapped to planes and so on. According to corollary \ref{cor:soTens}, there is a second order tensor $\tnr{A}\in\ete{\cam{F}}{S^2}$ where $\vtf{f}=\tnr{A}_1^*\circ\Phi$, and then  previous definition can be rewritten as 
\begin{equation}
\fua{\mathit{f}}{\vto{v}\oplus a}=(\vto{v}^*\odot_1\tnr{A})\oplus\fua{\mathit{g}}{a}\,,\,\,\forall\,a\in\eamd{S}{F}{n}\,,\,\vto{v}\in S_\cam{F}\,,
\end{equation} 
when $\tnr{A}$ is called an \textsb{affinity tensor}\index{tensor!afinity}\index{affinity!tensor}.
Moreover, an affinity $\mathit{f}$ is said to be centered at point $a$, represented by $\mathit{f}_a$, when $\mathit{g}$ is the identity function, that is, when point $\fua{\mathit{f}_a}{\vto{v}\oplus a}=\fua{\vtf{f}}{\vto{v}}\oplus a$. 
