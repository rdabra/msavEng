...
\section{Affinities}
The subject of Geometry is to study not only morphological sets and their elements, as we have done on the previous section, but also the correlation of geometrical objects, that is, functions whose domain and image are morphological sets. Thereby, in the context of Affine Geometry, this section deals exclusively with affinities, here understood as ``geometrical'' functions that correlate points, defined by linear vector functions. In more precise terms, given a Hilbert affine space $\eamd{U}{F}{m}$, a bijective mapping $\map{\mathit{g}}{\eamd{U}{F}{m}}{\eamd{U}{F}{m}}$ and a vector space $\evl{\cam{F}}{U}{U}$, we call  $\map{\mathit{f}}{\eamd{U}{F}{m}}{\eamd{U}{F}{m}}$ an \textsb{affine transformation}\index{affine!transformation} and its function an \textsb{affinity}\index{affinity} when there is a unique linear bijection $\vtf{f}\in\evl{\cam{F}}{U}{U}$ where
\begin{equation}\label{eq:affty}
\fua{\mathit{f}}{\vto{u}\oplus a}=\fua{\vtf{f}}{\vto{u}}\oplus\fua{\mathit{g}}{a}\,,\,\,\forall\,a\in\eamd{U}{F}{m}\,,\,\vto{u}\in U_\cam{F}\,.
\end{equation}
It is straightforward to note that since $\vtf{f}$ and $\mathit{g}$ are bijections, so results affinity $\mathit{f}$. Among other properties, it is possible to obtain that every affinity preserves parallelism and hyperplane dimensionality; in other words, parallel hyperplanes on the  domain remain parallel on the image and $n$-dimensional hyperplanes on the domain, $n\leq m$, remain $n$-dimensional on the image: for example, lines are mapped to lines, planes are mapped to planes and so on. According to corollary \ref{cor:soTens}, there is a second order tensor $\tnr{A}\in\ete{\cam{F}}{U^2}$ where linear function $\vtf{f}=\tnr{A}_1^*\circ\Phi$, and then  previous definition can be rewritten as
\begin{equation}
\fua{\mathit{f}}{\vto{u}\oplus a}=(\vto{u}^*\odot_1\tnr{A})\oplus\fua{\mathit{g}}{a}\,,\,\,\forall\,a\in\eamd{U}{F}{m}\,,\,\vto{v}\in U_\cam{F}\,,
\end{equation}
...
Considering previous conditions, an affinity $\mathit{f}$ is said to be centered at point $a$, represented by $\mathit{f}_a$, when $\mathit{g}$ is the identity function, that is, when point $\fua{\mathit{f}_a}{\vto{u}\oplus a}=\fua{\vtf{f}}{\vto{u}}\oplus a$. On the other hand, when $\vtf{f}$ is the identity function and $\fua{\mathit{g}}{a}=\vto{u}\oplus a$, we call affinity $\mathit{f}$ a \textsb{translation}\index{translation}, represented by $\mathit{t}_\vto{u}$, described by the rule
...
Still considering previous conditions, the affinity in $\map{\mathit{s}}{\eamd{U}{F}{m}}{\eamd{U}{F}{m}}$ is called a \textsb{dilation}\index{dilation} if bijection $\vtf{s}\in\evl{\cam{F}}{U}{U}$ in
\begin{equation}\label{eq:dilat}
\fua{\mathit{s}}{\vto{u}\oplus a}=\fua{\vtf{s}}{\vto{u}}\oplus\fua{\mathit{g}}{a}\,,\,\,\forall\,a\in\eamd{U}{F}{m}\,,\,\vto{u}\in U_\cam{F}\,,
\end{equation}
is a symmetric nonnegative operator, when we call this operator a \textsb{stretch}\index{function!stretch} function.
