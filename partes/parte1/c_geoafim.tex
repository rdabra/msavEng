...
\end{equation}
when $\tnr{F}$ is called an \textsb{affinity tensor}\index{tensor!afinity}\index{affinity!tensor}. Sometimes it is very useful to ``vectorize'' affinity $\mathit{f}$ in order to work only with vectors. In this sense, vector function $\tnr{\mathit{f}}:= \upsilon_o\circ\mathit{f}\circ\upsilon_o^{-1}$ described by 
\begin{equation}
\fua{\tnr{\mathit{f}}}{\vv{\vto{x}\oplus a}}=\vv{\fua{\mathit{f}}{\vto{x}\oplus a}}\,,
\end{equation}
is said to be a \textsb{vector affinity}\index{vector affinity}\index{affinity!vector}.
