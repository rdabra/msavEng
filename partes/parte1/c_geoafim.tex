...
Considering an Euclidean affine oriented space $(U_\real,\tnr{A}_B)$ where $B=\{\vun{u}_1,\cdots,\vun{u}_m\}$, the \textsb{simple bivector} $\vto{x}\times\vto{y}$ defined by vectors $\vto{x},\vto{y}\in U_\real$ is here understood as the mathematical object, endowed with magnitude and orientation, used to express rotational physical quantities such as angular momentum and torque. In geometrical terms, the magnitude of a simple bivector is defined to be the area of the parallelogram constructed from line segments $\overline{o\,\vto{x}\oplus o}$ and $\overline{o\,\vto{y}\oplus o}$ while its orientation is related to $\tnr{A}_B$ and to rotating $\overline{o\,\vto{x}\oplus o}$ to $\overline{o\,\vto{y}\oplus o}$, which is the inverse of rotating $\overline{o\,\vto{y}\oplus o}$ to $\overline{o\,\vto{x}\oplus o}$ for the case of the simple bivector $\vto{y}\times\vto{x}$. It is important to say that this new entity is here loosely defined because it is a particular case of a more general concept called multivector, which is out of the scope of this elementary study. Nevertheless, to our good fortune, this simple bivector can be represented as a vector if $\dim(U_\real)=3$, when it is called an \textsb{axial vector}\index{vector!axial}. In this context, considering vector $\vto{z}=\vto{x}\times\vto{y}$, the line $(\{\vto{z}\}_o)_\real^1$ is defined to be perpendicular to the plane $(\{\vto{x},\vto{y}\}_o)_\real^2$ and, if $\{\vto{x},\vto{y}\}$ is linearly independent, vector $\vto{z}$ must be oriented in such a way that basis $C=\{\vto{x},\vto{y},\vto{z}\}$ is positively oriented, that is, $\fua{\tnr{A}_C}{\vun{u}_1,\vun{u}_2,\vun{u}_3}=+1$.  


The orientation of an axial vector $\vv{ox}\times\vv{oy}$ is defined to be perpendi
...
where $\dim(U_\real)\leqslant 3$


since these quantities have orientation and  they can be 


Considering a rotation $\mathit{r}_a$ and a rotoreflection $\overline{\mathit{r}}_a$ where $\overline{\vtf{r}}=-\vtf{r}$, in the context of a three-dimensional Euclidean affine oriented space $(U_\real,\tnr{A}_B)$, we now search for a vector $\vto{u}\in U_\mathcal{\real}$, usually called an \textsb{axial vector}\index{vector!axial}, that observes the property $\fua{\vtf{r}}{\vto{u}}=-\fua{\vtf{r}}{\vto{u}}$ because certain physical quantities, such  are accordingly described by axial vectors. However, with the algebraic apparatus presented so far, there is no axial vector $\vto{u}\neq\vto{0}$ on $U_\real$ and then we need to define a binary operation whose resulting vector always observes this new property. Given two arbitrary vectors $\vto{x},\vto{y}\in U_\real$, from operation $\map{\times}{U_\real^2}{U_\real}$, vector $\vto{x}\times\vto{y}$, called the \textsb{cross product}\index{product!cross} of $\vto{x}$ and $\vto{y}$, is axial if  
\begin{equation}
