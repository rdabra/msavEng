...
\end{alignat}
where $|s|(x)=|\fua{s}{x}|$ and $|s|,g_1,g_2\in S$. In this context, if a Cauchy sequence of step functions $s_1,s_2,\cdots\in S$ exists and converges in measure $\mu$ to the measurable function in $\map{f}{M}{\cam{F}}$, we say that $f$ is integrable on $E$ and scalar 
\begin{equation}
\int_E f := \lim_{i\to \infty}\int_E s_i
\end{equation} 
is the integral of $f$ on $E$.
