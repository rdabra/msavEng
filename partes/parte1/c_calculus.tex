...
\end{equation*}
The tensor $\fua{[\fua{\tnr{\varphi}'_G}{\tnr{X}_0}]}{\tnr{X}}$ is called the \textsb{G\^ateaux differential}\index{differential!G\^ateaux} of $\tnr{\varphi}$ at $\tnr{X}_0$ and $\tnr{\varphi}'_G$ is the \textsb{G\^ateaux derivative}\index{derivative!G\^ateaux} of $\tnr{\varphi}$. Every Fr\'echet differentiable function is also G\^ateaux differentiable, but the converse is obviously not true. What is less obvious is the following: even when G\^ateaux derivative $\fua{\tnr{\varphi}'_G}{\tnr{X}_0}$ is bounded and linear, G\^ateaux differentiability, which means differentiability along all directions at a point, does not ensure Fr\'echet differentiability. This present study will require only the stronger restrictions of the Fr\'echet differentiation. For further developments of the G\^ateaux derivative, see \aut{Wouk}\cite{wouk_1979_1}, chapter 12.} of $\tnr{\psi}$ along $\tnr{H}$. 
...
\end{equation}
From the conditions presented so far, the property 
\begin{equation}
...
\end{equation*}
and $\pmb{\mathcal{U}}_i=\pmb{\mathcal{U}}$. By a similar development we performed for one order derivative, rule \eqref{eq:tangentOrderK1} leads to equality
...
Now, in the case of a vector valued scalar function $\tnr{\hat{\psi}}$, when $U_{\cam{F}}=\cam{F}$, from mapping $\map{\tnr{\hat{\psi}}}{\mathcal{U}}{V_\cam{F}}$, we can write that $\fua{[\tnr{\hat{\psi}}'(x)]}{\beta} = \beta\fua{[\tnr{\hat{\psi}}'(x)]}{1}$ for all $\beta\in\cam{F}$, since the derivative is linear. In this context, our interest is to describe the directional derivative  
...
\fua{\grad{\tnr{\hat{\psi}}}}{x} = \fua{\dfrac{d\tnr{\hat{\psi}}}{dx}}{x} \,.
\end{equation}
...
\begin{equation}\label{eq:parcMultScal}
\fua{\dfrac{\partial\tnr{\hat{\varphi}}}{\partial x_r}}{x_1,\cdots,x_r,\cdots,x_q} := [\partial_{x_r}\tnr{\hat{\varphi}}(x_1,\cdots,x_r,\cdots,x_q)](1)\,,
\end{equation}
from which conclude, considering the same idea of \eqref{eq:defGradient2}, that there is a vector 
\begin{equation}
...
\end{equation}
Similarly to definition \eqref{eq:parcMultScal}, for the case of a multivariate scalar valued scalar function $\hat{\varphi}$, we conclude that there is a scalar
\begin{equation}
...
\item[x.] \textsb{Mean Value Theorem}\index{theorem!mean value}\index{mean value theorem}: given the closed interval $[\alpha,\beta]\subset\real$ where $\alpha<\beta$, if the function in $\map{\hat{\psi}}{[\alpha,\beta]}{\cam{R}}$ is continuous on its domain and differentiable on open interval $(\alpha,\beta)$, then there is always a scalar $\kappa\in[\alpha,\beta]$ where
\begin{equation}\label{eq:meanValue}
\grad{\fua{{\hat{\psi}}}{\kappa}} =\dfrac{\hat{\psi}(\beta)-\hat{\psi}(\alpha)}{\beta-\alpha}\,.
\end{equation}

\begin{comment}
\item[xi.] \textsb{Mean Value Theorem for Real Valued Vector Functions}\index{theorem!mean value}\index{mean value theorem}: considering the mapping $\map{{\hat\psi}}{{\mathcal{{\mathcal{U}}}}}{\real}$, where ${\mathcal{U}}\subset U_\real$ is open, and an Euclidean affine space $\eamd{S}{\real}{n}$ defined by that $U_\real$, given arbitrary vectors $\vv{a},\vv{b}\in{\mathcal{U}}$, if every point $x$ in the line segment $\overline{ab}\subset\eamd{S}{\real}{n}$ defines a vector $\vv{x}\in{\mathcal{U}}$, there is always a point $c\in\overline{ab}$ where
...
\end{proof}}
\noindent Moreover, $\mu$ is called a \textsb{measure}\index{measure} if
...
\end{equation}
a condition that avoids terms like ``almost all'' or ``almost everywhere'' in forthcoming definitions. In this context, a double constituted by a measurable space and a measure, that is $((M,\css{M}),\mu)$, is called a \textsb{measure space}\index{space!measure}, which will hereafter be represented by the triple $(M,\css{M},\mu)$. In this context, a measure space is called bounded \index{space!bounded measure} when $\mu(M)$ is finite. 

Now, for the case of a measure space $(M,\css{M},\mu)$, where $M=\eamd{S}{\real}{n}$ is an Euclidean affine space, given an arbitrary $A\in\css{M}$, if each of the classes $\css{C}_i:=\{C_{i1},C_{i2},\cdots\}$ are covers of $A$, where $C_{ij}\subset A$ are hyperrectangles with side lengths $(l_{ij1},\cdots,l_{ijn})$ and hypervolumes $\operatorname{vol}(C_{ij}):=\prod_{k=1}^nl_{ijk}$, we define 
\begin{equation}
\lambda(A)=\inf\{\sum_{j=1}^\infty \operatorname{vol}(C_{1j}),\sum_{j=1}^\infty \operatorname{vol}(C_{2j}),\cdots\}\,,
\end{equation}
 where $\inf$ refers to the infimum (greatest lower bound) of the set involved\footnote{The infimum is used here because the minimum value of this infinite set of volumes does not exist.}. If
\begin{equation}
\lambda(A)=\lambda(A\cap B)+\lambda(A'\cap B)\,,\, \forall\, A,B\in\css{M}\,,
\end{equation}
then $\lambda$ results a specific measure $\mu$, called \textsb{Labesgue Measure}\index{measure!Labesgue}. It can be concluded that for the particular cases of one, two or three dimensional affine space $M$, real number $\lambda(A)$ corresponds to the length, area or volume of $A$ respectively. Moreover, from section \ref{sec:affine}, we recall that in an Euclidean affine space, a coordinate system is implicit, enabling points and vectors to be biunivocally correlated. Then, the Labesgue Measure can be used on a set constituted by the vectors correspondent to the points of $A$.

Given a measurable field $(\real,\css{F})$ and an arbitrary element $\con{B}\in\css{F}$, the function in $\map{s}{M}{\real}$ is called \textsb{measurable}\index{function!measurable} if the preimage $\con{R}^{-1}_{\con{B}} \in \css{M}$. 
Now, considering an arbitrary set $A\in\css{M}$, a class $\css{A}=\{A_1,\cdots,A_n\}\subseteq\css{M}$ is called a \textsb{partition}\index{set!partition of} of $A$ if it is disjoint and $A=\bigcup_{i=1}^nA_i$. Given a sequence $\alpha_1,\cdots,\alpha_n\in\real$, the measurable function in $\map{s_\css{A}}{M}{\real}$ is called \textsb{step}\index{function!step} or \textsb{simple}\index{function!simple} with respect to $\css{A}$ if its rule is described by
\begin{equation}
\fua{s_\css{A}}{x} = \sum_{i=1}^n\alpha_i\fua{\tnr{1}_{A_i}}{x}\,,
\end{equation}
where the function in $\map{\tnr{1}_{A_i}}{A_i}{\real}$ is commonly known as the \textsb{indicator} or \textsb{characteristic}\index{function!indicator}\index{function!characteristic} function of $A_i$, defined by
\begin{equation*}
\fua{\tnr{1}_{A_i}}{x}=
\begin{dcases}
0 & \text{if } x\notin A_i\\
...
\end{equation}
is the integral or Labesgue integral of $f$ on $E$. It is important to say that if $s_1,s_2,\cdots\in S$ and $t_1,t_2,\cdots\in S$ define convergent Chauchy sequences of integrable step functions, the sequence $(s_1+t_1, s_2+ t_2,\cdots)$ is also Cauchy. Moreover, for arbitrary $\alpha,\beta\in\real$, if the function in $\map{g}{M}{\real}$ is measurable, 
...
The following theorem is a fundamental result on the field of Labesgue integrations because it describes the sufficient conditions under which it is possible to make the integral of a limit of a function sequence equal to the limit of a sequence of integrals. In practical terms, the theorem lays down the conditions to commute the integral sign and the limit. The subsequent corollary, which describes a rule for differentiating under the integral sign, will be very useful in obtaining the expressions related to the Balance of Mass.  
...
\end{alignat*} 
According to the fundamental \textsb{Fatou's Lemma}\index{Fatou's Lemma}, which unfortunately we shall not prove here because it is quite long (see \aut{Lang}\cite{lang_1993_3}, p. 141), if the functions in $\map{h_i}{M}{\real^+}$ define a sequence $(h_1,h_2,\cdots)$ of integrable functions on $E$, then 
\begin{equation*}
...
\end{equation}
since function $f=\lim_{i\to\infty}\inf f_i=\lim_{i\to\infty}\sup f_i$ when sequence $(f_1,f_2,\cdots)$ converges to $f$, and $\lim_{i\to\infty}\inf (-\alpha_i)=-\lim_{i\to\infty}\sup \alpha_i$. Applying this same procedure for $h_i=g+f_i$, we arrive at 
\begin{equation}
\tag{b}
...
\begin{mcoro}{Leibniz Integral Rule}{leibIntRule}
...
\begin{proof}
{\footnotesize
If the function in $\map{f_{\alpha}}{M\times(a,b)}{\real}$ is described by 
\begin{equation*}
 f_{\alpha}(x,y) = \dfrac{f(x,y+\alpha)-f(x,y)}{\alpha}\,,
\end{equation*} 	
where real number $\alpha> 0$, then it is obvious that $\lim_{\alpha\to 0}f_{\alpha}(x,y) = \gradp{y}{f}(x,y)$. For a fixed $z\in M$, there is always a finite real number $\kappa\in[y,y+\alpha]$ so that $\gradp{y}{f}(z,\kappa)=f_{\alpha}(z,y)$, according to the Mean Value Theorem, which confers a boundedness to $f_{\alpha}$ not affected by the elements of $M$, that is, $f_{\alpha}$ is said to be uniformly bounded. Therefore, we conclude that any sequence $(\alpha_1,\alpha_2,\cdots)$ that converges to zero implies a sequence $(f_{\alpha_1},f_{\alpha_2},\cdots)$ of uniformly bounded functions that converges to $\gradp{y}{f}$, which is the same to say that every $f_{\alpha_i}$ is dominated by some function on $(a,b)$. In this context, from the Dominated Convergence Theorem, we prove the corollary through
\begin{align*}
\int_E \gradp{y}{f}(x,y)\, d\mu(x) &= \int_E \lim_{\alpha\to 0}f_{\alpha}(x,y)\, d\mu(x) \\
&= \lim_{\alpha\to 0}\int_E f_{\alpha}(x,y)\, d\mu(x) \\
&= \lim_{\alpha\to 0}\int_E\dfrac{f(x,y+\alpha)-f(x,y)}{\alpha}\, d\mu(x)\\
&= \lim_{\alpha\to 0}\dfrac{\int_Ef(x,y+\alpha)\, d\mu(x)-\int_Ef(x,y)\, d\mu(x)}{\alpha}\\
&= \lim_{\alpha\to 0}\dfrac{h(y+\alpha)-h(y)}{\alpha}\\
&= \grad{h}(y)\,.
\end{align*}
}
\end{proof}

Now, our goal is to integrate a broader class of functions other than functionals, namely, tensor and vector functions. Considering $\ete{\real}{U^{\times m}}$ and $\ete{\real}{V^{\times s}}$ Euclidean tensor spaces, let $\map{\tnr{\psi}}{\pmb{\mathcal{U}}}{\ete{\cam{R}}{V^{\times s}}}$ be a tensor mapping, where $\pmb{\mathcal{U}}\subseteq\ete{\real}{U^{\times m}}$, and $\map{f}{\pmb{\mathcal{U}}}{\real}$ a mapping whose functional is described by the rule $f = \frt{T}\circ\tnr{\psi}$, such that cotensor $\frt{T}$ is the representative tensor functional of an arbitrary tensor $\tnr{T}\in\ete{\real}{V^{\times s}}$, according to \eqref{eq:polyRepre} in the context of real fields. We define that function $\tnr{\psi}$ is integrable on $\pmb{\mathcal{U}}$ if and only if $f$ is integrable on $\pmb{\mathcal{U}}$. If this is the case, by specifying a rule
...
\begin{equation*}
\int_{\hat{\pmb{\mathcal{U}}}}\dvt{\vtf{\varphi}}\;d\lambda = \int_{\partial{\pmb{\mathcal{U}}}}\vtf{\varphi}_\odot\;d\lambda\,.
\end{equation*}
\end{mteo}
\hspace{1pt}
{\footnotesize
\begin{proof}
The proof of this theorem is very long. We invite the interested reader to see the proof presented by \aut{Amann \& Escher}\cite{amman_2001_1}, p.447.
\end{proof}
}
...
\begin{equation*}
	\int_{\hat{\pmb{\mathcal{U}}}}\nabla{\vtf{\varphi}}\;d\lambda = \int_{\partial{\pmb{\mathcal{U}}}}\vtf{\varphi}_\otimes\;d\lambda\,\,.
\end{equation*}
\end{mcoro}
{\footnotesize
\begin{proof}
...
\begin{align*}
\tnr{I}\odot_2	\int_{\hat{\pmb{\mathcal{U}}}}\nabla{\vtf{\varphi}}\;d\lambda & = \tnr{I}\odot_2 \int_{\partial{\pmb{\mathcal{U}}}}\vtf{\varphi}_\otimes\;d\lambda\\
\int_{\hat{\pmb{\mathcal{U}}}} \frt{I}\circ\nabla{\vtf{\varphi}}\;d\lambda& =\int_{\partial{\pmb{\mathcal{U}}}}\frt{I}\circ\vtf{\varphi}_\otimes\;d\lambda\,.
\end{align*}
...
\begin{equation*}
\int_{\hat{\pmb{\mathcal{U}}}}\crl{\vtf{\varphi}}\;d\lambda = \int_{\partial{\pmb{\mathcal{U}}}}\vtf{\varphi}_\times\;d\lambda\,\,.
\end{equation*}
\end{mcoro}
{\footnotesize
\begin{proof}
...
\begin{align*}
\int_{\hat{\pmb{\mathcal{U}}}}\crl{\vtf{\varphi}}\;d\lambda & = \int_{\partial{\pmb{\mathcal{U}}}}\vtf{\varphi}_\times\;d\lambda\\
\int_{\hat{\pmb{\mathcal{U}}}}\tnr{A}_B\odot_2\nabla{\vtf{\varphi}}\;d\lambda & = \int_{\partial{\pmb{\mathcal{U}}}}\frt{A}_B\circ\vtf{\varphi}_\otimes\;d\lambda\\
\tnr{A}_B\odot_2\int_{\hat{\pmb{\mathcal{U}}}}\nabla{\vtf{\varphi}}\;d\lambda & = \tnr{A}_B\odot_2\int_{\partial{\pmb{\mathcal{U}}}}\vtf{\varphi}_\otimes\;d\lambda\,.
\end{align*}
\end{proof}
}
...
\begin{equation*}
\int_{\widehat{{\pmb{\mathcal{S}}}}}\vtf{\varphi}_{\widehat{\pmb{\mathcal{S}}}}\;d\lambda = \int_{\partial{\pmb{\mathcal{S}}}}\vtf{\varphi}_{\partial\pmb{\mathcal{S}}}\;d\lambda\,\,.
\end{equation*}
\end{mteo}
%\hspace{1pt}
{\footnotesize
\begin{proof}
The proof of this theorem requires concepts out of the scope of this humble book. We recommend the reader to see the proof presented by \aut{Backus}\cite{backus_1997_1}, p.131. 
\end{proof}
}
