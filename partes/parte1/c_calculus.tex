...
Now, for the case of a measure space $(M,\css{M},\mu)$, where $M=\eamd{S}{\real}{n}$ is an Euclidean affine space, given an arbitrary $A\in\css{M}$, let $C_i\subset A$ be arbitrary hyperrectangles with side lengths $(l_1^{(i)},\cdots,l_n^{(i)})$ and hypervolumes $vol(C_i)=\prod_{k=1}^nl_k^{(i)}$ in such a way that each of the sequences $(C_1,C_2,\cdots)_j$ covers $A$. Considering $vol(C_i^{(j)})$ the hypervolume of the $i$-th hyperrectangle of the $j$-th sequence, we define the real number
\begin{equation}
\lambda(A)=\inf\{\sum_{i=1}^\infty vol(C_i^{(1)}),\sum_{i=1}^\infty vol(C_i^{(2)}),\cdots\}\,,
\end{equation}
...
\end{equation}
then $\lambda$ results a specific measure $\mu$, called \textsb{Labesgue Measure}\index{measure!Labesgue}. It can be concluded that for the particular cases of one, two or three dimensional affine space $M$, the value $\lambda(A)$ corresponds to the length, area or volume of $A$ respectively.  
