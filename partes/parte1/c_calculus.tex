...
\end{equation}           
for an arbitrary countable class $\{A_1,A_2,\cdots\}\subseteq\css{M}$. Thereby, the double constituted by a measurable space and a measure, that is $((M,\css{M}),\mu)$, is called a \textsb{measure space}\index{space!measure}, to which we shall hereafter refer by simply qualifying with ``measure'' its measurable space $(M,\css{M})$.  
