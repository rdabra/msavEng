...
\end{equation}
is said to be the directional derivative\footnote{It is important to say a few words about a less restricted or weaker type of derivative than the Fréchet derivative just presented. It is not defined from the concept of tangency but from a directional derivative in which the derivative involved is not necessarily bounded and linear. Considering the above conditions, the tensor function in $\map{\tnr{\varphi}}{\pmb{\mathcal{U}}}{\ete{\cam{F}}{V^{\times s}}}$ is said to be \textsb{Gâteaux differentiable}\index{function!Gâteaux differentiable} at $\tnr{X}_0$ if there exists a mapping $\map{\fua{\tnr{\varphi}'_G}{\tnr{X}_0}}{\pmb{\mathcal{U}}}{\ete{\cam{F}}{V^{\times s}}}$, not necessarily bounded and linear, where
\begin{equation*}
\fua{[\fua{\tnr{\varphi}'_G}{\tnr{X}_0}]}{\tnr{X}}=\lim_{\alpha\to 0} \dfrac{\fua{\tnr{\varphi}}{\tnr{X}_0+\alpha\tnr{X}}-\fua{\tnr{\varphi}}{\tnr{X}_0}}{\alpha}\,.
\end{equation*}
The tensor $\fua{[\fua{\tnr{\varphi}'_G}{\tnr{X}_0}]}{\tnr{X}}$ is called the \textsb{Gâteaux differential}\index{differential!Gâteaux} of $\tnr{\varphi}$ at $\tnr{X}_0$ and $\tnr{\varphi}'_G$ is the \textsb{Gâteaux derivative}\index{derivative!Gâteaux} of $\tnr{\varphi}$. Every Fréchet differentiable function is also Gâteaux differentiable, but the converse is obviously not true. What is less obvious is the following: even when Gâteaux derivative $\fua{\tnr{\varphi}'_G}{\tnr{X}_0}$ is bounded and linear, Gâteaux differentiability, which means differentiability along all directions at a point, does not assure Fréchet differentiability. This present study will require only the stronger restrictions from the Fréchet differentiation. For further developments of the Gâteaux derivative, see \aut{Wouk}\cite{wouk_1979_1}, chapter 12.} of $\tnr{\psi}$ along $\tnr{H}$. 

E QUANDO A FUNCAO FOR VETORIAL, COMO SERIA O ESQUEMA PARA SUA DIFERENCIACAO

For the particular case of $\pmb{\mathcal{U}}$ subset of a one-dimensional Hilbert space $U_\cam{F}$, there is only one ``direction'' on $U_\cam{F}$, which can be represented by the unique vector of its unique orthonormal basis $\{1\}$. Therefore, in the context of space $U_\cam{F}=\cam{F}$,
\begin{equation}
\fua{\dfrac{d\tnr{\psi}}{dx}}{x}:=\fua{[\fua{\tnr{\psi}'}{x}]}{1}=\lim_{\alpha\to 0} \dfrac{\fua{\tnr{\psi}}{x+\alpha}-\fua{\tnr{\psi}}{x}}{\alpha}\,,
\end{equation}
where ${d\tnr{\psi}}/{dx}$ is the notation we use for the derivative of $\tnr{\psi}$ when its domain is a scalar field. If $\ete{\cam{F}}{V^{\times s}}=\real$, given an arbitrary element $x_0\in\cam{F}$, we have the classical geometric interpretation of scalar $\fua{{d\tnr{\psi}}/{dx}}{x_0}$ as being the slope of the tangent line $\tnr{\psi_\mathit{d}}$ to $\tnr{\psi}$ at $x_0$.
...
\end{equation}
Now, for the particular case of a differentiable unary tensor operator on $\pmb{\mathcal{U}}$, its gradient assigns to a $m$-th order tensor a $2m$-th order tensor. In this context, if $m=1$ then tensor space $\ete{\cam{F}}{U}=U^*_\cam{F}$, gradient $\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}\in\ete{\cam{F}}{U^2}$, derivative $\fua{\tnr{\psi}'}{\vto{x}_0^*}\in\evlc{\cam{F}}{\pmb{\mathcal{U}}}{\ete{\cam{F}}{U}}$ and previous equality results
...
According to corollary \ref{cor:soTens} and definition \eqref{eq:repFunc2}, since the derivative $\fua{\tnr{\psi}'}{\vto{x}_0^*}$ is the $1$-cotensor of the second order tensor $\tnr{G}_{\vto{x}_0^*}:=\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}$, the linear bijection
 ...
 \tnr{\mathit{g}}'_{\vto{x}_0^*}:=\Phi^{-1}\circ\fua{\tnr{\psi}'}{\vto{x}_0^*}\circ\Phi
\end{equation}
is the representative function of $\tnr{G}_{\vto{x}_0^*}$ and then  
 \begin{equation}\label{eq:primiRepFuncGrad2}
 \tnr{\mathit{g}}:=\Phi^{-1}\circ\tnr{\psi}\circ\Phi\,.
\end{equation}
From these definitions and rule \eqref{eq:directDeriv}, given an arbitrary direction $\vto{h}^*\in\pmb{\mathcal{U}}$, it is possible to develop the following:
...
\end{align}
Considering equality \eqref{eq:secTens2Lin}, we also have for the present case that 
\begin{equation}\label{eq:gradFuncRep}
\fua{\tnr{G}_{\vto{x}_0^*}}{\vto{x},\vto{y}}=\vto{x}\cdot\fua{\tnr{\mathit{g}}'_{\vto{x}_0^*}}{\vto{y}^c}\,,\,\,\forall\, \vto{x}^*,\vto{y}^*\in\pmb{\mathcal{U}}\,.
\end{equation}
\emph{If the interest is to always inspect function values  $\fua{D\tnr{\psi}}{\vto{x}^*}=[\fua{\tnr{\psi}'}{\vto{x}^*}](\vto{x}^*)$, then we drop the subscripts that identify $\vto{x}^*$ resulting $\tnr{\mathit{g}}':=\tnr{\mathit{g}}'_{\vto{x}^*}$ and $\tnr{G}:=\tnr{G}_{\vto{x}^*}$}. Now, for the case of $m+s=0$, that is, field $\cam{F}=\ete{\cam{F}}{U^{\times m}}=\ete{\cam{F}}{V^{\times s}}$, we consider $\{\vun{u}\}$ the basis of a one dimensional Hilbert space $U_\cam{F}$ to write, according to theorem \ref{teo:repRiesz}, equalities
\begin{equation*}
\fua{\dfrac{d\psi}{dx}}{x_0} = [\fua{\psi'}{x_0}](1) = 1\fua{\psi'}{x_0} = [\fua{\psi'}{x_0}\vun{u}]\cdot\vun{u}\,,
\end{equation*}
from which we define that $\fua{\nabla\psi}{x_0} = \fua{\psi'}{x_0}\vun{u}$ and then
\begin{equation}
\fua{\dfrac{d\psi}{dx}}{x_0} = \fua{\nabla\psi}{x_0}\cdot\vun{u}\,.
\end{equation}

For the directional derivative of $\tnr{\psi}$ along $\tnr{H}\in\pmb{\mathcal{U}}$, the definition of gradient \eqref{eq:defGradient} and rule \eqref{eq:directDeriv} enable us to easily obtain that
\begin{equation}
