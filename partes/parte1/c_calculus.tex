...
Based on the concepts and definitions presented above for vector functions, when $U_{\cam{F}}=\cam{R}$, from mapping $\map{\tnr{\hat{\psi}}}{\mathcal{U}}{V_\real}$, we can write that $\fua{\tnr{\hat{\psi}}'_{x}}{\beta} = \beta\fua{\tnr{\hat{\psi}}'_{x}}{1}$ for all nonzero $\beta$, since the derivative is linear. In this context, our interest is to describe directional derivative $\fua{[d\tnr{\hat{\psi}}/dx]}{x}:=\fua{\tnr{\hat{\psi}}'_{x}}{1}$ by 
\begin{equation}\label{eq:derivVecScaFun}
\fua{\dfrac{d\tnr{\hat{\psi}}}{dx}}{x} = \lim_{\alpha\to 0} \dfrac{\fua{\tnr{\hat{\psi}}}{x+\alpha}-\fua{\tnr{\hat{\psi}}}{x}}{\alpha}\,. 
\end{equation} 
