...
\end{equation}
From the definitions above and rule \eqref{eq:directDeriv}, given an arbitrary direction $\vto{h}^*\in\pmb{\mathcal{U}}$, it is possible to develop the following:  
...
\end{align}
which results the rule for the directional derivative of the vector function in $\map{\tnr{\mathit{g}}}{\mathcal{U}}{U_{\mathbb{F}}}$ along $\vto{h}\in\mathcal{U}$, where $\mathcal{U}$ is constituted by the corresponding vectors of the elements of $\pmb{\mathcal{U}}$. Still in this context, when $U_{\cam{F}}=\cam{F}$, from mapping $\map{\mathit{g}}{\mathcal{U}}{{\cam{F}}}$ and the previous equality, we write that
\begin{equation*}
\fua{\mathit{g}'_{x^*}}{\beta} = \lim_{\alpha\to 0} \dfrac{\fua{\mathit{g}}{x+\alpha\beta}-\fua{\mathit{g}}{x}}{\alpha}\,,\forall\, \beta\in\cam{F}. 
\end{equation*} 
...
\end{equation*} 
If $\cam{F}=\real$, given an arbitrary real number $x_0$, we have the classical geometric interpretation of scalar $\fua{\mathit{g}'}{x_0}$ as being the slope of the tangent line $\mathit{g_d}$ to the curve $\mathit{g}$ at $x_0$. Now, if $\{\vun{u}\}$ is the orthonormal basis of a one dimensional Hilbert space $U_\cam{F}$, according to theorem \ref{teo:repRiesz}, it is valid that
