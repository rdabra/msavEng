...
\end{align}
which results the rule of the directional derivative of the vector function in $\map{\tnr{\hat\psi}}{\mathcal{U}}{V_\cam{F}}$ along $\vto{h}\in\mathcal{U}$. From this result, \emph{the derivative $\tnr{\hat\psi}'_{\vto{x}}$, as a linear vector function, inherits all the previous and forthcoming definitions of the linear tensor function $\fua{\tnr{\psi}'}{\vto{x}^*}$}. For example, from \eqref{eq:derivSameArg} and \eqref{eq:defGradient2}, we can write that vector
...
Based on the concepts and definitions presented above for vector functions, when $U_{\cam{F}}=\cam{R}$, from mapping $\map{\tnr{\hat{\psi}}}{\mathcal{U}}{V_\real}$, we can write that
\begin{equation*}
\fua{\tnr{\hat{\psi}}'_{x}}{\beta} = \lim_{\alpha\to 0} \dfrac{\fua{\tnr{\hat{\psi}}}{x+\alpha\beta}-\fua{\tnr{\hat{\psi}}}{x}}{\alpha}\,,\forall\, \beta\in\cam{R}. 
\end{equation*} 
Considering $\beta=1$ and $\fua{\tnr{\hat{\psi}}'}{x}:=\fua{\tnr{\hat{\psi}}'_{x}}{1}$ the corresponding scalar of coscalar $\tnr{\hat{\psi}}'_{x}$, 
\begin{equation*}
\fua{\tnr{\hat{\psi}}'}{x} = \lim_{\alpha\to 0} \dfrac{\fua{\tnr{\hat{\psi}}}{x+\alpha}-\fua{\tnr{\hat{\psi}}}{x}}{\alpha}\,. 
\end{equation*} 
Given an arbitrary real number $x_0$ in a context where $V_\real=\real$, we have the classical geometric interpretation of scalar $\fua{\tnr{\hat{\psi}}'}{x_0}$ as being the slope of the tangent line $\tnr{\hat{\psi}}_\mathit{d}$ to the curve $\tnr{\hat{\psi}}$ at $x_0$. Now, if $\{\vun{u}\}$ is the orthonormal basis of a one dimensional Hilbert space $U_\cam{R}$, according to theorem \ref{teo:repRiesz}, it is valid that
$\fua{\tnr{\hat{\psi}}'_{x}}{1} = 1\fua{\tnr{\hat{\psi}}'}{x} = [\fua{\tnr{\hat{\psi}}'}{x}\vun{u}]\cdot\vun{u}$, where vector  
\begin{equation}
\nabla\fua{\tnr{\hat{\psi}}}{x}:=\fua{\tnr{\hat{\psi}}'}{x}\vun{u}
\end{equation}
is called the gradient of $\tnr{\hat{\psi}}$ at $x$.
