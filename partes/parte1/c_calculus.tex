...
\end{dcases}\,.
\end{equation*} 
In the particular case of a measure space $(M,\css{M},\mu)$, the simple function $s$ is defined to be \textsb{integrable}\index{function!integrable simple} on set $E\in\css{M}$ when $\sum_{i=1}^n\fua{\mu}{E\cap A_i} < \infty$. If this is the case, scalar 
\begin{equation}
\int_E s := \sum_{i=1}^n\alpha_i\fua{\mu}{E\cap A_i} 
\end{equation}
is called the \textsb{integral}\index{integral!of simple function} of $s$ on $E$.