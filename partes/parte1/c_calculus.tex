...
\end{equation}
From this definition, it is clear that $\fua{\mu}{\emptyset}=\fua{\mu}{\emptyset\cup\emptyset}=2\fua{\mu}{\emptyset}$, from which we conclude that $\fua{\mu}{\emptyset}=0$. For the special case of a mapping $\map{\mu}{\css{C}}{\real^+}$ where $\css{C}$ is a ring, the additive nonegative set function $\mu$ is said to be a \textsb{continent}\index{ring!continent} of ring $\css{C}$. But in the case of the $\sigma$-ring $\css{M}$, its continent $\mu$ is called a \textsb{measure}\index{measure} if
...
Given a measurable field $(\real,\css{F})$, the function in $\map{s}{M}{\real}$ is called \textsb{measurable}\index{function!measurable} if, for every element $\con{B}\in\css{F}$, the preimage $\con{R}^{-1}_{\con{B}} \in \css{M}$. 
...
In the set of all measurable functions on $(M,\css{M},\mu)$, let $(S,\varrho)$ be a seminormed metric space of integrable step functions on $E$ with seminorm and metric defined by
\begin{alignat*} {3}
\|s\|=\int_E |s|&\qquad \text{ and } \qquad&\fua{\varrho}{s_1,s_2}=\|s_1-s_2\|\,,
\end{alignat*}
where $|s|(x):=|\fua{s}{x}|$ and $s,s_1,s_2\in S$. Note that in this case $\|\bullet\|$ is obviously a seminorm because $\|s\|=0$ does not necessarily imply $s=0$ . In this context, if a Cauchy sequence of integrable step functions $s_1,s_2,\cdots\in S$ exists and converges to the measurable functional in $\map{f}{M}{\real}$, we say that $f$ is integrable or Labesgue integrable on $E$ and scalar
...
Now, our goal is to integrate a broader class of functions other than functionals, namely, tensor functions. Considering $\ete{\real}{U^{\times m}}$ and $\ete{\real}{V^{\times s}}$ Euclidean tensor spaces, let $\map{\tnr{\psi}}{\pmb{\mathcal{U}}}{\ete{\cam{R}}{V^{\times s}}}$ be a tensor mapping, where $\pmb{\mathcal{U}}\subseteq\ete{\real}{U^{\times m}}$, and $\map{f}{\pmb{\mathcal{U}}}{\real}$ a mapping whose functional is described by the rule $f = \frt{T}\circ\tnr{\psi}$, such that function $\frt{T}$ is the representative tensor functional of an arbitrary tensor $\tnr{T}\in\ete{\real}{V^{\times s}}$, according to \eqref{eq:repTensFunc}. We define that function $\tnr{\psi}$ is integrable on $\pmb{\mathcal{U}}$ if and only if functional $f$ is integrable on $\pmb{\mathcal{U}}$. If this is the case, by specifying a rule
