...
\end{equation}
For the following important properties, the properties of derivatives presented on previous section were considered and the functions in gradients are obviously differentiable on their domains.
\begin{itemize}
	\setlength\itemsep{.1em}
...
\item[x.] \textsb{Mean Value Theorem}\index{theorem!mean value}\index{mean value theorem}: given the open interval $(a,b)\subset\real$ where $a<b$, if the function in $\map{\hat{\psi}}{(a,b)}{\cam{R}}$ is continuous on the closed interval $[a,b]$, then there is always a scalar $c\in[a,b]$ where
\begin{equation}\label{eq:meanValue}
\grad{\fua{{\hat{\psi}}}{c}} =\dfrac{\hat{\psi}(b)-\hat{\psi}(a)}{b-a}\,.
\end{equation}

\item[xi.] \textsb{Mean Value Theorem for Real Valued Vector Functions}\index{theorem!mean value}\index{mean value theorem}: if the function in $\map{\tnr{\hat\psi}}{{\mathcal{\tnr{\mathcal{U}}}}}{\real}$, where $\tnr{\mathcal{U}}\in U_\real$ is open,  
