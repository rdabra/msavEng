...
\end{align*}
Since item vi is a corollary of the product rule, its verification is trivial. Item vii is verified by the same procedure we used for the product rule.
\end{proof}}

  

In the context of Hilbert spaces, we already know that there is a biunivocal relationship between a linear tensor function space and a tensor space described by the Riesz-Fréchet Representation of Tensors, as presented on theorem \ref{teo:repRieszTensor}. For the present case, considering the terms of this theorem and the previous conditions restricted to Hilbert spaces, the derivative $\fua{\tnr{\psi}'}{\tnr{X}_0}\in\evl{\cam{F}}{\pmb{\mathcal{U}}}{\ete{\cam{F}}{V^{\times s}}}$ is the $m$-cotensor of tensor $\fua{\nabla\tnr{\psi}}{\tnr{X}_0}\in\ete{\cam{F}}{U^{\times m}\times V^{\times s}}$, called the \textsb{gradient}\index{gradient} of $\tnr{\psi}$ at $\tnr{X}_0$, if the following rule is defined:
\begin{equation}
...
\end{align*}  
Now, from the inner product rule we have
\begin{align*}
...
\begin{equation}
\fua{\dvt{\tnr{\varphi}}}{\tnr{X}} = ???????????\,,
\end{equation}	
and for $r=m=s=1$,
\begin{equation}
\fua{\crl{\tnr{\varphi}}}{\vto{x}} = ???????????\,;
\end{equation}	
...
\item[iv.] If $\fua{\tnr{\varphi}}{\vto{x}}=\fua{\tnr{\varphi}_1}{\vto{x}}\times\fua{\tnr{\varphi}_2}{\vto{x}}$, where mappings  $\map{\tnr{\psi}_1}{\pmb{\mathcal{U}}}{U_\cam{R}}$ and $\map{\tnr{\psi}_2}{\pmb{\mathcal{U}}}{U_\cam{R}}$ are defined, then
\begin{equation}
\fua{\crl{\tnr{\varphi}}}{\vto{x}} = ???????????\,;
\end{equation}
	
\item[iv.]	The curl of the gradient is always zero, or $\fua{\crl{\nabla\tnr{\varphi}}}{\vto{x}}=\vto{0}$;
\end{itemize}
