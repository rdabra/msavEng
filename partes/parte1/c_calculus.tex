...
The following theorem is a fundamental result on the field of Labesgue integrations because it describes the sufficient conditions under which it is possible to commute the integral sign and the limit. 
\begin{mteo}{Dominated Convergence Theorem}{boundedConv}
Given the bounded measure space $(M,\css{M},\mu)$ and a set $E\in\css{M}$, let the mappings $\map{f_i}{M}{\real}$ define a sequence $f_1,f_2,\cdots$ of integrable functions on $E$ that converges to the measurable function in $\map{f}{M}{\real}$, that is, $f=\lim_{i\to \infty} f_i$. If the function in $\map{g}{M}{\real}$ is integrable on $E$ and $|f_i(x)|\leqslant g(x)$ for all $f_i$ and $x\in E$, then $f$ is integrable on $E$ and 
\begin{equation*}
 \lim_{i\to \infty} \int_E f_i d\mu = \int_E f d\mu\,.
 \end{equation*} 
\end{mteo}
