...
Considering measure space $(M,\css{M},\mu)$, a measurable field $(\cam{F},\css{F})$ and a mapping  $\map{f}{M}{\cam{F}}$, a sequence of functions $f_1,f_2,\cdots$ that also map $M$ to $\cam{F}$ \textsb{converges in measure}\index{convergence!in measure} $\mu$ to function $f$ when, given an arbitrary real number $\epsilon>0$,
\begin{equation}
...
\end{equation}
is called the \textsb{integral}\index{integral!of step function} of $s$ on $E$. Let  $(S,\varrho)$ be a normed metric space of integrable step functions on $E$ with norm and metric respectively defined by
\begin{alignat} {3}
\|s\|=\int_E |s|&\qquad \text{ and } \qquad&\fua{\varrho}{g_1,g_2}=\|g_1-g_2\|\,,
\end{alignat}
where $|s|(x):=|\fua{s}{x}|$ and $|s|,g_1,g_2\in S$. In this context, if a Cauchy sequence of integrable step functions $s_1,s_2,\cdots\in S$ exists and converges in measure $\mu$ to the measurable function in $\map{f}{M}{\cam{F}}$, we say that $f$ is integrable or Labesgue integrable on $E$ and scalar 
\begin{equation}
\int_E f := \lim_{i\to \infty}\int_E s_i = \lim_{i\to \infty} \sum_{j=1}^n\alpha_{ij}\fua{\mu}{E\cap A_j}
\end{equation} 
