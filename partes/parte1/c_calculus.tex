...
\end{equation}
If tensor function $\tnr{\psi}$ is a differentiable unary tensor operator on $\pmb{\mathcal{U}}$, that is tensor space $\ete{\cam{F}}{U^{\times m}}=\ete{\cam{F}}{V^{\times s}}=\ete{\cam{F}}{U^{m}}$, its gradient assigns to a $m$-th order tensor a $2m$-th order tensor. Considering the conditions of expression \eqref{eq:toOrderZero}, it is valid to write that  
\begin{equation}\label{eq:orderGrad}
[[\fua{[\fua{\tnr{\psi}^{(k+1)}}{\tnr{X}}]}{\tnr{H}_{k+1}}](\tnr{H}_k)\cdots](\tnr{H}_{1})=\tnr{H}_1\otimes\cdots\otimes\tnr{H}_{k+1}\odot_{(k+1)m}\fua{\nabla^{(k+1)}\tnr{\psi}}{\tnr{X}}\,,
\end{equation}
where $\nabla^{k+1}\tnr{\varphi}$ is the gradient of $\tnr{\varphi}$ of order $(k+1)$. Similarly, from the conditions of \eqref{eq:partialDerivative}, which defines partial derivatives, we write 
...
\end{equation}
where $\nabla_{\tnr{X}_r}\tnr{\varphi}$ is the gradient of $\tnr{\varphi}$ with respect to $\tnr{X}_r$. Therefore, if $\tnr{\varphi}$ is bivariate and  differentiable of order two, from both expressions above, we also write
\begin{equation}
[[\fua{\partial_{\tnr{X}_1}\partial_{\tnr{X}_2}\tnr{\varphi}}{\tnr{X}_1,\tnr{X}_2}]{(\tnr{H}_2)}](\tnr{H}_1)=\tnr{H}_1\otimes\tnr{H}_2\odot_{2m}\fua{\nabla^{(2)}_{\tnr{X}_1\tnr{X}_2}\tnr{\varphi}}{\tnr{X}_1,\tnr{X}_2}\,.
\end{equation}
...
and then 
	...
	\end{equation}
	\item[ix.] If the function in $\map{\tnr{\hat{\varphi}}}{\pmb{\mathcal{U}}_1\times\pmb{\mathcal{U}}_2}{V_\cam{F}}$, where $\pmb{\mathcal{U}}_i\subset U_\cam{F}$, is $\mathcal{C}^2$ then
\begin{equation}
\nabla_{\vto{x}_1}\tnr{\varphi}
\end{equation}
