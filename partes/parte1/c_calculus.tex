...
{\footnotesize
\begin{proof}
The above theory would be useless if there were no tangent functions. Considering the rules $\fua{\tnr{\psi}}{\tnr{X}}=\alpha\tnr{X}+\tnr{Y}$ and $\fua{\tnr{\delta}}{\tnr{X}}=\alpha\tnr{X}_0+\alpha(\tnr{X}-\tnr{X}_0)=$, function $\tnr{\delta}$ tangent to $\tnr{\psi}$ can be proved by 
\end{proof}}
