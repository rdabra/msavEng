...
\end{equation}
where $\{B_1,B_2,\cdots\}\subseteq\css{M}$ is also an arbitrary disjoint countable class. Now, we choose to restrict our study to measures like size and mass, where
\begin{equation}
...
\end{equation}
For the particular case of $m=s=1$, tensor spaces $\ete{\cam{F}}{U}=U^*_\cam{F}$ and $\ete{\cam{F}}{V}=V^*_\cam{F}$. By defining a vector valued vector function $\tnr{\hat{\psi}}=\Phi^{-1}\circ\tnr{\psi}\circ\Phi$, where $\Phi$ is defined in theorem \ref{teo:repRiesz}, we can define a mapping $\map{g}{{\mathcal{U}}}{\real}$, where $\mathcal{U}$ is constituted by the corresponding vectors to the elements of $\tnr{\mathcal{U}}$.
