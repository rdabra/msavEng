...
If tensor function $\tnr{\psi}$ is a differentiable unary tensor operator on $\pmb{\mathcal{U}}$, that is tensor space $\ete{\cam{F}}{U^{\times m}}=\ete{\cam{F}}{V^{\times s}}=\ete{\cam{F}}{U^{m}}$, its gradient assigns to a $m$-th order tensor a $2m$-th order tensor. 
 ...
 For the particular case of $m=s=1$, tensor space $\ete{\cam{F}}{U}=U^*_\cam{F}$, $\ete{\cam{F}}{V}=V^*_\cam{F}$, gradient $\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}\in\ete{\cam{R}}{U\times V}$, derivative $\fua{\tnr{\psi}'}{\vto{x}_0^*}\in\evlc{\cam{F}}{\pmb{\mathcal{U}}}{V^*_\cam{F}}$ and then
\begin{equation}\label{eq:defGradient2}
[\fua{\tnr{\psi}'}{\vto{x}_0^*}](\vto{x}^*) = \vto{x}^*\odot_1\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}\,.
\end{equation}
According to corollary \ref{cor:soTens} and definition \eqref{eq:repFunc2}, since the derivative $\fua{\tnr{\psi}'}{\vto{x}_0^*}$ is the $1$-cotensor of the second order tensor $\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}$, the linear bijection
 \begin{equation}\label{eq:repFuncGrad2}
 \tnr{\hat{\psi}}'_{\vto{x}_0}:=\tnr{\Upsilon}[\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}]=\Phi^{-1}\circ\fua{\tnr{\psi}'}{\vto{x}_0^*}\circ\Phi
\end{equation}
is the representative function of $\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}$. Moreover, it is convenient to have a vector function  
\begin{equation}\label{eq:primiRepFuncGrad2}
 \tnr{\hat{\psi}}:=\Phi^{-1}\circ\tnr{\psi}\circ\Phi\,.
\end{equation}
and a function 
\begin{equation}
 \nabla\tnr{\hat{\psi}}:=\nabla\tnr{\psi}\circ\Phi^{-1}\,,
\end{equation}
called the gradient of $\tnr{\hat\psi}$, from which we conclude equality
\begin{equation}\label{eq:derivVecGradVec}
\fua{\tnr{\hat\psi}'_{\vto{x}_0}}{\vto{x}} = \vto{x}^*\hat{\odot}_1\fua{ \nabla\tnr{\hat{\psi}}}{\vto{x}_0}\,,
\end{equation}
according to \eqref{eq:genInnerProdVec} and \eqref{eq:defGradient2}. Now, considering equality \eqref{eq:secTens2Lin} and $\tnr{G}_{\vto{x}_0}:=\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}$, we also have for the present case that 
\begin{equation}
\fua{\tnr{G}_{\vto{x}_0}}{\vto{x},\vto{y}}=\vto{x}\cdot\fua{\tnr{\hat\psi}'_{\vto{x}_0}}{\vto{y}^{c}}\,,\,\,\forall\, \vto{x},\vto{y}\in\mathcal{U}\,,
\end{equation}
...
\end{align}
which results the rule of the directional derivative of the vector function in $\map{\tnr{\hat\psi}}{\mathcal{U}}{V_\cam{F}}$ along $\vto{h}\in\mathcal{U}$. From this result, \emph{the derivative $\tnr{\hat\psi}'_{\vto{x}^*}$, as a linear vector function, inherits all the previous and forthcoming definitions of a linear tensor function $\fua{\tnr{\psi}'}{\tnr{X}}$}. For example, by \eqref{eq:derivSameArg} and \eqref{eq:defGradient2}, we can write that
\begin{equation}\label{eq:derivSameArg2}
\fua{D\tnr{\hat\psi}}{\vto{x}} := \fua{\tnr{\hat\psi}'_{\vto{x}}}{\vto{x}}\,.
\end{equation}
Still in the context of vector functions, when $U_{\cam{F}}=V_{\cam{F}}=\cam{R}$, from mapping $\map{\hat{\psi}}{\mathcal{U}}{{\cam{R}}}$ and the previous equality, we write that
\begin{equation*}
\fua{\hat{\psi}'_{x}}{\beta} = \lim_{\alpha\to 0} \dfrac{\fua{\hat{\psi}}{x+\alpha\beta}-\fua{\hat{\psi}}{x}}{\alpha}\,,\forall\, \beta\in\cam{R}. 
\end{equation*} 
Considering $\beta=1$ and $\fua{\hat{\psi}'}{x}:=\fua{\hat{\psi}'_{x}}{1}$ the corresponding scalar of coscalar $\hat{\psi}'_{x}$, 
\begin{equation*}
\fua{\hat{\psi}'}{x} = \lim_{\alpha\to 0} \dfrac{\fua{\hat{\psi}}{x+\alpha}-\fua{\hat{\psi}}{x}}{\alpha}\,. 
\end{equation*} 
Given an arbitrary real number $x_0$, we have the classical geometric interpretation of scalar $\fua{\hat{\psi}'}{x_0}$ as being the slope of the tangent line $\hat{\psi}_\mathit{d}$ to the curve $\hat{\psi}$ at $x_0$. Now, if $\{\vun{u}\}$ is the orthonormal basis of a one dimensional Hilbert space $U_\cam{R}$, according to theorem \ref{teo:repRiesz}, it is valid that
$\fua{\hat{\psi}'_{x}}{1} = 1\fua{\hat{\psi}'}{x} = [\fua{\hat{\psi}'}{x}\vun{u}]\cdot\vun{u}$, where vector  
\begin{equation}
\nabla\fua{\hat{\psi}}{x}:=\fua{\hat{\psi}'}{x}\vun{u}
\end{equation}
is called the gradient of $\hat{\psi}$ at $x$.
...
\begin{itemize}
	...
	\end{equation}
	\item[iv.] If $\fua{\tnr{\hat\psi}}{\vto{x}}=\fua{\tnr{\hat\psi}_1}{\vto{x}}\times\fua{\tnr{\hat\psi}_2}{\vto{x}}$, where $U_\real$ in mappings  $\map{\tnr{\hat\psi}_1}{{\mathcal{U}}}{U_\cam{R}}$ and $\map{\tnr{\hat\psi}_2}{{\mathcal{U}}}{U_\cam{R}}$ is properly defined for enabling cross products, 
...
and then 
	...
	\fua{\nabla\tnr{\hat\psi}}{\vto{x}} = \fua{\nabla\tnr{\hat\psi}_1}{\vto{x}}\times\fua{\tnr{\hat\psi}_2}{\vto{x}}-\fua{\nabla\tnr{\hat\psi}_2}{\vto{x}}\times\fua{\tnr{\hat\psi}_1}{\vto{x}}\,.
	\end{equation}
\end{itemize}
...
\end{align*}
and then by adding and subtracting the limit on the right hand side by $\fua{\tnr{\hat\psi}_1}{\vto{x}+\alpha\vto{h}}\otimes\fua{\tnr{\hat\psi}_2}{\vto{x}}$ we obtain
\begin{equation*}
...
\end{equation*}
from which we easily verify the first equality of iv. From this same equality and \eqref{eq:derivVecGradVec}, development
\begin{align*}
\fua{\tnr{\hat\psi}'_{\vto{x}}}{\vto{h}} &= \fua{{\tnr{\hat\psi}_1'}_{\vto{x}}}{\vto{h}} \times \fua{\tnr{\hat\psi}_2}{\vto{x}}-\fua{{\tnr{\hat\psi}_2'}_{\vto{x}}}{\vto{h}}\times\fua{\tnr{\hat\psi}_1}{\vto{x}}\\
\Phi^{-1}\circ[\vto{h}^*\odot_{1}\fua{\nabla\tnr{\hat\psi}}{\vto{x}}]&=\Phi^{-1}\circ\{\vto{h}^*\odot_{1}[\fua{\nabla\tnr{\hat\psi}_1}{\vto{x}}\times\fua{\tnr{\hat\psi}_2}{\vto{x}}-\fua{\nabla\tnr{\hat\psi}_2}{\vto{x}}\times\fua{\tnr{\hat\psi}_1}{\vto{x}}]\}
\end{align*}
...
Now, let $(U_\real,\tnr{A}_B)$ be a three dimensional oriented Euclidean space, $V_\real$ another Euclidean space and $\map{\tnr{\varphi}}{\pmb{\mathcal{U}}}{\ete{\cam{R}}{U\times V}}$ a mapping where $\pmb{\mathcal{U}}$ is an open subset of $U_\real^*$. In this context, considering an open set $\mathcal{U}\subset U_\real$, it is possible to define a linear mapping  $\map{\gloref{curl}}{{\mathcal{U}}}{\ete{\cam{R}}{U\times V}}$, whose function is called the \textsb{curl}\index{curl} of $\tnr{\varphi}$, with rule
\begin{equation}\label{eq:curl}
\fua{\crl{\tnr{\varphi}}}{\vto{x}}=\tnr{A}_B\odot_{2}\fua{\nabla\tnr{\hat{\varphi}}}{\vto{x}}\,,
\end{equation}
where tensor $\fua{\nabla\tnr{\hat{\varphi}}}{\vto{x}}\in\ete{\cam{R}}{U^2\times V}$. Similarly to the case of gradients, there are some notable properties of divergence and curl, presented as follows, considering the appropriate previous conditions.
\begin{itemize}
\setlength\itemsep{.1em}
...
and for $m=s=1$,
\begin{equation}
\fua{\crl{\tnr{\varphi}}}{\vto{x}} = \fua{\crl{\tnr{\varphi}_1}}{\vto{x}}\odot_1\fua{\tnr{\hat{\varphi}}_2}{\vto{x}}+\fua{\crl{\tnr{\varphi}_2}}{\vto{x}}\odot_1\fua{\tnr{\hat{\varphi}}_1}{\vto{x}}\,;
\end{equation}

VERIFICAR AS DUAS PROPRIEDADES ABAIXO
\item[iv.] If $\fua{\tnr{\hat{\varphi}}}{\vto{x}}=\fua{\tnr{\hat{\varphi}}_1}{\vto{x}}\times\fua{\tnr{\hat{\varphi}}_2}{\vto{x}}$, where mappings  $\map{\tnr{\hat{\varphi}}_1}{{\mathcal{U}}}{U_\cam{R}}$ and $\map{\tnr{\hat{\varphi}}_2}{{\mathcal{U}}}{U_\cam{R}}$ are defined, then
\begin{equation}
\fua{\crl{\tnr{\varphi}}}{\vto{x}} = \fua{\crl{\tnr{\varphi}_1}}{\vto{x}}\times\fua{\tnr{\hat{\varphi}}_2}{\vto{x}}-\fua{\crl{\tnr{\varphi}_2}}{\vto{x}}\times\fua{\tnr{\hat{\varphi}}_1}{\vto{x}}\,;
\end{equation}
	
\item[v.]	If $\map{\tnr{\hat{\varphi}}}{\mathcal{U}}{\real}$, where $\mathcal{U}\subset U_\real$, then $\fua{\crl{\nabla \tnr{\varphi}}}{\vto{x}}=\vto{0}\,$.
\end{itemize}

{\footnotesize
\begin{proof}
Considering definitions \eqref{eq:divergence} and \eqref{eq:curl}, since partial inner products are left (right) distributive, proof of item i is trivial. Pre multiplying properties \eqref{eq:prop1Grad}, \eqref{eq:prop2Grad} and \eqref{eq:prop3Grad} adequately by $\tnr{I}$ or $\tnr{A}_B$, proofs of items ii, iii and iv are straightforward. For the item v, since we already know that $\vto{u}\cdot\tnr{A}_B\hat{\odot}_2\vto{v}\otimes\vto{w}=\tnr{A}_B(\vto{u},\vto{v},\vto{w})$, if we consider $\fua{{\nabla \tnr{\hat{\varphi}}}}{\vto{x}}=\vto{v}\otimes\vto{w}$, then $\vto{u}\cdot\fua{\crl{\nabla \tnr{\varphi}}}{\vto{x}}=\tnr{A}_B(\vto{u},\vto{v},\vto{w})$ for all $\vto{u}\in\pmb{\mathcal{U}}$. Thereby, choosing $\vto{u}=\vto{v}$, we have $\vto{v}\cdot\fua{\crl{\nabla \tnr{\varphi}}}{\vto{x}}=\tnr{A}_B(\vto{v},\vto{v},\vto{w})=0$ because $\tnr{A}_B$ is antisymmetric, and since $\vto{v}\neq\vto{0}$, we prove the item.
\end{proof}}
