...
\end{equation*}
The item is proved by both of these equalities and the so called Theorem of Schwarz or Clairaut's theorem, which states that second derivatives of $\mathcal{C}^2$ functions are symmetric or commute (see \aut{Lang}\cite{lang_1993_3}, Theorem 7.3), that is, $D_{\vto{x}_1}D_{\vto{x}_2}\tnr{\hat{\varphi}}(\vto{x}_1,\vto{x}_2)=D_{\vto{x}_2}D_{\vto{x}_1}\tnr{\hat{\varphi}}(\vto{x}_1,\vto{x}_2)$. Now we prove item x by considering a function rule $g(x)=\hat{\psi}(x)-\alpha x$, where $\alpha$ is a real constant and $g$ is obviously continuous on interval $[a,b]$. The condition where $g(a)=g(b)$ implies that $\alpha=(\hat{\psi}(b)-\hat{\psi}(a))/(b-a)$ and that there is a scalar $c\in(a,b)$ where $\grad{\fua{g}{c}}=0$, according to Rolle's Theorem (see \aut{Spivak}\cite{spivak_2008_1}, p.193). But since $\grad{\fua{g}{c}}=\grad{\fua{{\hat{\psi}}}{c}}-\alpha=0$, replacing $\alpha$ by its value results \eqref{eq:meanValue}. In order to prove item xi, let the function in $\map{f}{[0,1]}{\mathcal{U}}$ be described by the rule $f(t)=t\vv{b}+(1-t)\vv{a}$. Therefore, for an arbitrary $t\in[0,1]$, there is a point $x\in\overline{ab}$ where $\hat{\psi}(\vv{x})=\hat{\psi}\circ f(t)$. Since the function $\hat{\psi}\circ f$ observes the conditions of property x, there is always a real number $\kappa\in[0,1]$ where $\grad{\fua{{\hat{\psi}\circ f}}{\kappa}} =\hat{\psi}\circ f(1)-\hat{\psi}\circ f(0)$. From this equality, considering \eqref{eq:prop1Grad} and $\vv{c}=f(\kappa)$, the following equalities prove the property.
\begin{equation*}
\grad{\fua{{\hat{\psi}\circ f}}{\kappa}}=\grad{f}(\kappa)\cdot\grad{\fua{{\hat{\psi}\circ f}}{\kappa}}=(\vv{b}-\vv{a})\cdot\grad{\fua{{\hat{\psi}}}{\vv{c}}}=\hat{\psi}(\vv{b})-\hat{\psi}(\vv{a})\,.
\end{equation*}
\end{proof}}
...
\end{equation}
a condition that avoids terms like ``almost all'' or ``almost everywhere'' in forthcoming definitions. In this context, a double constituted by a measurable space and a measure, that is $((M,\css{M}),\mu)$, is called a \textsb{measure space}\index{space!measure}, which will hereafter be represented by the triple $(M,\css{M},\mu)$.  
...
\end{equation}
is called the \textsb{integral}\index{integral!of step function} of $s_\css{A}$ on $E$. Now, if $\css{B}=\{B_1,\cdots,B_m\}\subseteq\css{M}$ is another partition of $A$, we conclude that there is no element of $A$ which does not belong to some $A_i\cap B_j$ and then it is obvious that class 
\begin{equation*}
...
\begin{alignat*} {3}
\|s\|_1=\int_E |s|&\qquad \text{ and } \qquad&\fua{\varrho}{s_1,s_2}=\|s_1-s_2\|_1\,,
\end{alignat*}
...
\tnr{T}\cdot\int_{\pmb{\mathcal{U}}}\tnr{\psi}=\int_{\pmb{\mathcal{U}}} \frt{T}\circ\tnr{\psi}\,.
\end{equation}
