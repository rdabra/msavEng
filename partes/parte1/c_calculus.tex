...
\begin{itemize}
	...
	\item[ii.] If $\fua{\tnr{\psi}}{\tnr{X}}=\beta\tnr{X}$, tensor $\fua{[\fua{\tnr{\psi}'}{\tnr{X}}]}{\tnr{H}}=\beta\tnr{H}\,$ and therefore $\fua{\tnr{\psi}'}{\tnr{X}}=\tnr{i}$ for $\beta=1$;
...
\end{equation}
is the representative function of $\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}$. Moreover, it is convenient to have vector functions  
...
\end{equation}
according to \eqref{eq:genInnerProdVec} and \eqref{eq:defGradient2}. Now, considering equality \eqref{eq:secTens2Lin} and second order tensor $\fua{\nabla\tnr{\hat{\psi}}}{\vto{x}_0}=\fua{\nabla\tnr{\psi}}{\vto{x}_0^*}$, we also have for the present case that 
\begin{equation}\label{eq:gradFuncRep}
\fua{[\fua{\nabla\tnr{\hat{\psi}}}{\vto{x}_0}]}{\vto{x},\vto{y}}=\vto{x}\cdot\fua{\tnr{\hat\psi}'_{\vto{x}_0}}{\vto{y}^{c}}\,,\,\,\forall\, \vto{x},\vto{y}\in\mathcal{U}\,,
\end{equation}
...
\end{align}
which results the rule of the directional derivative of the vector function in $\map{\tnr{\hat\psi}}{\mathcal{U}}{V_\cam{F}}$ along $\vto{h}\in\mathcal{U}$. From this result, \emph{the derivative $\tnr{\hat\psi}'_{\vto{x}}$, as a linear vector function, inherits all the previous and forthcoming definitions of the linear tensor function $\fua{\tnr{\psi}'}{\vto{x}^*}$}. For example, by \eqref{eq:derivSameArg} and \eqref{eq:defGradient2}, we can write that vector
...
\begin{itemize}
	...
	\end{equation}	
	\item[iii.] If $\tnr{\psi}$ is a diffeomorphism then $\fua{\nabla\tnr{\psi}}{\tnr{X}}\neq\tnr{0}$ because
	\begin{equation}\label{eq:indentDiffeo}
	\tnr{I} = \fua{\nabla\tnr{\psi}}{\tnr{X}} \odot_s \fua{\nabla\tnr{\psi}^{-1}\circ\tnr{\psi}}{\tnr{X}}\,;
	\end{equation}	
	\item[iv.] If $\fua{\tnr{\psi}}{\tnr{X}}=\fua{\tnr{\psi}_1}{\tnr{X}}\cdot\fua{\tnr{\psi}_2}{\tnr{X}}$, where   $\map{\tnr{\psi}_1}{\pmb{\mathcal{U}}}{\ete{\real}{V^{\times s}}}$, $\map{\tnr{\psi}_2}{\pmb{\mathcal{U}}}{\ete{\real}{V^{\times s}}}$ and domain $\pmb{\mathcal{U}}\subset\ete{\real}{U^{\times m}}$, then
	...
	\end{equation}
	\item[v.] If $\fua{\tnr{\hat\psi}}{\vto{x}}=\fua{\tnr{\hat\psi}_1}{\vto{x}}\times\fua{\tnr{\hat\psi}_2}{\vto{x}}$, where $U_\real$ in mappings  $\map{\tnr{\hat\psi}_1}{{\mathcal{U}}}{U_\cam{R}}$ and $\map{\tnr{\hat\psi}_2}{{\mathcal{U}}}{U_\cam{R}}$ is properly defined for enabling cross products, 
...
\end{align*}
Now, applying property ii on $\tnr{i}=\tnr{\psi}^{-1}\circ\tnr{\psi}$ and considering \eqref{eq:cotensIdent}, proof of \eqref{eq:indentDiffeo} is straightforward and the statement of item iii is obvious. From the Leibniz's rule we have
\begin{align*}
...
&=\tnr{H}\odot_m\{\fua{[\nabla\tnr{\psi}_1]}{\tnr{X}}\odot_s\fua{\tnr{\psi}_2}{\tnr{X}}+\fua{[\nabla\tnr{\psi}_2]}{\tnr{X}}\odot_s\fua{\tnr{\psi}_1}{\tnr{X}}\}\,,
\end{align*}
which proves item iv. Considering $(U_\real,\tnr{A}_B)$ a three dimensional oriented Euclidean space, we prove the equalities of item v first by the development
\begin{align*}
...
\end{equation*}
from which we easily verify the first equality of v. By definition \eqref{eq:derivSameArg2}, proof of the second equality is straightforward. 
\end{proof}}
