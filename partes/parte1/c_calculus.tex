...
\item[x.] \textsb{Mean Value Theorem}\index{theorem!mean value}\index{mean value theorem}: given the closed interval $[a,b]\subset\real$ where $a<b$, if the function in $\map{\hat{\psi}}{(a,b)}{\cam{R}}$ is continuous on its domain and differentiable on open interval $(a,b)$, then there is always a scalar $c\in[a,b]$ where
