...
\section{Integral identities}\index{integral!identities}
...
Let $U_\real$ be a $n$-dimensional Euclidean space, $n\geqslant 2\,$, and $\pmb{\mathcal{V}}$ one of its proper subsets, which we assume to be a domain of a certain function. Considering an arbitrary non zero scalar $\alpha\in\real^+$, a disjoint cover $\css{C}=\{\widehat{C}_1,\cdots,\widehat{C}_r\}$ of boundary $\partial\pmb{\mathcal{V}}$ and a class $\{A_1,\cdots,A_r\}$ where $A_i\subset C_i$ defines a $(n-1)$-dimensional Euclidean subspace of $U_\real$ from the first $n-1$ dimensions of $U_\real$, the subset $\pmb{\mathcal{V}}$ is called a \textsb{Lipschitz domain}\index{domain!Lipschitz} and its boundary a \textsb{Lipschitz surface}\index{surface!Lipschitz} if the following conditions are observed. 

\begin{itemize}
	\setlength\itemsep{.1em}
	\item[i.] Every element $\widehat{C}_j$ of $\css{C}$ must have a specific shape such that arbitrary vectors $\vto{u},\vto{v}\in\{\vto{x}\in\pmb{\mathcal{V}}:\fua{\varrho}{\vto{x},\partial\pmb{\mathcal{V}}}<\alpha\}$, where distance $\fua{\varrho}{\vto{u},\vto{v}}<\alpha$, are also elements of the subset $\{\vto{x}\in\widehat{C}_j:\fua{\varrho}{\vto{x},\partial C_j}>\alpha\}\,$;

	\item[ii.] Every set $\pmb{\mathcal{V}}\cap \widehat{C}_j$ must be on the same ``side'' of boundary $\partial\pmb{\mathcal{V}}$, whose points coordinates must constitute the graph of a Lipschitz continuous function. In other words, there must exist a set of Lipschitz continuous functionals $\{f_1,\cdots,f_r\}$ where $\map{f_i}{A_i}{\real}$ in such a way that for an orthonormal basis $Z=\{\vun{v}_1,\cdots,\vun{v}_n\}$ of $\widehat{C}_j$ and arbitrary vector $\vto{w}\in \pmb{\mathcal{V}}\cap\widehat{C}_j$, 
\begin{equation*}
|\fua{\vtf{f}_n^Z}{\vto{w}}|<|f_j[\sum_{i=1}^{n-1}\fua{\vtf{f}_i^Z}{\vto{w}}\vun{v}_i]|\,.
\end{equation*}	
\end{itemize}

Considering $\pmb{\mathcal{V}}\subset U_\cam{R}$ a Lipschitz domain, we additionally require orientation of the Lipschitz surface $\partial\pmb{\mathcal{V}}$ on each of its points. In order to adequately accomplish this, let $\pmb{\mathcal{W}}$ be a $(n-1)$-dimensional subspace of $U_\cam{R}$ and the function in $\map{\vtf{g}}{\pmb{\mathcal{W}}}{\real}$ be Lipschitz continuous whose graph describes $\partial\pmb{\mathcal{V}}$ in such a way that 
\begin{equation*}
\fua{\vtf{f}_n^B}{\vto{v}} = \fua{\vtf{g}}{\sum_{i=1}^{n-1}\fua{\vtf{f}_i^B}{\vto{v}}\vun{u}_i}\,,\,\forall\,\vto{v}\in\partial\pmb{\mathcal{V}}\,, 
\end{equation*}
...
\begin{equation}\label{eq:impliSurf}
\fua{\vtf{s}}{\vto{v}} = \fua{\vtf{f}_n^B}{\vto{v}} - \fua{\vtf{g}}{\sum_{i=1}^{n-1}\fua{\vtf{f}_i^B}{\vto{v}}\vun{u}_i}=0\,,\,\forall\,\vto{v}\in\partial\pmb{\mathcal{V}}\,,
\end{equation}
is called the \textsb{implicit function}\index{function!implict} of $\partial\pmb{\mathcal{V}}$ and since it is constant, equality $[\fua{\vtf{s}'}{\vto{v}}](\vto{h})=0$ is valid along any direction $\vto{h}\in\partial\pmb{\mathcal{V}}$. Therefore,  from \eqref{eq:defGradient}, we can say that $\vto{h}\cdot\fua{\nabla\vtf{s}}{\vto{v}}=0$ for all $\vto{h}\in\{\viz{\vto{v}}\cap\partial\pmb{\mathcal{V}}\}$, that is, vector $\fua{\nabla\vtf{s}}{\vto{v}}$ is orthogonal to surface $\partial\pmb{\mathcal{V}}$ at arbitrary $\vto{v}\in\partial\pmb{\mathcal{V}}$. From this conclusion, surface $\partial\pmb{\mathcal{V}}$ is called \textsb{positively oriented}\index{surface!positively oriented} if it is possible to define a mapping $\map{\vtf{n}^+}{\partial\pmb{\mathcal{V}}}{U_\cam{R}}$ whose function, called the \textsb{outward unit normal function}\index{function!outward unit normal} on $\partial\pmb{\mathcal{V}}$, is described by 
\begin{equation}
\fua{\vtf{n}^+}{\vto{x}}=\dfrac{\fua{\nabla\vtf{s}}{\vto{x}}}{\|\fua{\nabla\vtf{s}}{\vto{x}}\|}\,.
\end{equation}
As a consequence of our definition of implicit function \eqref{eq:impliSurf}, the values of $\vtf{n}^+$ are outward unit normal vectors with respect to the surface. In some particular cases, it is useful to define an \textsb{inward unit normal function}\index{function!inward unit normal}  $\vtf{n}^-=-\vtf{n}^+$, when the surface in question is called \textsb{negatively oriented}\index{surface!negatively oriented}. 


%In our study and many other cases, it is necessary to \emph{specify} the direction of unit normal vectors: inward or outward, with respect to the surface. If we choose that there is always a non zero scalar $\alpha\in\real^+$ for all $\vto{x}\in\partial\pmb{\mathcal{V}}$ where $\vto{x}+\alpha\fua{\vtf{n}}{\vto{x}}\notin\pmb{\mathcal{V}}$, then 
%$\vtf{n}^+:=\vtf{n}$ is an \textsb{outward}\index{function!outward unit normal} unit normal function on $\partial\pmb{\mathcal{V}}$. When the previous restriction is chosen to be $\vto{x}+\alpha\fua{\vtf{n}}{\vto{x}}\in\pmb{\mathcal{V}}$, then $\vtf{n}^-:=\vtf{n}$ is called an \textsb{inward}\index{function!inward unit normal} unit normal function.
