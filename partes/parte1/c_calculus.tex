...
Considering $\pmb{\mathcal{V}}$ a Lipschitz domain, we additionally require orientation of the Lipschitz surface $\partial\pmb{\mathcal{V}}$ on each of its points. In order to adequately accomplish this, let $\pmb{\mathcal{W}}$ be a proper subset of $\partial\pmb{\mathcal{V}}$ and the function in $\map{\vtf{g}}{\pmb{\mathcal{W}}}{\real}$ be Lipschitz continuous whose graph describes $\partial\pmb{\mathcal{V}}$ in such a way that 
\begin{equation*}
\fua{\vtf{f}_n^B}{\vto{u}} = \fua{\vtf{g}}{\sum_{i=1}^{n-1}\fua{\vtf{f}_i^B}{\vto{u}}\vun{u}_i}\,,\,\forall\,\vto{u}\in\partial\pmb{\mathcal{V}}\,, 
\end{equation*}
where $B=\{\vun{u}_1,\cdots,\vun{u}_n\}$ is an arbitrary orthonormal basis of $U_\real$. Thereby, the constant vector function in $\map{\vtf{s}}{\partial\pmb{\mathcal{V}}}{\real}$, described by 
\begin{equation}
\fua{\vtf{s}}{\vto{x}} = \fua{\vtf{f}_n^B}{\vto{x}} - \fua{\vtf{g}}{\sum_{i=1}^{n-1}\fua{\vtf{f}_i^B}{\vto{x}}\vun{u}_i}=0\,,
\end{equation}
is called the \textsb{implicit function}\index{function!implict} of $\partial\pmb{\mathcal{V}}$ and it is obvious that       
