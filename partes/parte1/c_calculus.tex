...
\begin{itemize}
	...
	\item[ii.] Every set $\pmb{\mathcal{U}}\cap \widehat{C}_j$ must be on the same ``side'' of boundary $\partial\pmb{\mathcal{U}}$, whose points coordinates must constitute the graph of a Lipschitz continuous function. In other words, there must exist a set of Lipschitz continuous functionals $\{f_1,\cdots,f_r\}$ where $\map{f_i}{A_i}{\real}$ in such a way that for an orthonormal basis $B=\{\vun{u}_1,\cdots,\vun{u}_n\}$ of $\widehat{C}_j$ and arbitrary vector $\vto{w}\in \pmb{\mathcal{U}}\cap\widehat{C}_j$, 
\begin{equation*}
|\fua{\vtf{f}_n^B}{\vto{w}}|<|f_j[\sum_{i=1}^{n-1}\fua{\vtf{f}_i^B}{\vto{w}}\vun{u}_i]|\,.
\end{equation*}	
\end{itemize}

Considering $\pmb{\mathcal{U}}$ a Lipschitz domain, we additionally require orientation of the Lipschitz surface $\partial\pmb{\mathcal{U}}$ on each of its boundary points. In order to adequately accomplish this, let the function in $\map{h}{\real^{n-1}}{\real}$ be Lipschitz continuous whose graph describes $\partial\pmb{\mathcal{U}}$ in such a way that 
\begin{equation*}
\fua{\vtf{f}_n^B}{\vto{u}} = \fua{h}{\fua{\vtf{f}_1^B}{\vto{u}},\cdots,\fua{\vtf{f}_{n-1}^B}{\vto{u}}}\,,\,\forall\,\vto{u}\in\partial\pmb{\mathcal{U}}\,, 
\end{equation*}
where $B$ is an arbitrary orthonormal basis of $U_\real$.
