...
Now, for the case of a measure space $(M,\css{M},\mu)$, where $M=\eamd{S}{\real}{n}$ is an Euclidean affine space, given an arbitrary $A\in\css{M}$, if each of the classes $\css{C}_i:=\{C_{i1},C_{i2},\cdots\}$ are covers of $A$, where $C_{ij}\subset A$ are hyperrectangles with side lengths $(l_{ij1},\cdots,l_{ijn})$ and hypervolumes $vol(C_{ij}):=\prod_{k=1}^nl_{ijk}$, we define 
\begin{equation}
\lambda(A)=\inf\{\sum_{j=1}^\infty vol(C_{1j}),\sum_{j=1}^\infty vol(C_{2j}),\cdots\}\,,
\end{equation}
 where $\inf$ refers to the infimum (greatest lower bound) of the set involved\footnote{The infimum is used here because the minimum value of this set cannot be determined.}. If
\begin{equation}
\lambda(A)=\lambda(A\cap B)+\lambda(A'\cap B)\,,\, \forall\, A,B\in\css{M}\,,
\end{equation}
then $\lambda$ results a specific measure $\mu$, called \textsb{Labesgue Measure}\index{measure!Labesgue}. It can be concluded that for the particular cases of one, two or three dimensional affine space $M$, real number $\lambda(A)$ corresponds to the length, area or volume of $A$ respectively. Moreover, we recall that in an Euclidean affine space, a coordinate system is implicit and then points and vectors are biunivocally correlated. Therefore, $M=U\eamd{S}{\real}{n}$

in this context, the Labesgue Measure of set of vectors can be defined to be the Labesgue Measure of set of its corresponding points, that is, if $M$
