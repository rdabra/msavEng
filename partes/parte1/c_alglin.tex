...
Let's start the study of linear functions considering first an additive group $V^U_\cam{F}$ constituted of generic functions that define mappings of the type $U_\cam{F}\mapsto V_\cam{F}$, where $U_\cam{F}$ and $V_\cam{F}$ are complete vector spaces. This group is said to define a vector space \gloref{espacFunc}, usually called a \textsb{function space}\index{space!function}, if for arbitrary \textsb{vector functions}\index{vector!function} $\vtf{f},\vtf{g}\in V^U_\cam{F}$ and scalar $\alpha\in\cam{F}$ the following restrictions are observed:
