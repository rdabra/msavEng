...
\end{align}
for all $\alpha,\beta\in\mathcal{R}$ and $\vto{w},\vto{w}_i\in W_i$. In this context, when domain $U_\mathcal{R}=V_\mathcal{R}^q$, the vectors of $V^{V^q}_\mathcal{R}$ are called \textsb{multilinear operators}\index{operator!multilinear} and the mappings they define are \textsb{multilinear operations}\index{operation!multilinear}. Moreover, for the purposes of our study, it is useful to define a ``kind of'' multilinear function $\overline{\vtf{k}}\in\evl{\mathcal{R}}{W^{\times q}}{V}$, called \textsb{conjugate multilinear function}\index{function!conjugate multilinear}, where 
\begin{align}\label{eq:conjMultFun}
\lefteqn{\fua{\overline{\vtf{k}}}{\vto{w}_1,\cdots,\alpha\vto{w}_i+ \beta\vto{w},\cdots,\vto{w}_q}=} & & \nonumber\\
& &\overline{\alpha}\,\fua{\overline{\vtf{k}}}{\vto{w}_1,\cdots,\vto{w}_i,\cdots,\vto{w}_q}+\overline{\beta}\,\fua{\overline{\vtf{k}}}{\vto{w}_1,\cdots,\vto{w},\cdots,\vto{w}_q}\,,
\end{align}
for all $\alpha,\beta\in\mathcal{R}$ and $\vto{w},\vto{w}_i\in W_i$. In practice, we say that $\overline{\vtf{k}}$ is the conjugate correspondent of multilinear function $\vtf{k}$.
