...
\vto{u}^c=(\sum_{i=1}^{n}\fua{\vtf{f}^\con{B}_{i}}{\vto{u}}\vto{u}_i)^c=\sum_{i=1}^{n}\fua{\overline{\vtf{f}^\con{B}_{i}}}{\vto{u}}\vto{u}_i^c\,.
\end{equation}
...
\vun{u}_j^c\cdot\vun{u}_i^c=\vun{u}_j\cdot\vun{u}_i^c=\delta_{ji}\,.
\end{equation}
{\footnotesize
\begin{proof}
We'll prove only the second equality of this expression. Multiplying $\vun{u}_j\cdot\vun{u}_i=\delta_{ji}$ by $\alpha$ we have $\vun{u}_j\cdot\overline{\alpha}\vun{u}_i=\vun{u}_j\cdot\alpha\vun{u}_i^c=\overline{\alpha}\vun{u}_j\cdot\vun{u}_i^c=\alpha\delta_{ji}$ and by $\overline{\alpha}$ we arrive at $\alpha\vun{u}_j\cdot\vun{u}_i^c=\overline{\alpha}\delta_{ji}$. When $j\neq i$, it is obvious that $\alpha$ cannot be zero. If $j=i$, we obtain that $\vun{u}_j\cdot\vun{u}_i^c=\alpha/ \overline{\alpha}=\overline{\alpha}/\alpha$, and then $\alpha=\overline{\alpha}$.  
\end{proof}
}
