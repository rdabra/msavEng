...
\begin{mcoro}{Covectors and Linear Functions}{covLin}
A mapping $\map{\varphi}{\evl{\cam{F}}{U}{V}}{\evl{\cam{F}}{U^*}{V}}$, where $V_\cam{F}$ is a Hilbert space, with function rule  ${{}^*\vtf{g}} :=\fua{\varphi}{\vtf{g}}=\vtf{g}\circ\Phi^{-1}$ is a linear bijective transformation.
\end{mcoro}
{\footnotesize
	\begin{proof}
		Considering $\vto{u}$ and $\vto{v}$ vectors of $U_\cam{F}$, function $\Phi$ is antilinear from the following equalities:
		...
		\fua{\vtf{g}}{\vto{x}}=\fua{[\sum_{i=1}^n\alpha_i\vtf{f}^{\hat{B}}_i]}{\vto{x}}=\sum_{i=1}^n\alpha_i\fua{\vtf{f}^{\hat{B}}_i}{\vto{x}}=\sum_{i=1}^n\alpha_i\lpa\vto{x}\cdot\vun{u}_i\rpa=\vto{x}	\cdot ( \sum_{i=1}^n\overline{\alpha_i} \vun{u}_i)\,.
		\end{equation*}
		As $\fua{\vtf{g}}{\vto{x}}=\fua{\lco\fua{\Phi}{\vto{u}}\rco}{\vto{x}}=\vto{x}\cdot\vto{u}$, the existence of $\vto{u}=\sum_{i=1}^n\overline{\alpha_i} \vun{u}_i$ is verified. 
	\end{proof}
}
