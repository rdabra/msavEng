...
\end{equation}
for all $\alpha,\beta\in\cam{F},\,\vto{u}_1,\vto{u}_2\in U$. Thereby, we call $\vtf{h}$ a \textsb{linear function}\index{function!linear}\label{def:linear} and the corresponding mapping a \textsb{linear transformation}\index{transformation!linear}. If the function space $V^U_\cam{F}$ has only linear functions, its usual representation is $\gloref{evl}$. Now, considering $\{\vto{u}_1,\cdots,\vto{u}_n\}$ a basis of $U_\cam{F}$, a bijective 
...
\end{alignat}
where $\vto{x}\diamond\vto{y}:=\vto{x}\cdot\vto{y}^c$ is called the \textsb{contractive product}\index{contractive product!of vectors} of vectors $\vto{x}$ and $\vto{y}$, which is clearly bilinear. 

%REESCREVER A EXPRESSÃO DA FUNCAO TRANSPOSTA USANDO O C E PROVAR LOGO ABAIXO
