...
The theorem above also assures the existence of orthonormal bases because they can be obtained from normalization of orthogonal bases. Thereby, for an arbitrary orthonormal basis $\{\vun{u}_1,\cdots,\vun{u}_n\}$ of a Hilbert space, it is clear that inner products $\vto{u}_i\cdot\vto{u}_j/\|\vto{u}_i\|\overline{\|\vto{u}_j\|}=\delta_{ij}$ since $\|\vto{x}\|^2=\vto{x}\cdot\vto{x}$ and $\|\vto{x}\|=\overline{\|\vto{x}\|}$. Now, let $\vto{x}$ and $\vto{y}$ be arbitrary vectors of Euclidean space $E_\real$, of which $\hat{B}=\{\vun{v}_1,\cdots,\vun{v}_n\}$ is an orthonormal basis. Then, we can say that
\begin{equation}
