...
Considering a vector space $U_\mathcal{R}$, the elements of base $C$ can be written on basis $B$ according to the following expressions:
...
Gathering the components on $Z$ of all the elements of bases $B$ and $C$, we have
\begin{equation}
\begin{bmatrix}
...
{\footnotesize
\begin{proof}
Considering a function $\vtf{g}\in\evl{\mathcal{R}}{Y}{Y}$ and a vector $\vto{u}\in Y_\mathcal{R}$, the last equality of the development 
\begin{align}
...
\end{align}
and equality $\mav{\fua{\vtf{g}}{\vto{u}}}{C}=\maf{\vtf{g}}{C}{C}\mav{\vto{u}}{C}$ enables us to affirm that 
\begin{equation*}
\maf{\vtf{g}}{C}{C} = \maf{\vtf{i}}{B}{C}\maf{\vtf{g}}{B}{B}\maf{\vtf{i}}{C}{B}\,.
\end{equation*}
As $\maf{\vtf{i}}{B}{C}={\maf{\vtf{i}}{C}{B}}^{-1}$, we conclude that matrices $\maf{\vtf{g}}{C}{C}$ e $\maf{\vtf{g}}{B}{B}$ are similar.
\end{proof}
}
...
Considering a $n$-dimensional Hilbert space $U_\mathcal{R}$, we call $\alpha\vto{u}\in U_\mathcal{R}$, where $\alpha\in\mathcal{R}$, a \textsb{multiple scalar}\index{multiple scalar} of vector $\vto{u}$. The scalar $\alpha$, which specifies this multiplicity, on the context of norm $\|\alpha\vto{u}\|=|\alpha|\|\vto{u}\|$, performs a ``resizing'' of $\vto{u}$, that is, if $|\alpha|<1$, there is a decrease in its size or intensity; conversely, if $|\alpha|>1$, there is an increase. Given a linear operator $\vtf{l}\in\evl{\mathcal{R}}{U}{U}$, vector $\vto{u}$ is called an \textsb{eigenvector}\index{eigenvector} of $\vtf{l}$ if it is non zero and value $\fua{\vtf{l}}{\vto{u}}$ is its multiple scalar, that is, if $\fua{\vtf{l}}{\vto{u}}=\alpha\vto{u}$, where multiplicity $\alpha$ is said to be an \textsb{eigenvalue}\index{eigenvalueo} of $\vtf{l}$. In other words, \emph{a vector resized by a linear operator is its eigenvector and the sign-magnitude of this resizing is its eigenvalue}. 
...
\end{align*}
from which we conclude that the highlighted term is unitary. Thus, we can state that polar decomposition exists since bijection $\vtf{g}=\vtf{q}\circ\vtf{h}^{\nicefrac{1}{2}}$. As the square root  $\vtf{h}^{\nicefrac{1}{2}}$  is unique for 
$\vtf{g}\circ\vtf{g}^\dagger$, the unitary operator $\vtf{q}=\vtf{g}\circ\vtf{h}^{-\nicefrac{1}{2}}$ is also unique; which results a unique polar decomposition for $\vtf{g}$.
\end{proof}
}
...
{\footnotesize
\begin{proof}
The verification of the existence and uniqueness of left polar decomposition follows the same process of the right. Let's prove now that the unitary operators in both decompositions are really the same. 
