...
\item[ii.] $\tnr{G}^{-1}\odot_1\tnr{G}\in\ete{\cam{F}}{V^2}$ and $\tnr{G}\odot_1\tnr{G}^{-1}\in\ete{\cam{F}}{U^2}$ are identity tensors;
\item[iii.] If $\tnr{\mathit{g}}$ is hermitian then  $\tnr{G}^\dagger=\tnr{G}$.
\end{itemize}
...
\end{align*}
Considering \eqref{eq:cotensIdent}, if $\vtf{\mathit{g}}$ is the representative function of $\tnr{G}$, development 
\begin{align*}
\tnr{i}(\vto{v})&=\tnr{\mathit{g}}\circ\tnr{\mathit{g}^{-1}}(\vto{v})\\
[\vto{v}^*\hat{\odot}_1\tnr{I}]^c&=\tnr{\mathit{g}}([\vto{v}^*\hat{\odot}_1\tnr{G}^{-1}]^c)\\
&=[\vto{v}^*\odot_1\tnr{G}^{-1}]\hat{\odot}_1\tnr{G}\\
&=\vto{v}^*\hat{\odot}_1(\tnr{G}^{-1}\odot_1\tnr{G})
\end{align*}
proves the first identity tensor of the second item. Verification of $\tnr{G}\odot_1\tnr{G}^{-1}$ is similar. Proof of third item is trivial from \eqref{eq:Lin2secTens}.
\end{proof}
}
\noindent An important consequence of the first property is that 
\begin{equation}\label{eq:Lin2secTensTransp}
\fua{\tnr{\mathit{g}}}{\vto{u}} = \vto{u}^*\hat{\odot}_1\tnr{G}=[\tnr{G}^\dagger\hat{\odot}_1\vto{u}^*]^c\,.
\end{equation}
There is also an interesting property relating the composition of representative functions and second order tensors: let functions $\tnr{\mathit{g}}_1$ and $\tnr{\mathit{g}}_2$ represent $\tnr{G}_1\in\ete{\cam{F}}{U\times W}$ and $\tnr{G}_2\in\ete{\cam{F}}{W\times V}$ respectively. The following equalities show that $\tnr{G}_1\odot_1\tnr{G}_2\in\ete{\cam{F}}{U\times V}$ is represented by $\tnr{\mathit{g}}_2\circ\tnr{\mathit{g}}_1$. 
\begin{equation}\label{eq:compoFuncRepFunc}
\fua{\tnr{\mathit{g}}_2\circ\tnr{\mathit{g}}_1}{\vto{u}}=\tnr{\mathit{g}}_2[\vto{u}^*\hat{\odot}_1\tnr{G}_1]
=(\vto{u}^*\odot_1\tnr{G}_1)\hat{\odot}_1\tnr{G}_2
= \vto{u}^*\hat{\odot}_1(\tnr{G}_1\odot_1\tnr{G}_2)\,.
\end{equation}
...
In the context of an arbitrary second order tensor $\tnr{G}\in\ete{\cam{F}}{U\times V}$, we already know that it is represented by a linear bijection $\tnr{\mathit{g}}$ where $\fua{\tnr{\mathit{g}}}{\vto{u}}=\vto{u}^*\hat{\odot}_1\tnr{G}$. Considering $B_1$ and $B_2$ bases of $U_\cam{F}$ and $V_\cam{F}$, we can rewrite the previous equality as following:
\begin{equation}
[\vtf{g}_{B_1}]^{B_2}[\vto{u}]^{B_1} = [\tnr{G}]^{B}[\vto{u}]^{B_1}\,,
\end{equation}
where basis tensor $B$ is built from $B_1$ and $B_2$ according to theorem \ref{teo:geraEte}.
