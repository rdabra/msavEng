...
Additionally to symmetric and antisymmetric, there is another notable tensor that will enable very important concepts for our study, like isotropy and anti-isotropy for example: I'm referring to the identity tensor, which is defined in tensor spaces of even order. We call \gloref{tensorI}$\,\in\ete{\cam{F}}{V^{\times p}\times V^{\times p}}$ an \textsb{identity tensor}\index{tensor!identity} of order $2p$ if, considering $\vto{x}_i$ and $\vto{y}_i$ arbitrary vectors of  $(V_\cam{F})_i$, scalar 
 ...
 \end{equation}
From this definition, if $(V_\cam{F})_i$ is a Hilbert space and $B_i=\{\vto{v}_1^{(i)},\cdots,\vto{v}_{n_i}^{(i)}\}$ one of its basis, the following is valid: 
\begin{equation}\label{eq:identidade}
\fua{\tnr{I}}{\vto{x}_1,\cdots,\vto{x}_p,\vto{y}_1,\cdots,\vto{y}_p}=\sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\,\prod_{k=1}^p\fua{\vtf{f}^\con{B_k}_{i_k}}{\vto{x}_k}\fua{\overline{\vtf{f}^\con{B^\perp_k}_{i_k}}}{\vto{y}_k}\,,
\end{equation}
...
\end{equation}     
Moreover, if the tensor space in question has an even order $m=2q$, we can write  
\begin{equation}
\tnr{I}\cdot \tnr{B}=\fua{\tnr{I}}{\vto{b}_1,\cdots,\vto{b}_m}= \prod_{i=1}^q\vto{b}_i\cdot\vto{b}_{i+q}\,.
\end{equation}   
