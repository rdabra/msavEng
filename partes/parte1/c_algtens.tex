...
Considering function spaces $V^{U}_\cam{F}$ and  $\evl{\cam{F}}{\ete{\cam{F}}{U}}{V}$, where sets $V$ and $U$ define Hilbert spaces $V_\cam{F}$ and $U_\cam{F}=A^{\times p}_\cam{F}\times X^{\times m}_\cam{F}\times B^{\times q}_\cam{F}$ respectively, for every multiantilinear function $\vtf{c}\in V^{U}_\cam{F}$ there is a unique linear tensor function  \gloref{elevC}$\in\evl{\cam{F}}{\ete{\cam{F}}{U}}{V}$ such that 
\begin{multline}
\fua{\vtf{c}^\otimes}{\vto{a}_1^*\otimes\cdots\otimes\vto{a}_p^*\otimes\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*\otimes\vto{b}_1^*\otimes\cdots\otimes\vto{b}_q^*}=\\
\fua{{\vtf{c}}}{\vto{a}_1,\cdots,\vto{a}_p,\vto{x}_1,\cdots,\vto{x}_m,\vto{b}_1,\cdots,\vto{b}_q}\,,\,\,\forall\,\vto{a}_j\in A_j,\,\vto{x}_i\in X_i,\,\vto{b}_k\in B_k \nonumber\,.
\end{multline}
\end{mteo}
\hspace{1pt}
{\footnotesize
\begin{proof}
The following proof is adapted from \aut{Backus}\cite{backus_1997_1}, pp. 43-45. In order to shorten expression sizes, we shall consider that $U_\cam{F}=X^{\times m}_\cam{F}$ since spaces $A^{\times p}_\cam{F}$ and $B^{\times q}_\cam{F}$ do not alter the conceptual results of the following steps. According to theorem \ref{teo:geraEte}, let $\tnr{X}\in\ete{\cam{F}}{U}$ be an arbitrary tensor described by 
\begin{equation}
