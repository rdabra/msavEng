...
In the context of Hilbert spaces, the theorem above establishes a biunivocal relationship between a tensor space and a linear tensor function space. We already know that corollary \ref{cor:covLin} establishes a biunivocal relationship between a linear vector function space and a linear covector function space. Thereby, since a linear covector function is a tensor function whose domain is constituted by first order tensors (covectors), then we conclude that every linear vector function has its correspondent second order tensor. Let's now rewrite this rationale in a more concise form in the following corollary. 

\begin{mcoro}{Second Order Tensors and Linear Vector Functions}{soTens}
Considering Hilbert spaces $U_\cam{F}$ and $V_\cam{F}$, a mapping $\map{\tnr{\Upsilon}}{\ete{\cam{F}}{U\times V}}{\evl{\cam{F}}{U}{V}}$ is a bijective transformation if its function rule is $\fua{\tnr{\Upsilon}}{\tnr{X}}=\vtf{X}^*_1\circ\Phi$, where $\Phi$ is defined on theorem \ref{teo:repRiesz}. 
\end{mcoro}
{\footnotesize
\begin{proof}
If function $\tnr{\Upsilon}$ were not an injection, there would be two distinct tensors $\tnr{T}$ and $\tnr{W}$ where $\vtf{T}^*_1\circ\Phi(\vto{x})=\vtf{W}^*_1\circ\Phi(\vto{x})$; but this equality leads to $\vtf{T}^*_1(\vto{x}^*)=\vtf{W}^*_1(\vto{x}^*)$, which shows that $\vtf{T}^*_1=\vtf{W}^*_1$. Therefore, since $p$-cotensors are biunivocaly related to tensors, $\tnr{T}$ and $\tnr{W}$ cannot be distinct. Function $\tnr{\Upsilon}$ is also a surjection because, through corollary \ref{cor:covLin}, for every linear function $\vtf{g}\in\evl{\cam{F}}{U}{V}$ a covector function $\vtf{g}_\Phi$ can be defined and then, for every tensor function $\vtf{g}_\Phi$ there exists a unique second order tensor $G\in\ete{\cam{F}}{U\times V}$ where $1$-cotensor $G_1^*=\vtf{g}_\Phi$.
\end{proof}
}
