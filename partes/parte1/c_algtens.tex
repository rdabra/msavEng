...
For the purposes of our study, a tensor function establishes a functional relationship between tensors of any order. If these tensors are defined by Hilbert spaces, such functions are of the most general type: as presented in the previous section, tensors of order zero are scalars by definition and first order tensors are covectors, which are biunivocaly related to vectors, according to the Riesz-Fréchet Representation Theorem. This generality of a tensor function is particularly  important for us and that is why we shall be dealing mostly with Hilbert spaces hereafter, betaking all their particularities. This context is quite convenient to structure a tensor product space of any order likewise an ordinary vector space, 

we shall work with tensor product space of any order can be structured as an ordinary vector space, resulting that the former generalizes of the latter. 

following the same nomenclature described in figure \ref{fig:esquemaEspacos}, if inner product, norm and metric are defined accordingly. Therefore, subsequent definitions, such as function spaces constituted by tensor functions, are possible. Moreover, it is always possible to relate a multiantilinear function and a linear tensor function, which is a fundamental feature to enable uniqueness and existence of the fundamental linear tensor function to be presented in this section, which we call contraction. The following theorem\footnote{Adapted from \aut{Backus}\cite{backus_1997_1} after generalizing a property from  \aut{Marcus}\cite{marcus_1973_1}, p.14.}, borrowed from the realm of Multilinear Algebra, describes such feature by establishing a one to one correspondence between a multiantilinear function and a linear tensor function. From this biunivocal relationship, the multiantilinear function is usually said to be ``lifted'' to a tensor function.   

\begin{mteo}{Lifting of Multiantilinear Functions}{lift}
Considering function spaces $V^{U^{\times m}}_\mathcal{R}$ and  $\evl{\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m}}}{V}$, where sets $U_i$ and $V$ define Hilbert spaces, for every multiantilinear function $\vtf{c}\in V^{U^{\times m}}_\mathcal{R}$ there is one and only one linear tensor function  \gloref{elevC}$\in\evl{\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m}}}{V}$ such that 
\begin{equation*}
...
\nonumber
\end{equation}
where $B$ is a basis of $\ete{\mathcal{R}}{U^{\times m}}$ defined by orthonormal bases  $\con{B}_i=\{\vun{u}_{1}^{(i)},\cdots,\vun{u}_{n_i}^{(i)}\}$ of $U_i$. According to rule \eqref{eq:regraOrtonmFunc}, the function $\vtf{c}^\otimes$ that observes $\fua{\vtf{c}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\fua{{\vtf{c}}}{\vto{x}_1,\cdots,\vto{x}_m}$ really exists and is uniquely determined by $\vtf{c}$, as can be verified in  
\begin{eqnarray}
...
One of the special linear tensor functions mentioned in the previous paragraph, whose existence and uniqueness is assured by lifting multiantilinear functions, is called contraction. In simple words, this function relates a tensor of order $m\geq p$ and a tensor of order $m-p$, when we say that it ``contracts'' the order of its argument. Mathematically, in the context of the previous theorem, the tensor function \gloref{contrac}$\in\evl{\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m}}}{\ete{\mathcal{R}}{U^{\times m-p}}}$ is said to be a \textsb{contraction}\index{contraction} of order $p$ if it is lifted from the antilinear function in $\map{\tnr{c}_p}{U^{\times m}_\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m-p}}}$ in such a way that the $(m-p)$-th order tensor
\begin{equation}
...
\end{equation*}
where $\tnr{I}$ is the second order identity tensor of $\ete{\mathcal{R}}{U_r\times U_s}$. In other words, the lifting of this function $\vtf{c}_2$ results the $r,s$ trace function and thus
\begin{equation}
...
where $m=q+2p+n$, tensor $\tnr{A}:=\vto{a}_1^*\otimes\cdots\otimes\vto{a}_{q}^*\otimes\vto{x}_1^*\otimes\cdots\otimes\vto{x}_{p}^*$ belongs to tensor space $\ete{\mathcal{R}}{A^{\times q}\times W^{\times p}}$ and tensor $\tnr{B}:=\vto{y}_1^*\otimes\cdots\otimes\vto{y}_{p}^*\otimes\vto{b}_1^*\otimes\cdots\otimes\vto{b}_{n}^*$ belongs to $\ete{\mathcal{R}}{W^{\times p}\times B^{\times n}}$. In the case of 
\begin{align*}
...
\end{equation}
where $\tnr{C}\in\ete{\mathcal{R}}{W^{\times p}\times C^{\times n}\times Z^{\times r}}$ and $\tnr{D}\in\ete{\mathcal{R}}{Z^{\times r}\times D^{\times s}}$. 


When the order of a contractive product equals the order of its terms, resulting a tensor of order zero or a scalar, the order of the contractive product representation is not made explicit. In other words, for tensors $\tnr{W}_1,\tnr{W}_2\in\ete{\mathcal{R}}{W^{\times p}}$, we define $\tnr{W}_1\odot\tnr{W}_2=\tnr{W}_1\odot_p\tnr{W}_2$. In particular, the scalar values in equality \eqref{eq:ProdTens} are always real when only Euclidean spaces are considered and then, from \eqref{eq:identidade}, these real values can be arranged in such a way that 
\begin{equation*}
...
a function space  $\ete{\mathcal{R}}{V^{\times n}}_\mathcal{R}^{\ete{\mathcal{R}}{U^{\times m}}}$
