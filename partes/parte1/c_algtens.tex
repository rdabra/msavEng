...
\end{equation}
or $\fua{\tnr{G}}{\vto{u}^c,\vto{v}}=\vto{v}\cdot\fua{\tnr{\mathit{g}}}{\vto{u}}$. From definitions of Hilbert-adjoint and transpose functions described by \eqref{eq:HilbAdj}, it is interesting to note that if a representative operator $\tnr{\mathit{t}}\in\evl{\cam{F}}{U}{U}$ is Hermitian then
\begin{equation*}
\fua{\tnr{T}}{\vto{x}^c,\vto{y}}=\fua{\tnr{\mathit{t}}}{\vto{y}}\cdot\vto{x}=\overline{\vto{x}\cdot\fua{\tnr{\mathit{t}}}{\vto{y}}}=\overline{\fua{\tnr{T}}{\vto{y}^c,\vto{x}}}=\fua{\vto{t}_1^*}{(\vto{y}^c)^c}\fua{\vto{t}_2^*}{\vto{x}^c}=\fua{\tnr{T}}{\vto{y},\vto{x}^c}\,,
\end{equation*}
from which we can not conclude that $\tnr{T}$ is symmetric because a condition for symmetry in this case would be $\fua{\tnr{T}}{\vto{x}^c,\vto{y}}=\fua{\tnr{T}}{\vto{y}^c,\vto{x}}$; but, in the case of $\tnr{\mathit{t}}$ symmetric we can and this result leads to the following corollary.
...
{\footnotesize
\begin{proof}
From \eqref{eq:secTens2Lin}, if $\tnr{\mathit{t}}$ is a symmetric representative operator, we have
\begin{equation*}
\fua{\tnr{T}}{\vto{x},\vto{y}}=\fua{\tnr{\mathit{t}}}{\vto{y}}\cdot\vto{x}^c=\overline{\vto{x}^c\cdot\fua{\tnr{\mathit{t}}}{\vto{y}}}=\overline{\fua{\tnr{T}}{\vto{y}^c,\vto{x}^c}}=\fua{\vto{t}_1^*}{(\vto{y}^c)^c}\fua{\vto{t}_2^*}{(\vto{x}^c)^c}=\fua{\tnr{T}}{\vto{y},\vto{x}}.
\end{equation*}
\end{proof}
}
...
\begin{itemize}
\setlength\itemsep{.1em}
\item[i.] $\tnr{G}^{\dagger}:=\fua{\tnr{\Upsilon}^{-1}}{\tnr{\mathit{g}}^{\dagger}}$ and $\tnr{G}^\text{T}:=\fua{\tnr{\Upsilon}^{-1}}{\tnr{\mathit{g}}^\text{T}}$; 
