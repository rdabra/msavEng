...
In our study, we deal with three different types of contraction, expressed by three different rules for ${\vtf{c}}_p$. The first type is called trace of tensor, valid for tensor spaces of order greater than one and defined as follows. Still considering the above conditions, if $U^{\times m}=U_1\times\cdots\times U_{r}\times\cdots\times U_{s}\times\cdots\times U_m$ where $U_{r}=U_{s}$, the tensor $\fua{\vtf{c}_2^\otimes}{\tnr{X}}$ of order $m-2$ is a contraction called the $r,s$ trace of $\tnr{X}$, represented by \gloref{tracoRS}$\tnr{X}$, when 
\begin{equation*}
