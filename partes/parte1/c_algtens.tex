...
\begin{equation*}
\fua{\vtf{t}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\overline{\fua{\tnr{T}}{\vto{x}_1,\cdots,\vto{x}_m}}\,,\,\,\forall\,\vto{x}_i\in U_i.
\end{equation*}
\end{mteo}
\hspace{1pt}
{\footnotesize
...
\nonumber
\end{equation}
where $B$ is a basis of $\ete{\mathcal{R}}{U^{\times m}}$ defined by orthonormal bases  $\con{B}_i=\{\vun{u}_{1}^{(i)},\cdots,\vun{u}_{n_i}^{(i)}\}$ of $U_i$. According to rule \eqref{eq:regraOrtonmFunc}, the function $\vtf{t}^\otimes$ that observes $\fua{\vtf{t}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\fua{\tnr{T}}{\vto{x}_1,\cdots,\vto{x}_m}$ really exists and is uniquely determined by $\tnr{T}$, as can be verified in  
\begin{eqnarray}
...
i_m}^{B}}{\tnr{X}}\overline{\fua{\tnr{T}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}}\,,\nonumber
\end{eqnarray}
because the last equality results a rule for $\vtf{t}^\otimes$ since $\tnr{X}$ is arbitrary. Now, let's verify if equality $\fua{\vtf{t}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\overline{\fua{\tnr{T}}{\vto{x}_1,\cdots,\vto{x}_m}}$ is valid. Considering $\tnr{X}=\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*$, the properties of complex conjugates and rule \eqref{eq:regraCoordTens}, the previous development can proceed as follows:  
\begin{eqnarray}
\fua{\vtf{t}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}&=&\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
i_m}^{B}}{\tnr{X}}\overline{\fua{\tnr{T}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}}\nonumber\\
&=&
\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\tnr{X}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}\overline{\fua{\tnr{T}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}}\nonumber\\
&=&
\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\prod_{k=1}^m \overline{\fua{\vtf{f}_{i_k}^{\con{B}_k}}{\vto{x}_k}}\,\,\overline{\fua{\tnr{T}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}}\nonumber\\
&=&
\overline{\fua{\tnr{T}}{\sum_{i_1=1}^{n_1}\fua{\vtf{f}_{i_1}^{\con{B}_1}}{\vto{x}_1}
\vun{u}_{i_1}^{(1)},\cdots,\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_m}^{\con{B}_m}}{\vun{x}_m}\vun{u}_{i_m}^{(m)}}}\nonumber\\
&=& \overline{\fua{\tnr{T}}{\vto{x}_1,\cdots,\vto{x}_m}}\,.\nonumber
\end{eqnarray}
\end{proof}
}
