...
For the purposes of our study, a tensor function establishes a functional relationship between tensors of any order. Because of this definition such functions in Hilbert spaces are of the most general type: as presented in the previous section, tensors of order zero are scalars by definition and tensors of first order are covectors, which are biunivocaly related to vectors, according to the Riesz-Fréchet Representation Theorem. In the context of Hilbert spaces, this generality of tensor functions is particularly  important for us and that is why we shall be dealing mostly with these spaces hereafter, betaking all their particularities. Therefore, considering these spaces, it is always possible to relate a tensor and a linear tensor function, which is a fundamental feature to enable uniqueness and existence of the specific linear tensor functions to be presented in this section. The following theorem\footnote{Adapted from \aut{Backus}\cite{backus_1997_1} after generalizing a property from  \aut{Marcus}\cite{marcus_1973_1}, p.14.}, borrowed from the realm of Multilinear Algebra, describes such feature by establishing a one to one correspondence between a tensor and a linear tensor function. From this biunivocal relationship, the tensor is usually said to be ``lifted'' to a tensor function. Hereafter, in order to alleviate notational representation, we shall consider the scalar $\fua{\overline{\tnr{T}}}{\vto{x}_1,\cdots,\vto{x}_m}:=\overline{\fua{\tnr{T}}{\vto{x}_1,\cdots,\vto{x}_m}}$, where $\tnr{T}$ is an arbitrary tensor of order $m$ that uniquely determines $\overline{\tnr{T}}$ because $\vto{x}_i$ is arbitrary.     

\begin{mteo}{Lifting of Tensors}{lift}
Considering the tensor space $\ete{\mathcal{R}}{U^{\times m}}$ and the function space $\evl{\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m}}}{\mathcal{R}}$, where sets $U_i$ define Hilbert spaces, for every tensor $\tnr{T}\in\ete{\mathcal{R}}{U^{\times m}}$ there is one and only one linear tensor function $\vtf{t}^\otimes\in\evl{\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m}}}{\mathcal{R}}$ such that 
\begin{equation*}
\fua{\vtf{t}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\fua{\overline{\tnr{T}}}{\vto{x}_1,\cdots,\vto{x}_m}\,,\,\,\forall\,\vto{x}_i\in U_i.
\end{equation*}
\end{mteo}
\hspace{1pt}
{\footnotesize
...
\nonumber
\end{equation}
where $B$ is a basis of $\ete{\mathcal{R}}{U^{\times m}}$ defined by orthonormal bases  $\con{B}_i=\{\vun{u}_{1}^{(i)},\cdots,\vun{u}_{n_i}^{(i)}\}$ of $U_i$. According to rule \eqref{eq:regraOrtonmFunc}, the function $\vtf{t}^\otimes$ that observes $\fua{\vtf{t}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\fua{\overline{\tnr{T}}}{\vto{x}_1,\cdots,\vto{x}_m}$ really exists and is uniquely determined by $\tnr{T}$, as can be verified in  
\begin{eqnarray}
...
i_m}^{B}}{\tnr{X}}\fua{\overline{\tnr{T}}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}\,,\nonumber
\end{eqnarray}
because the last equality results a rule for $\vtf{t}^\otimes$ since $\tnr{X}$ is arbitrary. Now, let's verify if equality $\fua{\vtf{t}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}=\fua{\overline{\tnr{T}}}{\vto{x}_1,\cdots,\vto{x}_m}$ is valid. Considering $\tnr{X}=\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*$, the properties of complex conjugates and rule \eqref{eq:regraCoordTens}, the previous development can proceed as follows:  
\begin{eqnarray}
\fua{\vtf{t}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*}&=&\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
i_m}^{B}}{\tnr{X}}\fua{\overline{\tnr{T}}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}\nonumber\\
&=&
\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\tnr{X}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}\fua{\overline{\tnr{T}}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}\nonumber\\
&=&
\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\prod_{k=1}^m \overline{\fua{\vtf{f}_{i_k}^{\con{B}_k}}{\vto{x}_k}}\,\,\fua{\overline{\tnr{T}}}{\vun{u}_{i_1}^{(1)},\cdots,\vun{u}_{i_m}^{(m)}}\nonumber\\
&=&
\fua{\overline{\tnr{T}}}{\sum_{i_1=1}^{n_1}\fua{\vtf{f}_{i_1}^{\con{B}_1}}{\vto{x}_1}
\vun{u}_{i_1}^{(1)},\cdots,\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_m}^{\con{B}_m}}{\vun{x}_m}\vun{u}_{i_m}^{(m)}}\nonumber\\
&=& \fua{\overline{\tnr{T}}}{\vto{x}_1,\cdots,\vto{x}_m}\,.\nonumber
\end{eqnarray}
\end{proof}
}
