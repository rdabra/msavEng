...
{\footnotesize
\begin{proof}
Let's prove that the $r,s$ trace and the contractive product are indeed linear. From property ii on page \pageref{prop:tensorProd}, verification of equalities $\trt{r}{s} \alpha\tnr{ X}=\alpha\trt{r}{s}\tnr{X}$ and $(\alpha\tnr{A})\odot_p(\beta\tnr{B})=\alpha\beta\tnr{A}\odot_p\tnr{B}$ is straightforward. Thereafter, in order to shorten expression sizes, we shall consider that a contractive product always results a scalar, that is, the order of the contractive product equals the order of its terms. Firstly, let's verify the additive property of linear functions for a first order contractive product, that is, set $X^{\times 2}=W\times W$ and $\tnr{A},\tnr{B},\tnr{W}\in\ete{\cam{F}}{W}$. Since $\vto{x}_1^*+\vto{y}_1^*=(\vto{x}_1+\vto{y}_1)^*$, equalities
\begin{equation*}
...
& = [(\vto{x}_2^*\otimes\vto{x}_1^*) + (\vto{y}_2^*\otimes\vto{y}_1^*)]\odot_1\vto{w}_1^*\odot_1\vto{w}_2^*\\
& = \tnr{T}\otimes\tnr{Z}\odot_1\vto{w}_1^*\odot_1\vto{w}_2^*\\
& = \tnr{Z}\otimes\tnr{T}\odot_2\vto{w}_1^*\otimes\vto{w}_2^* \\
& = [(\vto{x}_1^*\otimes\vto{x}_2^*) + (\vto{y}_1^*\otimes\vto{y}_2^*)]\odot_2\vto{w}_1^*\otimes\vto{w}_2^*\\
& = (\tnr{A} + \tnr{B})\odot_2\tnr{W}\,,
\end{align*}
where $\tnr{T}\otimes\tnr{Z}:=(\vto{x}_2^*\otimes\vto{x}_1^*) + (\vto{y}_2^*\otimes\vto{y}_1^*)$. 
Applying the same strategy of this development, we can find that $\tnr{W}\odot_2\tnr{A} + \tnr{W}\odot_2\tnr{B}=\tnr{W}\odot_2(\tnr{A} + \tnr{B})$. Therefore, second  order contractive products are also linear. In the case of arbitrary tensors $\tnr{A},\tnr{B},\tnr{W}\in\ete{\cam{F}}{W^{\times p}}$, for any $p>2$, this procedure can also be applied, leading to linearity for a contractive product of any order.  
\end{proof}
}
...
(\alpha\tnr{A})\cdot(\beta\tnr{B})=((\alpha\vto{a}_1)\cdot(\beta\vto{b}_1))(\vto{a}_2\cdot\vto{b}_2)=\alpha\overline{\beta}(\vto{a}_1\cdot\vto{b}_1)(\vto{a}_2\cdot\vto{b}_2)=\alpha\overline{\beta}\tnr{A}\cdot\tnr{B}
\end{equation*}
assure the observation of linearity-antilinearity axioms.
\end{proof}
}
