...
{\footnotesize
\begin{proof}
Let's prove that the $r,s$ trace and the contractive product are indeed linear. According to property ii on page \pageref{prop:tensorProd}, verification of equalities $\trt{r}{s} \alpha\tnr{ X}=\alpha\trt{r}{s}\tnr{X}$ and $(\alpha\tnr{A})\odot_p(\beta\tnr{B})=\alpha\beta\tnr{A}\odot_p\tnr{B}$ is straightforward.   
\end{proof}
}
