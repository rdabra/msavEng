...
For the purposes of our study, the contraction that follows is more relevant than the $r,s$ trace because it generalizes the concept of inner product. In order to define it using the above conditions, let's consider that       
\begin{equation*}
...
\end{align*} 
we call tensor $\fua{{\vtf{c}}_{2p}^\otimes}{\tnr{A}\otimes\tnr{B}}$ the $p$-th order contractive product of $\tnr{A}$ and $\tnr{B}$, represented by $\tnr{A}\odot_p\tnr{B}$. Thereby, we can write in more simple terms that   
\begin{equation}
