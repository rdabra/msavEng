...
In simple terms, a tensor function establishes a functional relationship between tensors of any order. Because of this definition, in the context of Hilbert spaces, such functions are of the most general type: as presented in the previous section, tensors of order zero are scalars by definition and tensors of first order are covectors, which are biunivocaly related to vectors, according to the Riesz-Fréchet Representation. This generality of tensor functions relating tensors in Hilbert spaces is very important for our study and that is why we shall be dealing mostly with these spaces hereafter. In this context, it is always possible to relate a tensor and a linear tensor function, which is a fundamental feature to enable uniqueness and existence of the specific linear tensor functions to be presented in this section. The following classic theorem of Multilinear Algebra describes such relationship in a wider sense by establishing a one to one correspondence between a multilinear function and a linear tensor function.

\begin{mteo}{Lifting}{lift}
Given the function spaces $\evl{\mathcal{R}}{U^{\times m}}{V}$ and $\evl{\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m}}}{V}$, for every linear function $\vtf{g}\in$

 and a basis $B_i=\{\vto{u}_1^{(i)},\cdots,\vto{u}_{n_i}^{(i)}\}$ of each constituent vector space $(U_i)_\mathcal{R}$, $i=1,\cdots,m\,$, the set $B$ of all polyadic tensors $\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}$, $j_i=1,\cdots,n_i\,$, is a basis of $\ete{\mathcal{R}}{U^{\times m}}$. 
\end{mteo}
