...
\item[iv.] If $\tnr{\mathit{g}}$ is hermitian then  $\tnr{G}^\dagger=\tnr{G}$;
...
In the context of arbitrary second order tensor $\tnr{G}\in\ete{\cam{F}}{U\times V}$ and vector $\vto{u}\in U_\cam{F}$, we already know that a linear bijection $\tnr{\mathit{g}}$ represents $\tnr{G}$ in such a way that $\fua{\tnr{\mathit{g}}}{\vto{u}}=\tnr{G}^\dagger\hat{\odot}_1\vto{u}^*$. This equality can rewritten in matrix terms as following:
\begin{equation}
[\tnr{\mathit{g}}_{B_1}]^{B_2}[\vto{u}]^{B_1} = {[\tnr{G}]^{B}}^\dagger[\vto{u}]^{B_1}\,,
\end{equation}
where basis tensor $B$ is built from vector bases $B_1$ and $B_2$ of $U_\cam{F}$ and $V_\cam{F}$ respectively, according to theorem \ref{teo:geraEte}. Therefore, since $\vto{u}$ is arbitrary, matrix $[\tnr{\mathit{g}}_{B_1}]^{B_2}={[\tnr{G}]^{B}}^\dagger$ and then
