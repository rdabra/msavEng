...
\end{equation}
From equalities \eqref{eq:regraOrtonmFunc} and \eqref{eq:regraCoordTens}, given the orthonormal bases  $B_i=\{\vun{u}_{1}^{(i)},\cdots,\vun{u}_{n_i}^{(i)}\}$ and $C_i=\{\vun{v}_{1}^{(i)},\cdots,\vun{v}_{n_i}^{(i)}\}$ , we can write that the elements
\begin{equation}
