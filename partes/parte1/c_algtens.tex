...
In this context of array representations of tensors and linear tensor functions, it is also possible to develop change of coordinates and change of basis in a similar way already presented for vector and linear functions in section \ref{sec:Matrix}. For the sake of clarity and rigor we shall reproduce here both theorem \ref{teo:mudCoordVec} and its corollary \ref{cor:mudaBase} with minor changes in order to adapt them for the current case.    
...
$\{\fua{\tnr{\psi}}{\tnr{X}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{X}_p}}\}$ is also linearly independent. Therefore, $\tnr{\psi}$ must be an injection and consequently a bijection since it is an unary operator. Considering the idea in the development that leads to \eqref{eq:repMatMape} in order to verify item v, let's say that $\tnr{B}_l=(\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m})_l$ and $\tnr{C}_r=(\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{C_m})_r$ are arbitrary elements of bases $B$ and $C$ of $\ete{\cam{F}}{A}$, where ordinals $l$ and $r$ run from $1$ to $p$. Thereby, equalities
\begin{equation*}
