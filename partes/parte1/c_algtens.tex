...
Considering function spaces $\evl{\cam{F}}{U_\cam{F}}{V}$ and  $\evl{\cam{F}}{\ete{\cam{F}}{U}}{V}$, where sets $V$ and $U$ define Hilbert spaces $V_\cam{F}$ and $U_\cam{F}=X^{\times p}_\cam{F}\times \overline{X^{\times p}_\cam{F}}$ respectively, for every multilinear function $\vtf{c}\in \evl{\cam{F}}{U_\cam{F}}{V}$ there is a unique linear tensor function  \gloref{elevC}$\in\evl{\cam{F}}{\ete{\cam{F}}{U}}{V}$ such that 
\begin{equation*}
\fua{\vtf{c}^\otimes}{\vto{x}_1^*\otimes\cdots\otimes\vto{x}_p^*\otimes\vto{{x}_1^c}^*\otimes\cdots\otimes\vto{{x}_p^c}^*}=\fua{{\vtf{c}}}{\vto{x}_1,\cdots,\vto{x}_p,\vto{x}_1^c,\cdots,\vto{x}_p^c}\,,\,\,\forall\,\vto{x}_i\in X_i\,.
\end{equation*}
\end{mteo}
\hspace{1pt}
{\footnotesize
...
\begin{equation}
\tnr{X}=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_{m}}\fua{\vtf{f}_{i_1\cdots
i_m}^{B}}{\tnr{X}}\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_{p+1}}^{B_{p}}\otimes(\vtf{f}_{i_{p+1}}^c)^{B_{p+1}^c}\otimes\cdots\otimes(\vtf{f}_{i_{m}}^c)^{B_{m}^c}\,,
\nonumber
\end{equation}
where $m=2p$, $B$ and $B^c$ are basis of $\ete{\cam{F}}{X^{\times p}}$ and $\overline{\ete{\cam{F}}{X^{\times p}}}$ defined by orthonormal bases $\con{B}_i=\{\vun{u}_{1}^{(i)},\cdots,\vun{u}_{n_i}^{(i)}\}$ and $\con{B}_i^c=\{(\vun{u}_{1}^c)^{(i)},\cdots,(\vun{u}_{n_i}^c)^{(i)}\}$ of $U_i$ respectively. According to rule \eqref{eq:regraOrtonmFunc}, the function $\vtf{c}^\otimes$ that observes the equality of the theorem really exists and is uniquely determined by $\vtf{c}$, as can be verified in  
\begin{eqnarray}
