...
&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_p=1}^{n_p}\sum_{j_1=1}^{l_1}\cdots\sum_{j_m=1}^{l_m}\vtf{f}_{i_1\cdots i_p}^{C}(\fua{\tnr{\psi}}{\tnr{X}})\,\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{C_p}\nonumber\\
...
$\{\fua{\tnr{\psi}}{\tnr{X}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{X}_p}}\}$ is also linearly independent. Therefore, $\tnr{\psi}$ must be an injection and consequently a bijection since it is an unary operator. Considering the development that leads to \eqref{eq:repArrayTensF} in order to verify item v, let's say that $\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}$ and $\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{C_m}$ are arbitrary elements of bases $B$ and $C$ of $\ete{\cam{F}}{A}$. Thereby, equalities
\begin{multline*}
\fua{\tnr{\psi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}} = \sum_{i_1=1}^{n}\cdots\sum_{i_m=1}^{n}{\maf{\tnr{\psi}}{B}{C}}_{i_1\cdots i_m j_1\cdots j_m}
\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{C_m}=\\
\sum_{i_1=1}^{n}\cdots\sum_{i_m=1}^{n}\sum_{k_1=1}^{n}\cdots\sum_{k_m=1}^{n}{\maf{\tnr{\psi}}{B}{C}}_{i_1\cdots i_m j_1\cdots j_m}\maf{\tnr{i}}{C}{B}_{k_1\cdots k_m i_1\cdots i_m}\vtf{f}_{k_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m}^{B_m}=\\
\sum_{k_1=1}^{n}\cdots\sum_{k_m=1}^{n}{\maf{\tnr{\psi}}{B}{B}}_{k_1\cdots k_m j_1\cdots j_m}\vtf{f}_{k_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m}^{B_m}
\end{multline*}
...
\begin{multline*}
\fua{\tnr{P}}{\cdots,\fua{\tnr{\psi}}{\vtf{f}_{j_1^{(l)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m^{(l)}}^{B_m}},\cdots} = \cdots \sum_{k_1^{(l)}=1}^{n}\cdots\sum_{k_m^{(l)}=1}^{n}{\maf{\tnr{\psi}}{B}{B}}_{k_1^{(l)}\cdots k_m^{(l)} j_1^{(l)}\cdots j_m^{(l)}} \cdots\\
\fua{\tnr{P}}{\cdots,\vtf{f}_{k_1^{(l)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m^{(l)}}^{B_m},\cdots}\,
\end{multline*}
