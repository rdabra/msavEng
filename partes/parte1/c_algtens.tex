\chapter{Basic Tensor Algebra}\label{cp:tensor}
...
\end{itemize}
are valid properties for all $\alpha,\beta\in\cam{F}$ and  $\tnr{U},\tnr{U}_i,\tnr{V},\tnr{V}_i,\tnr{W}\in\ete{\cam{F}}{U^{\times m}}$. From the previous associativity property, it is valid to represent a polyadic tensor by a succession of tensor products, as we did for $\vtf{g}_1\otimes\cdots\otimes\vtf{g}_m$. 

%Thereby, if $\tnr{G}=\vtf{g}_1\otimes\cdots\otimes\vtf{g}_m$, it is useful to define a tensor $\tnr{G}^\text{T}\in\ete{\cam{F}}{U_{m}\times U_{m-1}\times\cdots\times U_1}$ where 
...
\begin{itemize}
\setlength\itemsep{.1em}
\item[i.] For an arbitrary basis $B\subset V_\cam{F}$, a set $\{\tnr{A}_B\}$ is a basis of alternating tensor space $\etng{m}{\cam{F}}{V^m}$ when
...
\item[v.] Given an arbitrary operator $\vtf{g}\in\evl{\cam{F}}{V}{V}$, an alternating tensor $\tnr{A}\in\etng{m}{\cam{F}}{V^m}$ and a basis $B=\{\vto{u}_1,\cdots,\vto{u}_m\}$ of $V_\cam{F}$, then
\begin{equation}\label{eq:alternDeteGen}
\fua{\tnr{A}}{\fua{\vtf{g}}{\vto{u}_1},\cdots,\fua{\vtf{g}}{\vto{u}_m}}=\dete{\vtf{g}}\fua{\tnr{A}}{\vto{u}_1,\cdots,\vto{u}_m}\,.
\end{equation}
\item[vi.] Given an arbitrary operator $\vtf{g}\in\grc{N}{\cam{F}}{V}$, then
\begin{equation}\label{eq:alternDete}
\fua{\tnr{A}}{\fua{\vtf{g}}{\vto{u}_1},\cdots,\fua{\vtf{g}}{\vto{u}_m}}=\pm\fua{\tnr{A}}{\vto{u}_1,\cdots,\vto{u}_m}\,.
\end{equation}
\end{itemize}
...
&=\dete{\vtf{g}}\fua{\tnr{A}}{\vto{u}_{1},\cdots,\vto{u}_{m}}\,.
\end{align*}
Item vi is an obvious consequence of item v.
\end{proof}
}

A tensor $\tnr{A}\in\etng{m}{\cam{F}}{U^m}$, where $U_\cam{F}$ is a Hilbert space, is called \textsb{unimodular}\index{tensor!unimodular} if scalar $|\fua{\tnr{A}}{\vun{u}_1,\cdots,\vun{u}_m}|=1$ for every orthonormal basis $B=\{\vun{u}_1,\cdots,\vun{u}_m\}$ of $U_\cam{F}$. If $\tnr{X}$ and $\tnr{Y}$ are arbitrary unimodular tensors, we have $|\fua{\tnr{X}}{\vun{u}_1,\cdots,\vun{u}_m}|=|\fua{\tnr{Y}}{\vun{u}_1,\cdots,\vun{u}_m}|$ and then $\tnr{X}=\pm\tnr{Y}$. Thereby, we can affirm that if tensor $\tnr{A}$ is unimodular, so is $-\tnr{A}$ and then conclude that a unique pair of unimodultar tensors $(\tnr{A},-\tnr{A})$ in $\etng{m}{\cam{F}}{U^m}$ is possible. Since this pair exists, if a tensor $\tnr{A}\in\etng{m}{\cam{F}}{U^m}$ is unimodular, the double $(U_\cam{F},\tnr{A})$ is said to be an \textsb{oriented vector space}\index{vector space!oriented}, where arbitrary basis $W=\{\vto{w}_1,\cdots,\vto{w}_m\}$ is called \textsb{positively}\index{basis!positively oriented} or \textsb{negatively oriented}\index{basis!negatively oriented} if scalar $\fua{\tnr{A}}{\vto{w}_1,\cdots,\vto{w}_m}$ is greater than zero or less than zero respectively. Particularly, if $W$ is orthonormal, it is positively or negatively oriented if $\fua{\tnr{A}}{\vto{w}_1,\cdots,\vto{w}_m}$ is $1$ or $-1$. In this context, for an arbitrary unitary operator $\vtf{h}\in\grc{U^+}{\cam{F}}{V}$, a basis $\{\fua{\vtf{h}}{\vun{u}_1},\cdots,\fua{\vtf{h}}{\vun{u}_m}\}$ is positively (negatively) oriented if $\{\vun{u}_1,\cdots,\vun{u}_m\}$ positively (negatively) oriented; and then we can say that \emph{basis orientation is immune to proper unitary operators}. Now, since basis $B$ is here orthonormal, considering previous property ii, \emph{the alternating tensor $\tnr{A}_B$ defined in \eqref{eq:basisAltern} results unimodular and then the oriented space $(U_\cam{F},\tnr{A}_B)$ specifies that $B$ is a positively oriented orthonormal basis}. Therefore, from \eqref{eq:igAntis}, given an arbitrary alternating tensor $\tnr{P}\in\etng{m}{\cam{F}}{U^m}$,
\begin{equation}\label{eq:coordAltern}
\vtf{f}_{i_1\cdots i_m}^{X}(\tnr{P})= \fua{\underbrace{\alpha\tnr{A}_B}_{\tnr{P}}}{\vun{x}_{i_1},\cdots,\vun{x}_{i_m}}=\alpha\epsilon_{i_1\cdots i_m}\fua{\tnr{A}_B}{\vun{x}_{1},\cdots,\vun{x}_{m}}=\pm\alpha\epsilon_{i_1\cdots i_m} 
\end{equation}
where basis $\{\vun{x}_{1},\cdots,\vun{x}_{m}\}$ of $(U_\cam{F},\tnr{A}_B)$ is positively or negatively oriented. In other words, the coordinates of an alternating tensor are identical for any positively (negatively) oriented basis. Therefore, we can conclude that $\vtf{f}_{i_1\cdots i_m}^{X}(\tnr{A}_B)=\pm\epsilon_{i_1\cdots i_m}$.
...
\end{equation}
is said to be the \textsb{trace}\index{tensor!trace of} of tensor $\tnr{X}$. Still in this context, following theorem \ref{teo:geraEte}, if a basis $B$ of tensor space $\ete{\cam{F}}{X^{\times q}\times X^{\times q}}$ of order $m=2q$ is constructed from bases $B_i=\{\vun{u}_1,\cdots,\vun{u}_{n_i}\}$ of $(X_i)_\cam{F}$, $i=1,\cdots,q\,$, we can calculate the value of the trace of $\tnr{X}$ from the last equality of the following development, where \eqref{eq:orthoConju} and \eqref{eq:tensCoord} are considered: 
\begin{align}\label{eq:calcTraco}
\trc{\tnr{X}}&=\sum_{j_1=1}^{n_1}\cdots\sum_{j_{m}=1}^{n_{m}} \vtf{f}_{j_1\cdots j_m}^{B}(\tnr{X})\,\trc{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_q}^{B_q}\otimes\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_q}^{B_q}}\nonumber\\
&=\sum_{j_1=1}^{n_1}\cdots\sum_{j_{m}=1}^{n_{m}} \vtf{f}_{j_1\cdots k_m}^{B}(\tnr{X})\,\prod_{k=1}^q(\vun{u}_{j_{k+q}}^{(k+q)})^c\cdot(\vun{u}_{j_k}^{(k)})_k\nonumber\\
&=\sum_{j_1=1}^{n_1}\cdots\sum_{j_{m}=1}^{n_{m}} \vtf{f}_{j_1\cdots k_m}^{B}(\tnr{X})\,\prod_{k=1}^q\delta_{j_{k+q}j_k}\nonumber\\
&=\sum_{j_1=1}^{n_1}\cdots\sum_{j_{q}=1}^{n_{q}} \vtf{f}_{j_1\cdots j_qj_1\cdots j_q}^{B}(\tnr{X})\,.
\end{align}

For the purposes of our study, the contraction that follows will take part in the majority of forthcoming definitions in this chapter. In order to define this important contraction using conditions above, let's consider that ${X^{\times m}=A^{\times q}\times W^{\times p}\times W^{\times p}\times B^{\times n}}$,
...
\fua{\tnr{\psi}^\dagger}{\tnr{V}}\cdot\vto{U}=\tnr{V}\cdot\fua{\tnr{\psi}}{\tnr{U}}\,,
\end{alignat}
where function $\tnr{\psi}^\dagger\in\evlc{\cam{F}}{\ete{\cam{F}}{V^{\times q}}}{\ete{\cam{F}}{U^{\times p}}}$ is called the Hilbert-adjoint of $\tnr{\psi}$, as defined on section \ref{sec:FuncLin}. Similarly to this section, if the fields involved are real, $\tnr{\psi}^\dagger$ is usually called the transpose of $\tnr{\psi}$, represented by $\tnr{\psi}^\text{T}$. All the properties of ordinary adjoint functions presented on page \pageref{eq:propAdj} are also valid and operator $\tnr{\varphi}\in\evlc{\cam{F}}{\ete{\cam{F}}{U^{\times p}}}{\ete{\cam{F}}{U^{\times p}}}$ is said to be Hermitian when  $\tnr{\varphi}=\tnr{\varphi}^\dagger$ and anti-Hermitian when $\tnr{\varphi}=-\tnr{\varphi}^\dagger$. Moreover, considering real fields, operator $\tnr{\varphi}$ is called symmetric if $\tnr{\varphi}=\tnr{\varphi}^\text{T}$ and antisymmetric when $\tnr{\varphi}=-\tnr{\varphi}^\text{T}$. As defined previously, if tensor function $\tnr{\psi}$ is an injection whose inverse equals the adjoint, it is called unitary; or orthogonal, in real fields. Similarly to unitary and orthogonal vector operators, the set of all unitary tensor operators on domain $\ete{\cam{F}}{U^{\times p}}$ defines a unitary group of tensors $\grt{N}{\cam{F}}{U^{\times p}}\subset\grt{G}{\cam{F}}{U^{\times p}}$ on this same domain and if real fields are considered the set of all orthogonal tensor operators on $\ete{\real}{U^{\times p}}$ defines an orthogonal group  $\grt{O}{}{U^{\times p}}:=\grt{N}{\real}{U^{\times p}}$. Again, like ordinary unitary operators, unitary tensor operators clearly preserve inner product. An operator $\tnr{\kappa}\in\evl{\cam{F}}{\ete{\cam{F}}{U^{\times p}}}{\ete{\cam{F}}{U^{\times p}}}$ is called isometric if  $\varrho\,[\fua{\tnr{\kappa}}{\tnr{U}_1},\fua{\tnr{\kappa}}{\tnr{U}_2}]=\fua{\varrho}{\tnr{U}_1,\tnr{U}_2}$, where tensors $\tnr{U}_1$ and $\tnr{U}_2$ are arbitrary. Thereby, the set of all isometric tensor operators defines an isometry group $\grt{I}{\cam{F}}{U^{\times p}}$. For Euclidean tensor spaces, an isometry group is also an unitary group whose elements preserve inner product; it is also true that adjoint, (anti-)Hermitian and unitary functions equal respectively to transpose, (anti)symmetric and orthogonal functions.
...
\end{equation*}
be a mapping where the tensor spaces involved are Hilbert spaces. If for every tensor $\tnr{T}$ of $\ete{\cam{F}}{W^{\times p}\times A^{\times q}}$, a $p$-cotensor \gloref{pCotensor}$:=\fua{\tnr{\Phi}}{\tnr{T}}$ is described by the rule $\fua{\frt{T}_p}{\tnr{X}}=\tnr{X}\odot_p\tnr{T}$ then $\tnr{\Phi}$ is an antilinear bijection.
\end{mteo}
\hspace{1pt}
{\footnotesize
\begin{proof}
...
\begin{equation*}
\fua{\lco\fua{\tnr{\Phi}}{\alpha\tnr{A}+\beta\tnr{B}}\rco}{\tnr{X}}=\tnr{X}\odot_p(\alpha\tnr{A}+\beta\tnr{B})=\overline{\alpha}\frt{A}_p(\tnr{X})+\overline{\beta}\tnr{B}^*_p(\tnr{X})=\fua{\lco\overline{\alpha}\fua{\tnr{\Phi}}{\tnr{A}}+\overline{\beta}\fua{\tnr{\Phi}}{\tnr{B}}\rco}{\tnr{X}}\,.
\end{equation*}
If $\tnr{\Phi}$ were not an injection, there would be different $p$-cotensors $\frt{A}_p$ and $\tnr{B}^*_p$ where $\tnr{X}\odot_p\tnr{A}=\tnr{X}\odot_p\tnr{B}$. From this supposition, equality $\tnr{X}\odot_p(\tnr{A}-\tnr{B})=\tnr{0}$ do not confirm $\frt{A}_p\neq\tnr{B}^*_p$ because it is valid for arbitrary $\tnr{X}$. In order to prove that $\tnr{\Phi}$ is a surjection, we need to obtain for an arbitrary tensor function ${\vtf{\psi}\in\evl{\cam{F}}{\ete{\cam{F}}{ W^{\times p}}}{\ete{\cam{F}}{A^{\times q}}}}$ a tensor ${\vto{A}\in \ete{\cam{F}}{W^{\times p}\times A^{\times q}}}$ such that $\fua{\tnr{\Phi}}{\tnr{A}}=\vtf{\psi}$. Using a strategy that we could call mathematical sagacity, let's consider a tensor
\begin{equation*}
...
Considering the conditions of the theorem, let's see what is the $p$-cotensor of identity tensor $\tnr{I}\in\ete{\cam{F}}{W^{\times p}\times W^{\times p}}$. If equality \eqref{eq:poliadIdentidade} and its definer conditions are considered, development
\begin{align}\label{eq:cotensIdent}
\fua{\frt{I}_p}{\tnr{X}}&=\tnr{X}\odot_p\tnr{I}\nonumber\\
...
&=\fua{\vtf{i}}{\tnr{X}}
\end{align}
shows that the identity operator $\tnr{i}\in\evl{\cam{F}}{\ete{\cam{F}}{ W^{\times p}}}{\ete{\cam{F}}{W^{\times p}}}$ is the $p$-cotensor of tensor $\tnr{I}$. Moreover, if scalar field $\cam{F}=\real$, then $\fua{\frt{I}_p}{\tnr{X}}=\tnr{X}\odot_p\tnr{I}=\tnr{I}\odot_p\tnr{X}=\fua{\vtf{i}}{\tnr{X}}$. Now, considering an arbitrary tensor $\tnr{Z}\in\ete{\cam{F}}{A^{\times q}\times Z^{\times r}}$, the associative property \eqref{eq:contractAsso} and previous conditions, we can write that
\begin{equation}\label{eq:compoCotens}
\fua{\frt{Z}_q\circ\frt{T}_p}{\tnr{X}}=(\tnr{X}\odot_p\tnr{T})\odot_q\tnr{Z}=\tnr{X}\odot_p(\tnr{T}\odot_q\tnr{Z})=\fua{(\vtf{T}\odot_q\vtf{Z})^\circledast_p}{\tnr{X}}\,,
\end{equation}
from which we conclude that the $p$-cotensor of $(\vtf{T}\odot_q\vtf{Z})\in\ete{\cam{F}}{W^{\times p}\times Z^{\times r}}$ is the composite function $\frt{Z}_q\circ\frt{T}_p$. In the particular case of $\tnr{Z}\in\ete{\cam{F}}{A^{\times q}\times W^{\times p}}$ and $\vtf{T}\odot_q\vtf{Z}=\tnr{I}$, we obtain from  previous equalities that $\frt{Z}_q\circ\frt{T}_p=\frt{I}_p=\vtf{i}$, that is, the $q$-cotensor of $\tnr{Z}$ is the inverse of the $p$-cotensor of $\tnr{T}$, or vice-versa.

\begin{mcoro}{Cotensors}{eqNorm}
Given $\map{\tnr{\Phi}}{\ete{\cam{F}}{W^{\times p}}}{\eted{\cam{F}}{W^{\times p}}}$, the rule of tensor functional \gloref{cotensor}$:=\frt{T}_p$, called the cotensor of $\tnr{T}$, results $\fua{\frt{T}}{\tnr{X}}=\tnr{X}\cdot\tnr{T}$ and when $\frt{T}$ is continuous, norm $\|\frt{T}\|_{\eted{\cam{F}}{W^{\times p}}}=\|\tnr{T}\|_{\ete{\cam{F}}{W^{\times p}}}$.
\end{mcoro}
{\footnotesize
\begin{proof}
The verification of previous theorem is not compromised by the conditions of this corollary. Now, let`s prove the equality of norms. Using definition \eqref{eq:normaFuncaoTens} on the present conditions, leaving space representation on norms implicit, we obtain equality $\|\frt{A}\|=\sup\{ |\tnr{X}\cdot\tnr{A}|/\|\tnr{X}\|\}$ for all non zero $\tnr{X}$. If $\tnr{A}$ is zero, it is evident that $\|\frt{A}\|=\|\tnr{A}\|$; otherwise, $\frt{A}$ is non zero and we conclude that $\|\frt{A}\|\geqslant |\, \tnr{X}\cdot\tnr{A}|/\|\tnr{X}\|$. Cauchy-Schwarz Inequality states that $|\, \tnr{X}\cdot\tnr{A}| \leqslant \|\tnr{X}\|\,\,\|\tnr{A}\|$. Subtracting these two previous inequalities, we arrive at $(\|\frt{A}\|-\|\tnr{A}\|)\|\tnr{X}\|\geqslant 0$, whose left side can be zero for arbitrary non zero $\frt{A}$, $\tnr{A}$ and $\tnr{X}$; therefore, $\|\frt{A}\|=\|\tnr{A}\|$.
\end{proof}
}
\noindent In the case of $p=1$ on the previous theorem, the tensors involved are covectors and then, for an arbitary $\vto{w}^*\in\ete{\cam{F}}{W}$ and according to equality \eqref{eq:prodIntTens}, the tensor functional $\vto{w}^\circledast:=(\vto{w}^*)^\circledast$ is described by
\begin{equation}\label{eq:funcCotens}
\vto{w}^\circledast(\vto{x}^*)=\vto{x}^*\cdot\vto{w}^*=\vto{w}\cdot\vto{x}=\vto{x}^*(\vto{w})\,.
\end{equation}

Given the conditions of this last corollary and a function space of tensor functionals $\cam{F}^V_\cam{F}$, where the set $V=\ete{\cam{F}}{U^{\times m}}$, it is convenient to define an antilinear tensor functional $\overline{\frt{T}}\in \cam{F}^V_\cam{F}$ described by the rule
\begin{equation}\label{eq:repTensFunc}
\fua{\overline{\frt{T}}}{\tnr{X}}=\tnr{T}\cdot\tnr{X}\,,
\end{equation}
whose existence and uniqueness are straightforward consequences of the previous theorem. Consdering the conditions of \eqref{eq:funcCotens}, where the tensors involved are covectors, scalar
\begin{equation}
\overline{\vto{w}^\circledast}(\vto{x}^*)=\vto{w}^*(\vto{x})\,.
\end{equation}
From rule \eqref{eq:repTensFunc}, it is obvious that every tensor $\tnr{T}$ has not only a correspondent cotensor $\frt{T}$ but also a correspondent \textsb{conjugate cotensor}\index{cotensor!conjugate} $\overline{\frt{T}}$. If the variable $\tnr{X}$ is expressed in its polyadic form $\vto{x}_1^*\otimes\cdots\otimes\vto{x}_m^*$,  the conjugate cotensor $\overline{\frt{T}}$ is convenient because it is linear on each variable $\vto{x}_i$, just like its correspondent tensor $\tnr{T}$. In this context, from the above definition and equalities \eqref{eq:propProdIndTens}, we can write that scalar
...
\end{equation}
where $\overline{\frt{T}}$ results a \textsb{representative tensor functional}\index{functional!representative tensor} of tensor $\tnr{T}$ since their values are the same when variables $\vto{x}_i\in (U_i)_\cam{F}$ are arranged as above. Note that while $\tnr{T}$ is a multilinear functional, its representative $\overline{\frt{T}}$ is univariate linear. Moreover, when the scalar fields involved are real, cotensor $\frt{T}$ is the representative tensor functional of $\tnr{T}$. Now, it is possible to study tensors through their conjugate cotensors, which substitute them numerically and have only one argument. If we consider the context of expression  \eqref{eq:identProdInt}, \emph{equalities
$\fua{\overline{\frt{I}}}{\tnr{X}} = \tnr{I}\cdot \tnr{X} = \overline{\mathrm{tr}}\,\tnr{X}$ show that the conjugate trace represents the identity tensor}. Moreover, if the identity tensor described by \eqref{eq:poliadIdentidade} is the argument of a representative function $\overline{(\tnr{A}\otimes\tnr{B})^\circledast}$ where $\tnr{A},\tnr{B}\in\ete{\cam{F}}{V^{\times p}}$, we have the following:
\begin{align}
\fua{\overline{(\tnr{A}\otimes\tnr{B})^\circledast}}{\tnr{I}} &= \sum_{{i_1}=1}^{n_1}\cdots\sum_{{i_p}=1}^{n_p}\fua{\overline{(\tnr{A}\otimes\tnr{B})^\circledast}}{\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}\otimes\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_p}^{B_p}}\nonumber\\
...
In the context of Hilbert spaces, we already know that corollary \ref{cor:covLin} establishes a biunivocal relationship between a linear vector function space and a linear covector function space, that is, if $\vtf{g}$ is a linear vector function, there is always a linear covector function $\vtf{g}_\Phi$, which is a linear tensor function whose domain is constituted by first order tensors (covectors). But the theorem above establishes a biunivocal relationship between a linear tensor function space and a tensor space, when we conclude that covector function $\vtf{g}_\Phi$ has its correspondent second order tensor $\tnr{G}$. Therefore, if tensor function $\vtf{g}_\Phi=\frt{G}_1$ has a correspondent linear function $\vtf{g}$ through corollary \ref{cor:covLin} and a correspondent second order tensor $\tnr{G}$ through theorem above, we can conclude that $\vtf{g}$ and $\tnr{G}$ are correspondents. Let's now rewrite this rationale in a more concise form.

\begin{mcoro}{Second Order Tensors and Linear Functions}{soTens}
Considering Hilbert spaces $U_\cam{F}$ and $V_\cam{F}$, a mapping $\map{\tnr{\Upsilon}}{\ete{\cam{F}}{U\times V}}{\evl{\cam{F}}{U}{V}}$ is a bijective transformation if its function rule is $\fua{\tnr{\Upsilon}}{\tnr{X}}=\Phi^{-1}\circ\frt{X}_1\circ\Phi$, where $\Phi$ is defined on theorem \ref{teo:repRiesz}.
\end{mcoro}
{\footnotesize
\begin{proof}
If function $\tnr{\Upsilon}$ were not an injection, there would be two distinct tensors $\tnr{T}$ and $\tnr{W}$ where $\Phi^{-1}\circ\frt{T}_1\circ\Phi(\vto{x})=\Phi^{-1}\circ\frt{W}_1\circ\Phi(\vto{x})$; but this equality leads to $\frt{T}_1(\vto{x}^*)=\frt{W}_1(\vto{x}^*)$, which shows that $\frt{T}_1=\frt{W}_1$. Therefore, since $p$-cotensors are biunivocally related to tensors, $\tnr{T}$ and $\tnr{W}$ cannot be distinct. Function $\tnr{\Upsilon}$ is also a surjection because, through corollary \ref{cor:covLin}, for every linear function $\vtf{g}\in\evl{\cam{F}}{U}{V}$ a covector function $\vtf{g}_\Phi$ can be defined and then, for every tensor function $\vtf{g}_\Phi$ there exists a unique second order tensor $\tnr{G}\in\ete{\cam{F}}{U\times V}$ where $1$-cotensor $\frt{G}_1=\Phi\circ\vtf{g}_\Phi$.
\end{proof}
}


\noindent From this corollary, the linear bijection 
\begin{equation}\label{eq:repFunc2}
\gloref{reprFun}:=\fua{\tnr{\Upsilon}}{\tnr{G}}
\end{equation}
is called the \textsb{representative function}\index{function!representative} of second order tensor $\tnr{G}\in\ete{\cam{F}}{U\times V}$. Considering  $\vto{g}_1^*\otimes\vto{g}_2^*$ the dyadic description of $\tnr{G}$, arbitrary vectors $\vto{u}\in U_\cam{F}$ and $\vto{v}\in V_\cam{F}$, we can write that
\begin{equation}\label{eq:Lin2secTens}
\fua{\tnr{\mathit{g}}}{\vto{u}} = \fua{\Phi^{-1}\circ\tnr{G}^*_1\circ\Phi}{\vto{u}} = \fua{\Phi^{-1}}{\vto{u}^*\odot_1\tnr{G}} =[\vto{u}^*\hat{\odot}_1\tnr{G}]^c=(\vto{g}_2^*\otimes\vto{g}_1^*)\hat{\odot}_1\vto{u}^* = (\vto{u}\cdot\vto{g}_1)\vto{g}_2
\end{equation}
because $\Phi^{-1}$ is antilinear, and from this result scalar
\begin{equation}\label{eq:secTens2Lin}
\fua{\tnr{G}}{\vto{u},\vto{v}} = \fua{\vto{g}_1^*}{\vto{u}}\fua{\vto{g}_2^*}{\vto{v}} = \vto{v}\cdot\overline{\fua{\vto{g}_1^*}{\vto{u}}}\vto{g}_2 = \vto{v}\cdot\fua{\vto{g}_1^*}{\vto{u}^c}\vto{g}_2= \vto{v}\cdot[(\vto{u}^c\cdot\vto{g}_1)\vto{g}_2]= \vto{v}\cdot\fua{\tnr{\mathit{g}}}{\vto{u}^c}\,.
\end{equation}

\begin{mcoro}{Symmetric Second Order Tensors and Operators}{soSymTens}
The set of symmetric operators, subset of $\evlc{\real}{U}{U}$, is biunivocally related to the space of symmetric second order tensors $\ets{\real}{U^2}$.
\end{mcoro}
\hspace{.1pt}
{\footnotesize
\begin{proof}
The corollary is straightforwardly obtained from \eqref{eq:secTens2Lin} for real fields and symmetric operators. 
\end{proof}
}

\noindent Still considering an arbitrary second order tensor $\tnr{G}\in\ete{\cam{F}}{U\times V}$ and its representative function $\tnr{\mathit{g}}$, we now sacrifice mathematical rigor in favor of mathematical practicality by specifying\footnote{This is a little notational permissiveness since there are no such things as the Hermitian-adjoint, the inverse or the power of a tensor.} three tensors in $\ete{\cam{F}}{V\times U}$:   
\begin{itemize}
\setlength\itemsep{.1em}
\item[i.] $\tnr{G}^{\dagger}:=\fua{\tnr{\Upsilon}^{-1}}{\tnr{\mathit{g}}^{\dagger}}$ ; 
\item[ii.] $\tnr{G}^{-1}:=\fua{\tnr{\Upsilon}^{-1}}{\tnr{\mathit{g}}^{-1}}$, if $\tnr{\mathit{g}}$ is invertible;
\item[iii.] $\tnr{G}^{n}:=\fua{\tnr{\Upsilon}^{-1}}{\underbrace{\tnr{\mathit{g}}\circ\cdots\circ\tnr{\mathit{g}}}_{n\text{ times}}}$, where $V=U$ and $n>0$.
\end{itemize}
They belong to $\ete{\cam{F}}{V\times U}$ as a straightforward consequence of the definitions of their representative functions together with equalities \eqref{eq:Lin2secTens}. From these definitions, we have the following properties:
\begin{itemize}
\setlength\itemsep{.1em}
\item[i.] $(\vto{a}^*\otimes\vto{b}^*)^\dagger = \vto{b}^*\otimes\vto{a}^*$ for arbitrary $\vto{a}\in U_\cam{F}$ and $\vto{b}\in V_\cam{F}$;
\item[ii.] For arbitrary $\vto{u}\in U_\cam{F}$, vector
\begin{equation}\label{eq:Lin2secTensTransp}
\fua{\tnr{\mathit{g}}}{\vto{u}} = [\vto{u}^*\hat{\odot}_1\tnr{G}]^c=\tnr{G}^\dagger\hat{\odot}_1\vto{u}^*\,;
\end{equation}
\item[iii.] If functions $\tnr{\mathit{g}}_1$ and $\tnr{\mathit{g}}_2$ represent $\tnr{G}_1\in\ete{\cam{F}}{U\times W}$ and $\tnr{G}_2\in\ete{\cam{F}}{W\times V}$, then $\tnr{\mathit{g}}_2\circ\tnr{\mathit{g}}_1$ represents $\tnr{G}_1\odot_1\tnr{G}_2$ or  
\begin{equation}\label{eq:compoFuncRepFunc}
\fua{\tnr{\mathit{g}}_2\circ\tnr{\mathit{g}}_1}{\vto{u}}
=(\tnr{G}_2^\dagger\odot_1\tnr{G}_1^\dagger)\hat{\odot}_1\vto{u}^*
= (\tnr{G}_1\odot_1\tnr{G}_2)^\dagger\hat{\odot}_1\vto{u}^*\,.
\end{equation}
Thereby, for $\tnr{G}\in\ete{\cam{F}}{U^2}$,
\begin{equation}\label{eq:tensPot}
\tnr{G}^n = \underbrace{\tnr{G}\odot_1\cdots\odot_1\tnr{G}}_{n\text{ times}}\,;
\end{equation}
\item[iv.] If a representative function $\tnr{\mathit{g}}$ of tensor $\tnr{G}$ is hermitian then
\begin{equation}\label{eq:hemritianTensor}
\tnr{G}^\dagger=\tnr{G}\,;
\end{equation}
\item[v.] Considering tensors $\tnr{G}^{-1}\odot_1\tnr{G}\in{\ete{\cam{F}}{V^2}}$ and $\tnr{G}\odot_1\tnr{G}^{-1}\in{\ete{\cam{F}}{U^2}}$, 
\begin{alignat}{3}\label{eq:identTensor}
\tnr{G}^{-1}\odot_1\tnr{G}=\tnr{I} &\quad \text{and}\quad& \tnr{G}\odot_1\tnr{G}^{-1}=\tnr{I}\,.
\end{alignat}
\end{itemize}

{\footnotesize
\begin{proof}
From \eqref{eq:funcaoTransposta} and \eqref{eq:Lin2secTens}, if $\vtf{\mathit{f}}$ is the representative function of $\vto{a}^*\otimes\vto{b}^*$, $\vto{u}\in U_\cam{F}$ and $\vto{v}\in V_\cam{F}$, the following procedure proves the first item. 
\begin{align*}
\fua{\vtf{\mathit{f}}^\dagger}{\vto{u}}\cdot\vto{v}&=\vto{u}\cdot\fua{\vtf{\mathit{f}}}{\vto{v}}\\
[\vto{u}^*\hat{\odot}_1(\vto{a}^*\otimes\vto{b}^*)^\dagger]^c\cdot\vto{v}&=\vto{u}\cdot[\vto{v}^*\hat{\odot}_1(\vto{a}^*\otimes\vto{b}^*)]^c\\
\vto{u}\cdot[(\vto{a}^*\otimes\vto{b}^*)^\dagger\hat{\odot}_1\vto{v}^*]&=\vto{u}\cdot[(\vto{b}^*\otimes\vto{a}^*)\hat{\odot}_1\vto{v}^*]\,.
\end{align*}
Proof of item ii is trivial. Now, let arbitrary functions $\tnr{\mathit{g}}_1$ and $\tnr{\mathit{g}}_2$ represent $\tnr{G}_1\in\ete{\cam{F}}{U\times W}$ and $\tnr{G}_2\in\ete{\cam{F}}{W\times V}$. Then, from item ii, development
\begin{equation*}
\fua{\tnr{\mathit{g}}_2\circ\tnr{\mathit{g}}_1}{\vto{u}}=\tnr{\mathit{g}}_2(\tnr{G}_1^\dagger\hat{\odot}_1\vto{u}^*)
=(\tnr{G}_2^\dagger\odot_1\tnr{G}_1^\dagger)\hat{\odot}_1\vto{u}^*
\end{equation*}
proves the first equality of \eqref{eq:compoFuncRepFunc}. In order to prove the second equality, we will consider functions $\tnr{\mathit{h}}_1=\tnr{\mathit{g}}_1^\dagger$ and $\tnr{\mathit{h}}_2=\tnr{\mathit{g}}_2^\dagger$ representing tensors $\tnr{H}_1=\tnr{G}_1^\dagger$ and $\tnr{H}_2=\tnr{G}_2^\dagger$. It is true to say that $\tnr{\mathit{h}}_2\circ\tnr{\mathit{h}}_1=(\tnr{\mathit{h}}_1^\dagger\circ\tnr{\mathit{h}}_2^\dagger)^\dagger$ represents $(\tnr{H}_2^\dagger\odot_1\tnr{H}_1^\dagger)^\dagger$ and then to conclude that, from the definition of $\tnr{G}^\dagger$, function $\tnr{\mathit{g}}=\tnr{\mathit{h}}_1^\dagger\circ\tnr{\mathit{h}}_2^\dagger$ represents $\tnr{G}=\tnr{H}_2^\dagger\odot_1\tnr{H}_1^\dagger$, which proves the second equality of \eqref{eq:compoFuncRepFunc}. Equality \eqref{eq:tensPot} is a straightforward consequence of \eqref{eq:compoFuncRepFunc}. Verification of item iv is trivial from \eqref{eq:Lin2secTens}. Considering \eqref{eq:cotensIdent}, if $\vtf{\mathit{g}}$ is the representative function of $\tnr{G}$, development 
\begin{align*}
\tnr{i}(\vto{v})&=\tnr{\mathit{g}}\circ\tnr{\mathit{g}^{-1}}(\vto{v})\\
[\vto{v}^*\hat{\odot}_1\tnr{I}]^c&=\tnr{\mathit{g}}([\vto{v}^*\hat{\odot}_1\tnr{G}^{-1}]^c)\\
&=[(\vto{v}^*\odot_1\tnr{G}^{-1})\hat{\odot}_1\tnr{G}]^c\\
&=[\vto{v}^*\hat{\odot}_1(\tnr{G}^{-1}\odot_1\tnr{G})]^c
\end{align*}
proves the first identity tensor of item v. Verification for $\tnr{G}\odot_1\tnr{G}^{-1}$ is similar. 
\end{proof}
}

Now, from equalities \eqref{eq:secTens2Lin}, we extend the concept of positivity that can be applied to the representative operator $\tnr{\mathit{t}}\in\evl{\cam{F}}{U}{U}$, described by definition \eqref{eq:linPosDef}, to tensor $\tnr{T}\in\ete{\cam{F}}{U^2}$. Thereby, a second order tensor $\tnr{T}$ is said to be \textsb{nonnegative}\index{tensor!nonnegative} if
\begin{equation}\label{eq:nonnegTensor}
\Re(\fua{\tnr{\mathit{t}}}{\vto{y}}\cdot\vto{y})=\Re(\vto{y}\cdot\fua{\tnr{\mathit{t}}}{\vto{y}})=\Re(\fua{\tnr{T}}{\vto{y},\vto{y}})\geqslant 0\,,\,\forall\,\vto{y}\in U_\cam{F}\,,
\end{equation}
and \textsb{positive-definite}\index{tensor!positive-definite} if the real value on the left side is never zero. Moreover, we also extend to a second order tensor $\tnr{T}\in\ete{\cam{F}}{U^2}$ the concepts of eigenvalues and eigenvectors of their representative operator $\tnr{\mathit{t}}\in\evl{\cam{F}}{U}{U}$, that is, the eigenvalues and eigenvectors of $\tnr{\mathit{t}}$ are also of $\tnr{T}$, by definition.
...
\end{alignat}
Now, we shall verify if it is possible to establish a relation between a Hilbert-adjoint tensor function and a conjugate transpose array. If $\tnr{\psi}\in\evl{\cam{F}}{\ete{\cam{F}}{U^{\times m}}}{\ete{\cam{F}}{V^{\times m}}}$ is Hilbert-adjoint function, with sets $B$ and $D$ bases of $\ete{\cam{F}}{U^{\times m}}$ and $\ete{\cam{F}}{V^{\times m}}$ respectively, then, from the definition of Hilbert-adjoint tensor function,
...
\end{align}
where equalities \eqref{eq:regraCoordTens} and \eqref{eq:propProdIndTens} were considered. 

In the context of arbitrary second order tensor $\tnr{G}\in\ete{\cam{F}}{U\times V}$ and vector $\vto{u}\in U_\cam{F}$, we already know from \eqref{eq:Lin2secTensTransp} that a linear bijection $\tnr{\mathit{g}}$ represents $\tnr{G}$ in such a way that $\fua{\tnr{\mathit{g}}}{\vto{u}}=\tnr{G}^\dagger\hat{\odot}_1\vto{u}^*$. This equality can be rewritten in matrix terms as following: 
\begin{equation*}
[\tnr{\mathit{g}}_{B_1}]^{B_2}[\vto{u}]^{B_1} = {[\tnr{G}]^{B}}^\dagger[\vto{u}]^{B_1}\,,
\end{equation*}
where basis tensor $B$ is built from vector bases $B_1$ and $B_2$ of $U_\cam{F}$ and $V_\cam{F}$, according to theorem \ref{teo:geraEte}. Therefore, since $\vto{u}$ is arbitrary, matrix 
\begin{equation}\label{eq:matTensorRepre}
[\tnr{G}]^{B}={[\tnr{\mathit{g}}_{B_1}]^{B_2}}^\dagger=[\tnr{\mathit{g}}_{B_2}^\dagger]^{B_1}.
\end{equation}
From these equalities, if $V_\cam{F}=U_\cam{F}$, that is, tensor $\tnr{G}\in\ete{\cam{F}}{U^2}$, it is possible to obtain that 
\begin{equation}\label{eq:trTentrMat}
\trc{\tnr{G}}=\trc{[\tnr{G}]^{B}}=\trc{\tnr{\mathit{g}}}.
\end{equation}
Still in this context, from equality \eqref{eq:matOpeHermit}, if $\tnr{\mathit{g}}$ is hermitian,  
\begin{equation}\label{eq:hermitTensor}
[\tnr{G}]^{B}=[\tnr{\mathit{g}}_{B_2}]^{B_1}=[\tnr{\mathit{g}}_{B_1}]^{B_2}={[\tnr{G}]^{B}}^\dagger.
\end{equation}

{\footnotesize
\begin{proof}
Let's prove equalities \eqref{eq:trTentrMat}. Considering basis $B_1=B_2=\{\vun{u}_1,\cdots,\vun{u}_n\}$, from \eqref{eq:calcTraco} we can write that equalities $\trc{\tnr{G}}=\sum_{j=1}^n \vtf{f}_{jj}^{B}(\tnr{G})=\trc{[\tnr{G}]^B}$. But since $\tnr{\mathit{g}}$ is a linear operator on $U_\cam{F}$, from \eqref{eq:traceOpLin} we have $\trc{[\tnr{G}]^B}=\trc{\tnr{\mathit{g}}}$.
\end{proof}
}
...
Before we develop mathematically this notion of tensor invariance, there is a preliminary definition that needs to be addressed. The feature of proper orthogonality -- to which anti-isotropic tensors, defined ahead, are invariant -- is also valid for tensor operators. As presented in section \ref{sec:Matrix}, an orthogonal operator is called proper if its determinant is $1$, but we still don't know what a determinant of a tensor operator is. In order to present this unusual definition as concisely as possible, we shall sacrifice self-containedness and tacitly accept the approach of \aut{Spivak}\cite{spivak_1965_1}\footnote{See also \aut{Bishop \& Goldberg}\cite{bishop_1980_1}, pp.97-100, and \aut{Backus}\cite{backus_1997_1}, pp.17-21.}, adapting to our case his theorem 4-6. Firstly, let's consider $\tnr{\mathit{P}}\in\etng{p}{\cam{F}}{A^p}$ an arbitrary non-zero alternating tensor where set $A=U^{\times m}$, that is, the arguments of $\tnr{\mathit{P}}$ are $p$ tensors of order $m$. Let tensor function $\tnr{\psi}\in\evl{\cam{F}}{\eteg{n}{\cam{F}}{U^{\times m}}}{\eteg{n}{\cam{F}}{U^{\times m}}}$ and alternating tensor   $\tnr{\mathit{P}}^{\tnr{\psi}}\in\etng{p}{\cam{F}}{A^p}$ such that scalar
\begin{equation}\label{eq:defDetPsi}
\fua{\tnr{\mathit{P}}^{\tnr{\psi}}}{\tnr{X}_1,\cdots,\tnr{X}_p}:=\fua{\tnr{\mathit{P}}}{\fua{\tnr{\psi}}{\tnr{X}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{X}_p}}}\,,\,\forall\,\tnr{X}_i\in\eteg{n}{\cam{F}}{A}\,.
\end{equation}
We already know the interesting feature that any alternating tensor space is one-di\-men\-sional and thereby the set $\{\tnr{\mathit{P}}\}$ can be a basis of $\etng{p}{\cam{F}}{A^p}$, when it is valid to write that $\tnr{\mathit{P}}^{\tnr{\psi}}=\alpha\tnr{\mathit{P}}$, where $\alpha\in\cam{F}$ is the coordinate of $\tnr{\mathit{P}}^{\tnr{\psi}}$ on $\{\tnr{\mathit{P}}\}$. If a non-zero arbitrary tensor $\tnr{K}\in\etng{p}{\cam{F}}{A^p}$ is considered, let $\beta\in\cam{F}$ be its coordinate on $\{\tnr{\mathit{P}}\}$. Since the previous definition \eqref{eq:defDetPsi} is valid for an arbitrary tensor of $\etng{p}{\cam{F}}{A^p}$, the following development
\begin{align*}
\fua{\tnr{K}^{\tnr{\psi}}}{\tnr{X}_1,\cdots,\tnr{X}_p}&=\fua{\beta\tnr{\mathit{P}}}{\fua{\tnr{\psi}}{\tnr{X}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{X}_p}}}\\
&=\beta\fua{\tnr{\mathit{P}}^{\tnr{\psi}}}{\tnr{X}_1,\cdots,\tnr{X}_p}\\
&=\beta\fua{\alpha\tnr{\mathit{P}}}{\tnr{X}_1,\cdots,\tnr{X}_p}\\
...
\fua{\tnr{\mathit{P}}}{\fua{\tnr{\psi}}{\tnr{X}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{X}_p}}}=\hdet{\tnr{\psi}}{\fua{\tnr{\mathit{P}}}{\tnr{X}_1,\cdots,\tnr{X}_p}}\,,
\end{equation}
is valid for all $\tnr{\mathit{P}}\in\{\etng{p}{\cam{F}}{A^p}\setminus \tnr{0}\}$ and all $\tnr{X}_i\in\eteg{n}{\cam{F}}{U^{\times m}}$. From this equality, given a basis $B=\{\tnr{B}_1,\cdots,\tnr{B}_p\}$ of tensor space $\eteg{n}{\cam{F}}{U^{\times m}}$ where $p=n^m$, if operator $\tnr{\psi}$ is proper unitary (see property vii below), then
\begin{equation*}
\fua{\tnr{\mathit{P}}}{\fua{\tnr{\psi}}{\tnr{B}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{B}_p}}}={\fua{\tnr{\mathit{P}}}{\tnr{B}_1,\cdots,\tnr{B}_p}}\,,
\end{equation*}
when we say that $\{\fua{\tnr{\psi}}{\tnr{B}_1},\cdots,\fua{\tnr{\psi}}{\tnr{B}_p}\}$, which is also a basis according to theorem \ref{teo:basesUnit}, has the same orientation of $B$. Now, according to theorem \ref{teo:repRieszTensor}, in the particular case where operator $\tnr{\psi}\in\evl{\cam{F}}{\eteg{p}{\cam{F}}{U}}{\eteg{p}{\cam{F}}{U}}$ is a $1$-cotensor of a second order tensor $\tnr{T}\in \eteg{p}{\cam{F}}{U^2}$, that is, $\tnr{\psi}=\tnr{T}_1^*$, it is possible to state that $\hdet{\tnr{T}}:=\hdet{\tnr{T}_1^*}$ and then
\begin{equation}\label{eq:detTensorSeg}
\fua{\tnr{\mathit{P}}}{\fua{\tnr{T}_1^*}{\vto{x}_1^*},\cdots,{\fua{\tnr{T}_1^*}{\vto{x}_{p}^*}}}=\hdet{\tnr{T}}{\fua{\tnr{\mathit{P}}}{\vto{x}_1^*,\cdots,\vto{x}_{p}^*}}
\end{equation}
or
\begin{equation}\label{eq:detTensorSeg2}
\fua{\tnr{P}}{\fua{\tnr{\mathit{t}}}{\vto{x}_1},\cdots,{\fua{\tnr{\mathit{t}}}{\vto{x}_{p}}}}=\hdet{\tnr{T}}{\fua{\tnr{{P}}}{\vto{x}_1,\cdots,\vto{x}_{p}}}\,,
\end{equation}
from corollary \ref{cor:soTens}, where tensor $\tnr{P}\in\etng{p}{\cam{F}}{U^{p}}$ and operator $\tnr{\mathit{t}}\in\evl{\cam{F}}{U}{U}$ is the representative function of $\tnr{T}$. 

Now, considering arbitrary functions $\tnr{\psi},\tnr{\varphi}\in\evl{\cam{F}}{\eteg{n}{\cam{F}}{U^{\times m}}}{\eteg{n}{\cam{F}}{U^{\times m}}}$ and tensors $\tnr{T},\tnr{T}_1, \tnr{T}_2\in \eteg{p}{\cam{F}}{U^2}$, the following properties of determinant are valid:
\begin{itemize}
\setlength\itemsep{.1em}
\item[i.] $\hdet{\tnr{i}} =  1$;
...
\item[v.] If $B$ and $C$ are bases of $\eteg{n}{\cam{F}}{U^{\times m}}$, then 
\begin{equation}
\hdet{\tnr{\psi}}=\hdet{\maf{\tnr{\psi}}{B}{C}}\,;
\end{equation}
\item[vi.] If $\tnr{\mathit{t}}$ is the representative function of $\tnr{T}$, then 
\begin{equation}\label{eq:detTensRep}
\hdet{\tnr{T}}=\dete{\tnr{\mathit{t}}}=\dete{[\tnr{\mathit{t}}]}\,;
\end{equation}
\item[vii.] $\hdet{\tnr{\psi}^\dagger}=\overline{\hdet{\tnr{\psi}}}$;
\item[viii.] If $\tnr{\psi}$ is unitary, then $\hdet{\tnr{\psi}}=\pm 1$;
\item[ix.] If $\tnr{T}=\tnr{T}_1\odot_1\tnr{T}_2$, then $\hdet{\tnr{T}}=\hdet{\tnr{T}_1}\hdet{\tnr{T}_2}$.
\end{itemize}
...
\begin{align*}
\hdet{\tnr{\psi}\circ\tnr{\varphi}}\fua{\tnr{\mathit{P}}}{\tnr{X}_1,\cdots,\tnr{X}_p}&=\fua{\tnr{\mathit{P}}}{\tnr{\psi}[\fua{\tnr{\varphi}}{\tnr{X}_1}],\cdots,\tnr{\psi}[\fua{\tnr{\varphi}}{\tnr{X}_p}]}\\
&=\hdet{\tnr{\psi}}\fua{\tnr{\mathit{P}}}{\fua{\tnr{\varphi}}{\tnr{X}_1},\cdots,\fua{\tnr{\varphi}}{\tnr{X}_p}}\\
&=\hdet{\tnr{\psi}}\hdet{\tnr{\varphi}}\fua{\tnr{\mathit{P}}}{\tnr{X}_1,\cdots,\tnr{X}_p}\,.
\end{align*}
Equalities $1=\hdet{\tnr{\psi}^{-1}\circ\tnr{\psi}}=\hdet{\tnr{\psi}^{-1}}\,\hdet{\tnr{\psi}}$ prove the third item. For the item iv, let's consider  $\hdet{\tnr{\psi}}\neq 0$ and as equality $\eqref{eq:detFunDef}$ is valid for any $\tnr{X}_i\in\eteg{n}{\cam{F}}{A}$, let's choose a linearly independent set $\{\tnr{X}_1,\cdots,\tnr{X}_p\}$, which causes $\fua{\tnr{\mathit{P}}}{\tnr{X}_1,\cdots,\tnr{X}_p}\neq 0$ since $\tnr{\mathit{P}}$ is antisymmetric. These conditions imply inequality $\fua{\tnr{\mathit{P}}}{\fua{\tnr{\psi}}{\tnr{X}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{X}_p}}}\neq 0$, from which we conclude that
...
\begin{align*}
\fua{\tnr{\mathit{P}}}{\fua{\tnr{\psi}}{\tnr{B}_1},\cdots,\fua{\tnr{\psi}}{\tnr{B}_p}}&=\sum_{i_1=1}^{p}\cdots\sum_{i_p=1}^{p}{\maf{\tnr{\psi}}{B}{B}}_{i_11}\cdots{\maf{\tnr{\psi}}{B}{B}}_{i_pp}\fua{\tnr{\mathit{P}}}{{\tnr{B}_{i_1}},\cdots,{\tnr{B}_{i_p}}}\\
&=\sum_{i_1=1}^{p}\cdots\sum_{i_p=1}^{p}{\maf{\tnr{\psi}}{B}{B}}_{i_11}\cdots{\maf{\tnr{\psi}}{B}{B}}_{i_pp}\epsilon_{i_1\cdots i_p}\fua{\tnr{\mathit{P}}}{{\tnr{B}_{1}},\cdots,{\tnr{B}_{p}}}\\
&=\dete{\maf{\tnr{\psi}}{B}{B}}\fua{\tnr{\mathit{P}}}{{\tnr{B}_{1}},\cdots,{\tnr{B}_{p}}}\,,
\end{align*}
according to \eqref{eq:igAntis} and \eqref{eq:DeterminanteSimplificado}. Thus, from \eqref{eq:detFunDef}, we have $\hdet{\tnr{\psi}}=\dete{\maf{\tnr{\psi}}{B}{B}}=\hdet{\maf{\tnr{\psi}}{B}{B}}$. Now, considering item v and comparing \eqref{eq:alternDeteGen} with \eqref{eq:detTensorSeg2}, proof of item vi is trivial. For the item vii verification, let's consider the previous development and equality \eqref{eq:propHilbAjd}, when we have
\begin{equation*}
...
and consequently
\begin{align*}
\fua{\tnr{\mathit{P}}}{\fua{\tnr{\psi}^\dagger}{\tnr{B}_1},\cdots,\fua{\tnr{\psi}^\dagger}{\tnr{B}_p}}
&=\sum_{i_1=1}^{p}\cdots\sum_{i_p=1}^{p}\overline{{\maf{\tnr{\psi}}{B}{B}}_{1i_1}\cdots{\maf{\tnr{\psi}}{B}{B}}_{pi_p}}\epsilon_{i_1\cdots i_p}\fua{\tnr{\mathit{P}}}{{\tnr{B}_{1}},\cdots,{\tnr{B}_{p}}}\\
&=\overline{\dete{\maf{\tnr{\psi}}{B}{B}}}\fua{\tnr{\mathit{P}}}{{\tnr{B}_{1}},\cdots,{\tnr{B}_{p}}}\\
&=\overline{\hdet{\tnr{\psi}}}\fua{\tnr{\mathit{P}}}{{\tnr{B}_{1}},\cdots,{\tnr{B}_{p}}}\,,
\end{align*}
since an equality similar to \eqref{eq:DeterminanteSimplificado} can be obtained by considering $i_k=k$ on \eqref{eq:Determinante}. Now, equalities $1=\hdet{\tnr{i}}=\hdet{\tnr{\psi}\circ\tnr{\psi}^\dagger} =  \hdet{\tnr{\psi}}\,\hdet{\tnr{\psi}^\dagger}=|\hdet{\tnr{\psi}}|^2$ prove item viii. Property ix is a straightforward consequence of property ii and equality \eqref{eq:compoFuncRepFunc}.
\end{proof}
}
