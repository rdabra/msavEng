...
At this point, we sacrifice once again self-containednes and humbly stand upon the shoulders of a giant. By proving his Theorem 2.9.A, \aut{Weyl}\cite{weil_1939_1}\footnote{See also \aut{Backus}\cite{backus_1997_1}, p. 80.} shows that identity tensor $\tnr{I}\in\eto{\cam{F}}{A^p}$, where $A=U^{m}$ and $p$ is even, its transpositions and their linear combinations constitute the isotropy group, that is,
\begin{equation}
\grt{U}{\cam{F}}{A^p}= \spn(\{\tnr{I}\})\,\cup\,\spn(\{\tnr{I}_\pi\})\,. 
\end{equation}
For the case of anti isotropic tensors, he arrives at
\begin{equation}
\tnr{\mathcal{U}}^+_{\cam{F}}(A^p\times A^q)= \spn(\{\tnr{I}\otimes\tnr{A}\})\,\cup\,\spn(\{(\tnr{I}\otimes\tnr{A})_\pi\})\,, 
\end{equation}
where $\tnr{A}\in\etng{m}{\cam{F}}{A^q}$. Compiling the results obtained by \aut{Backus}\cite{backus_1997_1}, pp. 80-88, for tensor spaces of order from 0 to 4, we build table XXX.
...
\titfigura{Tensores Isotrópicos e Anti-Isotrópicos em $\ete{\con{V}^p}{\con{F}}$. Fonte: \aut{Backus}\cite{backus_1997_1}.}\label{tb:Isotropicos}
\end{table}
