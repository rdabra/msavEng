...
\end{equation}
where linear functional $(\vtf{t}_j)_i\in(U_j)_\mathcal{R}^*$ is the $j$-th polyad of the $i$-th polyadic tensor member of the additive decomposition of $\tnr{T}$.
...
One of the special linear tensor functions mentioned in the previous paragraph, whose existence and uniqueness is assured by lifting multilinear functions, is called contraction. In simple words, this function relates a tensor of order $m$ and a tensor of order $m-p$, when we say that it ``contracts'' the order of its argument. Mathematically, the tensor function $\vtf{c}_p^\otimes\in\evl{\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m}}}{\ete{\mathcal{R}}{U^{\times m-p}}}$ is said to be a contraction of order $p$ if it is lifted from $\tnr{c}_p\in\evl{\mathcal{R}}{U^{\times m}}{\ete{\mathcal{R}}{U^{\times m-p}}}$ in such a way that the $(m-p)$-th order tensor
\begin{equation}
\fua{\vtf{c}_p^\otimes}{\tnr{X}}=\sum_{i=1}^m\fua{\overline{\vtf{c}}_p}{(\vtf{x}_1)_i,\cdots,(\vtf{x}_m)_i}\,,
\end{equation}
where, according to decomposition \eqref{eq:decompAditTens}, 
\begin{equation}
\tnr{X}=\sum_{i=1}^m (\vtf{x}_1^*)_i\otimes\cdots\otimes(\vtf{x}_m^*)_i
\end{equation}
