...
\end{equation}
Moreover, since the sum of two (anti)symmetric tensors is an (anti)symmetric tensor and multiplication by scalar also preserves (anti)symmetry, the set of all (anti)symmetric tensors defines a tensor subspace of $\ete{\cam{F}}{V^{m}}$: if this subspace is constituted by all symmetric tensors, it is represented by $\ets{\cam{F}}{V^m}$; if it is constituted by antisymmetric tensors, we have $\etn{\cam{F}}{V^m}$. Then, it is possible, and intriguing, to obtain that \emph{every tensor subspace defined by antisymmetric tensors is one-dimensional.}
...
Before we develop mathematically this notion of tensor invariance, there is a preliminary definition that needs to be addressed. The feature of proper orthogonality -- to which anti-isotropic tensors are invariant -- is also valid for tensor operators. As presented in section \ref{sec:Matrix}, an orthogonal operator is called proper if its determinant is $1$, but we don't know yet what a determinant of a tensor operator is. In order to present this unusual definition as concisely as possible, we shall sacrifice self-containedness and tacitly accept the approach of SPIVAK for the subject\footnote{See also \aut{Bishop \& Goldberg}\cite{bishop_1980_1}, pp.97-100, and \aut{Backus}\cite{backus_1997_1}, pp.17-21.}, adapting it to our case. Firstly, let's consider $\tnr{P}\in\etn{\cam{F}}{A^p}$ an arbitrary non-zero antisymmetric tensor where the set $A=V^{\times m}$, a function $\tnr{\psi}\in\evl{\cam{F}}{\ete{\cam{F}}{A}}{\ete{\cam{F}}{A}}$ and the tensor $\tnr{P}^{\tnr{\psi}}\in\ete{\cam{F}}{A^p}$ such that
\begin{equation}\label{eq:defDetPsi}
\fua{\tnr{P}^{\tnr{\psi}}}{\tnr{X}_1,\cdots,\tnr{X}_p}:=\fua{\tnr{P}}{\fua{\tnr{\psi}}{\tnr{X}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{X}_p}}}\,,\,\forall\,\tnr{X}_i\in\ete{\cam{F}}{V^m}\,,
\end{equation}  
where $\tnr{P}^{\tnr{\psi}}$ is clearly antisymmetric, that is, $\tnr{P}^{\tnr{\psi}}$ also belongs to $\etn{\cam{F}}{A^p}$. We already know the interesting feature that any antisymmetric tensor space is one-di\-men\-sional and thereby the set $\{\tnr{P}\}$ can be a basis of $\etn{\cam{F}}{A^p}$, when it is valid to write that $\tnr{P}^{\tnr{\psi}}=\alpha\tnr{P}$, where $\alpha\in\cam{F}$ is the coordinate of $\tnr{P}^{\tnr{\psi}}$ on $\{\tnr{P}\}$. If a non-zero arbitrary tensor $\tnr{T}\in\etn{\cam{F}}{A^p}$ is considered, let $\beta\in\cam{F}$ be its coordinate on $\{\tnr{P}\}$. As the previous definition \eqref{eq:defDetPsi} is valid for an arbitrary tensor of $\etn{\cam{F}}{A^p}$, then the following development
\begin{align*}
\fua{\tnr{T}^{\tnr{\psi}}}{\tnr{X}_1,\cdots,\tnr{X}_p}&=\fua{\beta\tnr{P}}{\fua{\tnr{\psi}}{\tnr{X}_1},\cdots,{\fua{\tnr{\psi}}{\tnr{X}_p}}}\\
&=\beta\fua{\tnr{P}^{\tnr{\psi}}}{\tnr{X}_1,\cdots,\tnr{X}_p}\\
&=\beta\fua{\alpha\tnr{P}}{\tnr{X}_1,\cdots,\tnr{X}_p}\\
&=\alpha\fua{\tnr{T}}{\tnr{X}_1,\cdots,\tnr{X}_p}
\end{align*}  
leads to equality $\tnr{T}^{\tnr{\psi}}=\alpha\tnr{T}$. Therefore, it is possible to conclude that the coordinate of any tensor $\tnr{T}^{\tnr{\psi}}$ is always $\alpha$ and thus this scalar value is related only to function $\tnr{\psi}$. 
