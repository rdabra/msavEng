...
A fun?o cujos elementos do dom?io s? tensores e os elementos da
imagem tamb? s? tensores ? chamada fun?o tensorial. Dados os
espa?s tensoriais $\ete{\crt{V}{p}}{F}$ e $\ete{\crt{W}{q}}{F}$,
se um determinado mapeamento
$\map{\psi}{\cft{\crt{V}{p}}{F}}{\cft{\crt{W}{q}}{F}}$ for uma
transforma?o linear, ent? $\psi$ ? uma \emph{fun?o tensorial
linear}\index{fun?o!tensorial!linear}.

\subsection{Arrays Associados}\index{array!associado!a fun?o tensorial}
Sejam os espa?s tensoriais $\ete{\crt{V}{p}}{\con{F}}$,
$\ete{\crt{W}{q}}{\con{F}}$ e o espa? vetorial de fun?es
tensoriais
\begin{equation}
...
geradoras das bases de tensores poli?icos poli?icos
...
Considerando as condi?es anteriores e o espa? vetorial
\begin{equation}
...
A uma fun?o multilinear qualquer $\vtf{g}$, cujos argumentos s?
vetores, ? sempre poss?el associar uma fun?o linear
$\vtf{g}^\otimes$ que tem um tensor como argumento. Diz-se que
$\vtf{g}$ ? ``elevada'' ? $\vtf{g}^\otimes$. Esta
eleva?o\rodape{O termo em ingl? ? ``lifting''.} estabelece uma
conveniente e importante rela?o, descrita no teorema a seguir,
entre uma seq??cia qualquer de vetores e um tensor. Fica
impl?ito que este teorema fundamenta quest?es de exist?cia e
...
Sejam os espa?s de Hilbert
$\ehr{V_1}{\con{F}},\cdots,\ehr{V_p}{\con{F}}$ e o espa? vetorial
$\evt{W}{\con{F}}$. Seja o espa? vetorial
$\evl{\crt{V}{p}}{W}{\con{F}}$ de fun?es multilineares que mapeiam uma tupla ordenada de vetores para um vetor de $\con{W}$ e
$\evl{\cft{\crt{V}{p}}{\con{F}}}{W}{\con{F}}$ o espa? vetorial
...
\end{eqnarray}
Os termos destacados s? unicamente determinados para qualquer
...
Seja um espa? tensorial $\ete{\con{V}_{(p)}}{\con{F}}$, onde
$\con{V}_{(p)}:=\crt{V}{p}$, e o conjunto $\con{V}_{(p-n)}$,
formado a partir da retirada de $n$ conjuntos quaisquer de
$\con{V}_{(p)}$. Considerando o espa? tensorial
$\ete{\con{V}_{(p-n)}}{\con{F}}$ e o espa? vetorial
...
\subsubsection{Tra? de Tensor}\index{tra?!de tensor}
...
\end{equation}
Com base nos espa?s $\ete{\con{V}_{(p)}}{\con{F}}$,
...
\end{eqnarray}
? o tra? em $r,s$, representada $\trt{r}{s}$. Nas condi?es da
...
\end{equation}
\end{itemize}
Al? destas propriedades, o produto contrativo pode ser
...
\ref{subsec:EspacoProdutoInterno}. Na pr?ica, a nota?o ?
...
Diz-se ent? que $\ete{\crt{V}{s}}{\con{F}}$ ? um \emph{espa?
tensorial produto interno}\index{espa?!tensorial!produto
interno}. Caso sejam definidas uma m?rica e uma norma para este
espa?, pode-se ter, de forma similar ao caso de espa?s
vetoriais, \emph{espa?s tensoriais de Banach e Hilbert}.


\subsection{Operador Tensorial Identidade}\index{operador!tensorial!identidade} Seja um espa? tensorial
$\ete{\crt{V}{p}}{\con{F}}$ e o espa? vetorial
...
Seja uma base qualquer de tensores poli?icos
...
\subsection{Fun?o Tensorial Inversa}\index{fun?o!tensorial!inversa} Sejam os espa?s
tensoriais $\ete{\crt{V}{p}}{\con{F}}$,
$\ete{\crt{W}{q}}{\con{F}}$. Sejam os espa?s vetoriais de fun?es
tensoriais
$\evl{\cft{\crt{V}{p}}{\con{F}}}{\cft{\crt{W}{q}}{\con{F}}}{\con{F}}$,
$\evl{\cft{\crt{W}{q}}{\con{F}}}{\cft{\crt{V}{p}}{\con{F}}}{\con{F}}$
e
...
\subsection{Mudan? de Base}\index{tensor!mudan? de base} Considerando o espa? tensorial
...
{\vto{w}_{i_p}}^{(p)}\,,
\end{equation}
? uma fun?o linear que promove uma mudan? de base de
...
\end{equation}
Utilizando procedimento semelhante, obt?-se as igualdades:
\begin{eqnarray}
...
arrays associados a $\psi$ podem ser constru?as alternando-se
...
Sejam os espa?s tensoriais $\ete{\crt{V}{p}}{\con{F}}$, $\ete{\crt{W}{q}}{\con{F}}$ e os
tensores quaisquer $\tnr{X}\in\cft{\crt{V}{p}}{\con{F}}$,
$\tnr{Y}\in\cft{\crt{W}{q}}{\con{F}}$. Dados os espa?s vetoriais de fun?es tensoriais
\begin{eqnarray}
...
Caso os espa?s tensoriais considerados forem de Euclidianos, dadas duas bases $\hat{\con{T}}^{\otimes}_{\crt{V}{\times p}}$ e
...
\subsubsection{Operador Tensorial Sim?rico}\index{operador!tensorial!sim?rico}
Considerando as condi?es anteriores, tendo
$\crt{W}{q}=\crt{V}{p}$, um operador tensorial $\psi$ ? sim?rico
se $\psi=\psi^T$. Ele ?
\emph{anti-sim?rico}\index{operador!tensorial!anti-sim?rico} se
$\psi=-\psi^T$.
...
ortogonais pr?rios e impr?rios respectivamente, geram os grupos
...
\end{eqnarray}
s? transforma?es lineares bijetoras se, respectivamente,
\begin{eqnarray}
...
\begin{itemize}
\item[i.] se $q=p$, ent? $\stackrel{\rightarrow}{\Psi} \,=\,
...
$\stackrel{\leftarrow}{\psi}_{\tnr{T}}$ s? operadores lineares;

\item[iv.] Dada uma tupla qualquer
$(\vto{v}_1,\cdots,\vto{v}_p)\in\crt{V}{p}$, ? sempre v?ido que
\begin{equation}
...
\end{equation}
Pode-se fazer, ent?, o seguinte desenvolvimento:
\begin{eqnarray}
...
Sejam os espa?s tensoriais $\ete{\crt{V}{p}}{\con{F}}$, $\ete{\crt{W}{q}}{\con{F}}$ e
\begin{equation}
\evl{\cft{\crt{V}{p}}{\con{F}}}{\cft{\crt{W}{q}}{\con{F}}}{\con{F}}\,.\nonumber
\end{equation}
Dados os espa?s tensoriais $\ete{\crt{W}{q}\times\crt{V}{p}}{\con{F}}$ e $\ete{\crt{V}{p}\times\crt{W}{q}}{\con{F}}$, uma fun?o qualquer
\begin{equation}
\psi\in\cfl{\cft{\crt{V}{p}}{\con{F}}}{\cft{\crt{W}{q}}{\con{F}}}\,,\nonumber
\end{equation}
...
um vetor pode tamb? ser considerado um funcional linear, conforme
(\ref{eq:VetorFuncional}).

No caso de tensores, segundo o teorema
\ref{teo:RieszGeneralizado}, as fun?es
$\stackrel{\rightarrow}{\psi}_{\tnr{T}}$ e
$\stackrel{\leftarrow}{\psi}_{\tnr{T}}$ t?, cada uma, rela?o
...
\end{eqnarray}
s? chamadas fun?es representantes do tensor $\tnr{T}$ com
contra?o $q$ ? direita e ? esquerda, respectivamente. Se o tensor
$\tnr{T}$ tiver ordem $p$, $\rft{T}{q}$ e $\lft{T}{q}$ s? fun?es
tensoriais lineares associadas a $\tnr{T}$, segundo
(\ref{eq:funcaoTensorial}), cujas imagens s? constitu?as por
tensores de ordem $p-q$. Pode-se concluir, ent?, que
...
import?cia, j? que ele torna poss?el apresentar o estudo de
...
igualdades do ?em iv da proposi?o
\ref{prp:propsRieszGeneralizado} ? considerada\rodape{Grande parte
dos trabalhos em Mec?ica do Cont?uo que tratam de Teoria de
Tensores n? mostra, de forma expl?ita, qual das fun?es
representantes ? utilizada para substituir o conceito de tensor.
No caso de tensores de segunda ordem, por exemplo, a cl?sica
...
Seja o espa? tensorial $\ete{\crt{V}{p}\times\crt{V}{p}}{\con{F}}$. A partir da
...
\lpa\vto{y}_1\otimes\cdots\otimes\vto{y}_p\rpa\nonumber\,.
\end{eqnarray}
Com base no ?em iv da proposi?o
\ref{prp:propsRieszGeneralizado}, pode-se concluir ent? que a
...
Sejam os espa?s tensoriais $\ete{\crt{V}{p}\times\crt{V}{p}}{\con{F}}$,
$\ete{\crt{V}{p}\times\crt{V}{p}\times\crt{W}{q}}{\con{F}}$ e
$\ete{\crt{W}{q}\times\crt{V}{p}\times\crt{V}{p}}{\con{F}}$. Considerando um base
gen?ica qualquer $\con{U}_i=\{ {\vto{u}_1}^{(i)} ,\cdots, {\vto{u}_{n_i}}^{(i)}\}$ de
...
espa? tensorial $\ete{\crt{V}{p}}{\con{F}}$ e os tensores quaisquer
$\tnr{A}$,$\tnr{B}\in\cft{\crt{V}{p}}{\con{F}}$, onde $p\geqslant s+q$, tal que $s$ e $q$
s? escalares positivos n? nulos. Das defini?es (\ref{eq:funcaoTensorial}), dado um
...
contra?o ? direita ? mais simples, j? que n? ? necess?io
inverter a se\-q??\-cia dos tensores envolvidos\rodape{Esta ? a
principal raz? pela qual a maioria dos textos introdut?ios optam
por fun?es representantes com contra?o ? direita para substituir
o conceito de tensor.}.

Neste contexto, se for poss?el $n-1$ composi?es de uma mesma fun?o representante $\tnr{A}_{\odot q}$, tem-se
...
Seja o espa? tensorial $\ete{\crt{V}{p}\times\crt{V}{p}}{\con{F}}$, onde $2p\geqslant
s+q$, tal que $s$ e $q$ s? positivos n? nulos. Dado um tensor qualquer
...
\end{equation}
Em termos de nota?o, tal defini?o n? gera ambig?idade, j? que
inexiste o conceito de ``tensor inverso''.

\subsubsection{Fun?o Representante Transposta}\index{fun?o!representante!transposta} Sejam os quatro espa?s tensoriais
...
representante transposta de $\ftr{A}{p}\,$. Nestas circunst?cias,
...
$\crt{W}{q}=\crt{V}{p}$, n? h? este problema e pode-se ter uma
fun?o
...
\subsubsection{Tra? de Fun?o Representante}\label{sec:tracoFuncaoRepresentante}\index{tra?!de fun?o representante}
Sejam os espa?s tensoriais $\ete{\con{V}_{(p)}}{\con{F}}$  e
...
\end{equation}
onde $q_1\leqslant p$ e $q_2\leqslant p-2$, ? denominada tra? da fun?o representante $\ftr{T}{q_1}$ em $r,s$.
Com base nesta defini?o, considerando o caso particular de tensores de segunda ordem, cujo espa? tensorial ? definido por $\con{V}_{(2)}=\con{V}^{2}$, tem-se que o tra? $\fua{\trt{1}{2}}{\ftr{T}{q_1}}$, onde $q_1\leqslant 2$, resulta uma fun?o escalar, pois
\begin{equation}
 \fua{\trt{1}{2}}{\tnr{T}}\in\con{F}.
\end{equation}
Ainda neste contexto, se $\con{V}$ definir um espa? vetorial de Hilbert com dimens? $n$, seja $\con{T}^{\otimes}_{\con{V}{2}}$ base de $\ete{\con{V}^2}{\con{F}}$, gerada pelos tensores poli?icos $\vto{u}_{i_1}\otimes\vto{u}^*_{i_2}$, $i_j=1,\cdots,n\,$. Dado um tensor qualquer $\tnr{A}\in\cft{\con{V}^{2}}{\con{F}}$, pode-se realizar o seguinte desenvolvimento:
\begin{eqnarray}
...
Seja o espa? tensorial $\ete{\crt{V}{p}}{\con{F}}$, gerado por espa?s de Hilbert
$m$-dimensionais, e o espa? vetorial de fun?es tensoriais lineares
...
tensores anti-sim?ricos n? nulos $\tnr{P}^\psi,\tnr{P}\in\cfa{(\crt{V}{p})^n}{F}$ e o
...
do subespa? $\etn{(\crt{V}{p})^n}{F}$ de $\ete{(\crt{V}{p})^n}{F}$, tal que
...
que tal defini?o seja plaus?el, deve ser mostrado que o valor do escalar $\alpha$ em
...
\end{equation}
de onde se conclui que $\alpha$ depende somente de $\psi$. Nos termos do Teorema \ref{teo:RieszGeneralizado}, como a fun?o $\psi$ pode ser representante de um tensor $\tnr{S}\in\cft{(\crt{V}{p})^n}{\con{F}}$, ent? o escalar
\begin{equation}
\hpd{\tnr{S}}:=\hpd{\psi}
\end{equation}
? dito o \emph{hiperdeterminante do tensor} $\tnr{S}$.\index{hiperdeterminante!de tensor}

Com base na defini?o (\ref{eq:determinanteUm}), ? poss?el escrever que
...
\item[iii.] $\psi_1$ autom?fico $\implies \hpd{\psi_1^{-1}} = \lpa\hpd{\psi_1}\rpa^{-1}$\,;
\item[iv.] $\psi_1$ autom?fico $\Longleftrightarrow\hpd{\psi_1}\neq 0$;
\item[v.]
$\hpd{\psi_1}=
...
\end{itemize}
Considerando o ?ltimo ?em, caso seu hiperdeterminante seja positivo,
$\psi_1$ ? chamado operador ortogonal \emph{pr?rio}\index{operador!tensorial!ortogonal pr?rio}, sen?, ele
? \emph{impr?rio}\index{operador!tensorial!ortogonal impr?rio}.
...
\end{eqnarray*}
\item[ii.]  De (\ref{eq:determinanteTensor}), a obten?o da igualdade deste ?em ? imediata.
...
\end{equation*}
Mas como $\tnr{X}_\alpha=\tnr{X}_\beta$, ent?
$\fua{\tnr{T}_{\odot_q}}{\tnr{X}}=-\fua{\tnr{T}_{\odot_q}}{\tnr{X}}$, c.q.d. Com base
neste resultado, vamos demonstrar agora que se o conjunto
$\con{Y}=\{\tnr{Y}_1,\cdots,\tnr{Y}_n\}$ for linearmente dependente, ent?
\begin{equation*}
\fua{\tnr{T}_{\odot_q}}{\tnr{Y}_1\otimes\cdots\otimes\tnr{Y}_n}=0\,.
\end{equation*}
Para tal, vamos dizer que um tensor $\tnr{Y}_\alpha$ de $\con{Y}$ ? combina?o linear dos demais elementos do conjunto. Pode-se realizar ent? o seguinte desenvolvimento:
\begin{eqnarray*}
...
\end{eqnarray*}
A ?ltima igualdade ocorre porque para cada valor do ?dice $i\neq\alpha$, o produto
...
\end{equation*}
Como conseq??cia os conjuntos $\{\tnr{Z}_1,\cdots,\tnr{Z}_n\}$ e
$\{\fua{\psi_1}{\tnr{Z}_1},\cdots,\fua{\psi_1}{\tnr{Z}_n}\}$ s? linearmente
independentes. Desta conclus?, como $\tnr{Z}_i$ s? tensores quaisquer, dada uma base de
tensores poli?icos $\con{T}^{\otimes}_{\crt{V}{p}}$ de $\ete{\crt{V}{p}}{\con{F}}$,
constru?a a partir de $\con{U}_i=\{{\vto{v}_1}^{(i)} ,\cdots, {\vto{v}_{m}}^{(i)}\}$,
onde $n_i$ ? a dimens? de $\ehr{V_i}{F}$, pode-se afirmar que o conjunto
\begin{equation*}
...
$\tnr{K}_1,\tnr{K}_2\in\cft{\crt{V}{p}}{\con{F}}$, pode-se dizer ent? que os valores
\begin{equation*}
...
\end{equation*}
tamb? s? distintos. Se ? assim, ent? o operador $\psi_1$ ? uma inje?o. Nos termos da
proposi?o \ref{teo:operadorBijecao}, $\psi_1$ ? tamb? uma bije?o.
\item[v.] Dada uma base qualquer de
tensores poli?icos $\hat{\con{T}}^{\otimes}_{\crt{V}{p}}$ de $\ete{\crt{V}{p}}{\con{F}}$,
constru?a a partir de $\con{U}_i=\{{\vto{v}_1}^{(i)} ,\cdots, {\vto{v}_{m}}^{(i)}\}$ e
um tensor anti-sim?rico  qualquer $\tnr{T}\in\cfa{(\crt{V}{p})^n}{F}$, onde $n=m^p$.
...
\cdots\otimes{\vto{v}_{(i_p)_n}}^{(p)}}&&
\end{eqnarray*}
porque o ?ltimo termo foi colocado em evid?cia j? que o tensor $\tnr{T}$ ? anti-sim?rico. ? importante observar que este resultado fica inalterado se fossem adotadas duas bases diferentes de tensores poli?icos. Comparando esta igualdade com (\ref{eq:hiperdeterminante}) e com (\ref{eq:determinanteTensor}), conclui-se  a igualdade do ?em.
\item[vi.] A ?ltima igualdade da demonstra?o do ?em anterior, escrita para $\psi_1^T$, pode ser apresentada na forma:
\begin{eqnarray*}
...
\end{equation*}
tem-se que o termo destacado na primeira igualdade ? igual ao da ?ltima igualdade do ?em anterior. Isto ocorre porque os fatores da soma global, promovida pelo diversos somat?ios envolvidos, sofrem apenas altera?o de posi?o.
\item[vii.] Se $\psi_1^T=\psi_1^{-1}$ ent?
\begin{equation*}
...
% Seja o espa? tensorial $\ete{\crt{V}{p}}{\con{F}}$ e duas de suas bases de tensores
% poli?icos $\hat{\con{T}}^{\otimes}_{\crt{V}{p}}$ e
% $\tilde{\con{T}}^{\otimes}_{\crt{V}{p}}$. Dado o espa? vetorial de fun?es tensoriais
...
% (\ref{eq:conversoesTensores}), obt?-se que
% \begin{eqnarray}
...
invari?cia. A grosso modo, diz-se que uma quantidade ou medida ?
isotr?ica se seu valor independe do efeito de qualquer fun?o
ortogonal que eventualmente aja no seu dom?io de atua?o. Se esta
independ?cia se restringir somente a fun?es ortogonais pr?rias,
a medida ? anti-isotr?ica\index{anti-isotropia}. Medidas que n?
apresentam nenhuma destas caracter?ticas s? chamadas
\emph{anisotr?icas}\index{anisotr?rica!medida}.

Considerando um espa? vetorial Euclidiano, medidas isotr?icas,
representadas por grandezas de segunda ordem (matrizes), por
exemplo, s? \emph{indiferentes} a rota?es e reflex?es. Ocorrendo
reflex?es, medidas an\-ti-iso\-tr?-pi\-cas podem sofrer
altera?o.

\subsection{Tensores Invariantes}\index{tensor!invariante}
Seja o espa? tensorial $\ete{\crt{V}{p}}{\con{F}}$ e o grupo de
...
\end{equation}
Caso a igualdade anterior seja v?ida para qualquer tensor
$\tnr{T}$ de $\cft{\crt{V}{p}}{\con{F}}$, diz-se que o espa?
...
\subsubsection{Tensor Isotr?ico}\index{tensor!isotr?ico} Um tensor ? dito isotr?ico se
...
$\tnr{T}\in\cft{\crt{V}{p}}{\con{F}}$ ? isotr?ico se
\begin{equation}
\fua{\psi}{\tnr{T}}=\tnr{T}\,,\,\forall\,\psi\in\con{O}_{\cft{\crt{V}{p}}{\con{F}}}\,.
\end{equation}

\subsubsection{Tensor Anti-Isotr?ico}\index{tensor!anti-isotr?ico} Um tensor ? anti-isotr?ico
se ele for invariante ao grupo ortogonal pr?rio. Desta forma, com
base no conjunto $\con{O}^+_{\cft{\crt{V}{p}}{\con{F}}}$ formado
por operadores ortogonais pr?rios, para um tensor anti-isotr?ico
...
tensor isotr?ico ? sempre anti-isotr?ico.
...
\con{O}_{\cft{\crt{V}{p}}{\con{F}}}$, ent? o conjunto
$\cti{\crt{V}{p}}{\con{F}}$ ? formado por tensores isotr?icos,
...
anti-isotr?icos ? representado $\ctt{\crt{V}{p}}{\con{F}}$.

\subsubsection{Exemplos de Tensores Isotr?icos e
Anti-Isotr?icos} Seja o espa? vetorial $\ehr{V}{\con{F}}$, o
espa? tensorial $\ete{\con{V}^p}{\con{F}}$ , o tensor identidade
$\tnr{I}\in\cft{\con{V}^2}{\con{F}}$ e o tensor anti-sim?rico n?
...
isotr?icos n? vazios s? gerados (span) pela permuta?o de
tensores identidade. Segundo ele, conjuntos de tensores
anti-isotr?icos n? vazios s? gerados pela permuta?o de
tensores identidade e$\backslash$ou de tensores anti-sim?ricos
n? nulos e$\backslash$ou dos tensores $\tnr{I}\otimes\tnr{A}$,
$\tnr{I}\otimes\tnr{I}\otimes\tnr{A}$,
$\tnr{I}\otimes\tnr{I}\otimes\tnr{I}\otimes\tnr{A}$, etc\ldots Em
termos espec?icos, para espa?s tensoriais de ordem 0 a 4,
...
\begin{tabular}{|c|c|c|c|c|}
  ...
  $\ete{\con{V}^p}{\con{F}}$ & $\mathrm{dim}(\ehr{\con{V}}{\con{F}})$  & $\cts{\con{V}^{p}}{\con{F}}$  & $\ctt{\con{V}^{p}}{\con{F}}$  & \textbf{P?-condi?o} \\
  \hline
  & & & & \\
  $p=0$ & $>0$ & $\con{F}$ & $\con{F}$ & - \\
  & & & & \\
...
\hline
\end{tabular}
}  \titfigura{Tensores Isotr?icos e Anti-Isotr?icos em
$\ete{\con{V}^p}{\con{F}}$. Fonte:
\aut{Backus}\cite{backus_1997_1}.}\label{tb:Isotropicos}
\end{table}


\subsection{Operadores Tensoriais de Invari?cia}\index{operador!tensorial!de invari?cia}
Seja o espa? tensorial $\ete{\crt{V}{p}}{\con{F}}$ e o grupo de
...
$\psi\in\con{P}_{\cft{\crt{V}{p}}{\con{F}}}$ ? dito de invari?cia
...
de todos os operadores de invari?cia que atuam em
$\cti{\crt{V}{p}}{\con{F}}$ define o grupo de invari?cia
...
um operador de invari?cia em $\cts{\crt{V}{p}}{\con{F}}$. Com
...
Neste caso, s? operadores de invari?cia em
...
Seja o espa? m?rico completo $\lpa \con{A}, \varrho \rpa$ e o
espa? tensorial $\ete{\crt{W}{q}}{\con{F}}$. Dado o subconjunto
$\con{B}\subset\con{A}$, que tamb? define um espa? m?rico
completo, a fun?o no mapeamento
\begin{equation}
\map{\fac{F}}{B}{\cft{\crt{W}{q}}{\con{F}}}
\end{equation}
? dita um campo tensorial. Caso $q=1$, tem-se ent? um \emph{campo
...
$\saf{U}{S}{\ele{a}}{F}$ um subespa? afim do espa? m?rico
completo $\eaf{V}{A}{F}$ e um sistema de coordenadas $(\ele{o} ,
\tilde{\con{U}})$ deste subespa?. Considerando o espa? tensorial
$\ete{\crt{W}{q}}{\con{F}}$ e o espa? vetorial $\evt{U}{F}$ (ou
...
$\fac{F}$. Desta forma, o estudo de campos tensoriais pode ser substitu?o pelo
estudo de fun?es tensoriais.

\subsubsection{Campo Caracter?tico}\index{campo!caracter?tico}
Seja um espa? afim m?rico completo $\eaf{V}{A}{F}$ com
dimens? maior que 2 e dois de seus subespa?s
...
\vto{w}\in\con{W}\,.
\end{equation}
Nestas condi?es, diz-se que $\fac{F}$ ? um campo caracter?tico de $\epo{S}_\ele{b}$
...
%\end{equation}
%A partir desta igualdade, a fun?o $\chi$ ? dita caracter?tica de
%$\epo{S}_\ele{b}$ em $\con{U}$, representada
%$\fac{F}_{\epo{S}_\ele{b}}^{\con{U}}$. ? importante notar que fun?es caracter?ticas permitem substituir o uso de pontos pelo de vetores, viabilizando manipula?es alg?ricas.
\end{comment} 