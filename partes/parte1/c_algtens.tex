...
\end{align*} 
we call tensor $\fua{{\vtf{c}}_{2p}^\otimes}{\tnr{A}\otimes\tnr{B}}$ the $p$-th order \textsb{contractive product}\index{contractive!product} of $\tnr{A}$ and $\tnr{B}$, represented by $\tnr{A}\odot_p\tnr{B}$. Thereby, we can write in simpler terms that   
\begin{equation}\label{eq:ProdTens}
\tnr{A}\odot_p\tnr{B}=(\prod_{i=1}^p\vto{y}_i^c\cdot\vto{x}_i)\vto{a}_1^*\otimes\cdots\otimes\vto{a}_{q}^*\otimes\vto{b}_{1}^*\otimes\cdots\otimes\vto{b}_n^*.
\end{equation}

{\footnotesize
\begin{proof}
Let's prove that the $r,s$ trace and the contractive product are indeed linear. From property ii on page \pageref{prop:tensorProd}, verification of equalities $\trt{r}{s} \alpha\tnr{ X}=\alpha\trt{r}{s}\tnr{X}$ and $(\alpha\tnr{A})\odot_p(\beta\tnr{B})=\alpha\beta\tnr{A}\odot_p\tnr{B}$ is straightforward. Henceforth, in order to shorten expression sizes, without losing generality, we shall consider that a contractive product always results a scalar, that is, the order of the contractive product equals the order of its terms. Considering the above conditions, let's verify the additive property of linear functions for a first order contractive product, that is, set $X^{\times 2}=W\times W$ and tensors $\tnr{A},\tnr{B},\tnr{W}\in\ete{\cam{F}}{W}$. Since $\vto{x}_1^*+\vto{y}_1^*=(\vto{x}_1+\vto{y}_1)^*$, equalities
\begin{equation*}
(\tnr{A}+\tnr{B})\odot_1\tnr{W} = (\vto{x}_1+\vto{y}_1)^*\odot_1\vto{w}_1^*=\vto{w}^c_1\cdot(\vto{x}_1+\vto{y}_1)= \vto{w}^c_1\cdot\vto{x}_1+\vto{w}^c_1\cdot\vto{y}_1=\tnr{A}\odot_1\tnr{W}+\tnr{B}\odot_1\tnr{W}
\end{equation*}
...
\begin{align*}
\tnr{A}\odot_2\tnr{W} + \tnr{B}\odot_2\tnr{W}  & = (\vto{x}_1^*\otimes\vto{x}_2^*) \odot_2(\vto{w}_1^*\otimes\vto{w}_2^*)+(\vto{y}_1^*\otimes\vto{y}_2^*) \odot_2(\vto{w}_1^*\otimes\vto{w}_2^*)\\
...
& = [(\vto{x}_1\cdot\vto{w}_1^c)\vto{x}_2 + (\vto{y}_1\cdot\vto{w}_1^c)\vto{y}_2]^*\odot_1\vto{w}_2^*\\
& = [(\vto{w}_1^c\cdot\vto{x}_1)\vto{x}_2^* + (\vto{w}_1^c\cdot\vto{y}_1)\vto{y}_2^*]\odot_1\vto{w}_2^*\\
& = [(\vto{x}_2^*\otimes\vto{x}_1^*)\odot_1\vto{w}_1^* + (\vto{y}_2^*\otimes\vto{y}_1^*)\odot_1\vto{w}_1^*]\odot_1\vto{w}_2^*\\
& = \underbrace{[(\vto{x}_2^*\otimes\vto{x}_1^*) + (\vto{y}_2^*\otimes\vto{y}_1^*)]}_{\vto{u}^*\otimes\vto{v}^*}\odot_1\vto{w}_1^*\odot_1\vto{w}_2^*\\
& = \vto{v}^*\otimes\vto{u}^*\odot_2\vto{w}_1^*\otimes\vto{w}_2^* \\
& = [(\vto{x}_1^*\otimes\vto{x}_2^*) + (\vto{y}_1^*\otimes\vto{y}_2^*)]\odot_2\vto{w}_1^*\otimes\vto{w}_2^*\\
& = (\tnr{A} + \tnr{B})\odot_2\tnr{W}\,.
\end{align*}
By the same strategy of this development, we can find $\tnr{W}\odot_2\tnr{A} + \tnr{W}\odot_2\tnr{B}=\tnr{W}\odot_2(\tnr{A} + \tnr{B})$. Therefore, it results that second  order contractive products are also linear. In the case of arbitrary tensors $\tnr{A},\tnr{B},\tnr{W}\in\ete{\cam{F}}{W^{\times p}}$, for any $p>2$, this procedure can also be applied, leading to linearity for higher order contractive products. Linearity for $r,s$ trace can be verified by this procedure as well.  
\end{proof}
}

For the sake of generality, when manipulating contractive products we consider that $\tnr{A}\odot_0\tnr{B}:=\tnr{A}\otimes\tnr{B}$. Moreover, it is trivial to obtain that these type of products are also associative:
\begin{equation}
\tnr{A}\odot_p(\tnr{C}\odot_r\tnr{D}) = (\tnr{A}\odot_p\tnr{C})\odot_r\tnr{D}\,,
\end{equation}
where $\tnr{C}\in\ete{\cam{F}}{W^{\times p}\times C^{\times n}\times Z^{\times r}}$ and $\tnr{D}\in\ete{\cam{F}}{Z^{\times r}\times D^{\times s}}$. In the context of Euclidean spaces, it is not unusual to see a first order contractive product of a dyadic tensor and a covector being used to define the so called ``tensor product of vectors''\footnote{See \aut{Gurtin}\cite{gurtin_1981}, p.4; \aut{Truesdell \& Noll}\cite{truesdell_1992}, p.15; \aut{Ogden}\cite{ogden_1997}, p.16.}. Although we shall not adopt this rather obscure concept, its frequent use by renowned authors forces us to clarify it in terms of our rigorous approach as follows. Firstly, since vectors and covectors have a one to one correspondence, these authors inadvertently use the term ``vector'' either as an element of a vector space or as a covector, depending on the context: in $\vto{u}(\vto{v})=\vto{v}\cdot\vto{u}$, for example, $\vto{u}$ is a covector on the left-hand side and a vector on the right. Thereby, they get rid of the burden of defining a covector and all of its preliminary concepts. In this sense, our representation of a dyadic tensor $\vto{u}^*\otimes\vto{v}^*$ is subverted to $\vto{u}\otimes\vto{v}$ and the contractive product operator is suppressed from the precise representation $(\vto{u}^*\otimes\vto{v}^*)\odot_1 \vto{x}^*=(\vto{v}\cdot\vto{x})\vto{u}^*$ in order to avoid defining a contraction and its supporting concepts. Finally, they define the ``tensor product'' $\vto{u}\otimes\vto{v}$ through its effects, that is, as a ``tensor'' which causes $(\vto{u}\otimes\vto{v})\vto{x}=(\vto{v}\cdot\vto{x})\vto{u}$, for an arbitrary $\vto{x}$. Again,  \emph{this pragmatic approach of taking vectors as first order tensors and all of its consequences will not be adopted in this text}. 


When the order of a contractive product and the order of its terms are equal, the result is a scalar. In this context, considering arbitrary tensors $\tnr{A},\tnr{B}\in\ete{\cam{F}}{U^{\times m}}$, where $\tnr{A}=\vto{a}^*_1\otimes\cdots\otimes\vto{a}^*_m$ and $\tnr{B}=\vto{b}^*_1\otimes\cdots\otimes\vto{b}^*_m$, the scalar
\begin{equation}\label{eq:prodIntTens}
\tnr{A}\cdot\tnr{B}:=\vto{b}^*_1\otimes\cdots\otimes\vto{b}^*_m\,\,\odot_m\,\,(\vto{a}^c_1)^*\otimes\cdots\otimes(\vto{a}^c_m)^*=\prod_{i=1}^m\vto{a}_i\cdot\vto{b}_i
\end{equation}
is called the \textsb{inner product of tensors}\index{inner product!of tensors} $\tnr{A}$ and $\tnr{B}$ because it observes the axioms of ordinary inner products. From this definition, if $\ete{\cam{F}}{U^{\times m}}$ is a first order tensor space, we have equalities
\begin{equation*}
\tnr{A}\cdot \tnr{B}=\vto{a}^*_1\cdot\vto{b}^*_1=\vto{b}^*_1\odot_1(\vto{a}^c_1)^*=\vto{a}_1\cdot\vto{b}_1\,, 
\end{equation*}     
from which we conclude that the inner product of covectors, as defined by \eqref{eq:prodIntTens}, equals the inner product of the corresponding vectors. Moreover, if an inner product can be defined on the elements of $\ete{\cam{F}}{U^{\times m}}$, then this space results an inner product tensor space.  


{\footnotesize
\begin{proof}
Let's verify if definition \eqref{eq:prodIntTens} really observes the axioms of inner products (see p. \pageref{prop:produtoInterno}). We do not compromise generality if we assume $\tnr{A},\tnr{B}\in\ete{\cam{F}}{U^{\times 2}}$ in order to shorten expression sizes. Equality $\tnr{A}\cdot\tnr{A}=(\vto{a}_1\cdot\vto{a}_1)(\vto{a}_2\cdot\vto{a}_2)$ assures nonnegativity for the left-hand side since each inner product on the right-hand side is nonnegative. Now, let's say that $(\vto{a}_1\cdot\vto{a}_1)(\vto{a}_2\cdot\vto{a}_2)=0$. If this is true, one of the the inner products on the left-hand side has to be zero. Therefore, according to the axiom of definition of ordinary inner products, vector $\vto{a}_1=\vto{0}$ or vector $\vto{a}_2=\vto{0}$ and as a consequence, $\tnr{A}=\tnr{0}$, according to property i on page \pageref{prop:tensorProd}. Equalities $\tnr{A}\cdot\tnr{B}=(\vto{a}_1\cdot\vto{b}_1)(\vto{a}_2\cdot\vto{b}_2)=\overline{(\vto{b}_1\cdot\vto{a}_1)(\vto{b}_2\cdot\vto{a}_2)}=\overline{\tnr{B}\cdot\tnr{A}}$ show that the inner product of tensors is conjugate symmetric. Finally, according to property ii on page \pageref{prop:tensorProd},  we have equalities
\begin{equation*}
(\alpha\tnr{A})\cdot(\beta\tnr{B})=((\alpha\vto{a}_1)\cdot(\beta\vto{b}_1))(\vto{a}_2\cdot\vto{b}_2)=\alpha\overline{\beta}(\vto{a}_1\cdot\vto{b}_1)(\vto{a}_2\cdot\vto{b}_2)=\alpha\overline{\beta}\tnr{A}\cdot\tnr{B}
\end{equation*}
and  development
\begin{align*}
...
& = (\vto{a}_1\cdot\vto{w}_1)(\vto{a}_2\cdot\vto{w}_2) + (\vto{b}_1\cdot\vto{w}_1)(\vto{b}_2\cdot\vto{w}_2)\\
& = (\vto{a}_2\cdot\vto{w}_2)\vto{a}_1\cdot\vto{w}_1 + (\vto{b}_2\cdot\vto{w}_2)\vto{b}_1\cdot\vto{w}_1\\
& = [(\vto{a}_2\cdot\vto{w}_2)\vto{a}_1+(\vto{b}_2\cdot\vto{w}_2)\vto{b}_1]\cdot\vto{w}_1\\
& = [(\vto{a}_1^*\otimes\vto{a}_2^*)\odot_1(\vto{w}^c_2)^*+(\vto{b}_1^*\otimes\vto{b}_2^*)\odot_1(\vto{w}^c_2)^*]\cdot\vto{w}_1\\
& =[(\vto{a}_1^*\otimes\vto{a}_2^*)+(\vto{b}_1^*\otimes\vto{b}_2^*)]\odot_1(\vto{w}^c_2)^*\}\cdot\vto{w}_1\\
& =(\vto{w}_1^c)^*\odot_1(\tnr{A}+\tnr{B})\odot_1(\vto{w}^c_2)^*\\
& =(\tnr{A}+\tnr{B})\odot_2(\vto{w}^c_1)^*\otimes(\vto{w}^c_2)^*\\
& = (\tnr{A} + \tnr{B})\cdot\tnr{W}\,,
\end{align*}
...
\begin{equation*}
\fua{\vtf{T}^*}{\tnr{X}}=\tnr{T}\cdot\tnr{X}\,,
\end{equation*}
then $\tnr{\Phi}$ results a linear bijection. If $\tnr{T}^*$ is continuous,  $\|\tnr{T}^*\|_{\eted{\cam{F}}{U^{\times m}}}=\|\tnr{T}\|_{\ete{\cam{F}}{U^{\times m}}}$.
\end{mteo}

{\footnotesize
\begin{proof}
Considering $\tnr{A},\tnr{B}\in\ete{\cam{F}}{U^{\times m}}$, function $\tnr{\Phi}$ is linear from the following equalities:
\begin{equation*}
\fua{\lco\fua{\tnr{\Phi}}{\alpha\tnr{A}+\beta\tnr{B}}\rco}{\tnr{X}}=(\alpha\tnr{A}+\beta\tnr{B})\cdot\tnr{X}=\alpha\tnr{A}^*(\tnr{X})+\beta\tnr{B}^*(\tnr{X})=\fua{\lco\alpha\fua{\tnr{\Phi}}{\tnr{A}}+\beta\fua{\tnr{\Phi}}{\tnr{B}}\rco}{\tnr{X}}\,.
\end{equation*}
...
Considering the same conditions of equality \eqref{eq:coordTensor} and definition \eqref{eq:prodIntTens}, we can say that
\begin{align*}
\fua{\vtf{\psi}}{\tnr{X}}&=\fua{[\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\vtf{f}_{i_1\cdots i_m}^{B}]}{\tnr{X}}\\
&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\fua{\tnr{X}}{\vto{u}_{i_1}^{(1)},\cdots,\vto{u}_{i_m}^{(m)}}\\
&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\prod_{k=1}^m\fua{\vto{x}_k^*}{\vto{u}_{i_k}^{(k)}}\\
&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\prod_{k=1}^m\vto{u}_{i_k}^{(k)}\cdot\vto{x}_k\\
&=\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}(\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m})\cdot\tnr{X}\\
&=\underbrace{(\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\alpha_{i_1\cdots i_m}\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m})}_{\tnr{A}}\cdot\tnr{X}=[\fua{\tnr{\Phi}}{\tnr{A}}](\tnr{X})
\end{align*}
Finally, using definition \eqref{eq:normaFuncao} on the present conditions, leaving space representation on norms implicit, we obtain equality $\|\vtf{u}^*\|=\sup\{ |\vto{x}\cdot\vto{u}|/\|\vto{x}\|\}$ for all non zero $\vto{x}$.
If $\vto{u}$ is zero, it is evident that $\|\vtf{u}^*\|=\|\vto{u}\|$; otherwise, $\vtf{u}^*$ is non zero and we conclude that $\|\vtf{u}^*\|\geqslant |\, \vto{x}\cdot\vto{u}|/\|\vto{x}\|$. Cauchy-Schwarz Inequality states that $|\, \vto{x}\cdot\vto{u}| \leqslant \|\vto{x}\|\,\,\|\vto{u}\|$. Subtracting these two previous inequalities, we arrive at $(\|\vtf{u}^*\|-\|\vto{u}\|)\|\vto{x}\|\geqslant 0$, whose left side can be zero for arbitrary non zero $\vtf{u}^*$, $\vto{u}$ and $\vto{x}$; therefore, $\|\vtf{u}^*\|=\|\vto{u}\|$.    
\end{proof}
}
