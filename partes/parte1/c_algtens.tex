...
\begin{equation}
\fua{\tnr{P}}{\fua{\tnr{T}_m^*}{\tnr{X}_1},{\fua{\tnr{T}_m^*}{\tnr{X}_2}}}=\hdet{\tnr{T}}{\fua{\tnr{P}}{\tnr{X}_1,\tnr{X}_2}}\,.
\end{equation}  
...
{\footnotesize
\begin{proof}
Verification of item i is a straightforward consequence of considering $\tnr{\psi}=\tnr{i}$ in \eqref{eq:detFunDef}. In order to shorten expression sizes, we will admit hereafter that $p=2$, since generality is not compromised. The second item is proved through the following equalities:
\begin{align*}
\hdet{\tnr{\psi}\circ\tnr{\varphi}}\fua{\tnr{P}}{\tnr{X}_1,\tnr{X}_2}&=\fua{\tnr{P}}{\tnr{\psi}[\fua{\tnr{\varphi}}{\tnr{X}_1}],\tnr{\psi}[\fua{\tnr{\varphi}}{\tnr{X}_2}]}\\
&=\hdet{\tnr{\psi}}\fua{\tnr{P}}{\fua{\tnr{\varphi}}{\tnr{X}_1},\fua{\tnr{\varphi}}{\tnr{X}_2}}\\
&=\hdet{\tnr{\psi}}\hdet{\tnr{\varphi}}\fua{\tnr{P}}{\tnr{X}_1,\tnr{X}_2}\,.
\end{align*}  
Equalities $1=\hdet{\tnr{\psi}^{-1}\circ\tnr{\psi}}=\hdet{\tnr{\psi}^{-1}}\,\hdet{\tnr{\psi}}$ prove the third item. For the item iv, let's consider  $\hdet{\tnr{\psi}}\neq 0$ and as equality $\eqref{eq:detFunDef}$ is valid for any $\tnr{X}_i\in\ete{\cam{F}}{A}$, let's choose a linearly independent set $\{\tnr{X}_1,\tnr{X}_2\}$, which causes $\fua{\tnr{P}}{\tnr{X}_1,\tnr{X}_2}\neq 0$. These conditions imply inequality $\fua{\tnr{P}}{\fua{\tnr{\psi}}{\tnr{X}_1},{\fua{\tnr{\psi}}{\tnr{X}_2}}}\neq 0$, from which we conclude that $\fua{\tnr{\psi}}{\tnr{X}_1}\neq\gamma\fua{\tnr{\psi}}{\tnr{X}_2}$ or $\fua{\tnr{\psi}}{\tnr{X}_1}\neq\fua{\tnr{\psi}}{\gamma\tnr{X}_2}$, where $\gamma\in\cam{F}$. As $\{\tnr{X}_1,\tnr{X}_2\}$ is linearly independent, then $\tnr{\psi}$ must be an injection and consequently a bijection since it is an unary operator. Considering the development that leads to \eqref{eq:repArrayTensF} in order to verify item v, let's say that $\vtf{f}_{i_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{B_m}$ and $\vtf{f}_{j_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{C_m}$ are arbitrary elements of bases $B$ and $C$ of $\ete{\cam{F}}{A}$. Thereby, 
\begin{multline*}
\fua{\tnr{P}}{\fua{\tnr{\psi}}{\vtf{f}_{k_1^{(1)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m^{(1)}}^{B_m}},\fua{\tnr{\psi}}{\vtf{f}_{k_1^{(2)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m^{(2)}}^{B_m}}} = \sum_{i_1^{(1)}=1}^{n_1}\cdots\sum_{i_m^{(1)}=1}^{n_m}\sum_{j_1^{(1)}=1}^{n_1}\cdots\sum_{j_m^{(1)}=1}^{n_m}\sum_{i_1^{(2)}=1}^{n_1}\cdots\sum_{i_m^{(2)}=1}^{n_m}\\
\sum_{j_1^{(2)}=1}^{n_1}\cdots\sum_{j_m^{(2)}=1}^{n_m}{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(1)}\cdots i_m^{(1)} j_1^{(1)}\cdots j_m^{(1)}}\,{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(2)}\cdots i_m^{(2)} j_1^{(2)}\cdots j_m^{(2)}}\delta_{k_1^{(1)}j_1^{(1)}}\cdots\delta_{k_m^{(1)}j_m^{(1)}}\\
...
and then
\begin{multline*}
\fua{\tnr{P}}{\fua{\tnr{\psi}}{\vtf{f}_{k_1^{(1)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m^{(1)}}^{B_m}},\fua{\tnr{\psi}}{\vtf{f}_{k_1^{(2)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m^{(2)}}^{B_m}}} = \sum_{i_1^{(1)}=1}^{n_1}\cdots\sum_{i_m^{(1)}=1}^{n_m}\sum_{i_1^{(2)}=1}^{n_1}\cdots\sum_{i_m^{(2)}=1}^{n_m}\\
...
\begin{multline*}
\sum_{i_1^{(1)}=1}^{n_1}\cdots\sum_{i_m^{(1)}=1}^{n_m}\sum_{i_1^{(2)}=1}^{n_1}\cdots\sum_{i_m^{(2)}=1}^{n_m}
{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(1)}\cdots i_m^{(1)} k_1^{(1)}\cdots k_m^{(1)}}\,{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(2)}\cdots i_m^{(2)} k_1^{(2)}\cdots k_m^{(2)}}
\\\sum_{j_1^{(1)}=1}^{n_1}\cdots\sum_{j_m^{(1)}=1}^{n_m}\sum_{j_1^{(2)}=1}^{n_1}\cdots\sum_{j_m^{(2)}=1}^{n_m}\maf{\tnr{i}}{C}{B}_{j_1^{(1)}\cdots j_m^{(1)} i_1^{(1)}\cdots i_m^{(1)}}\maf{\tnr{i}}{C}{B}_{j_1^{(2)}\cdots j_m^{(2)} i_1^{(2)}\cdots i_m^{(2)}}\\\fua{\tnr{P}}{\vtf{f}_{j_1^{(1)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m^{(1)}}^{B_m},\vtf{f}_{j_1^{(2)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m^{(2)}}^{B_m}}\,,
\end{multline*}
and from \eqref{eq:compTensFunct} this expression results
\begin{multline*}
\sum_{j_1^{(1)}=1}^{n_1}\cdots\sum_{j_m^{(1)}=1}^{n_m}\sum_{j_1^{(2)}=1}^{n_1}\cdots\sum_{j_m^{(2)}=1}^{n_m}
