...
\end{equation}     
Moreover, if tensors $\tnr{C},\tnr{D}\in\ete{\cam{F}}{V^{\times p}}$, it is straightforward to see that
\begin{equation}
(\tnr{A}\otimes \tnr{C}) \cdot (\tnr{B}\otimes \tnr{D})=(\tnr{A}\cdot \tnr{B})(\tnr{C}\cdot \tnr{D})\,.
\end{equation} 
If a tensor $\tnr{B}=\vto{x}^*_1\otimes\cdots\otimes\vto{x}^*_q\otimes\vto{y}^*_1\otimes\cdots\otimes\vto{y}^*_q$ belongs to  $\ete{\cam{F}}{U^{\times m}}=\ete{\cam{F}}{V^{\times q}\otimes V^{\times q}}$, 
\begin{equation}
\tnr{I}\cdot \tnr{B}=\fua{\tnr{I}}{\vto{x}_1,\cdots,\vto{x}_q,\vto{y}_1,\cdots,\vto{y}_q}= \prod_{i=1}^q\vto{x}_i\cdot\vto{y}_{i}^c=\overline{\mathrm{tr}}\,\tnr{B}\,,
\end{equation}   
