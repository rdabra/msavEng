...
For the purposes of our study, a tensor function establishes a functional relationship between tensors of any order. Because of this definition, in the context of Hilbert spaces, such functions are of the most general type: as presented in the previous section, tensors of order zero are scalars by definition and tensors of first order are covectors, which are biunivocaly related to vectors, according to the Riesz-Fréchet Representation Theorem. In the context of Hilbert spaces, this generality of tensor functions is particularly  important for us and that is why we shall be dealing mostly with these spaces hereafter. So, considering Hilbert spaces, it is always possible to relate a tensor and a linear tensor function, which is a fundamental feature to enable uniqueness and existence of the specific linear tensor functions to be presented in this section. The following theorem\footnote{As presented by \aut{Backus}\cite{backus_1997_1} after adapting and generalizing a property from  \aut{Marcus}\cite{marcus_1973_1}, p.14.}, borrowed from the realm of Multilinear Algebra, describes such feature in a wider sense by establishing a one to one correspondence between a multilinear function and a linear tensor function. From this biunivocal relationship, the multilinear function is usually said to be ``lifted'' to a tensor function. More specifically, when the images involved are constituted by scalars, we have a tensor, instead of a generic multilinear function, being lifted to a tensor function.

\begin{mteo}{Lifting of Multilinear Functions}{lift}
Considering the function spaces $\evl{\mathcal{R}}{U^{\times m}}{V}$ and $\evl{\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m}}}{V}$, where sets $U_i$ define Hilbert spaces and $V$ define a vector space, for every function $\vtf{g}\in\evl{\mathcal{R}}{U^{\times m}}{V}$ there is one and only one 
linear tensor function $\vtf{g}^\otimes\in\evl{\mathcal{R}}{\ete{\mathcal{R}}{U^{\times m}}}{V}$ such that $\fua{\vtf{g}}{\vto{x}_1,\cdots,\vto{x}_m}=\fua{\vtf{g}^\otimes}{\vto{x}_1\otimes\cdots\otimes\vto{x}_m}$ for all $\vto{x}_i\in U_i$.
\end{mteo}

{\footnotesize
\begin{proof}
The following proof is adapted from \aut{Backus}\cite{backus_1997_1}, pp. 43-45. According to definition \eqref{eq:poliadicoHilbert}, let $\tnr{T}\in\ete{\mathcal{R}}{U^{\times m}}$ be an arbitrary tensor described by 
\begin{equation}
\tnr{T}=\sum_{i_1=1}^{n_1}\cdots\sum_{i_p=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
i_p}^{B}}{\tnr{T}}\vto{u}_{i_1}^{(1)}\otimes\cdots\otimes\vto{u}_{i_m}^{(m)}\,,
\nonumber
\end{equation}
where $B$ is a basis of $\ete{\mathcal{R}}{U^{\times m}}$ defined by arbitrary bases  $\con{B}_i=\{\vto{u}_{1}^{(i)},\cdots,\vto{u}_{n_i}^{(i)}\}$ of $U_i$. If we define that $\fua{\vtf{g}^\otimes}{\vto{u}_{i_1}^{(1)}\otimes\cdots\otimes\vto{u}_{i_m}^{(m)}}=\fua{\vtf{g}}{\vto{u}_{i_1}^{(1)},\cdots,\vto{u}_{i_m}^{(m)}}$, since $\tnr{T}$ and $\vtf{g}$ and $\con{B}_i$ exist, the development
\begin{eqnarray}
\fua{\vtf{g}^\otimes}{\tnr{T}}&=&\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
i_m}^{B}}{\tnr{T}}\fua{\vtf{g}^\otimes}{\vto{u}_{i_1}^{(1)}\otimes\cdots\otimes\vto{u}_{i_m}^{(m)}}\nonumber\\
&=&\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
i_m}^{B}}{\tnr{T}}\fua{\vtf{g}}{\vto{u}_{i_1}^{(1)},\cdots,\vto{u}_{i_m}^{(m)}}\nonumber
\end{eqnarray}
shows that $\vtf{g}^\otimes$ also exists and it is uniquely determined by $\vtf{g}$ because $\tnr{T}$ and $B_i$ are arbitrary. Therefore, our previous definition relating the functions in question results valid and if we consider $\tnr{T}=\vto{x}_1\otimes\cdots\otimes\vto{x}_m$, through the rule \eqref{eq:regraCoordTens}, development can proceed as follows:
\begin{eqnarray}
\fua{\vtf{g}^\otimes}{\vto{x}_1\otimes\cdots\otimes\vto{x}_m}&=&\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_1\cdots
i_m}^{B}}{\tnr{T}}\fua{\vtf{g}}{\vto{u}_{i_1}^{(1)},\cdots,\vto{u}_{i_m}^{(m)}}\nonumber\\
&=&
\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\fua{\tnr{T}}{\vto{u}_{i_1}^{(1)},\cdots,\vto{u}_{i_m}^{(m)}}\fua{\vtf{g}}{\vto{u}_{i_1}^{(1)},\cdots,\vto{u}_{i_m}^{(m)}}\nonumber\\
&=&
\sum_{i_1=1}^{n_1}\cdots\sum_{i_m=1}^{n_m}\prod_{k=1}^m \fua{\vtf{f}_{i_k}^{\con{U}_k}}{\vto{x}_k}\fua{\vtf{g}}{\vto{u}_{i_1}^{(1)},\cdots,\vto{u}_{i_m}^{(m)}}\nonumber\\
&=&
\fua{\vtf{g}}{\sum_{i_1=1}^{n_1}\fua{\vtf{f}_{i_1}^{\con{U}_1}}{\vto{x}_1}
	\vto{u}_{i_1}^{(1)},\cdots,\sum_{i_m=1}^{n_m}\fua{\vtf{f}_{i_m}^{\con{U}_m}}{\vto{x}_m}\vto{u}_{i_m}^{(m)}}\nonumber\nonumber\\
&=& \fua{\vtf{g}}{\vto{x}_1,\cdots,\vto{x}_m}\,,\nonumber
\end{eqnarray}
which proves the generic equality of the theorem.
\end{proof}
}
