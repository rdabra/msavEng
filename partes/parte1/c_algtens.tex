...
When the order of a contractive product equals the order of its terms, the result is a scalar. In this context, considering arbitrary tensors $\tnr{A},\tnr{B}\in\ete{\cam{F}}{U^{\times m}}$, the scalar
\begin{equation}
\tnr{A}\cdot\tnr{B}:=\tnr{B}\odot_m\tnr{A}^c
\end{equation}
is called the \textsb{inner product of tensors}\index{inner product!of tensors} $\tnr{A}$ and $\tnr{B}$ because it observes the axioms of ordinary inner products\footnote{See p.\pageref{prop:produtoInterno}.}. From this definition, if $\ete{\cam{F}}{U^{\times m}}$ is a first order tensor space, where $\tnr{A}=\vto{x}^*$ and $\tnr{B}=\vto{y}^*$, we have equalities
\begin{equation}
\vto{x}^*\cdot\vto{y}^*=\vto{y}^*\odot_1(\vto{x}^*)^c=
\end{equation}     
