...
\end{align*}
according to \eqref{eq:igAntis} and \eqref{eq:DeterminanteSimplificado}. Thus, from \eqref{eq:detFunDef}, we have $\hdet{\tnr{\psi}}=\dete{\maf{\tnr{\psi}}{B}{B}}=\hdet{\maf{\tnr{\psi}}{B}{B}}$. For the item vi verification, let's consider the previous development and equality \eqref{eq:transpDefTens} in the context of $\cam{F}=\real$, when we have 
\begin{equation*}
\fua{\tnr{\psi}^\text{T}}{\tnr{B}_l}=\sum_{k=1}^{p}{\maf{\tnr{\psi}^\text{T}}{B}{B}}_{kl}\tnr{B}_k=\sum_{k=1}^{p}{\maf{\tnr{\psi}}{B}{B}}^\text{T}_{kl}\tnr{B}_k=\sum_{k=1}^{p}{\maf{\tnr{\psi}}{B}{B}}_{lk}\tnr{B}_k
\end{equation*}
and consequently
\begin{align*}
\fua{\tnr{P}}{\fua{\tnr{\psi}^\text{T}}{\tnr{B}_1},\cdots,\fua{\tnr{\psi}^\text{T}}{\tnr{B}_p}}
&=\sum_{i_1=1}^{p}\cdots\sum_{i_p=1}^{p}{\maf{\tnr{\psi}}{B}{B}}_{1i_1}\cdots{\maf{\tnr{\psi}}{B}{B}}_{pi_p}\epsilon_{i_1\cdots i_p}\fua{\tnr{P}}{{\tnr{B}_{1}},\cdots,{\tnr{B}_{p}}}\\
&=\dete{\maf{\tnr{\psi}}{B}{B}}\fua{\tnr{P}}{{\tnr{B}_{1}},\cdots,{\tnr{B}_{p}}}\\
&=\hdet{\tnr{\psi}}\fua{\tnr{P}}{{\tnr{B}_{1}},\cdots,{\tnr{B}_{p}}}\,,
\end{align*}
since an equality similar to \eqref{eq:DeterminanteSimplificado} can be obtained by considering $i_k=k$ on \eqref{eq:Determinante}. Now, equalities $1=\hdet{\tnr{i}}=\hdet{\tnr{\psi}\circ\tnr{\psi}^\text{T}} =  \hdet{\tnr{\psi}}\,\hdet{\tnr{\psi}^\text{T}}=\hdet{\tnr{\psi}}^2$ prove item vii.  
\end{proof}
}

Now that we know the determinant of linear tensor operators and its properties, let $\ete{\cam{F}}{U^{\times m}}$ be a Hilbert tensor space and function $\tnr{\varphi}$ be an arbitrary element of a group $\grt{G}{\cam{F}}{U^{\times m}}$ of invertible continuous linear tensor operators whose domain is $\ete{\cam{F}}{U^{\times m}}$. A tensor $\tnr{T}\in\ete{\cam{F}}{U^{\times m}}$ is said to be \textsb{invariant}\index{tensor!invariant} on group $\grt{G}{\cam{F}}{U^{\times m}}$ if $\fua{\tnr{\varphi}}{\tnr{T}}=\tnr{T}$. This equality and the linearity of $\tnr{\varphi}$ enable us to affirm that a set $V$ constituted by all tensors of $\ete{\cam{F}}{U^{\times m}}$ that are invariant on group $\grt{G}{\cam{F}}{U^{\times m}}$ is a tensor subspace of $\ete{\cam{F}}{U^{\times m}}$, when $V$ is represented by the invariant tensor space $\eti{\cam{F}}{U^{\times m}}$ on group $\grt{G}{\cam{F}}{U^{\times m}}$. This group is represented by $\grt{V}{\cam{F}}{U^{\times m}}$ and called the \textsb{invariance group}\index{group!invariant} of $\eti{\cam{F}}{U^{\times m}}$ if it is constituted by all the invertible continuous linear tensor operators on which every element of $\eti{\cam{F}}{U^{\times m}}$ is invariant. Therefore, it is evident that operator $\tnr{i}\in\grt{V}{\cam{F}}{U^{\times m}}$. The invariance orthogonal group $\grt{O}{\cam{F}}{U^{\times m}}\subseteq\grt{V}{\cam{F}}{U^{\times m}}$ is called the \textsb{isotropy group}\index{group!isotropy} of the invariant tensor space $\eto{\cam{F}}{U^{\times m}}$ whose tensors are said to be \textsb{isotropic}\index{tensor!isotropic} and then $\eti{\cam{F}}{U^{\times m}}\subseteq\eto{\cam{F}}{U^{\times m}}$. Moreover, the invariance proper orthogonal group $\tnr{\mathcal{O}}^+_{\cam{F}}({U^{\times m}})\subset\grt{O}{\cam{F}}{U^{\times m}}$, where $\hdet{\tnr{\varphi}}=1$,  $\forall\,\tnr{\varphi}\in\tnr{\mathcal{O}}^+_{\cam{F}}({U^{\times m}})$, is said to be the \textsb{anti isotropy group}\index{group!anti isotropy} of $\etom{\cam{F}}{U^{\times m}}$ whose tensors are called \textsb{anti isotropic}\index{tensor!anti isotropic} and $\eto{\cam{F}}{U^{\times m}}\subset\etom{\cam{F}}{U^{\times m}}$. Therefore, every isotropic tensor is also anti isotropic. Tensors that are not isotropic are called \textsb{anisotropic}\index{tensor!anisotropic}.


From development \eqref{eq:cotensIdent}, the identity operator $\tnr{i}\in\evl{\cam{F}}{\ete{\cam{F}}{ U^{\times m}}}{\ete{\cam{F}}{U^{\times m}}}$ is the $m$-cotensor of identity tensor $\tnr{I}\in\ete{\cam{F}}{U^{\times m}\times U^{\times m}}$ and then this tensor results an example of . 

{\footnotesize
\begin{proof}
Let's verify if set $V$ is indeed a vector space of invariant tensors on $\grt{G}{\cam{F}}{U^{\times m}}$. Considering $\tnr{\varphi}\in\grt{G}{\cam{F}}{U^{\times m}}$, $\alpha,\beta\in\cam{F}$ and $\tnr{A},\tnr{B}\in V$ arbitrary elements of their respective sets, the following equalities $\alpha\tnr{A}+\beta\tnr{B}=\fua{\alpha\tnr{\varphi}}{\tnr{A}}+\beta\fua{\tnr{\varphi}}{\tnr{B}}=\fua{\tnr{\varphi}}{\alpha\tnr{A}+\beta\tnr{B}}$ prove that sum and multiplication by scalar of invariant tensors result in an invariant tensor, since $\tnr{\varphi}$ is linear. This condition also justifies that $\tnr{0}\in V$ because $\fua{\tnr{\varphi}}{\tnr{0}}=\tnr{0}$. The verification of other axioms of vector spaces is trivial. 
\end{proof}
}
