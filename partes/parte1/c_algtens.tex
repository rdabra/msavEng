...
In the context of arbitrary second order tensor $\tnr{G}\in\ete{\cam{F}}{U\times V}$ and vector $\vto{u}\in U_\cam{F}$, we already know from the previous section that a linear bijection $\tnr{\mathit{g}}$ represents $\tnr{G}$ in such a way that $\fua{\tnr{\mathit{g}}}{\vto{u}}=\tnr{G}^\dagger\hat{\odot}_1\vto{u}^*$. This equality can be rewritten in matrix terms as following: 
\begin{equation*}
...
\end{equation}
From equality \eqref{eq:matOpeHermit}, if $\tnr{\mathit{g}}$ is hermitian, $V_\cam{F}=U_\cam{F}$ and $B_1=B_2$, 
...
\end{equation}
where tensor $\tnr{P}\in\etng{p}{\cam{F}}{U^{p}}$ and $\tnr{\mathit{t}}$ is the representative function of $\tnr{T}$. Now, considering arbitrary functions $\tnr{\psi},\tnr{\varphi}\in\evl{\cam{F}}{\eteg{n}{\cam{F}}{U^{\times m}}}{\eteg{n}{\cam{F}}{U^{\times m}}}$ and $\tnr{T}_1, \tnr{T}_2\in \eteg{p}{\cam{F}}{U^{\times 2}}$, the following properties of determinant are valid:
\begin{itemize}
\setlength\itemsep{.1em}
\item[i.] $\hdet{\tnr{i}} =  1$;
...
\item[viii.] If $\tnr{T}=\tnr{T}_1\odot_1\tnr{T}_2$, then $\hdet{\tnr{T}}=\hdet{\tnr{T}_1}\hdet{\tnr{T}_2}$.
\end{itemize}
