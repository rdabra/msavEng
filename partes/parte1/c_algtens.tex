...
\end{equation}
where $\overline{\frt{T}}$ results a \textsb{representative tensor functional}\index{functional!representative tensor} of tensor $\tnr{T}$ since their values are the same when variables $\vto{x}_i\in (U_i)_\cam{F}$ are arranged as above. Note that if the scalar fields involved are real, cotensor $\frt{T}$ is the representative tensor functional of $\tnr{T}$. Now, it is possible to study tensors through their conjugate cotensors, which substitute them numerically and have only one argument. If we consider the context of expression  \eqref{eq:identProdInt}, \emph{equalities
