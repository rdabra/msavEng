...
\begin{align*}
\fua{\tnr{P}}{\fua{\tnr{\psi}}{\tnr{B}_1},\cdots,\fua{\tnr{\psi}}{\tnr{B}_p}}=\sum_{i_1=1}^{p}\cdots\sum_{i_p=1}^{p}{\maf{\tnr{\psi}}{B}{B}}_{i_11}\cdots{\maf{\tnr{\psi}}{B}{B}}_{i_pp}
\end{align*}
...
\begin{multline*}
\fua{\tnr{\psi}}{\vtf{f}_{k_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m}^{B_m}} = \sum_{i_1=1}^{n}\cdots\sum_{i_m=1}^{n}\sum_{l_1=1}^{n}\cdots\sum_{l_m=1}^{n}{\maf{\tnr{\psi}}{B}{C}}_{i_1\cdots i_m l_1\cdots j_m}\delta_{l_1k_1}\cdots\delta_{l_mk_m}
\\\vtf{f}_{i_1}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_m}^{C_m}=
\sum_{i_1=1}^{n}\cdots\sum_{i_m=1}^{n}\sum_{j_1=1}^{n}\cdots\sum_{j_m=1}^{n}{\maf{\tnr{\psi}}{B}{C}}_{i_1\cdots i_m k_1\cdots k_m}\maf{\tnr{i}}{C}{B}_{j_1\cdots j_m i_1\cdots i_m}\\\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}=
\sum_{j_1=1}^{n}\cdots\sum_{j_m=1}^{n}{\maf{\tnr{\psi}}{B}{B}}_{j_1\cdots j_m k_1\cdots k_m}\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}
\end{multline*}
assure that the use of one basis $B$ to represent $\tnr{\psi}$ is valid for the following results. In order to shorten expression sizes, we will omit the first and the last terms in the expressions. From the left hand side of \eqref{eq:detFunDef} and considering the previous development, we can write  
\begin{multline*}
\fua{\tnr{P}}{\cdots,\fua{\tnr{\psi}}{\vtf{f}_{k_1^{(l)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m^{(l)}}^{B_m}},\cdots} = \cdots \sum_{j_1^{(l)}=1}^{n}\cdots\sum_{j_m^{(l)}=1}^{n}{\maf{\tnr{\psi}}{B}{B}}_{j_1^{(l)}\cdots j_m^{(l)} k_1^{(l)}\cdots k_m^{(l)}} \cdots\\
\fua{\tnr{P}}{\cdots,\vtf{f}_{j_1^{(l)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m^{(l)}}^{B_m},\cdots}\,
\end{multline*}
where ordinal $l=1,\cdots,p$. 
...
\begin{multline*}
\fua{\tnr{P}}{\cdots,\fua{\tnr{\psi}}{\vtf{f}_{j_1}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m}^{B_m}},\cdots} = 
 \sum_{i_1^{(1)}=1}^{n}\cdots\sum_{i_m^{(1)}=1}^{n}\sum_{j_1^{(1)}=1}^{n}\cdots\sum_{j_m^{(1)}=1}^{n}\sum_{i_1^{(2)}=1}^{n}\cdots\sum_{i_m^{(2)}=1}^{n}\\
\sum_{j_1^{(2)}=1}^{n}\cdots\sum_{j_m^{(2)}=1}^{n}{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(1)}\cdots i_m^{(1)} j_1^{(1)}\cdots j_m^{(1)}}\,{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(2)}\cdots i_m^{(2)} j_1^{(2)}\cdots j_m^{(2)}}\delta_{k_1^{(1)}j_1^{(1)}}\cdots\delta_{k_m^{(1)}j_m^{(1)}}\\
\delta_{k_1^{(2)}j_1^{(2)}}\cdots\delta_{k_m^{(2)}j_m^{(2)}}\fua{\tnr{P}}{\vtf{f}_{i_1^{(1)}}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_m^{(1)}}^{C_m},\vtf{f}_{i_1^{(2)}}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_m^{(2)}}^{C_m}}\,,
\end{multline*}


and then
\begin{multline*}
\fua{\tnr{P}}{\fua{\tnr{\psi}}{\vtf{f}_{k_1^{(1)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m^{(1)}}^{B_m}},\fua{\tnr{\psi}}{\vtf{f}_{k_1^{(2)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{k_m^{(2)}}^{B_m}}} = \sum_{i_1^{(1)}=1}^{n}\cdots\sum_{i_m^{(1)}=1}^{n}\sum_{i_1^{(2)}=1}^{n}\cdots\sum_{i_m^{(2)}=1}^{n}\\
{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(1)}\cdots i_m^{(1)} k_1^{(1)}\cdots k_m^{(1)}}\,{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(2)}\cdots i_m^{(2)} k_1^{(2)}\cdots k_m^{(2)}}
\fua{\tnr{P}}{\vtf{f}_{i_1^{(1)}}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_m^{(1)}}^{C_m},\vtf{f}_{i_1^{(2)}}^{C_1}\otimes\cdots\otimes\vtf{f}_{i_m^{(2)}}^{C_m}}\,.
\end{multline*}
From \eqref{eq:elemChangeCoord} and the development that leads to \eqref{eq:changeBasisTens}, we can write that the left hand side of the previous equality equals
\begin{multline*}
\sum_{i_1^{(1)}=1}^{n}\cdots\sum_{i_m^{(1)}=1}^{n}\sum_{i_1^{(2)}=1}^{n}\cdots\sum_{i_m^{(2)}=1}^{n}
{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(1)}\cdots i_m^{(1)} k_1^{(1)}\cdots k_m^{(1)}}\,{\maf{\tnr{\psi}}{B}{C}}_{i_1^{(2)}\cdots i_m^{(2)} k_1^{(2)}\cdots k_m^{(2)}}
\\\sum_{j_1^{(1)}=1}^{n}\cdots\sum_{j_m^{(1)}=1}^{n}\sum_{j_1^{(2)}=1}^{n}\cdots\sum_{j_m^{(2)}=1}^{n}\maf{\tnr{i}}{C}{B}_{j_1^{(1)}\cdots j_m^{(1)} i_1^{(1)}\cdots i_m^{(1)}}\maf{\tnr{i}}{C}{B}_{j_1^{(2)}\cdots j_m^{(2)} i_1^{(2)}\cdots i_m^{(2)}}\\\fua{\tnr{P}}{\vtf{f}_{j_1^{(1)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m^{(1)}}^{B_m},\vtf{f}_{j_1^{(2)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m^{(2)}}^{B_m}}\,,
\end{multline*}
and from \eqref{eq:compTensFunct} this expression results
\begin{multline*}
\sum_{j_1^{(1)}=1}^{n}\cdots\sum_{j_m^{(1)}=1}^{n}\sum_{j_1^{(2)}=1}^{n}\cdots\sum_{j_m^{(2)}=1}^{n}
{\maf{\tnr{\psi}}{B}{B}}_{j_1^{(1)}\cdots j_m^{(1)} k_1^{(1)}\cdots k_m^{(1)}}\,{\maf{\tnr{\psi}}{B}{B}}_{j_1^{(2)}\cdots j_m^{(2)} k_1^{(2)}\cdots k_m^{(2)}}
\\\fua{\tnr{P}}{\vtf{f}_{j_1^{(1)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m^{(1)}}^{B_m},\vtf{f}_{j_1^{(2)}}^{B_1}\otimes\cdots\otimes\vtf{f}_{j_m^{(2)}}^{B_m}}\,.
\end{multline*}
