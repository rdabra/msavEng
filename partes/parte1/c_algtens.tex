...
\end{equation*}
where $\frt{T}$ results a \textsb{substitute function}\index{function!substitute} of tensor $\tnr{T}$ since their values are the same when variables $\vto{x}_i\in (U_i)_\cam{F}$ are arranged as above. Now, it is possible to study tensors through their conjugate cotensors, which substitute them numerically.
