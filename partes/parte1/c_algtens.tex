...
\otimes\cdots\otimes\vto{x}_{s-1}^*\otimes\vto{x}_{s+1}^*\otimes\cdots\otimes\vto{x}_m^*\,.
\end{equation}
It is important to observe that the product $\vto{x}_{r}^c\cdot\vto{x}_{s}$ is antilinear also on the left term since  $\vtf{c}_2$ has $\vto{x}_{r}$ as one of its arguments\footnote{See property \eqref{eq:antiLinConjVect}.}. Moreover, in the context of Euclidean spaces, from equalities \eqref{eq:prodIntGen} and \eqref{eq:identidade}, we can write that $\vto{x}_{r}^c\cdot\vto{x}_{s}=\vto{x}_{r}\cdot\vto{x}_{s}\,$. 
...
\\&(\prod_{i=1}^p\vto{x}_i^c\cdot\vto{y}_i)\,\vto{a}_1^*\otimes\cdots\otimes\vto{a}_{q}^*\otimes\vto{b}_{1}^*\otimes\cdots\otimes\vto{b}_n^*\,,
\end{align*} 
...
\begin{equation}\label{eq:ProdTens}
\tnr{A}\odot_p\tnr{B}=(\prod_{i=1}^p\vto{x}_i^c\cdot\vto{y}_i)\vto{a}_1^*\otimes\cdots\otimes\vto{a}_{q}^*\otimes\vto{b}_{1}^*\otimes\cdots\otimes\vto{b}_n^*.
\end{equation}
...
When the order of a contractive product equals the order of its terms, resulting a scalar, the order of the contractive product representation is omitted: for arbitrary tensors $\tnr{A},\tnr{B}\in\ete{\cam{F}}{U^{\times m}}$, we define $\tnr{A}\odot\tnr{B}=\tnr{A}\odot_m\tnr{B}$. In this context, the scalar 
\begin{equation}
\tnr{A}\cdot\tnr{B}:=\tnr{A}\odot\tnr{B}^c
\end{equation}
is called the inner product of $\tnr{A}$ and $\tnr{B}$ because it observes the axioms of inner product spaces on page \pageref{prop:produtoInterno}. 
