...
\end{bmatrix}\,.
\end{equation}
The Levi-Civita symbol enables us to define a notable mapping $\map{\text{Det}}{\bar{A}}{\cam{F}}$, where $\bar{A}$ is the set of all $q$-th order hypercubic arrays of size $n$, $\cam{F}$ is a real or complex field and function $\text{Det}$ is called \textsb{hyperdeterminant}\index{array!hyperdeterminant of}, whose rule is
...
\end{equation}
when the order $q$ is even and $\hdet{\mat{X}}=0$ when it is odd. Although this definition lacks an adequate justification at this point, it will support future concepts that will have their own intuitive meaning.   
