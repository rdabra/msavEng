...
\end{equation}
On the other hand, the multiplication $\mat{AB}$ requires that the last $q$ elements of the dimension of $A$ and the first $q$ elements of the dimension of $B$ are equal; in other words, if array $\mat{A}$ has dimension $m_1\times\cdots\times m_p\times n_1\times\cdots\times n_q$, then $\mat{B}$ must have dimension $n_1\times\cdots\times n_q\times l_1\times\cdots\times l_s$. Thereby, the resulting dimension of array $\mat{AB}$ is $m_1\times\cdots\times m_p\times l_1\times\cdots\times l_s$ and each element
\begin{equation}
...
$M\subset Y$ a set of matrices and  $\overline{M}\subset M$ a set of square matrices with order $n$. For example, the set $\overline{M}$ has the $n\times n$ Kronecker Delta, represented in \eqref{eq:matrizIdentidade}, as an element, which we usually call \textsb{identity matrix}\index{matrix!identity} $\mat{I}$ because $\mat{A}\mat{I} = \mat{I}\mat{A} = \mat{A}$, for an arbitrary $\mat{A}\in\overline{M}$.

If there exists a matrix $\mat{B}\in\overline{M}$ such that $\mat{A}\mat{B} = \mat{I}$, we call it the \textsb{inverse}\index{matrix!inverse} of $\mat{A}$, notated by $\mat{A}^{-1}$. The condition of invertibility of a matrix depends on the value of a function called \textsb{determinant}\index{determinant}, which defines a mapping $\map{\gloref{determ}}{\overline{M}}{\cam{F}}$ where $\cam{F}$ is a real or complex field and $\det$ is described by the equality
...
\end{equation}
A mapping $\map{q}{\overline{M}}{\overline{N}}$ is a \textsb{similarity transformation}\index{similarity!transformation} if
\begin{equation}
...
Now let's clarify the term ``spectrum'' cited by the theorem above; but before that, we need some important definitions. The function in $\map{\gloref{traco}}{\overline{M}}{\cam{F}}$ is called \textsb{trace}\index{matrix!trace of} when its rule is defined by
\begin{equation}
...
A matrix $\mat{A}\in\overline{M}$ is said to be \textsb{nonnegative}\index{matrix!nonnegative} or \textsb{positive-semidefinite}\index{matrix!positive-semidefinite} if
\begin{equation}
