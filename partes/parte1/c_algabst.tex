...
\begin{itemize}
	\setlength\itemsep{1pt}	
	\item[a)] ``al-jabr'' involves the acts of adding to the side where there is a subtraction a value that ``restores'' the subtracted term and of balancing the equation by adding this same value to the other side. The word ``al-jabr'' is the etymological ancestor of the word ``algebra''\footnote{In \aut{Cervantes}\cite{cervantes_2010}, there's an interesting passage at p. 476: \textit{En esto fueron razonando los dos,  hasta que llegaon a un pueblo donde fue ventura hallar un algebrista, con quien se curó...}. In the quotation, Dom Quixote and his faithful esquire are lucky to find an algebraist, who was a healer that restores displaced bones. It should be said that the spanish language was strongly influenced by the successive arab invasions from the south. In this same novel, Cervantes also states that every spanish word started by ``al'' has an arab source.}, the latter being constructed from pronouncing the former. Using a symbolic notation, this is the example presented by al-Khw\=arizm\={\i} in his handbook: 
	\begin{eqnarray*}
		x^2&=&40x-4x^2\\
		5x^2&=&40x\,.
	\end{eqnarray*}
...
In conceptual terms, the most unrestricted algebraic collection is called set\index{set}, which can have a finite or infinite number of distinct elements, or none\footnote{This is the point of view of the Naive Set Theory. For an axiomatic approach, see \aut{Cameron}\cite{cameron_1999_1}.}. Therefore, from the finite collection $a,b,b,a,c$, we can define a finite set $\lch \gloref{eleA},b,c \rch$ of three distinct\footnote{A set does not admit repeated elements. See \aut{Shen \& Vereshchagin}\cite{shen_2002_1}.} elements. This distinction, represented by different letters, depends on the element characteristics elected to analysis, as in fig. \ref{fg:conjunto}. It is important to say that the descriptive sequence of its elements does not alter the set definition: for example, definitions $\gloref{conjA}\gloref{defPor}\lch \ele{a},\ele{b}\rch$ e $\con{A}:=\lch \ele{b},\ele{a}\rch$ are identical. Speaking of comparisons, the sets $\con{A}$ e $\con{B}$ are said to be equal, $\con{A}=\con{B}$, if they have the same elements; otherwise, they are different: $\con{A}\neq\con{B}$. 
...
\begin{figure}[!ht]
	...
	\end{center}
	\titfigura{If the selection criteria was ``\texttt{triangles} with height < 15'', then $\varDelta$ is only a collection; if it was ``\texttt{triangles with height > 5}'', $\varDelta$ is also a collection; but, if the selection criteria was ``\texttt{white or black or gray triangles with height > 5}'', then $\varDelta$ is a set.}\label{fg:conjunto}
\end{figure}


The \textsb{empty set}\index{set!empty} $\gloref{emptyset}$ has no elements and it enforces the idea of set as a restricted collection built from some selection criteria: the empty set results from a criteria that no object obeyed. For example, if the selection criteria is ``\texttt{plane triangles whose angles sum ninety degrees}'', the result will be an empty set. In set theory, this ``selection criteria'' is called \textsb{specification}\index{set!specification of}, whose mathematical syntax is the following:   
\begin{equation}
...
\end{equation}
is the set of even numbers. This specification reads ``the set $\con{E}$ is defined by every element of the set of integers whose the division by two remainder is zero´´.
